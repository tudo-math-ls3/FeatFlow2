/* $Header: /src4/opennurbs/opennurbs_offsetsurface.cpp 4     9/07/06 9:03a Dalelear $ */
/* $NoKeywords: $ */
/*
//
...
float FloatMaker(double x)
{
  float f = (float)x;
  return f;
}

ON_Mesh* RhinoMeshSurface( const ON_Surface& surface, 
                         int mesh_density,
                         ON_Mesh* mesh )
{
  //if (ON::Version() >= 200401010 )
  //  return ON_MeshSurface(surface,mesh_density,mesh);

  // Fixes to get around bugs in older opennurbs code
  ...
  int fcnt_s = scount*sdegree;
  int fcnt_t = tcount*tdegree;
  int vcnt_s = fcnt_s + 1;
  int vcnt_t = fcnt_t + 1;
    ...
    for ( j = i?1:0; j <= sdegree; j++ )
    {
      ...
      for ( k = 0; k < tcount; k++ )
      {
        ...
        for ( n = k?1:0; n <= tdegree; n++ )
        {
          ...
          V_point = P;
          N_vector = N;
          T_coord.x = FloatMaker(u); //(float)u;
          T_coord.y = FloatMaker(v); //(float)v;
          mesh->m_V.Append( V_point );
          mesh->m_N.Append( N_vector );
          mesh->m_T.Append( T_coord );
          if ( j && n ) // bug fix here
          {
            vindex = mesh->m_V.Count()-1;
            mf.vi[2] = vindex;        // NE vertex index
            mf.vi[3] = vindex-vcnt_t; // SW vertex index
  ...
  mesh->m_tex_domain[0] = mesh->m_srf_domain[0];
  mesh->m_tex_domain[1] = mesh->m_srf_domain[1];

  // airtight seams on closed surfaces
  for ( i = 0; i < 2; i++ )
  {
    bool bIsClosed = surface.IsClosed(i)?true:false;
    if ( !bIsClosed )
      continue;
    bool bIsPeriodic = surface.IsPeriodic(i)?true:false;
    // j = first face
    // n = j increment
    // k = max value of j
