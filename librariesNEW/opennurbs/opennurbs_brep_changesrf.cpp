/* $Header: /src4/opennurbs/opennurbs_brep_changesrf.cpp 4     9/01/06 4:37p Dalelear $ */
/* $NoKeywords: $ */
/*
//
...

{
  ...
  */
  ...
  if (a.Length() < 10.0*ON_ZERO_TOLERANCE)
    return false;
  if (fabs(a[0] - b[0]) > ON_ZERO_TOLERANCE || fabs(a[0] - b[0]) > ON_ZERO_TOLERANCE)
    return false;
  ...
  if (!pBrep->JoinEdges(*pEA, *pEB, ON_ZERO_TOLERANCE))
    return false;
