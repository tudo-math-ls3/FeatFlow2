/* $Header: /src4/opennurbs/opennurbs_3dm_attributes.h 19    8/01/06 11:29p A-steve $ */
/* $NoKeywords: $ */
/*
//
...
public:
  ...
  */
  ...
  //   1        boundary and knot wires and one interior wire if there are no interior knots
  ...
  */
  ...
  /*
  Description:
    Get a list of the display matrial references.
  Parameters:
    list - [out] The attributes display material reference list
                 is appended to list.
  Returns:
    Number of elements append to list.
  */
  int GetDisplayMaterialRefList(
    ON_SimpleArray<ON_DisplayMaterialRef>& list
    ) const;

  // If m_bSpecialViewport attribute is set to true, the object only
  // works with viewports in the m_dmref list
  // Default is false
  bool m_bSpecialViewport;
private:
