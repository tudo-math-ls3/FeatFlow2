/* $Header: /src4/opennurbs/opennurbs_array_defs.h 9     8/29/06 1:43p Dalelear $ */
/* $NoKeywords: $ */
/*
//
...
int ON_SimpleArray<T>::BinarySearch( const T* key, int (*compar)(const T*,const T*) ) const
{
  ...
  if ( 0 != found )
  {
    ...
    //   rc = found ? ((((unsigned int)found) - ((unsigned int)m_a))/sizeof(T)) : -1;
    // worked on a wide range of 32 bit compilers.  The following should work
    // on 32 and 64 bit compilers.
    //
    unsigned long fptr = (unsigned long)found;
    unsigned long aptr = (unsigned long)m_a;
    unsigned long sz   = (unsigned long)sizeof(T);
    unsigned long i    = (fptr - aptr)/sz;
    rc = (int)i;
#endif
  }
  else
