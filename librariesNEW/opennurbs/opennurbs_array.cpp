/* $Header: /src4/opennurbs/opennurbs_array.cpp 10    9/06/06 1:52p Dalelear $ */
/* $NoKeywords: $ */
/*
//
...
{
  int i = ON_UuidPair::CompareFirstUuid(a,b);
  if ( !i && a)
...
{
  return ON_UuidList::CompareUuid(&a->m_uuid,&b->m_uuid);
}
    ...
    ) const
{
  ...
  for ( i = 0; i < m_count; i++ )
  {
    if ( ON_max_uuid == m_a[i].m_uuid )
      continue;
    uuid_list.Append(m_a[i].m_uuid);
  }
  return uuid_list.Count() - count0;
}
...
ON_UuidIndex* ON_UuidIndexList::SearchHelper(const ON_UUID* uuid) const
{
  if ( m_count - m_sorted_count > 8 || m_removed_count > 0 )
  {
    ...
    if ( m_removed_count > 0 )
    {
      ...
      removed_uuid.m_uuid = ON_max_uuid;
      removed_uuid.m_i = 0;
  ...
  if (0 == p)
  {
    ...
    for ( i = m_sorted_count; i < m_count; i++ )
    {
      if ( 0 == ON_UuidList::CompareUuid(uuid,&m_a[i].m_uuid) )
      {
        p = m_a+i;
        break;
      }
