/* $Header: /src4/opennurbs/opennurbs_mesh.h 35    9/05/06 11:32a Dalelear $ */
/* $NoKeywords: $ */
/*
//
...
public:
  ...
  */
  ...
  int m_texture_range;       // 0: surface domain
                             //
                             //          each face in a polysurface uses the
                             //          surface's domain as the texture range.
                             //
                             // 1: normalized
                             //
...
public:
  ...
  */
  ...
  bool SetTextureDomain( 
        const ON_Interval&, // first texture parameter domain
        const ON_Interval&  // second texture parameter domain
        );
  ...
  */
  ...
  bool NormalizeTextureCoordinates();
	...
	//	Parameters:
	//		mode -[in] 
	//						0   -   means surface coords were also Transposed.
	//										For example see bottom if ON_BrepFace::Transpose().
	//						1   -   means surface coords were not changed (like called on stand alone mesh)
	//  Returns
	//			true  -  success
	bool TransposeTexture( int mode = 0 );
	...
	//		mode -[in] 
	//						0   -   means surface coords were also Reversed.
	//										For example see bottom if ON_BrepFace::Reverse().
	//						1   -   means surface coords were not changed (like called on stand alone mesh)
	//  Returns
	//			true  -  success
	bool ReverseTexture( int dir, int mode = 0 );
  ...
  */
  ...
  /*
  Description:
    Uses the ON_Mesh m_tex_domain[], m_srf_domain[], and m_srf_tex_rotate
    fields to calculate the tranformation that maps the surface evaluation
    parameters m_S[] to a textures coordinates.
  Parameters:
    mapping - [out]
      If successful, 
        mapping.m_mapping_id = ON_TextureMapping::srfp_mapping_id
        mapping.m_uvw = identity
        mapping.m_xyz = transformation that maps surface evaluation
                        parameters in m_S[] to texture coordinates.
  Returns:
    True if m_tex_domain[], m_srf_domain[], and m_srf_tex_rotate
    are valid and the m_xyz value can be calculated.
  */
  bool GetSurfaceParameterTextureMapping( 
  ...
  //////////
  // If the mesh was created by meshing a parametric surface, 
  // then GetSurfaceParameters() can compute the surface
  // parameters from texture coordinates that may have been 
  // normalized, packed, or rotated
  bool GetSurfaceParameters(
         double, double,   // texture coordinates
         double*, double*  // associated surface parameters
         ) const;
  ...
  */
  ...
  ON_3fPointArray             m_V;  // vertex locations
  ...
  ON_SimpleArray<ON_MeshFace> m_F;  // facets (triangle or quad)

  ON_3fVectorArray            m_N;  // OPTIONAL vertex unit normals
                                    // Either m_N[] has zero count or it has the same
                                    // count as m_V[], in which case m_N[j] reports
                                    // the unit normal at m_V[j].

  ON_3fVectorArray            m_FN; // OPTIONAL face unit normals
  ...
  /////////////////////////////////////////////////////////////////
  // Implementation - surface parameters
  ON_2dPointArray m_S; // OPTIONAL surface parameters for each vertex

  // If m_texture_scale[] has positive values, then they report a world
  // coordinate texture map size that would minimize texture distortion
  // if the texture were mapped using texture coordinates
  //   m_T[i].x = m_srf_domain[0].NormalizedParameterAt(m_S[i].x);
  //   m_T[i].y = m_srf_domain[1].NormalizedParameterAt(m_S[i].y);
  ON_Interval m_tex_domain[2];
  double m_texture_scale[2];
  ON_Interval m_srf_domain[2];
  bool m_srf_tex_rotate;
  ...
  // m_V[j].
  //
  // The m_tex_domain[] intervals record the texture coordinate 
  // intervals that coorespond to the surface parameter intervals.
  // These texture coordinate intervals can be decreasing, but 
  // "canonical" texture coordinates satisfy
  //
  //     m_tex_domain[0].Min() <= tex_u <= m_tex_domain[0].Max()
  //     m_tex_domain[1].Min() <= tex_v <= m_tex_domain[1].Max()
  //
  // If m_S[] is not present, m_srf_domain[] are increasing intervals,
  // m_tex_domain[] are nonempty intervals (possibly decreasing),
  // m_mapping_id = ON_TextureMapping::m_srfuv_mapping_id, and 
  // m_srf_tex_rotate = 0 or 1, then the following code will calculate
  // the surface parameters from the "canonical" texture coordinates.
  //
  //           // (tex_u, tex_v) = known "canonical" texture coordinate
  //           double a, b;
  //           if ( m_srf_tex_rotate ) 
  //           {
  //              a = m_tex_domain[1].NormalizedParameterAt( tex_v );
  //              b = 1.0 - m_tex_domain[0].NormalizedParameterAt( tex_u );
  //           }
  //           else 
  //           {
  //             a = m_tex_domain[0].NormalizedParameterAt( tex_u );
  //             b = m_tex_domain[1].NormalizedParameterAt( tex_v );
  //           }
  //           // surface parameters
  //           srf_s = m_srf_domain[0].ParameterAt(a); 
  //           srf_t = m_srf_domain[1].ParameterAt(b);
  //
  //
  // If m_S[] is present, m_srf_domain[] are increasing intervals,
  // m_tex_domain[] are nonempty intervals (possibly decreasing),
  // and m_srf_tex_rotate = 0 or 1, then the following code can calculate
  // the "canonical" texture coordinates from the surface parameters.
  //
  //           // (srf_s, srf_t) = known surface parameter
  //           double a, b;
  //           if ( m_srf_tex_rotate ) 
  //           {
  //              a = m_srf_domain[1].NormalizedParameterAt( srf_t );
  //              b = 1.0 - m_srf_domain[0].NormalizedParameterAt( srf_s );
  //           }
  //           else 
  //           {
  //             a = m_srf_domain[0].NormalizedParameterAt( srf_s );
  //             b = m_srf_domain[1].NormalizedParameterAt( srf_t );
  //           }
  //           // corresponding "canonical" texture coordinates
  //           tex_u = m_tex_domain[0].ParameterAt(a);
  //           tex_v = m_tex_domain[1].ParameterAt(b);
  //
  /////////////////////////////////////////////////////////////////
  ...
  // UNUSED ?? // int m_material_index;      // -1 = default if >= 0, this material overrides any other
