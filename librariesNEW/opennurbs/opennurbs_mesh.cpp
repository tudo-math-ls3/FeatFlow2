/* $Header: /src4/opennurbs/opennurbs_mesh.cpp 67    9/06/06 3:46p Dalelear $ */
/* $NoKeywords: $ */
/*
//
            ...
            )
{
  ...
  mesh->m_tex_domain[0].Set(0.0, 1.0);
  mesh->m_tex_domain[1].Set(0.0, 1.0);

  ON_TextureMapping srfp_mapping;
  if ( mesh->GetSurfaceParameterTextureMapping(srfp_mapping) )
  {
    mesh->m_Ttag.Set(srfp_mapping);
  }
           ...
           ) const
{
  if ( m_surface )
  {
    ...
    if ( mesh && m_bTransposed )
    {
      ...
      ON_Interval tmp = mesh->m_tex_domain[0];
      mesh->m_tex_domain[0] = mesh->m_tex_domain[1];
      mesh->m_tex_domain[0] = tmp;
      
      tmp = mesh->m_srf_domain[0];
      mesh->m_srf_domain[0] = mesh->m_srf_domain[1];
      mesh->m_srf_domain[0] = tmp;

      double t = mesh->m_texture_scale[0];
      mesh->m_texture_scale[0] = mesh->m_texture_scale[1];
      mesh->m_texture_scale[0] = t;
...
ON_Mesh::ON_Mesh() : m_srf_tex_rotate(0), 
                     m_parent(0),
                     m_mesh_parameters(0), 
                     m_invalid_count(0), 
                     m_quad_count(0), 
...
{
  m_top.m_mesh = this;
  m_texture_scale[0] = 1.0;
  m_texture_scale[1] = 1.0;
  m_kstat[0] = 0;
  m_kstat[1] = 0;
  m_kstat[2] = 0;
  m_kstat[3] = 0;
...
ON_Mesh& ON_Mesh::operator=( const ON_Mesh& src )
{
  if ( this != &src ) 
  {
    ...
    m_Ctag = src.m_Ctag;
    m_Ttag = src.m_Ttag;
    m_tex_domain[0] = src.m_tex_domain[0];
    m_tex_domain[1] = src.m_tex_domain[1];
    m_srf_domain[0] = src.m_srf_domain[0];
    m_srf_domain[1] = src.m_srf_domain[1];
    m_texture_scale[0] = src.m_texture_scale[0];
    m_texture_scale[1] = src.m_texture_scale[1];
    m_srf_tex_rotate = src.m_srf_tex_rotate;
...
void ON_Mesh::Dump( ON_TextLog& dump ) const
{
  ...
  dump.Print("m_srf_tex_rotate: %s\n",m_srf_tex_rotate?"true":"false");
  dump.Print("m_tex_domain: (%g,%g)x(%g,%g)\n",m_tex_domain[0][0],m_tex_domain[0][1],m_tex_domain[1][0],m_tex_domain[1][1]);
  dump.Print("m_srf_domain: (%g,%g)x(%g,%g)\n",m_srf_domain[0][0],m_srf_domain[0][1],m_srf_domain[1][0],m_srf_domain[1][1]);
  dump.Print("m_texture_scale: %g,%g\n",m_texture_scale[0],m_texture_scale[0]);
...
BOOL ON_Mesh::Write( ON_BinaryArchive& file ) const
{
  ...
  if (rc) rc = file.WriteInterval( m_tex_domain[0] );
  if (rc) rc = file.WriteInterval( m_tex_domain[1] );
  if (rc) rc = file.WriteInterval( m_srf_domain[0] );
  if (rc) rc = file.WriteInterval( m_srf_domain[1] );
  if (rc) rc = file.WriteDouble( 2, m_texture_scale );
...
BOOL ON_Mesh::Read( ON_BinaryArchive& file )
{
  ...
  if (rc && (major_version == 1 || major_version == 3) ) 
  {
    int vcount = 0;
    int fcount = 0;
    ...
    if (rc) rc = file.ReadInterval( m_tex_domain[0] );
    if (rc) rc = file.ReadInterval( m_tex_domain[1] );
    if (rc) rc = file.ReadInterval( m_srf_domain[0] );
    if (rc) rc = file.ReadInterval( m_srf_domain[1] );
    if (rc) rc = file.ReadDouble( 2, m_texture_scale );
    ...
    if ( minor_version >= 2 ) 
    {
      int b = m_srf_tex_rotate;
      if (rc) rc = file.ReadInt( &b );
      m_srf_tex_rotate = b?true:false;
    }
    ...
    if ( 3 == major_version )
    {
      ...
      {
        if (   m_srf_domain[0].IsIncreasing() 
            && m_srf_domain[1].IsIncreasing() 
            && 0.0 != m_tex_domain[0].Length() 
            && 0.0 != m_tex_domain[1].Length()
            && ON_UuidIsNil(m_Ttag.m_mapping_id)
            ) 
        {
          // This is a mesh from an old file - but I have enough 
          ...
          for ( i = 0; i < vcount; i++)
          {
            ...
            sp.x = tc.x;
            sp.y = tc.y;
            GetSurfaceParameters(sp.x,sp.y,&sp.x,&sp.y);
            m_S.Append(sp);
          }
          ON_TextureMapping srfp_mapping;
          if ( GetSurfaceParameterTextureMapping(srfp_mapping) )
          {
            m_Ttag.Set(srfp_mapping);
          }
          else
          {
            m_Ttag.m_mapping_id = ON_TextureMapping::m_default_srfp_mapping_id;
          }
        }
        else if ( ON_UuidIsNil(m_Ttag.m_mapping_id) 
                  && file.ArchiveOpenNURBSVersion() < 20060526
                )
        {
          m_Ttag.m_mapping_id = ON_TextureMapping::m_default_srfp_mapping_id;
        }
...
bool ON_Mesh::SetTextureDomain( 
      const ON_Interval& d0, // second texture parameter domain
      const ON_Interval& d1  // second texture parameter domain
      )
{
  m_tex_domain[0] = d0;
  m_tex_domain[1] = d1;
  return (d0.IsIncreasing() && d1.IsIncreasing());
}
...
{ // appends a copy of mesh to this and updates
  ...
  const bool bHasVertexNormals = HasVertexNormals() && m.HasVertexNormals();
  const bool bHasFaceNormals   = HasFaceNormals() && m.HasFaceNormals();
  const bool bHasTextureCoordinates = HasTextureCoordinates() && m.HasTextureCoordinates();
  const bool bHasPrincipalCurvatures = HasPrincipalCurvatures() && m.HasPrincipalCurvatures();
  const bool bHasVertexColors = HasVertexColors() && m.HasVertexColors();
  const bool bHasSurfaceParameters = HasSurfaceParameters() && m.HasSurfaceParameters();
...
{
  bool rc = HasTextureCoordinates() 
           && (m_tex_domain[0].IsIncreasing() || m_tex_domain[0].IsDecreasing() )
           && (m_tex_domain[1].IsIncreasing() || m_tex_domain[1].IsDecreasing() );
  if ( rc ) 
  {
    if ( m_srf_tex_rotate ||
         m_tex_domain[0].m_t[0] != 0.0 || m_tex_domain[0].m_t[1] != 1.0 ||
         m_tex_domain[0].m_t[1] != 0.0 || m_tex_domain[1].m_t[1] != 1.0 ) 
    {
      const int vertex_count = m_T.Count();
      int ti;
      ON_2fPoint t0, t1;
      for (ti = 0; ti < vertex_count; ti++ ) 
      {
        ...
        if ( m_srf_tex_rotate ) 
        {
          ...
          // 31 Jan 2003 Dale Lear - fixed bug
          //t1.y = (float)(m_tex_domain[0].NormalizedParameterAt(t0.x));
          //t1.x = (float)(1.0-m_tex_domain[1].NormalizedParameterAt(t0.y));
          t1.x = (float)(m_tex_domain[1].NormalizedParameterAt(t0.y));
          t1.y = (float)(1.0 - m_tex_domain[0].NormalizedParameterAt(t0.x));
        }
        else 
        {
         // normalize
          t1.x = (float)(m_tex_domain[0].NormalizedParameterAt(t0.x));
          t1.y = (float)(m_tex_domain[1].NormalizedParameterAt(t0.y));
        }
        m_T[ti] = t1;
      }
      m_tex_domain[0].Set( 0.0, 1.0 );
      m_tex_domain[1].Set( 0.0, 1.0 );
      m_srf_tex_rotate = 0;
    }
    if ( m_mesh_parameters )
...
bool ON_Mesh::TransposeTexture( int mode )
{
	bool rc = HasTextureCoordinates() && (mode==0 || mode==1) 
           && (m_tex_domain[0].IsIncreasing() || m_tex_domain[0].IsDecreasing() )
           && (m_tex_domain[1].IsIncreasing() || m_tex_domain[1].IsDecreasing() );
	if(rc)
  {
		// swap m_texture scale
		double temp = m_texture_scale[0];
		m_texture_scale[0] = m_texture_scale[1];
		m_texture_scale[1] = temp;

		// We will reflect the texture coordinates stored in the m_T array
		// about either the diagonal between the lower left to the upper right 
		// corners (llur==true) of the texture domain, or the diagonal between 
		...
		if(mode==0)
    {
			...
			llur = !((m_tex_domain[0][0]==m_tex_domain[0].Min()) ^ (m_tex_domain[1][0]==m_tex_domain[1].Min())); 
			if( m_srf_tex_rotate)
				llur = !llur;
		}
		else if(mode==1) 
    {
			m_srf_tex_rotate = !m_srf_tex_rotate;
			m_tex_domain[1].Swap();
		}

		
		ON_Interval TD[2]={m_tex_domain[0],m_tex_domain[1]};
		TD[0].MakeIncreasing(); 
		TD[1].MakeIncreasing(); 
		for(int i=0; i<m_T.Count(); i++)
    {
			ON_2fPoint tc = m_T[i];
			double x = TD[1].NormalizedParameterAt(tc[1]);
			double y = TD[0].NormalizedParameterAt(tc[0]);
			if(!llur){
				x = 1-x;
				y = 1-y;
			}	
			x= TD[0].ParameterAt(x);
			y= TD[1].ParameterAt(y);
			m_T[i].Set((float)x,(float)y);
		}
	}
	return rc;
}

bool ON_Mesh::ReverseTexture( int dir, int mode )
{
	bool rc = HasTextureCoordinates() && (mode==0 || mode==1) && (dir==0 || dir==1)
           && (m_tex_domain[0].IsIncreasing() || m_tex_domain[0].IsDecreasing() )
           && (m_tex_domain[1].IsIncreasing() || m_tex_domain[1].IsDecreasing() );
	int xdir =dir;
	if(rc)
  {
		if(mode==0)
    {
			...
			m_tex_domain[dir].Swap();	//TODO is this right???
		

		//reverse dir-th component of texture coordinates within m_tex_domain[dir]  
		for(int i=0; i<m_T.Count(); i++)
    {
			ON_2fPoint tc = m_T[i];
			double x = m_tex_domain[xdir].NormalizedParameterAt(tc[xdir]);
			tc[xdir]= (float)m_tex_domain[xdir].ParameterAt(1-x);
			m_T[i] = tc;
		}
	}
	return rc;
...
bool ON_Mesh::GetSurfaceParameters( double tex_x, double tex_y, double* srf_s, double* srf_t ) const
{
  ...
  if ( m_srf_tex_rotate ) 
  {
    // undo rotation and normalize
    // 31 Jan 2003 Dale Lear - fixed bug
    //tex_s = 1.0 - m_tex_domain[1].NormalizedParameterAt( tex_y );
    //tex_t = m_tex_domain[0].NormalizedParameterAt( tex_x );
    tex_s = m_tex_domain[1].NormalizedParameterAt( tex_y );
    tex_t = 1.0 - m_tex_domain[0].NormalizedParameterAt( tex_x );
  }
  else 
  {
    // normalize
    tex_s = m_tex_domain[0].NormalizedParameterAt( tex_x );
    tex_t = m_tex_domain[1].NormalizedParameterAt( tex_y );
  }
  *srf_s = m_srf_domain[0].ParameterAt(tex_s);
  *srf_t = m_srf_domain[1].ParameterAt(tex_t);
  return true;
}
...
bool ON_Mesh::EvaluateMeshGeometry( const ON_Surface& srf )
{
  ...
  if ( bHasSurfaceParameters
       || (HasTextureCoordinates() && m_srf_domain[0].IsIncreasing() && m_srf_domain[1].IsIncreasing() )  )
  {
    ...
    if ( HasPrincipalCurvatures() ) 
    {
      for ( vi = 0; vi < vcount; vi++ ) 
      {
        ...
        {
          GetSurfaceParameters( tex.x, tex.y, &s, &t );
        }
  ...
  if ( !m_partition ) {
         ...
         triangle_count <= partition_max_triangle_count ) 
    {
      ...
      for ( fi = 0; fi < face_count; fi++ ) {
        ...
        fpt[fi].x = (int)floor(m_tex_domain[0].NormalizedParameterAt(fcenter.x)*100);
        fpt[fi].y = (int)floor(m_tex_domain[1].NormalizedParameterAt(fcenter.y)*100);
      }
...
void ON_MappingTag::Transform( const ON_Xform& xform )
{
  if ( !ON_UuidIsNil(m_mapping_id) 
       && m_mapping_id != ON_TextureMapping::m_default_srfp_mapping_id
     )
  {
