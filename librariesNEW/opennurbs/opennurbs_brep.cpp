/* $Header: /src4/opennurbs/opennurbs_brep.cpp 37    9/05/06 3:21p Chuck $ */
/* $NoKeywords: $ */
/*
//
...
ON_Brep::IsValidTrim( int trim_index, ON_TextLog* text_log ) const
{
  ...
  else if ( trim.m_type != ON_BrepTrim::ptonsrf )
  {
    ...
    BOOL bFoundIt = false;
    int i;
    for ( i = 0; i < edge.m_ti.Count() && !bFoundIt; i++ ) 
    {
      bFoundIt  = ( edge.m_ti[i] == trim_index );
    }
    if ( !bFoundIt )
    {
      if ( text_log )
  ...
  case ON_BrepTrim::seam:
    {
      ...
      if ( edge.m_ti.Count() != 2 )
      {
        if ( text_log )
        {
          text_log->Print("brep.m_T[%d] trim is not valid.\n",trim_index);
          text_log->PushIndent();
          text_log->Print("trim.m_type = seam but brep.m_E[trim.m_ei=%d] does not reference two seam trims.\n",trim.m_ei);
          text_log->PopIndent();
        }
        return false;
      }
      int other_ti = edge.m_ti[ (edge.m_ti[0] == trim_index)?1:0 ];
      if ( other_ti >= 0 && other_ti < m_T.Count() )
      {
        const ON_BrepTrim& other_trim = m_T[other_ti];
        if ( other_trim.m_type != ON_BrepTrim::seam )
        {
          if ( text_log )
          {
            text_log->Print("brep.m_T[%d] trim is not valid.\n",trim_index);
        ...
        if ( other_trim.m_li != trim.m_li )
        {
          if ( text_log )
          {
            text_log->Print("brep.m_T[%d] trim is not valid.\n",trim_index);
            text_log->PushIndent();
            text_log->Print("trim.m_type = seam but trim.m_li=%d is not the same as other_trim.m_li=%d\n",
                            trim.m_li, other_trim.m_li);
            text_log->PopIndent();
          }
          return false;
        }
      ...
      if ( loop.m_type != ON_BrepLoop::outer )
      {
        if ( text_log )
        {
          text_log->Print("brep.m_T[%d] trim is not valid.\n",trim_index);
          text_log->PushIndent();
          text_log->Print("trim.m_type = seam but brep.m_L[trim.m_li=%d].m_type is not outer.\n",trim.m_li);
          text_log->PopIndent();
        }
        return false;
      }
      // 31 Jan 2002 - The definition of a seam trim is a trim that is connected to
      //               an edge, is part of an outer loop, and one other trim is the
      //               same outer loop is connected to the same edge.  This can happen
...
ON_Brep::IsValidLoop( int loop_index, ON_TextLog* text_log  ) const
{
  ...
  if ( loop.m_type != ON_BrepLoop::outer && loop.m_type != ON_BrepLoop::inner )
  {
    if ( text_log )
    {
      text_log->Print("brep.m_L[%d] loop is not valid.\n",loop_index);
      text_log->PushIndent();
      text_log->Print("loop.m_type = %d (must be %d=outer or %d=inner)\n",
                      loop.m_type,ON_BrepLoop::outer,ON_BrepLoop::inner);
      text_log->PopIndent();
    }
    return false;
  }
...
ON_Brep::IsValid( ON_TextLog* text_log ) const
{
  ...
  for ( fi = 0; fi < face_count; fi++ )
  {
    ...
    for ( lti = 0; lti < outer_trim_count; lti++ )
    {
      ...
      // search for seam
      for ( lti1 = lti+1; lti1 < outer_trim_count; lti1++ )
      {
...
// 2/18/03 GBA.  Destroy surface cache on face.
  ...
  if (m_brep->m_is_solid == 1 || m_brep->m_is_solid == 2) m_brep->m_is_solid = 0;
	...
	if(m_render_mesh)
		m_render_mesh->ReverseTexture(dir);
	if(m_analysis_mesh)
		m_analysis_mesh->ReverseTexture(dir);
...
// 2/18/03 GBA.  Destroy cache on the face.
  ...
  if (m_brep->m_is_solid == 1 || m_brep->m_is_solid == 2) m_brep->m_is_solid = 0;
	...
	// Greg Arden 10 April 2003.  Fix TRR#9624.  Update analysis mesh and render mesh.
	if(m_render_mesh)
		m_render_mesh->TransposeTexture();
	if(m_analysis_mesh)
		m_analysis_mesh->TransposeTexture();
  ...
  if ( ei >= 0 && ei < m_E.Count() ) {
    ...
    for (evi = 0; evi < 2; evi++ ) {
      ...
      if ( vi >= 0 && vi < m_V.Count() ) {
        ...
        if ( bDeleteEdgeVertices && v.m_ei.Count() == 1 && v.m_ei[0] == ei ) {
          v.m_ei.Empty();
          DeleteVertex(v);
        }
        else {
