...
module cccallback
  ...
  !real(DP) :: gravity1 = 10.0_DP, gravity2 = 0.0_DP
...
! ***************************************************************************
  ...
  !</subroutine>
    ...
    allocate(dwhere(1,npointsPerElement,nelements))
    call fevl_evaluate_sim4(p_rvector%RvectorBlock(1), &
   	                         rdomainIntSubset, DER_func, dwhere, 1)
...
!<subroutine>
  ...
  !</subroutine>
    ...
    rho1 = rcollection%DquickAccess(4)
    rho2 = rcollection%DquickAccess(5)
    gravity = rcollection%DquickAccess(1)
    p_rvector => rcollection%p_rvectorQuickAccess1
    allocate(dwhere(1,npointsPerElement,nelements))
...
!<subroutine>
  ...
  !</subroutine>
    ...
    rho1 = rcollection%DquickAccess(4)
    rho2 = rcollection%DquickAccess(5)
    gravity = rcollection%DquickAccess(2)
...
!    END DO
 ...
 ! ***************************************************************************
  ...
  subroutine coeff_Mass (rdiscretisationTrial,rdiscretisationTest,rform, &
                  nelements,npointsPerElement,Dpoints, &
                  IdofsTrial,IdofsTest,rdomainIntSubset, &
                  Dcoefficients,rcollection)
...
! ***************************************************************************
 ! ***************************************************************************
  ...
  subroutine coeff_LS (rdiscretisationTrial,rdiscretisationTest,rform, &
                  nelements,npointsPerElement,Dpoints, &
                  IdofsTrial,IdofsTest,rdomainIntSubset, &
                  Dcoefficients,rcollection)
  ...
  !  Dcoefficients = 1.0_DP
    ...
    p_rvector => rcollection%p_rvectorQuickAccess1
    allocate(dwhere(npointsPerElement,nelements))
    
    call PositionTracking (rdiscretisationTrial,p_rvector%rvectorBlock(1),rdomainIntSubset,nelements,&
...
! ***************************************************************
   subroutine Coeff_conv (rdiscretisationTest,rdiscretisationTrial,rform, &
                  nelements,npointsPerElement,Dpoints, &
                  IdofsTrial,IdofsTest,rdomainIntSubset,&
                  Dcoefficients, rcollection)
...
! <subroutine>
  ...
  real(DP), dimension(:,:), intent(INOUT) :: dwhere
...
! type(t_triangulation), pointer   :: p_rtriangulation   
  ...
  p_elementDistributionTrial =>&
    rvector%p_rspatialDiscr%RelementDistr(rdomainIntSubset%ielementDistribution)
  indoftrial = elem_igetNDofLoc(p_elementDistributionTrial%celement)
  indof = elem_igetNDofLoc(p_elementDistributionTrial%celement)
  NEL = p_elementDistribution%NEL
  IELset = rdomainIntSubset%ielementStartIdx
  IELmax = min(NEL,IELset+999)
 !print *, indoftrial
 !print *, indof
  
  
  allocate(DbasTrial(indoftrial,elem_getMaxDerivative(p_elementDistributionTrial%celement), &
      ...
      do j = 1, npointsperelement
       ...
       if (present(rcollection)) then
         if (DB.GE.0.0_DP) THEN
           dwhere(j,i) = rcollection%DquickAccess(4)
         else
           dwhere(j,i) = rcollection%DquickAccess(5)
         end if
       else
...
!</subroutine>
    ...
    REAL(DP) :: dx,dy
    REAL(DP) :: x1,x2,y1,y2,sin_theta
    real(DP) :: yy,dist,pai
    !X1=0.0_DP
    !X2= 10.0_DP
    !Y1= 7.0_DP
    !Y2=3.0_DP
    !SIN_theta=(Y1-Y2)/SQRT((X1-X2)**2.0_DP+(Y1-Y2)**2.0_DP)
    CALL boundary_getCoords(rdiscretisation%p_rboundary, &
         rboundaryRegion%iboundCompIdx, dwhere, dx, dy)
