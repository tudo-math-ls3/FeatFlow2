...
module ccmatvecassembly
  ...
  !real(DP) :: density1 = 2.0_DP, density2 = 1.0_DP
...
!<subroutine>
  ...
  subroutine cc_assembleMatrix (coperation,cmatrixType,rmatrix,&
      rnonlinearCCMatrix,rvector,rt,rfineMatrix)
...
!<input>
  ...
  type(t_vectorBlock), intent(IN), optional :: rt
...
!</subroutine>
    ...
    if (iand(coperation,CCMASM_COMPUTE) .ne. 0) then
      ...
      call assembleVelocityBlocks (&
          rnonlinearCCMatrix,rmatrix,rvector,rt,1.0_DP)
  ...
  contains
    ...
    subroutine assembleVelocityBlocks (rnonlinearCCMatrix,rmatrix,rvector,rt,dvectorWeight)
    ...
    type(t_vectorBlock), optional :: rvector
     type(t_vectorBlock), target,optional :: rt
    ...
    real(DP) :: rho1,rho2
      ...
      rho1 = rnonlinearCCMatrix%rho1
      rho2 = rnonlinearCCMatrix%rho2
      ...
      if (rnonlinearCCMatrix%dgamma .ne. 0.0_DP) then
        ...
        case (CCMASM_STAB_EDGEORIENTED)
         ...
         !                     rmatrix)   
          ...
          rcollection%p_rvectorQuickAccess1 => rt
          rcollection%DquickAccess(4) = rnonlinearCCMatrix%dgamma*rho1
          ...
          rtriform%Dcoefficients(1)= rnonlinearCCMatrix%dgamma
          rtriform%Idescriptors(1,1)=DER_FUNC
          rtriform%Idescriptors(2,1)=DER_DERIV_X
          rtriform%Idescriptors(3,1)=DER_FUNC
         
         CALL trilf_buildMatrixScalar (rtriform,.FALSE.,rmatrix%rmatrixblock(1,1),&
...
!<subroutine>
  ...
  subroutine cc_nonlinearMatMul (rnonlinearCCMatrix,rx,rt,rd,dcx,dcd,ry)
...
!</description>
  ...
  type(t_vectorBlock), intent(IN) :: rt
...
!</subroutine>
    ...
    call assembleVelocityDefect (rnonlinearCCMatrix,rmatrix,rx,rd,rt,p_ry,-dcx)
  ...
  contains
    ...
    subroutine assembleVelocityDefect (rnonlinearCCMatrix,&
        rmatrix,rvector,rdefect,rt,rvelocityVector,dvectorWeight)
    ...
    type(t_vectorBlock),intent(in),target :: rt
    ...
    ! DEBUG!!!
    real(dp), dimension(:), pointer :: p_DdataX,p_Ddata
    real(DP) :: rho1,rho2
