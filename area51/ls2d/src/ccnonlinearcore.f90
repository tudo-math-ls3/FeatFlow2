...
contains
  ...
  !<subroutine>
    ...
    subroutine cc_getDefect (rproblem,rnonlinearIteration,ite,rx,rt,rb,rd)
  ...
  !</subroutine>
      ...
      call cc_nonlinearMatMul (rnonlinearCCMatrix,rx,rt,rd,-1.0_DP,1.0_DP)        
...
!    ilvmax = collct_getvalue_int (p_rcollection,'NLMAX')
  ...
  !<subroutine>
    ...
    subroutine cc_precondDefect (rproblem,rnonlinearIteration,&
        ite,rd,rx,rt,rb,domega,bsuccess)
  ...
  !</subroutine>
    ...
    !call lsysbl_getbase_double (rx,p_vec)
      ...
      case (CCPREC_LINEARSOLVER,CCPREC_NEWTON,CCPREC_NEWTONDYNAMIC)
        ...
        call assembleLinsolMatrices (rproblem,rnonlinearIteration,rproblem%rcollection,&
            bassembleNewton,rx,rt,rnonlinearIteration%NLMIN,rnonlinearIteration%NLMAX)
    ...
    contains
      ...
      subroutine assembleLinsolMatrices (rproblem,rnonlinearIteration,rcollection,&
          bassembleNewton,rx,rt,NLMIN,NLMAX)
    ...
    !    call lsysbl_getbase_double (rx,p_vec)
        ...
        nullify(p_rmatrix)
        call lsysbl_copyVector(rt,rvectortmp)
        ...
        do ilev=NLMAX,NLMIN,-1
          ...
          if (ilev .eq. NLMAX) then
            call cc_assembleMatrix (CCMASM_COMPUTE,CCMASM_MTP_AUTOMATIC,&
                p_rmatrix,rnonlinearCCMatrix,p_rvectorCoarse,rvectortmp)
          else
            call cc_assembleMatrix (CCMASM_COMPUTE,CCMASM_MTP_AUTOMATIC,&
                p_rmatrix,rnonlinearCCMatrix,p_rvectorCoarse,rvectortmp,p_rmatrixFine)
          end if
          ...
          call matfil_discreteNLSlipBC (p_rmatrix,.true.)
           IF(ilev.GT.NLMIN) &
        !CALL prjTrace(rvectorTmp,rproblem%RlevelInfo(ilev-1)) 
         CALL prjF2C(rvectorTmp,rproblem%rlevelinfo(ilev-1)%rdiscretisationLS)     
        end do
...
!</subroutine>
  ...
  logical :: bconvergence,bdivergence,bsuccess
    ...
    call cc_getDefect (rproblem,rnonlinearIteration,0,rx,rt,rb,rd)
    ...
    if ((.not. bconvergence) .and. (.not. bdivergence)) then
      ...
      do ite = 1,rsolverNode%nmaxIterations
        ...
        if (domega .eq. 0.0_DP) then
          ...
          call cc_getDefect (rproblem,rnonlinearIteration,ite,rx,rt,rb,rd)
