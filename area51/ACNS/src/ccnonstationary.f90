...
module ccnonstationary
 ...
 ! use Allen Cahn module to provide perform one time step for Allen Cahn equation    
...
contains
  ...
  ! *********************Main subroutine for ACNS eqn*********************
  
!<subroutine>

!  subroutine ACNS_solveNonstationary (rproblem,rvector,rrhs,rpostprocessing)
!~~~~It should be modified as ACNS_solveNonstationary (rproblem,rvector,rrhs,rpostprocessing, &
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!!<inputoutput> for Allen Cahn problem 
!  ! A problem structure saving problem-dependent information.
...
!
!  ! Postprocessing structure. Defines what to do with solution vectors.
   type(t_c2d2postprocessing), intent(INOUT) :: rACpostprocessing
!</inputoutput>
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
!~~~~~~~~~~~~~~~~AC problem, postprocessing~~~~~~~~~~~~~~~~
    call output_line ('AC problem, Starting postprocessing of initial solution...')

!   Debug,
!    print *, rproblem%rtimedependence%dtime
!    print *, rACproblem%rtimedependence%dtime
!    print *, rACproblem%rtimedependence%iiteration

    call AC_postprocessing (rACproblem,rACvector,rACproblem%rtimedependence%iiteration,&
                                                          rproblem%rtimedependence%dtime)
    rACproblem%rtimedependence%dtime = rACproblem%rtimedependence%dtimemin
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	...
	! we treat AC_postprocessing after 1 time step
	rACproblem%rtimedependence%iiteration=1

    !----------------------------------------------------
    ! Timeloop
    !----------------------------------------------------
   ! write(*,*)'no problem before times loop'
   ! stop 
    ...
    do while ((rproblem%rtimedependence%itimeStep .le. &
              ...
              (dtimederivative .ge. &
               rproblem%rtimedependence%dminTimeDerivative) .and. &
              (irepetition .le. rACproblem%rtimedependence%niterations) .and. &
              (rACproblem%rtimedependence%dtime .lt. rACproblem%rtimedependence%dtimemax))

! evaluate rACproblem%rtimedependence%iiteration, so that it is equal to iprepetion
! it is better increase iiteration once after each time step
!      rACproblem%rtimedependence%iiteration=iprepetion

!~~~~~~~~~ Combined it with AC problem
!     DO WHILE ((iiteration .LE. rACproblem%rtimedependence%niterations) .AND. &
!              (rACproblem%rtimedependence%dtime .LT. rACproblem%rtimedependence%dtimemax))
 ...
 !~~~~~~~~~~~~~~~Perform one step for NS equation~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 !~~~~We may need other input parameters, like rACproblem, rACvector, rACrhs
 !~~~~from Allen-Cahn problem..........
...
!Mcai, we use the following to realize coupling

         call cc_performTimestep (rproblem,rpredictedSolution,rrhs,&
		  rtimesteppingPredictor,rnonlinearIteration,rnlSol,rtempBlock1,rtempBlock2,&
		         rACproblem, rACvector, rACrhs)

 !~~~~~~~~~~~~~~~We add a code for one time step of Allen-Cahn Eqn~~~~~~~~~~~~~~
 !
 !~~~~~~~We may need other input parameters, like rproblem, rvector, rrhs from NS problem
 !~~~~~ 
!          call AC_timestep (rACproblem,rACvector,rACrhs)   !~~~~We may need also time step...
!Mcai, we use the following to realize coupling 

        call output_separator(OU_SEP_MINUS)
        call output_line ('Time step '//TRIM(sys_siL(rACproblem%rtimedependence%iiteration,6))// &
                        '     Time '//TRIM(sys_sdL(rACproblem%rtimedependence%dtime,5)))
        call output_lbrk ()
     
         call AC_timestep (rACproblem,rACvector,rACrhs, &
		             rproblem,rvector,rrhs)   !~~~~We may need also time step... 
!          call AC_timestep (rACproblem,rACvector,rACrhs)
 !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
!Mcai
        call cc_performTimestep (rproblem,rvector,rrhs,&
            rtimestepping,rnonlinearIteration,rnlSol,rtempBlock1,rtempBlock2,&
    			rACproblem, rACvector, rACrhs)
!~~~~~~~~~~~~~~~We add a code for one time step of Allen-Cahn Eqn~~~~~~~~~~~~~~
!         call AC_performTimestep (rACproblem, rACrhs, rACtempBlock)
!        call AC_timestep (rACproblem,rACvector,rACrhs)
        call output_separator(OU_SEP_MINUS)
        write(*,*)'we have finished one step of NS----Then AC problem'
        ...
        call AC_timestep(rACproblem,rACvector,rACrhs, &
                                      rproblem, rvector, rrhs)
         write(*,*)'we finished one AC problem  step '
...
!~~~~~~~~~~~~~~~~AC problem, postprocessing~~~~~~~~~~~~~~~~~~`~~~~
        call output_separator(OU_SEP_MINUS)
        call output_line ('AC problem, Starting postprocessing of the time step...')

        ! Postprocessing. Write out the solution if it was calculated successfully.
!        call AC_postprocessing (rACproblem,rACvector,rACpostprocessing)
        call AC_postprocessing (rACproblem,rACvector,rACproblem%rtimedependence%iiteration,&
                    rproblem%rtimedependence%dtime)
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      ...
      rACproblem%rtimedependence%iiteration=rACproblem%rtimedependence%iiteration+1
...
!~~~~~~~~~~~~~~~~~~~AC problem~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  type(t_ACproblem), intent(INOUT) :: rACproblem    !~~~Q: target or not?
  type(t_vectorBlock), intent(INOUT) :: rACvector
  type(t_vectorBlock), intent(INOUT) :: rACrhs
