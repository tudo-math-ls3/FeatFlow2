...
!<typeblock>
  ...
  type t_boundaryDescriptor
    
    ! handle to the edges on boundary level 1
    ! this array contains all the edges on the boundary
    ! in real coordinates at level 1 in the following encoding:
    ! edgesAtBoundary(1:2,j),edgesAtBoundary(1:2,j+1) contain the
    ! i-th edge at the boundary and j=2*(i-1)+1
    integer :: h_DedgesAtBoundary
...
!<typeblock description="Application-specific type block for the Nav.St. problem">
  ...
  type t_problem
    ...
    real(dp), dimension(:,:), pointer :: dDistMatrix
