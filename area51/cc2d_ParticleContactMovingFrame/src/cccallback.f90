...
!</subroutine>
    ...
    case (1)
      ...
      rcollection%Dquickaccess(4) = rproblem%rtimedependence%dtimestep
    end select
...
!</subroutine>
  ...
  !Dvelocity(2) = -p_rparticleCollection%p_rParticles(ipart)%dAccel(1)
...
!<subroutine>
  ...
  !</subroutine>
    ...
    ! local variables
    real(DP) :: dxcenter, dycenter, dradius, dx, dy,dtime,lvx,lvy,robx,roby
    real(DP), dimension(:,:), pointer :: p_DvertexCoordinates
...
!        Revaluation(1)%p_Dvalues (idx,1) = 0.0_dp !rcollection%Dquickaccess(10) + lvx
!        Revaluation(2)%p_Dvalues (idx,1) = 0.0_dp !rcollection%Dquickaccess(11) + lvy
