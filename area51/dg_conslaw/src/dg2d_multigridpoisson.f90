!##############################################################################
!# ****************************************************************************
!# <name> dg2d_multigridpoisson </name>
!# ****************************************************************************
!#
!# <purpose>
...
!<subroutine>
  ...
  subroutine dgmgp_initParlist (rproblem)
...
!<subroutine>
  ...
  subroutine dgmgp_initParamTriang (rproblem)
...
!<subroutine>
  ...
  subroutine dgmgp_initDiscretisation (rproblem)
...
!<subroutine>
  ...
  subroutine dgmgp_projectInitialConditions (rproblem)
...
!<subroutine>
  ...
  subroutine dgmgp_solvePoisson (rproblem)
...
!</inputoutput>
  ...
  type(t_matrixScalar), dimension(:), allocatable :: rmatrixMC, rmatrixCx, rmatrixCy,&
                                                     rmatrixF1, rmatrixFn1, rmatrixFn2, &
                                                     rmatrixFn12, rmatrixFn22
  ...
  !!! Build convection matrices (Cx, Cy) on all levels !!!
  
  ! Allocate the matrices
  allocate(rmatrixCx(ilvmin:ilvmax),rmatrixCy(ilvmin:ilvmax))
  ...
  do ilevel = ilvmin, ilvmax
    ...
    call bilf_createMatrixStructure (rproblem%RlevelInfo(ilevel)%p_rdiscretisation%RspatialDiscr(1),&
                                     ...
                                     BILF_MATC_EDGEBASED)
    ! Calculate entries
    call bilf_buildMatrixScalar (rform,.true.,rmatrixCx(ilevel))
  ...
  do ilevel = ilvmin, ilvmax
    ...
    call bilf_createMatrixStructure (rproblem%RlevelInfo(ilevel)%p_rdiscretisation%RspatialDiscr(1),&
                                     ...
                                     BILF_MATC_EDGEBASED)
    ! Calculate entries
    call bilf_buildMatrixScalar (rform,.true.,rmatrixCy(ilevel))
  ...
  !!! Build flux matrices on all levels !!!
  
  allocate(rmatrixF1(ilvmin:ilvmax),rmatrixFn1(ilvmin:ilvmax),rmatrixFn2(ilvmin:ilvmax))
  allocate(rmatrixFn12(ilvmin:ilvmax),rmatrixFn22(ilvmin:ilvmax))
  ...
  do ilevel = ilvmin, ilvmax
    ...
    call bilf_dg_buildMatrixScEdge2D_ss (rform, CUB_G5_1D, .true.,&
                                       rmatrixF1(ilevel),&
                                       rproblem%RlevelInfo(ilevel)%rvector%RvectorBlock(1),&
                                       rproblem%RlevelInfo(ilevel)%raddTriaData,&
                                       flux_poisson_rmatrixF1,&
    ...
    ! Set up the corresponding linear form (f,Phi_j):
     rlinformcell%itermCount = 1
     rlinformcell%Idescriptors(1) = DER_FUNC
     rlinformcell%Idescriptors(2) = DER_FUNC
!     rlinformcell%ballCoeffConstant = .true.
!     rlinformcell%BconstantCoeff = .true.
!     rlinformcell%Dcoefficients(1) = 1.0
     
     ! Call the linearformroutine
     call linf_buildVectorScalar2 (rlinformcell, .true., rproblem%rrhs%RvectorBlock(1),&
                                   coeff_RHS_DG_Poisson)
...
!      call lsysbl_createMatBlockByDiscr (rproblem%RlevelInfo(ilevel)%p_rdiscretisation,rproblem%RlevelInfo(ilevel)%rmatrix)
      ...
      ! Add the matrices into the big matrix
      
      call lsyssc_matrixLinearComb (rmatrixMC(ilevel),rproblem%RlevelInfo(ilevel)%rmatrix%RmatrixBlock(2,2),&
                                    -1.0_dp,1.0_dp,&
                                    .false.,.false.,.true.,.false.)
                                    
      call lsyssc_matrixLinearComb (rmatrixMC(ilevel),rproblem%RlevelInfo(ilevel)%rmatrix%RmatrixBlock(3,3),&
                                    -1.0_dp,1.0_dp,&
                                    .false.,.false.,.true.,.false.)



      call lsyssc_matrixLinearComb (rmatrixCx(ilevel),rproblem%RlevelInfo(ilevel)%rmatrix%RmatrixBlock(1,2),&
                                    -1.0_dp,1.0_dp,&
                                    .false.,.false.,.true.,.false.)
                                    
      call lsyssc_matrixLinearComb (rmatrixCy(ilevel),rproblem%RlevelInfo(ilevel)%rmatrix%RmatrixBlock(1,3),&
                                    -1.0_dp,1.0_dp,&
                                    .false.,.false.,.true.,.false.)  
                                                                
      call lsyssc_matrixLinearComb (rmatrixCx(ilevel),rproblem%RlevelInfo(ilevel)%rmatrix%RmatrixBlock(2,1),&
                                    -1.0_dp,1.0_dp,&
                                    .false.,.false.,.true.,.false.)
                                    
      call lsyssc_matrixLinearComb (rmatrixCy(ilevel),rproblem%RlevelInfo(ilevel)%rmatrix%RmatrixBlock(3,1),&
                                    -1.0_dp,1.0_dp,&
                                    .false.,.false.,.true.,.false.)
                                    
                                    
                                    
      call lsyssc_matrixLinearComb (rmatrixFn1(ilevel),rproblem%RlevelInfo(ilevel)%rmatrix%RmatrixBlock(1,2),&
                                    1.0_dp,1.0_dp,&
                                    .false.,.false.,.true.,.false.)
                                    
      call lsyssc_matrixLinearComb (rmatrixFn2(ilevel),rproblem%RlevelInfo(ilevel)%rmatrix%RmatrixBlock(1,3),&
                                    1.0_dp,1.0_dp,&
                                    .false.,.false.,.true.,.false.)  
                                                                
      call lsyssc_matrixLinearComb (rmatrixFn12(ilevel),rproblem%RlevelInfo(ilevel)%rmatrix%RmatrixBlock(2,1),&
                                    1.0_dp,1.0_dp,&
                                    .false.,.false.,.true.,.false.)
                                    
      call lsyssc_matrixLinearComb (rmatrixFn22(ilevel),rproblem%RlevelInfo(ilevel)%rmatrix%RmatrixBlock(3,1),&
                                    1.0_dp,1.0_dp,&
                                    .false.,.false.,.true.,.false.)



      call lsyssc_matrixLinearComb (rmatrixF1(ilevel),rproblem%RlevelInfo(ilevel)%rmatrix%RmatrixBlock(1,1),&
                                    1.0_dp,1.0_dp,&
                                    .false.,.false.,.true.,.false.)      
 ...
 !         call linsol_initJacobi (p_rsmoother)
  ...
  !        call linsol_initBiCGStab (p_rsmoother,p_rpreconditioner)!,RfilterChain)
          ...
          call linsol_convertToSmoother (p_rsmoother,8,0.7_DP)
...
!    pause
    ...
    ! Add solution update
    call lsysbl_vectorLinearComb (rproblem%RlevelInfo(ilvmax)%rvector,p_rvector,1.0_dp,0.0_dp)
  ...
  do ilevel = ilvmin, ilvmax
    ...
    call lsyssc_releaseMatrix(rmatrixCx(ilevel))
    call lsyssc_releaseMatrix(rmatrixCy(ilevel))
    call lsyssc_releaseMatrix(rmatrixF1(ilevel))
    call lsyssc_releaseMatrix(rmatrixFn1(ilevel))
    call lsyssc_releaseMatrix(rmatrixFn2(ilevel))
    call lsyssc_releaseMatrix(rmatrixFn12(ilevel))
    call lsyssc_releaseMatrix(rmatrixFn22(ilevel))
  end do
  deallocate(rmatrixMC,rmatrixCx,rmatrixCy,rmatrixF1,rmatrixFn1,rmatrixFn2)
...
!<subroutine>
  ...
  subroutine dgmgp_postprocessing (rproblem)
...
!</inputoutput>
  ...
  ! local variables
    ...
    ! Output solution to gmv file
    call dg2gmv(p_rvector%Rvectorblock(2),3,sofile,-1)

    ! Output solution to vtk file
    call dg2vtk(p_rvector%Rvectorblock(2),3,sofile,-1)
    
        sofile = './gmv/u2dq2'

    ! Output solution to gmv file
    call dg2gmv(p_rvector%Rvectorblock(3),3,sofile,-1)

    ! Output solution to vtk file
    call dg2vtk(p_rvector%Rvectorblock(3),3,sofile,-1)
...
!<subroutine>
  ...
  subroutine dgmgp_doneMatVec (rproblem)
...
!<subroutine>
  ...
  subroutine dgmgp_doneDiscretisation (rproblem)
...
!<subroutine>
  ...
  subroutine dgmgp_doneParamTriang (rproblem)
...
!<subroutine>
  ...
  subroutine dgmgp_doneParlist (rproblem)
...
!    integer :: i
    ...
    call dgmgp_initParlist (p_rproblem)
   
    ! Initialise triangulation and parametrisation
    call dgmgp_initParamTriang (p_rproblem)
    
    ! Initialise discretisation
    call dgmgp_initDiscretisation (p_rproblem)
    
    ! Set initial conditions via L2 projection
    call dgmgp_projectInitialConditions (p_rproblem)
    
    ! Perform the timestepping
    call dgmgp_solvePoisson (p_rproblem)
    
!    ! Calculate matrix and vectors
!    call dgmgp_initMatVec (p_rproblem)
!    
!    ! Solve
!    call cpu_time(dtime1)
!    call dgmgp_solve (p_rproblem)
!    call cpu_time(dtime2)
!    write(*,*) 'Solver took: ', dtime2-dtime1
    
    ! Write solution to file
    call dgmgp_postprocessing (p_rproblem)
    
    ! Release memory
    call dgmgp_doneMatVec (p_rproblem)
    call dgmgp_doneDiscretisation (p_rproblem)
    call dgmgp_doneParamTriang (p_rproblem)
    call dgmgp_doneParList (p_rproblem)
