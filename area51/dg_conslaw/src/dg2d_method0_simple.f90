...
!    NLMAX = 4
    ...
    ! the cubature formula to use. Standard: Gauss 5x5.
    call spdiscr_createDefCubStructure(&  
        rdiscretisation%RspatialDiscr(1),rcubatureInfo,CUB_GEN_AUTO_G5)
    ...
    ! data.
    call bilf_buildMatrixScalar (rform,.true.,rmatrixMC,rcubatureInfo,coeff_Laplace_2D)
    ...
    if(ielementType .ne. EL_DG_T0_2D) call stdop_assembleSimpleMatrix(rmatrixCX, DER_FUNC, DER_DERIV_X,rcubatureInfo=rcubatureInfo)

    ! Now we do the same for CY
    call lsyssc_duplicateMatrix(rmatrixMC, rmatrixCY,&
         LSYSSC_DUP_SHARE, LSYSSC_DUP_EMPTY)

    if(ielementType .ne. EL_DG_T0_2D) call stdop_assembleSimpleMatrix(rmatrixCY, DER_FUNC, DER_DERIV_Y,rcubatureInfo=rcubatureInfo)
...
!       p_Ddata(1)=0.0_DP
       ...
       if(ielementType .ne. EL_DG_T0_2D) then
         ...
         call linf_buildVectorScalar (rlinformconv, .false., rrhs,&
                                       rcubatureInfo,flux,rcollection)
...
!    call output_line ('H1-error: ' // sys_sdEL(derror,10) )
    ...
    call pperr_scalar (PPERR_L2ERROR,derror,rsol,&
