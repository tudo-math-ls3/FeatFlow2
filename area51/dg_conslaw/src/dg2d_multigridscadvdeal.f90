!##############################################################################
!# ****************************************************************************
!# <name> dg2d_multigridscalaradvectiondeal </name>
!# ****************************************************************************
!#
!# <purpose>
!# This module solves the steady transport equation
!# 
!#   \nabla \cdot (\beta u) = 0
!#   u given on inflow boundary by a function g
!#   with \beta = 1/|x|(-x_2,x_1)
!#    
...
!# 
!# -\sum_{T}   (u_h,\beta\cdot\nabla v_h)_T
!# +\sum_{F^i} <u_h^-,\beta\cdot[v_h n]>_F^i 
!# +           <u_h,v_h \beta\cdot n>_\Gamma^+ 
!# =
!# -           <g,v_h \beta\cdot n>_\Gamma^+ 
!# 
!# where [v n] = v^+n^+ + v^-n^-
!#
!# </purpose>
...
!<subroutine>
  ...
  subroutine dgmgsadvd_initParlist (rproblem)
...
!<subroutine>
  ...
  subroutine dgmgsadvd_initParamTriang (rproblem)
...
!<subroutine>
  ...
  subroutine dgmgsadvd_initDiscretisation (rproblem)
...
!<subroutine>
  ...
  subroutine dgmgsadvd_initMatVec (rproblem)
...
!       call bilf_buildMatrixScalar (rform,.true.,p_rmatrix%RmatrixBlock(1,1))
       ...
       if (ielementtype.ne.0) then
          call bilf_buildMatrixScalar (rform, .true., p_rmatrix%RmatrixBlock(1,1),&
               dgmgsadvd_fcoeff_MatrixScalarMgCell)
       else
      ...
      call lsyssc_createVecIndMat (p_rmatrix%RmatrixBlock(1,1),rvectorSolTemp,.true.)
       ...
       call bilf_dg_buildMatrixScEdge2D_ss (rform, CUB_G5_1D, .false., p_rmatrix%RmatrixBlock(1,1),&
            ...
            dgmgsadvd_flux_dg_MatrixScalarMgEdge)!,&
...
!                                  .true., 0, './test.txt','(F6.1)')
    ...
    call linf_dg_buildVectorScalarEdge2d (rlinformedge, CUB_G5_1D, .true.,&
         ...
         dgmgsadvd_flux_dg_VectorScalarMgEdge,&
...
!<subroutine>
  ...
  subroutine dgmgsadvd_solve (rproblem)
...
!<subroutine>
  ...
  subroutine dgmgsadvd_postprocessing (rproblem)
...
!<subroutine>
  ...
  subroutine dgmgsadvd_doneMatVec (rproblem)
...
!<subroutine>
  ...
  subroutine dgmgsadvd_doneDiscretisation (rproblem)
...
!<subroutine>
  ...
  subroutine dgmgsadvd_doneParamTriang (rproblem)
...
!<subroutine>
  ...
  subroutine dgmgsadvd_doneParlist (rproblem)
...
!</subroutine>
  ...
  subroutine dg2d_multigridscalaradvectiondeal_run
...
!    integer :: i
    ...
    call dgmgsadvd_initParlist (p_rproblem)
   
    ! Initialise triangulation and parametrisation
    call dgmgsadvd_initParamTriang (p_rproblem)
    
    ! Initialise discretisation
    call dgmgsadvd_initDiscretisation (p_rproblem)
    
    ! Calculate matrix and vectors
    call dgmgsadvd_initMatVec (p_rproblem)
    
    ! Solve
    call cpu_time(dtime1)
    call dgmgsadvd_solve (p_rproblem)
    call cpu_time(dtime2)
    write(*,*) 'Solver took: ', dtime2-dtime1
    
    ! Write solution to file
    call dgmgsadvd_postprocessing (p_rproblem)
    
    ! Release memory
    call dgmgsadvd_doneMatVec (p_rproblem)
    call dgmgsadvd_doneDiscretisation (p_rproblem)
    call dgmgsadvd_doneParamTriang (p_rproblem)
    call dgmgsadvd_doneParList (p_rproblem)
