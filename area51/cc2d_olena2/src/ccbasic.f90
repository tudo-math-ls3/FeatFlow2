...
!<typeblock>
  ...
  type t_asmTemplates
    ...
    ! A template FEM matrix that defines the structure of concentration
    ! matrices (C). The matrix contains only a stucture, no content.
    type(t_matrixScalar) :: rmatrixTemplateFEMConcentration

    ! A template FEM matrix that defines the structure of coupling
    ! matrices (K). The matrix contains only a stucture, no content.
    type(t_matrixScalar) :: rmatrixTemplateK
    ...
    type(t_spatialDiscretisation) :: rdiscrMassConcentration
    ...
    ! A scalar discretisation structure that specifies how to generate 
    ! the concentration matrix.
    type(t_spatialDiscretisation) :: rdiscretisationConcentration

    ! Precalculated mass matrix for the concentration space.
    type(t_matrixScalar) :: rmatrixMassConcentration

    ! Precalculated Laplace matrix for that specific level (=nu*Laplace) in the
    ! concentration space.
    type(t_matrixScalar) :: rmatrixLaplaceConcentration
