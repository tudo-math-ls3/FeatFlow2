...
!</subroutine>
  ...
  character(LEN=SYS_NAMELEN) :: sstr
    ...
    type(t_spatialDiscretisation), pointer :: p_rdiscrMassConcentration
    ...
    do i=rproblem%NLMIN,rproblem%NLMAX
      ...
      call spdiscr_duplicateDiscrSc (p_rdiscretisation%RspatialDiscr(4), &
          rproblem%RlevelInfo(i)%rasmTempl%rdiscrMassConcentration,.true.)
      ...
      p_rdiscrMassConcentration => &
          rproblem%RlevelInfo(i)%rasmTempl%rdiscrMassConcentration
      ...
      if (icubM .ne. SPDISC_CUB_AUTOMATIC) then
        ...
        do k = 1,p_rdiscretisationMass%inumFESpaces
          ...
          p_rdiscrMassConcentration%RelementDistr(k)%ccubTypeBilForm = icubM
        end do
      end if
...
!<input>
  ...
  !  3 = Q1~(EM30) / Q1~(EM30) / Q0 / Q1 = standard
...
!</subroutine>
  ...
  logical :: btriallowed, bquadallowed
    ...
    ! For the concentration (4th component), we set up a separate discretisation 
    ! structure, as this uses different finite elements for trial and test
    ! functions.
    call spdiscr_deriveSimpleDiscrSc (rdiscretisation%RspatialDiscr(1),  &
        ieltypeC, icubA,rdiscretisation%RspatialDiscr(4))
...
!</subroutine>
  ...
  integer(I32) :: ieltypeUV, ieltypeP, ieltypeC
    ...
    call spdiscr_deriveSimpleDiscrSc (rsourceDiscretisation%RspatialDiscr(4),  &
        ieltypeC, SPDISC_CUB_NOCHANGE,rdestDiscretisation%RspatialDiscr(4))
...
!</subroutine>
  ...
  integer :: i
    ...
    do i=rproblem%NLMAX,rproblem%NLMIN,-1
      ...
      call spdiscr_releaseDiscr (rproblem%RlevelInfo(i)%rasmTempl%rdiscrMassConcentration)
...
!</subroutine>
  ...
  integer :: iprojTypeVelocity, iprojTypePressure
    ...
    ! Create the matrices structure of the B-matrix.
    call bilf_createMatrixStructure (&
              rdiscretisation%RspatialDiscr(3),LSYSSC_MATRIX9,&
    ...
    ! Create the matrix structure of the C-matrix.
    call bilf_createMatrixStructure (&
              rdiscretisation%RspatialDiscr(4),LSYSSC_MATRIX9,&
              rasmTempl%rmatrixTemplateFEMConcentration)

    ! Create the matrices structure of the K-matrix.
    call bilf_createMatrixStructure (&
              rdiscretisation%RspatialDiscr(4),LSYSSC_MATRIX9,&
              rasmTempl%rmatrixTemplateK,&
    ...
    ! THe same in the concentration space.
    call lsyssc_duplicateMatrix (rasmTempl%rmatrixTemplateFEMConcentration,&
                rasmTempl%rmatrixLaplaceConcentration,LSYSSC_DUP_SHARE,LSYSSC_DUP_REMOVE)
    call lsyssc_allocEmptyMatrix (rasmTempl%rmatrixLaplaceConcentration,LSYSSC_SETM_UNDEFINED)
...
!                                 rproblem%rcollection)
    ...
    ! Laplace matrix in the concentration space.
    call stdop_assembleLaplaceMatrix (rasmTempl%rmatrixLaplaceConcentration,.true.)
    ...
    call lsyssc_releaseMatrix (rasmTempl%rmatrixMassConcentration)
    ...
    call lsyssc_duplicateMatrix (rasmTempl%rmatrixTemplateFEMConcentration,&
                rasmTempl%rmatrixMassConcentration,LSYSSC_DUP_SHARE,LSYSSC_DUP_REMOVE)
    ...
    call lsyssc_assignDiscrDirectMat (rasmTempl%rmatrixMassConcentration,&
        rasmTempl%rdiscrMassConcentration)
    ...
    call stdop_assembleSimpleMatrix (rasmTempl%rmatrixMassConcentration,DER_FUNC,DER_FUNC)
...
!</subroutine>
    ...
    call lsyssc_releaseMatrix (rasmTempl%rmatrixLaplaceConcentration)
    ...
    call lsyssc_releaseMatrix (rasmTempl%rmatrixTemplateK)
    call lsyssc_releaseMatrix (rasmTempl%rmatrixTemplateFEMConcentration)
...
!        end if
      ...
      call linf_buildVectorScalar (&
                ...
                rrhs%RvectorBlock(3),coeff_RHS_div,&
                rproblem%rcollection)

      ! Concentration part:
      call linf_buildVectorScalar (&
                p_rdiscretisation%RspatialDiscr(4),rlinform,.false.,&
                rrhs%RvectorBlock(4),coeff_RHS_C,&
                rproblem%rcollection)
