...
!<typeblock>
  ...
  type t_timeDerivatives
    ...
    ! $||rel. change in C||_{L2}$
    real(DP) :: drelCL2

    ! $||rel. change in C||_{\max}$
    real(DP) :: drelCmax
  end type
...
!<constantblock description="Identifiers for norms of time-dependent solutions.">
  ...
  ! $ max(||u||_{l2}, ||c||_{l2}) $
  integer, parameter :: TNRM_L2UL2C = 1

  ! $ ||u||_{l2} $
  integer, parameter :: TNRM_L2U    = 9  
...
!</function>
    ...
    neqc = rSolutionNew%RvectorBlock(4)%NEQ
    ...
    p_rtimeNorm%drelCL2 = Dnorms1(4) / (sqrt(real(neqc,DP)) * dtstep)
    ...
    p_rtimeNorm%drelCmax = Dnorms1(4) / dtstep
    
    ! Return the value in the desired norm
    select case (ctimeErrorControl)
    case DEFAULT
    case (TNRM_L2UL2C)
      dtimenorm = max(p_rtimeNorm%drelUL2,p_rtimeNorm%drelCL2)
    case (TNRM_L2U)
