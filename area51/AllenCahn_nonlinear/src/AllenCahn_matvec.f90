...
!#
!# 3.) AC5_doneMatVec
!#     -> Release memory allocated in AC5_initMatVec.
!# </purpose>
!##############################################################################
...
CONTAINS
  ...
  subroutine AC_assembleMatJacobian(rACproblem, rACvector)
...
!  ( (\phi(t_n)^2 -1) \phi(t_n+1), \psi)
  ...
  type(t_vectorBlock), intent(INOUT) :: rACvector
    ...
    type(t_matrixBlock), POINTER :: p_rmatrixJacobian
    integer :: i
    ...
    do i = rACproblem%NLMAX, rACproblem%NLMIN,-1
      ...
      p_rmatrixJacobian => rACproblem%RlevelInfo(i)%rmatrixJacobian
      ...
      ! the discretisation.
      call lsysbl_createMatBlockByDiscr (p_rdiscretisation,p_rmatrixJacobian)
      ...
      call bilf_buildMatrixScalar (rform,.TRUE.,&
                                   p_rmatrixJacobian%RmatrixBlock(1,1),coeff_Jacobian,&
                                   rcollectionTmp)
...
!<subroutine>
  ...
  subroutine AC5_initMatVec (rACproblem,rparams, &
...
!      call storage_new ('AC5_initMatVec', 'Iresort', &
...
!<subroutine>
!MCai
  subroutine AC5_calcRHS (rACproblem, rACvector, rACrhs,&
...
!<subroutine>
  ...
  subroutine AC5_doneMatVec (rACproblem)
