        ...
        do isegment = 1,parlst_querysubstrings (p_rbdcond, cstr)
	      ...
	      ! of NVS1 and NVS2
              ...
              if (sbdex3 .ne. '') then
                ...
                ! Assemble the BC`s.
                call bcasm_newDirichletBConRealBD (&
                    rdiscretisation,3,rboundaryRegion,rdynamicLevelInfo%rdiscreteBC,&
                    cc_getBDconditions,rcoll,casmComplexity)
              ...
              if (sbdex4 .ne. '') then
                ...
                ! Assemble the BC`s.
                call bcasm_newDirichletBConRealBD (&
                    rdiscretisation,4,rboundaryRegion,rdynamicLevelInfo%rdiscreteBC,&
                    cc_getBDconditions,rcoll,casmComplexity)
            ...
            case (2)
              ...
              if (sbdex1 .ne. '') then  ! sbdex1: for pressure drop
...
!<input>
  ...
  ! =0: Any velocity component, =1: X-velocity, =2: Y-velocity / NVS1
  !                  component, =3: X-velocity, =4: Y-velocity / NVS2
        ...
        do isegment = 1,parlst_querysubstrings (p_rbdcond, cstr)
          ...
          if (dpar2 .ge. dpar1) then
            ...
            case (1)
             ! Simple Dirichlet boundary
             ! Read the line again, get the expressions for X- and Y-velocity
	     ! of NVS1 (sbdex1,sbdex2) and X- and Y-velocity of NVS2 (sbdex3,sbdex4)
              read(cstr,*) dvalue,iintervalEnds,ibctyp,sbdex1,sbdex2,sbdex3,sbdex4
              
              ! If the type is a double precision value, set the DquickAccess(4)
              ! to that value so it can quickly be accessed.
              if (((icomponent .eq. 0) .and. ((sbdex1 .ne. '') .or. (sbdex2 .ne. '') .or. (sbdex3 .ne. '') .or. (sbdex4 .ne. ''))) .or. &
                  ((icomponent .eq. 1) .and. (sbdex1 .ne. '')) .or. &
                  ((icomponent .eq. 2) .and. (sbdex2 .ne. '')) .or. &
                  ((icomponent .eq. 3) .and. (sbdex3 .ne. '')) .or. &
                  ((icomponent .eq. 4) .and. (sbdex4 .ne. ''))) then
...
!<input>
  ...
  !   5=5th solution component, e.g. pressure)
