...
!<typeblock>
  ...
  type t_c2d2postprocessing
    ...
    ! A vector that describes the X-velocity field in the vertices for nvs1 /***
    type(t_vectorScalar) :: rvectorVelX


    ! A vector that describes the Y-velocity field in the vertices for nvs1 /***
...
!</subroutine>
    ...
    if ((dtime .ge. dminTime-1000.0*SYS_EPSREAL_DP) .and. &
      ...
      ! Figure out if we have to write the solution. This is the case if
      ! 1.) Previous and current time is the same or
...
!</subroutine>
    ...
    ! local variables
    real(DP),dimension(5) :: Derr
    real(DP) :: derrorVel1,derrorVel2, derrorP, denergy1, denergy2
    ...
    if (icalcL2 .ne. 0) then
      ...
      call pperr_scalar (rsolution%RvectorBlock(1),PPERR_L2ERROR,Derr(1),&
                         ffunction_TargetX1,rproblem%rcollection)

      call pperr_scalar (rsolution%RvectorBlock(2),PPERR_L2ERROR,Derr(2),&
                         ffunction_TargetY1,rproblem%rcollection)

      call pperr_scalar (rsolution%RvectorBlock(3),PPERR_L2ERROR,Derr(3),&
                         ffunction_TargetX2,rproblem%rcollection)

      call pperr_scalar (rsolution%RvectorBlock(4),PPERR_L2ERROR,Derr(4),&
                         ffunction_TargetY2,rproblem%rcollection)
                         
      derrorVel1 = sqrt(Derr(1)**2+Derr(2)**2)
      derrorVel2 = sqrt(Derr(3)**2+Derr(4)**2)
      ...
      call output_line ('||u-reference||_L2 = '//trim(sys_sdEP(derrorVel1,15,6)) )
      call output_line ('||v-reference||_L2 = '//trim(sys_sdEP(derrorVel2,15,6)) )
      ...
      if (iwriteErrorAnalysisL2 .ne. 0) then
        ...
        if ((cflag .eq. SYS_REPLACE) .or. (.not. bfileexists)) then
          ! Write a headline
          write (iunit,'(A)') '# timestep time ||u-reference||_L2 ||v-reference||_L2 ||p-reference||_L2'
        end if
        stemp = trim(sys_siL(rproblem%rtimedependence%itimeStep,10)) // ' ' &
            // trim(sys_sdEL(dtime,10)) // ' ' &
            // trim(sys_sdEL(derrorVel1,10)) // ' ' &
            // trim(sys_sdEL(derrorVel2,10)) // ' ' &
    ...
    if (icalcH1 .ne. 0) then
      ...
      if ((Derr(1) .ne. -1.0_DP) .and. (Derr(2) .ne. -1.0_DP)) then
        ...
        call output_line ('||u-reference||_H1 = '//trim(sys_sdEP(derrorVel1,15,6)),&
      ...
      if ((Derr(3) .ne. -1.0_DP) .and. (Derr(4) .ne. -1.0_DP)) then
        ...
        call output_line ('||v-reference||_H1 = '//trim(sys_sdEP(derrorVel2,15,6)),&
            coutputMode=OU_MODE_STD+OU_MODE_BENCHLOG )
      else
        call output_line ('||v-reference||_H1 = not available',&
            coutputMode=OU_MODE_STD+OU_MODE_BENCHLOG )
      end if
      ...
      if (iwriteErrorAnalysisH1 .ne. 0) then
        ...
        if ((cflag .eq. SYS_REPLACE) .or. (.not. bfileexists)) then
          ! Write a headline
          write (iunit,'(A)') '# timestep time ||u-reference||_H1 ||v-reference||_H1'
        end if
        stemp = trim(sys_siL(rproblem%rtimedependence%itimeStep,10)) // ' ' &
            // trim(sys_sdEL(dtime,10)) // ' ' &
            // trim(sys_sdEL(derrorVel1,10)) // ' ' &
    ...
    if (icalcEnergy .ne. 0) then
      ...
      denergy1 = 0.5_DP*(Derr(1)**2+Derr(2)**2)
      denergy2 = 0.5_DP*(Derr(3)**2+Derr(4)**2)

      call output_line ('||u_1||_L2         = '//trim(sys_sdEP(Derr(1),15,6)),&
          coutputMode=OU_MODE_STD+OU_MODE_BENCHLOG )
      call output_line ('||u_2||_L2         = '//trim(sys_sdEP(Derr(2),15,6)),&
          coutputMode=OU_MODE_STD+OU_MODE_BENCHLOG )
      call output_line ('||v_1||_L2         = '//trim(sys_sdEP(Derr(3),15,6)),&
          coutputMode=OU_MODE_STD+OU_MODE_BENCHLOG )
      call output_line ('||v_2||_L2         = '//trim(sys_sdEP(Derr(4),15,6)),&
          coutputMode=OU_MODE_STD+OU_MODE_BENCHLOG )
      call output_line ('||u||_L2           = '//&
          trim(sys_sdEP(sqrt(Derr(1)**2+Derr(2)**2),15,6)),&
          coutputMode=OU_MODE_STD+OU_MODE_BENCHLOG )
      call output_line ('||v||_L2           = '//&
          trim(sys_sdEP(sqrt(Derr(3)**2+Derr(4)**2),15,6)),&
          coutputMode=OU_MODE_STD+OU_MODE_BENCHLOG )
      call output_line ('1/2||u||^2_L2      = '//trim(sys_sdEP(denergy1,15,6)),&
          coutputMode=OU_MODE_STD+OU_MODE_BENCHLOG )
      call output_line ('1/2||v||^2_L2      = '//trim(sys_sdEP(denergy2,15,6)),&
          coutputMode=OU_MODE_STD+OU_MODE_BENCHLOG )      
      call cc_doneCollectForAssembly (rproblem,rproblem%rcollection)
      ...
      if (iwriteKineticEnergy .ne. 0) then
        ...
        if ((cflag .eq. SYS_REPLACE) .or. (.not. bfileexists)) then
          ! Write a headline
          write (iunit,'(A)') '# timestep time 1/2||u||^2_L2 1/2||v||^2_L2 ||u||_L2' // ' ' &
                                 // '||v||_L2 ||u_1||_L2 ||v_1||_L2 ||u_2||_L2 ||v_2||_L2'
        end if
        stemp = trim(sys_siL(rproblem%rtimedependence%itimeStep,10)) // ' ' &
            // trim(sys_sdEL(dtime,10)) // ' ' &
            // trim(sys_sdEL(denergy1,10)) // ' ' &
            // trim(sys_sdEL(denergy2,10)) // ' ' &
...
!<subroutine>
  ...
  subroutine ffunctionBDForcesVisco1 (cderivative,rdiscretisation, &
...
!</subroutine>
    ...
    call ffunctionViscoModel1 (1,rdiscretisation, &
              nelements,npointsPerElement,Dpoints, &
              IdofsTest,rdomainIntSubset, &
              Dvalues,rcollection)
...
!<subroutine>
  ...
  subroutine ffunctionBDForcesVisco2 (cderivative,rdiscretisation, &
                nelements,npointsPerElement,Dpoints, &
                IdofsTest,rdomainIntSubset, &
                Dvalues,rcollection)
...
!</subroutine>
    ...
    ! cderivative will always be DER_FUNC here. Call ffunctionViscoModel
    ! to calculate the viscosity into Dvalues at first.
    call ffunctionViscoModel2 (1,rdiscretisation, &
              nelements,npointsPerElement,Dpoints, &
              IdofsTest,rdomainIntSubset, &
              Dvalues,rcollection)
...
!</subroutine>
    ...
    real(DP), dimension(NDIM2D) :: Dforces1
    real(DP), dimension(NDIM2D) :: Dforces2
    real(DP) :: dbdForcesCoeff1,dbdForcesCoeff2,dbdForcesCoeff3,dbdForcesCoeff4
    type(t_boundaryRegion) :: rregion
    integer :: cformulation1,cformulation2
    integer :: ibodyForcesFormulation1,ibodyForcesFormulation2
    integer :: icalcBodyForces,ibodyForcesBdComponent
...
!******************************/////////////**************************
    type(t_blockDiscretisation), pointer :: p_sub_rdiscretisation
    type(t_vectorBlock), target :: temp_rsolution
!***************************////////////////***************************    
    ...
    call parlst_getvalue_int (rproblem%rparamList, 'CC-POSTPROCESSING', &
        'ibodyForcesFormulation1', ibodyForcesFormulation1, -1)
!
    call parlst_getvalue_int (rproblem%rparamList, 'CC-POSTPROCESSING', &
        'ibodyForcesFormulation2', ibodyForcesFormulation2, -1)
!
    call parlst_getvalue_int (rproblem%rparamList, 'CC-POSTPROCESSING', &
        'ibodyForcesBdComponent', ibodyForcesBdComponent, 2)
    call parlst_getvalue_double (rproblem%rparamList, 'CC-POSTPROCESSING', &
        'dbdForcesCoeff1', dbdForcesCoeff1, rproblem%rphysics%dnu1)
    call parlst_getvalue_double (rproblem%rparamList, 'CC-POSTPROCESSING', &
        'dbdForcesCoeff2', dbdForcesCoeff2, 0.1_DP * 0.2_DP**2)
!
    call parlst_getvalue_double (rproblem%rparamList, 'CC-POSTPROCESSING', &
        'dbdForcesCoeff3', dbdForcesCoeff3, rproblem%rphysics%dnu2)
    call parlst_getvalue_double (rproblem%rparamList, 'CC-POSTPROCESSING', &
        'dbdForcesCoeff4', dbdForcesCoeff4, 0.1_DP * 0.2_DP**2)
!
    ...
    if ((rsolution%p_rblockDiscr%RspatialDiscr(1)% &
      ...
      case (1)
        ! Old implementation:
!****************//////////////////////////////*********************************
      allocate(p_sub_rdiscretisation)
!
      call spdiscr_initBlockDiscr (p_sub_rdiscretisation,3,&
	     rsolution%p_rblockDiscr%p_rtriangulation, rproblem%rboundary)
! 
      call spdiscr_duplicateDiscrSc(rsolution%p_rblockDiscr%RspatialDiscr(1),&
	    p_sub_rdiscretisation%RspatialDiscr(1))

      call spdiscr_duplicateDiscrSc(rsolution%p_rblockDiscr%RspatialDiscr(1),&
	    p_sub_rdiscretisation%RspatialDiscr(2))

      call spdiscr_duplicateDiscrSc(rsolution%p_rblockDiscr%RspatialDiscr(5),&
	    p_sub_rdiscretisation%RspatialDiscr(3))

      call lsysbl_createVecBlockByDiscr(p_sub_rdiscretisation,temp_rsolution)

      call lsyssc_duplicateVector (rsolution%RvectorBlock(1),temp_rsolution%RvectorBlock(1),&
				LSYSSC_DUP_COPY,LSYSSC_DUP_SHARE)

      call lsyssc_duplicateVector (rsolution%RvectorBlock(2),temp_rsolution%RvectorBlock(2),&
				LSYSSC_DUP_COPY,LSYSSC_DUP_SHARE)

      call lsyssc_duplicateVector (rsolution%RvectorBlock(5),temp_rsolution%RvectorBlock(3),&
				LSYSSC_DUP_COPY,LSYSSC_DUP_SHARE)

      call ppns2D_bdforces_uniform (temp_rsolution,rregion,Dforces1,CUB_G1_1D,&
            dbdForcesCoeff1,dbdForcesCoeff2)

! 			for the Dforces 2: 
      call lsyssc_duplicateVector (rsolution%RvectorBlock(3),temp_rsolution%RvectorBlock(1),&
				LSYSSC_DUP_COPY,LSYSSC_DUP_SHARE)

      call lsyssc_duplicateVector (rsolution%RvectorBlock(4),temp_rsolution%RvectorBlock(2),&
				LSYSSC_DUP_COPY,LSYSSC_DUP_SHARE)

      call lsyssc_duplicateVector (rsolution%RvectorBlock(5),temp_rsolution%RvectorBlock(3),&
				LSYSSC_DUP_COPY,LSYSSC_DUP_SHARE)

      call ppns2D_bdforces_uniform (temp_rsolution,rregion,Dforces2,CUB_G1_1D,&
            dbdForcesCoeff3,dbdForcesCoeff4)

! release the unnecessary things
   
      call spdiscr_releaseBlockDiscr(p_sub_rdiscretisation)
      call lsysbl_releaseVector(temp_rsolution)

!****************//////////////////////////////*********************************
!         call ppns2D_bdforces_uniform (rsolution,rregion,Dforces1,CUB_G1_1D,&
!             dbdForcesCoeff1,dbdForcesCoeff2)
      ...
      case (2)      
        ...
        select case (ibodyForcesFormulation1)
        case (0)
          cformulation1 = PPNAVST_GRADIENTTENSOR_SIMPLE
        case (1)
          cformulation1 = PPNAVST_GRADIENTTENSOR
        case (2)
          cformulation1 = PPNAVST_DEFORMATIONTENSOR
        case default
          ! Automatic mode.
          ! Use the deformation tensor formulation for the forces if
          ! we are in the deformation tensor formulation
          cformulation1 = PPNAVST_GRADIENTTENSOR_SIMPLE
          if (rproblem%rphysics%isubequation1 .eq. 1) &
            cformulation1 = PPNAVST_DEFORMATIONTENSOR
        end select
!****************//////////////////////////////*********************************
        ! Select the tensor formulation to use.
        select case (ibodyForcesFormulation2)
        case (0)
          cformulation2 = PPNAVST_GRADIENTTENSOR_SIMPLE
        case (1)
          cformulation2 = PPNAVST_GRADIENTTENSOR
        case (2)
          cformulation2 = PPNAVST_DEFORMATIONTENSOR
        case default
          ! Automatic mode.
          ! Use the deformation tensor formulation for the forces if
          ! we are in the deformation tensor formulation
          cformulation2 = PPNAVST_GRADIENTTENSOR_SIMPLE
          if (rproblem%rphysics%isubequation2 .eq. 1) &
            cformulation2 = PPNAVST_DEFORMATIONTENSOR
        end select
!****************//////////////////////////////*********************************          
        ...
        call ccmva_prepareViscoAssembly1 (rproblem,rproblem%rphysics,&
            rcollection,rsolution)
          
        if (rproblem%rphysics%cviscoModel1 .eq. 0) then
!****************//////////////////////////////*********************************
...
! 
	  ...
	  call ppns2D_bdforces_line (temp_rsolution,rregion,Dforces1,CUB_G1_1D,&
              dbdForcesCoeff1,dbdForcesCoeff2,cformulation1)
!
	  call spdiscr_releaseBlockDiscr(p_sub_rdiscretisation)
	  call lsysbl_releaseVector(temp_rsolution)
!            To here
...
! 
	  ...
	  call lsyssc_duplicateVector (rsolution%RvectorBlock(5),temp_rsolution%RvectorBlock(3),&
          ...
          call ppns2D_bdforces_line (temp_rsolution,rregion,Dforces1,CUB_G1_1D,&
              dbdForcesCoeff1,dbdForcesCoeff2,cformulation1,ffunctionBDForcesVisco1,rcollection)
!
	  call spdiscr_releaseBlockDiscr(p_sub_rdiscretisation)
	  call lsysbl_releaseVector(temp_rsolution)
!            To here
...
!...................................[for the second nvs 2]..................
        call ccmva_prepareViscoAssembly2 (rproblem,rproblem%rphysics,&
            rcollection,rsolution)  
        if (rproblem%rphysics%cviscoModel2 .eq. 0) then
!****************//////////////////////////////*********************************
!            Start from Here */*/*
	  allocate(p_sub_rdiscretisation)
!
...
! 
	  ...
	  call lsyssc_duplicateVector (rsolution%RvectorBlock(5),temp_rsolution%RvectorBlock(3),&
          ...
          call ppns2D_bdforces_line (temp_rsolution,rregion,Dforces2,CUB_G1_1D,&
              dbdForcesCoeff3,dbdForcesCoeff4,cformulation2)
...
! 
	  ...
	  call lsyssc_duplicateVector (rsolution%RvectorBlock(5),temp_rsolution%RvectorBlock(3),&
          ...
          call ppns2D_bdforces_line (temp_rsolution,rregion,Dforces2,CUB_G1_1D,&
              dbdForcesCoeff3,dbdForcesCoeff4,cformulation2,ffunctionBDForcesVisco2,rcollection)
...
! 
      ...
      call ppns2D_bdforces_uniform (temp_rsolution,rregion,Dforces1,CUB_G4_2D,&
				dbdForcesCoeff1,dbdForcesCoeff2)
...
! 			for the Dforces 2: 
      ...
      call ppns2D_bdforces_uniform (temp_rsolution,rregion,Dforces2,CUB_G4_2D,&
				dbdForcesCoeff3,dbdForcesCoeff4)
...
!              dbdForcesCoeff1,dbdForcesCoeff2)
      ...
      case (4)
        ...
        ! Select the tensor formulation1 to use.
        select case (ibodyForcesFormulation1)
        case (0)
          cformulation1 = PPNAVST_GRADIENTTENSOR_SIMPLE
        case (1)
        ...
        ! Select the tensor formulation2 to use.
        select case (ibodyForcesFormulation2)
        case (0)
          cformulation2 = PPNAVST_GRADIENTTENSOR_SIMPLE
        case (1)
...
! 
	   ...
	   call lsyssc_duplicateVector (rsolution%RvectorBlock(5),temp_rsolution%RvectorBlock(3),&
           ...
           call ppns2D_bdforces_vol(temp_rsolution,rcharfct,Dforces1,&
              dbdForcesCoeff1,dbdForcesCoeff2,cformulation1)
!
	   call spdiscr_releaseBlockDiscr(p_sub_rdiscretisation)
	   call lsysbl_releaseVector(temp_rsolution)
...
! 
	   ...
	   call lsyssc_duplicateVector (rsolution%RvectorBlock(5),temp_rsolution%RvectorBlock(3),&
          ...
          call ppns2D_bdforces_vol(temp_rsolution,rcharfct,Dforces2,&
				dbdForcesCoeff3,dbdForcesCoeff4,cformulation2)
!
	   call spdiscr_releaseBlockDiscr(p_sub_rdiscretisation)
	   call lsysbl_releaseVector(temp_rsolution)
...
! ......................... ///////////////////  .................................................
      ...
      call output_line ('-----------')
      call output_line ('Body forces real bd., bdc/horiz/vert/horiz/vert',&
          coutputMode=OU_MODE_STD+OU_MODE_BENCHLOG)
      call output_line (' '//trim(sys_siL(ibodyForcesBdComponent,10)) // ' / ' &
          //trim(sys_sdEP(Dforces1(1),15,6)) // ' / '&
          //trim(sys_sdEP(Dforces1(2),15,6)) // ' / '&
          //trim(sys_sdEP(Dforces2(1),15,6)) // ' / '&
          //trim(sys_sdEP(Dforces2(2),15,6)),&
          coutputMode=OU_MODE_STD+OU_MODE_BENCHLOG )
      ...
      if (iwriteBodyForces .ne. 0) then
        ...
        if ((cflag .eq. SYS_REPLACE) .or. (.not. bfileexists)) then
          ! Write a headline
          write (iunit,'(A)') '# timestep time bdc horiz vert horiz vert'
        end if
        stemp = trim(sys_siL(rproblem%rtimedependence%itimeStep,10)) // ' ' &
            // trim(sys_sdEL(dtime,10)) // ' ' &
            // trim(sys_siL(ibodyForcesBdComponent,10)) // ' ' &
            // trim(sys_sdEL(Dforces1(1),10)) // ' '&
            // trim(sys_sdEL(Dforces1(2),10)) // ' '&
            // trim(sys_sdEL(Dforces2(1),10)) // ' '&
            // trim(sys_sdEL(Dforces2(2),10))
        write (iunit,'(A)') trim(stemp)
        close (iunit)
      end if
...
!</subroutine>
    ...
    if (rsolution%p_rblockDiscr%RspatialDiscr(1)% &
      ...
      case (EL_Q1T, EL_P1T)
        ...
        call lsyssc_createVecByDiscr (rsolution%RvectorBlock(5)%p_rspatialDiscr,&
            rtempVector,.true.)

        ! Calculate divergence =  D1 u1 + D2 u2 + D3 u3 + D4 u4
        call lsyssc_scalarMatVec (&
            rproblem%RlevelInfo(rproblem%nlmax)%rasmTempl%rmatrixD1, rsolution%RvectorBlock(1), &
            rtempVector, rproblem%rphysics%dmu1, 0.0_DP)
        call lsyssc_scalarMatVec (&
            rproblem%RlevelInfo(rproblem%nlmax)%rasmTempl%rmatrixD2, rsolution%RvectorBlock(2), &
            rtempVector, rproblem%rphysics%dmu1, 1.0_DP)
        call lsyssc_scalarMatVec (&
            rproblem%RlevelInfo(rproblem%nlmax)%rasmTempl%rmatrixD3, rsolution%RvectorBlock(3), &
            rtempVector, rproblem%rphysics%dmu2, 1.0_DP)
        call lsyssc_scalarMatVec (&
            rproblem%RlevelInfo(rproblem%nlmax)%rasmTempl%rmatrixD4, rsolution%RvectorBlock(4), &
            rtempVector, rproblem%rphysics%dmu2, 1.0_DP)
...
!</subroutine>
    ...
    character(LEN=10), dimension(5,3), parameter :: Sfctnames = reshape (&
      (/ "       u1 ","     u1_x ","     u1_y " , &
         "       u2 ","     u2_x ","     u2_y " , &
         "       u3 ","     u3_x ","     u3_y " , &
         "       u4 ","     u4_x ","     u4_y " , &
...
!</subroutine>
    ...
    integer :: iderType, cflag
    logical :: bfileExists
    real(dp), dimension(:), allocatable :: Dvalues1
...
!***************************////////////////***************************  
    ...
    ! Allocate memory for the values
    allocate(Dvalues1(nlines))
    allocate(Dvalues2(nlines))
...
!****************//////////////////////////////*********************************
    allocate(p_sub_rdiscretisation)
!
    call spdiscr_initBlockDiscr (p_sub_rdiscretisation,2,&
	     rsolution%p_rblockDiscr%p_rtriangulation, rproblem%rboundary)
! 
    call spdiscr_duplicateDiscrSc(rsolution%p_rblockDiscr%RspatialDiscr(1),&
	    p_sub_rdiscretisation%RspatialDiscr(1))
...
!****************//////////////////////////////*********************************
    ...
    ! Calculate the flux
    do i=1,nlines
      call ppns2D_calcFluxThroughLine (temp_rsolution,Dcoords(1:2,i,1),Dcoords(1:2,i,2),Dvalues1(i))
    end do
!****************//////////////////////////////*********************************
! 			for the Dforces 2: 
    call lsyssc_duplicateVector (rsolution%RvectorBlock(3),temp_rsolution%RvectorBlock(1),&
    ...
    ! Calculate the flux
    do i=1,nlines
      call ppns2D_calcFluxThroughLine (temp_rsolution,Dcoords(1:2,i,1),Dcoords(1:2,i,2),Dvalues2(i))
    end do
...
! 	release the unnecessary things
    ...
    call output_line ('-----------')
    do i=1,nlines
      write (sstr,"(A,F9.4,A,F9.4,A,F9.4,A,F9.4,A,E16.10)") "flux_NVS1 (",&
          Dcoords(1,i,1),",",Dcoords(2,i,1),")->(",&
          Dcoords(1,i,2),",",Dcoords(2,i,2),") = ",Dvalues1(i)
      call output_line(trim(sstr),coutputMode=OU_MODE_STD+OU_MODE_BENCHLOG )
    end do
!
    do i=1,nlines
      write (sstr,"(A,F9.4,A,F9.4,A,F9.4,A,F9.4,A,E16.10)") "flux_NVS2 (",&
          Dcoords(1,i,1),",",Dcoords(2,i,1),")->(",&
          Dcoords(1,i,2),",",Dcoords(2,i,2),") = ",Dvalues2(i)
      call output_line(trim(sstr),coutputMode=OU_MODE_STD+OU_MODE_BENCHLOG )
    end do
!    
    ! Get information about writing the stuff into a DAT file.
    ...
    if (iwriteFluxValues .ne. 0) then
      ...
      if ((cflag .eq. SYS_REPLACE) .or. (.not. bfileexists)) then
        ! Write a headline
        write (iunit,'(A)') &
          '# timestep time x1 y1 x2 y2 value1_NVS1 value1_NVS2 x1 y1 x2 y2 value2_NVS1 value2_NVS2 ...'
      end if
      stemp = &
          trim(sys_siL(rproblem%rtimedependence%itimeStep,10)) // ' ' // &
          trim(sys_sdEL(dtime,10))
      ...
      do i=1,nlines
        stemp = ' ' //&
            ...
            trim(sys_sdEL(Dvalues1(i),10)) // ' ' // &
	    trim(sys_sdEL(Dvalues2(i),10))
        write (iunit,ADVANCE='NO',FMT='(A)') trim(stemp)
      end do
      write (iunit,ADVANCE='YES',FMT='(A)') ""
      close (iunit)
...
!<description>
  ! Writes a UCD postprocessing file as configured in the DAT file.
  ! (-> GMV, AVS, Paraview,...)
!</description>
...
!</subroutine>
    ...
    ! a GMV file.
    real(DP), dimension(:), pointer :: p_Ddata,p_Ddata2,p_Ddata3,p_Ddata4
    ...
    character(SYS_STRLEN) :: sfile,sfilename,sfile2,sfilename2
    
!******************************/////////////**************************
    type(t_blockDiscretisation), pointer :: p_sub_rdiscretisation
    type(t_vectorBlock), target :: temp_rvector
!***************************////////////////***************************  
    ...
    case (1)
      ...
      call ucd_startGMV (rexport2,UCD_FLAG_STANDARD,p_rtriangulation,sfile2)

    case (2)
      call ucd_startAVS (rexport,UCD_FLAG_STANDARD,p_rtriangulation,sfile)
      call ucd_startAVS (rexport2,UCD_FLAG_STANDARD,p_rtriangulation,sfile2)
          
    case (3)
      call ucd_startVTK (rexport,UCD_FLAG_STANDARD,p_rtriangulation,sfile)
      call ucd_startVTK (rexport2,UCD_FLAG_STANDARD,p_rtriangulation,sfile2)

    case (5)
      call ucd_startBGMV (rexport,UCD_FLAG_STANDARD,p_rtriangulation,sfile)
      call ucd_startBGMV (rexport2,UCD_FLAG_STANDARD,p_rtriangulation,sfile2)
    ...
    if (present(dtime)) then
      ...
      call ucd_setSimulationTime (rexport2,dtime)
    end if
    ...
    call ucd_addCommentLine (rexport2,'Configuration: ')
    call ucd_addCommentLine (rexport2,'---------------')
    call ucd_addParameterList (rexport2,rproblem%rparamList)
    call ucd_addCommentLine (rexport2,'---------------')
...
!    end if
    ...
    call ucd_addVarVertBasedVec (rexport,'velocity_NVS1',&
        p_Ddata(1:p_rtriangulation%NVT),p_Ddata2(1:p_rtriangulation%NVT))

    call ucd_addVarVertBasedVec (rexport,'velocity_NVS2',&
        p_Ddata3(1:p_rtriangulation%NVT),p_Ddata4(1:p_rtriangulation%NVT))

    call ucd_addVarVertBasedVec (rexport2,'velocity_NVS2',&
        p_Ddata3(1:p_rtriangulation%NVT),p_Ddata4(1:p_rtriangulation%NVT))
    ...
    call ucd_addVariableElementBased (rexport2,'pressure',UCD_VAR_STANDARD, &
        p_Ddata(1:p_rtriangulation%NEL))

    ! If we have a simple Q1 discretisation in the pressure, write it out as it is
    if (rvector%p_rblockDiscr%RspatialDiscr(5)% &
        ccomplexity .eq. SPDISC_UNIFORM) then
      ieltype = rvector%p_rblockDiscr%RspatialDiscr(5)% &
                RelementDistr(1)%celement
      ...
      if (elem_getPrimaryElement(ieltype) .eq. EL_Q1) then
        ...
        call ucd_addVariableVertexBased (rexport,'pressure',UCD_VAR_STANDARD, &
            p_Ddata(1:p_rtriangulation%NVT))
        call ucd_addVariableVertexBased (rexport2,'pressure',UCD_VAR_STANDARD, &
            p_Ddata(1:p_rtriangulation%NVT))
      else
        ! If this is QP1 or something else, project to Q1.
        call lsyssc_getbase_double (rprjVector%RvectorBlock(1),p_Ddata)
        call spdp_projectToVertices (rvector%RvectorBlock(5),p_Ddata)
        call ucd_addVariableVertexBased (rexport,'pressure',UCD_VAR_STANDARD, &
            p_Ddata(1:p_rtriangulation%NVT))
        call ucd_addVariableVertexBased (rexport2,'pressure',UCD_VAR_STANDARD, &
            p_Ddata(1:p_rtriangulation%NVT))
...
!
	call spdiscr_initBlockDiscr (p_sub_rdiscretisation,2,&
	     rvector%p_rblockDiscr%p_rtriangulation, rproblem%rboundary)
! 
	call spdiscr_duplicateDiscrSc(rvector%p_rblockDiscr%RspatialDiscr(1),&
	    p_sub_rdiscretisation%RspatialDiscr(1))
	...
	call lsysbl_createVecBlockByDiscr(p_sub_rdiscretisation,temp_rvector)

	call lsyssc_duplicateVector (rvector%RvectorBlock(1),temp_rvector%RvectorBlock(1),&
				LSYSSC_DUP_COPY,LSYSSC_DUP_SHARE)

	call lsyssc_duplicateVector (rvector%RvectorBlock(2),temp_rvector%RvectorBlock(2),&
				LSYSSC_DUP_COPY,LSYSSC_DUP_SHARE)

!****************//////////////////////////////*********************************          
        call ppns2D_streamfct_uniform (temp_rvector,rprjVector%RvectorBlock(1))
        
        call lsyssc_getbase_double (rprjVector%RvectorBlock(1),p_Ddata)
        call ucd_addVariableVertexBased (rexport,'streamfunction_NVS1',&
            UCD_VAR_STANDARD, p_Ddata(1:p_rtriangulation%NVT))

!  		repeat the same steps but for NVS2    
	call lsysbl_createVecBlockByDiscr(p_sub_rdiscretisation,temp_rvector)

	call lsyssc_duplicateVector (rvector%RvectorBlock(3),temp_rvector%RvectorBlock(1),&
				LSYSSC_DUP_COPY,LSYSSC_DUP_SHARE)

	call lsyssc_duplicateVector (rvector%RvectorBlock(4),temp_rvector%RvectorBlock(2),&
				LSYSSC_DUP_COPY,LSYSSC_DUP_SHARE)  
 
        call ppns2D_streamfct_uniform (temp_rvector,rprjVector%RvectorBlock(1))   
        
        call lsyssc_getbase_double (rprjVector%RvectorBlock(1),p_Ddata)
        call ucd_addVariableVertexBased (rexport2,'streamfunction_NVS2',&
            UCD_VAR_STANDARD, p_Ddata(1:p_rtriangulation%NVT))
