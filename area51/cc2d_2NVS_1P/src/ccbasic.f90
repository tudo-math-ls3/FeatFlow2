...
!<typeblock>
  ...
  type t_asmTemplates
    ...
    type(t_matrixScalar) :: rmatrixStokes1

    ! Precalculated Stokes matrix for that specific level (=nu2*Laplace) / Obaid 12.11.2011
    type(t_matrixScalar) :: rmatrixStokes2
    ...
    ! Precalculated B3-matrix for that specific level.  obaid 12.10.2011
    type(t_matrixScalar) :: rmatrixB3
    
    ! Precalculated B4-matrix for that specific level. obaid 12.10.2011
    type(t_matrixScalar) :: rmatrixB4
    ...
    ! Precalculated D3-matrix for that specific level.  obaid 12.10.2011
    type(t_matrixScalar) :: rmatrixD3

    ! Precalculated D4-matrix for that specific level.  obaid 12.10.2011 
    type(t_matrixScalar) :: rmatrixD4
    ...
    ! Precalculated D3^T-matrix for that specific level. 
    ! This matrix either coincides with rmatrixB3 (in case D3=B3^T)
    ! or describes an independent D3 matrix.	obaid 12.10.2011
    type(t_matrixScalar) :: rmatrixD3T

    ! Precalculated D4-matrix for that specific level. 
    ! This matrix either coincides with rmatrixB4 (in case D4=B4^T)
    ! or describes an independent D4 matrix.	obaid 12.10.2011
    type(t_matrixScalar) :: rmatrixD4T
    ...
    type(t_matrixScalar) :: rmatrixMass1

    ! Precalculated mass matrix for the velocity space of the 2nd equation/ Obaid 12.10.2011
    type(t_matrixScalar) :: rmatrixMass2
    ...
    type(t_matrixScalar) :: rmatrixStabil1
    type(t_matrixScalar) :: rmatrixStabil2 ! to be reviewed letter on
    
    ! A prolongation matrix for the velocity.
    type(t_matrixScalar) :: rmatrixProlVelocity1

    ! A prolongation matrix for the velocity 2.  obaid 12.10.2011 !!!!
    type(t_matrixScalar) :: rmatrixProlVelocity2
    
    ! A prolongation matrix for the pressure.
    type(t_matrixScalar) :: rmatrixProlPressure
    
    ! An interpolation matrix for the velocity.
    type(t_matrixScalar) :: rmatrixInterpVelocity1

    ! An interpolation matrix for the velocity2.  obaid 12.10.2011 !!!! ?
    type(t_matrixScalar) :: rmatrixInterpVelocity2
...
!<typeblock>
  ...
  type t_rhsAssembly
    ...
    ! =-1 if this is undetermined.type t_problem_physics
...
!<typeblock>
  ...
  type t_problem_physics
    ...
    ! Viscosity parameter nu2 = 1/Re2  for the seond equation. Obaid 12.11.2011
    real(DP) :: dnu2
    ...
    ! Type of problem for the second equation.  Obaid 12.10.2011
    ! =0: Stokes.
    ! =1: Navier-Stokes.
    integer :: iequation2
    ...
    integer :: isubEquation1
    
    ! Type of subproblem of the main problem. Depending on iequationType. For the 2nd equation 
    !                                                                       / Obaid 12.10.2011
    ! If iequationType2=0 or =1:
    ! =0: (Navier-)Stokes with gradient tensor
    ! =1: (Navier-)Stokes with deformation tensor
    integer :: isubEquation2
    ...
    integer :: cviscoModel1

    ! Model for the viscosity. of the second equation / Obaid 12.10.2011
    ! =0: Constant viscosity.
    ! =1: Power law: nu = nu_0 * z^(dviscoexponent/2 - 1), nu_0 = 1/RE, z=||D(u)||^2+dviscoEps
    ! =2: Bingham fluid: nu = nu_0 + dviscoyield / sqrt(|D(u)||^2+dviscoEps^2), nu_0 = 1/RE
    integer :: cviscoModel2
        
    ! Exponent parameter for the viscosity model
    real(DP) :: dviscoexponent1

    ! Exponent parameter for the viscosity model for the 2nd equation/ Obaid 12.10.2011
    real(DP) :: dviscoexponent2

    ! Epsilon regularisation for the viscosity model
    real(DP) :: dviscoEps1

    ! Epsilon regularisation for the viscosity model of the 2nd equation/ Obaid 12.10.2011
    real(DP) :: dviscoEps2
    
    ! Yield stress for Bingham fluid
    real(DP) :: dviscoYield1

    ! Yield stress for Bingham fluid of the 2nd equation / Obaid 12.10.2011
    real(DP) :: dviscoYield2
