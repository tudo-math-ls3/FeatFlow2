...
!</subroutine>
    ...
    call parlst_getvalue_int (rproblem%rparamList,'CC-DISCRETISATION',&
                              'iEquation1',rproblem%rphysics%iequation1,0)
    call parlst_getvalue_int (rproblem%rparamList,'CC-DISCRETISATION',&
                              'iEquation2',rproblem%rphysics%iequation2,0)


    ! Type of subproblem (gradient tensor, deformation tensor,...)
    call parlst_getvalue_int (rproblem%rparamList,'CC-DISCRETISATION',&
                              'isubEquation1',rproblem%rphysics%isubEquation1,0)
    call parlst_getvalue_int (rproblem%rparamList,'CC-DISCRETISATION',&
                              'isubEquation2',rproblem%rphysics%isubEquation2,0)
    ...
    call parlst_getvalue_int (rproblem%rparamList,'CC-DISCRETISATION',&
                                 'cviscoModel1',rproblem%rphysics%cviscoModel1,0)
    call parlst_getvalue_int (rproblem%rparamList,'CC-DISCRETISATION',&
                                 'cviscoModel2',rproblem%rphysics%cviscoModel2,0)

                                 
    call parlst_getvalue_double (rproblem%rparamList,'CC-DISCRETISATION',&
                                 'dviscoexponent1',rproblem%rphysics%dviscoexponent1,2.0_DP)
    call parlst_getvalue_double (rproblem%rparamList,'CC-DISCRETISATION',&
                                 'dviscoexponent2',rproblem%rphysics%dviscoexponent2,2.0_DP)

                                 
    call parlst_getvalue_double (rproblem%rparamList,'CC-DISCRETISATION',&
                                 'dviscoEps1',rproblem%rphysics%dviscoEps1,0.01_DP)
    call parlst_getvalue_double (rproblem%rparamList,'CC-DISCRETISATION',&
                                 'dviscoEps2',rproblem%rphysics%dviscoEps2,0.01_DP)


    call parlst_getvalue_double (rproblem%rparamList,'CC-DISCRETISATION',&
                                 'dviscoYield1',rproblem%rphysics%dviscoYield1,1.0_DP)
    call parlst_getvalue_double (rproblem%rparamList,'CC-DISCRETISATION',&
                                 'dviscoYield2',rproblem%rphysics%dviscoYield2,1.0_DP)

    ! Get the 2 constants of the 2 divergence terms, save them to the problem structure
    ! as well as into the collection.
    ! Note that the parameter in the DAT file is 1/nu !
    call parlst_getvalue_double (rproblem%rparamList,'CC-DISCRETISATION',&
                                 'dmu1',rproblem%rphysics%dmu1,0.5_DP)
    call parlst_getvalue_double (rproblem%rparamList,'CC-DISCRETISATION',&
                                 'dmu2',rproblem%rphysics%dmu2,0.5_DP)

    ! Get the viscosity parameter, save it to the problem structure
    ! as well as into the collection.
    ! Note that the parameter in the DAT file is 1/nu !
    call parlst_getvalue_double (rproblem%rparamList,'CC-DISCRETISATION',&
                                 'RE1',rproblem%rphysics%dnu1,1000.0_DP)
    call parlst_getvalue_double (rproblem%rparamList,'CC-DISCRETISATION',&
                                 'RE2',rproblem%rphysics%dnu2,1000.0_DP)

    
    rproblem%rphysics%dnu1 = 1.0_DP/rproblem%rphysics%dnu1
    rproblem%rphysics%dnu2 = 1.0_DP/rproblem%rphysics%dnu2
