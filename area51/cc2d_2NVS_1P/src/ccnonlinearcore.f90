...
!<typeblock>
  ...
  ! When a callback routine is called, the structure is rebuilt from
  ...
  type t_ccNonlinearIteration
    ...
    ! ALPHA-parameter that controls the weight of the mass matrices (M1 & M2)
    ! in the core equation. =0.0 for stationary simulations.
    real(DP) :: dalpha1 = 0.0_DP
    real(DP) :: dalpha2 = 0.0_DP
    
    ! THETA-parameter that controls the weight of the Stokes matrices (L1 & L2)
    ...
    real(DP), dimension(3) :: DresidualInit = 0.0_DP
    
    ! Auxiliary variable: Saves the last defect in the nonlinear iteration
    real(DP), dimension(3) :: DresidualOld = 0.0_DP
...
contains
  ...
  !</subroutine>
      ...
      rnonlinearCCMatrix%dalpha1 = rnonlinearIteration%dalpha1
      rnonlinearCCMatrix%dalpha2 = rnonlinearIteration%dalpha2
      rnonlinearCCMatrix%dtheta1 = rnonlinearIteration%dtheta1
      rnonlinearCCMatrix%dtheta2 = rnonlinearIteration%dtheta2
      rnonlinearCCMatrix%dgamma1 = rnonlinearIteration%dgamma1
      rnonlinearCCMatrix%dgamma2 = rnonlinearIteration%dgamma2
      rnonlinearCCMatrix%deta1 = rnonlinearIteration%deta1
      rnonlinearCCMatrix%deta2 = rnonlinearIteration%deta2
      rnonlinearCCMatrix%dtau1 = rnonlinearIteration%dtau1
      rnonlinearCCMatrix%dtau2 = rnonlinearIteration%dtau2
...
!    ilvmax = collct_getvalue_int (p_rcollection,'NLMAX')
      ...
      !
      ! (u1)     (u1)                     ( (f1)   [ A11  0    0    0    B1] (u1) )
      ! (u2)     (u2)			  ( (f2)   [ 0    A22  0    0    B2] (u2) )
      ! (v1)  := (v1)  + OMEGA * C^{-1} * ( (f3) - [ 0    0    A33  0    B3] (v1) )
      ! (v2)     (v2)			  ( (f4)   [ 0    0    0    A44  B4] (v2) )
      ! (p )     (p )                     ( (fp)   [ D1   D2   D3   D4   0 ] (p ) )
      !
      !                                   |----------------------------------------|
      !                                              = d_n
      !                            |-----------------------------------------------|
      !                                        = Y = (y1,y2,y3,y4,yp) = rd
      !
      ! with KST1=KST1(u1,u2,v1,v2,p) and Y=rd being the solution from
      ! the Oseen equation with
      !
      !                  [ A11  0    0    0    B1]
      !    C = T(u_n) =  [ 0    A22  0    0    B2]
      !			 [ 0    0    A33  0    B3]
      !			 [ 0    0    0    A44  B4]
      !                  [ D1   D2   D3   D4   0 ]
      !
  ...
  !</subroutine>
      ...
      case (CCPREC_LINEARSOLVER,CCPREC_NEWTON,CCPREC_NEWTONDYNAMIC)
        ...
        if (rnonlinearIteration%rpreconditioner%ctypePreconditioning .eq. &
          ...
          if (p_rnewton%cinexactNewton .ne. 0) then
            ...
            ! Just for safeness, gain at least one digit.
            ...
            if (associated(p_rcgrSolver)) then
              ...
              ! But just for safeness, the coarse grid solver should gain at least
              ! one digit!
        ...
        ! Note that no pointers and no handles are changed, so we can safely do
    ...
    !    call lsysbl_getbase_double (rx,p_vec)
        ...
        do ilev=NLMAX,NLMIN,-1
          ...
          if (bassembleNewton) rnonlinearCCMatrix%dnewton1 = rnonlinearIteration%dgamma1
          if (bassembleNewton) rnonlinearCCMatrix%dnewton2 = rnonlinearIteration%dgamma2
        ...
        if (rnonlinearIteration%rprecSpecials%bpressureIndefinite) then
          ...
          ! The 5,5-matrix must exist! This is ensured by the initialisation routine.
          !
          ...
          if (rnonlinearIteration%rprecSpecials%isolverType .eq. 0) then
            ...
            call mmod_replaceLinesByUnitBlk (&
                rnonlinearIteration%RcoreEquation(NLMAX)%p_rmatrixPreconditioner,5,Irows)
          end if
          ...
          if (rnonlinearIteration%rprecSpecials%isolverType .eq. 1) then
            ...
            if (rnonlinearIteration%rprecSpecials%icoarseGridSolverType .eq. 0) then
              ! Include a unit vector
              call mmod_replaceLinesByUnitBlk (&
                  rnonlinearIteration%RcoreEquation(NLMIN)%p_rmatrixPreconditioner,5,Irows)
            end if
          ...
          if (lsysbl_isSubmatrixPresent(p_rmatrix,5,5)) &
            call lsyssc_clearMatrix (p_rmatrix%RmatrixBlock(5,5))
  ...
  !<inputoutput>
    ...
    ! The nonlinear solver node that configures tDirichlet0he solution process.
    type(t_nlsolNode), intent(inout)              :: rsolverNode
  ...
  !<output>
    ...
    ! Must be set to TRUE by the callback routineDirichlet0 if the residuum rd
  ...
  !</output>
      ...
      real(DP) :: dresOld,drhoNL,ddelP,ddelU,ddelV,dtmp_u,dtmp_v,dtmp_p
      real(DP) :: dresU,dresV,dresDIV,dres,dresINIT
      real(DP) :: depsD_u,depsD_v,depsDiv,depsUR,depsVR,depsPR,depsRES
      integer, dimension(5) :: Cnorms

      ! Calculate norms of the solution/defect vector
      call cc_getDefectNorm (rx,rb,rd,Dresiduals)
      
      Dresiduals(4) = sqrt(Dresiduals(1)**2 + Dresiduals(2)**2 + Dresiduals(3)**2)
      ...
      if (ite .eq. 0) then
        ...
        call output_line (' IT  RELU     RELV     RELP     DEF-U    DEF-V    '// &
                          'DEF-DIV  DEF-TOT  RHONL    OMEGNL   RHOMG',&
	    ...
	    trim(sys_sdEP(Dresiduals(3),9,2))//&
            trim(sys_sdEP(Dresiduals(4),9,2)),coutputMode=rsolverNode%coutputMode)
        call output_separator (OU_SEP_MINUS,coutputMode=rsolverNode%coutputMode)

        rnonlinearIteration%DresidualInit (1:3) = Dresiduals(1:3)
        rnonlinearIteration%DresidualOld (1:3) = Dresiduals(1:3)
        ...
        drhoNL = (Dresiduals(4)/dresINIT) ** (1.0_DP/real(ite,DP))
        
        ! Calculate norms of the solution/defect vector, calculated above
        dresU   = Dresiduals(1)
	dresV   = Dresiduals(2)
        dresDIV = Dresiduals(3)
        dres    = sqrt(dresU**2 + dresV**2 + dresDIV**2)
	...
	! Relative change of solution vector:
        !
        !            || (Y3,Y4) ||_max    || Vnew - Vold ||_max
        !   DELU := ------------------- = --------------------- 
        !           || (KU3,KU4) ||_max       || Vnew ||_max
	!
        ...
        dtmp_u = max(Dresiduals(1),Dresiduals(2))
	dtmp_v = max(Dresiduals(3),Dresiduals(4))
        if (dtmp_u .lt. 1.0E-8_DP) dtmp_u = 1.0_DP
        if (dtmp_v .lt. 1.0E-8_DP) dtmp_v = 1.0_DP
        ddelU = max(rnonlinearIteration%DresidualCorr(1),&
                    rnonlinearIteration%DresidualCorr(2))/dtmp_u
        ddelV = max(rnonlinearIteration%DresidualCorr(3),&
                    rnonlinearIteration%DresidualCorr(4))/dtmp_v
        
        dtmp_p = Dresiduals(5)
        if (dtmp_p .lt. 1.0E-8_DP) dtmp_p = 1.0_DP
        ddelP = rnonlinearIteration%DresidualCorr(5)/dtmp_p
        ...
        depsD_u = rnonlinearIteration%DepsNL(1)
	depsD_v = rnonlinearIteration%DepsNL(2)
        depsDiv = rnonlinearIteration%DepsNL(3)
	depsUR  = rnonlinearIteration%DepsNL(4)
        depsVR  = rnonlinearIteration%DepsNL(5)
        depsPR  = rnonlinearIteration%DepsNL(6)
        depsRES = rnonlinearIteration%DepsNL(7)*dresINIT ! -> ddampingD
        ...
        if((ddelU .le. depsUR).and.(ddelV .le. depsVR).and.  &
           (ddelP .le. depsPR).and.(dresU .le. depsD_u).and. &
	   (dresV .le. depsD_v).and.(dresDiv .le. depsDiv).and. &
	    ...
	    (ddelV .lt. SYS_EPSREAL_DP*1E2_DP) .and. &
        ...
        call output_line ( &
            ...
            trim(sys_sdEP(ddelV,9,2))// &
            trim(sys_sdEP(ddelP,9,2))// &
            trim(sys_sdEP(dresU,9,2))// &
            trim(sys_sdEP(dresV,9,2))// &
...
!<output>
  ...
  ! Dresiduals(1) = RESU   = ||defect_u|| / ||rhs_u|| = displacement residual
  ! Dresiduals(2) = RESV   = ||defect_v|| / ||rhs_v|| = velocity residual
  ! Dresiduals(3) = RESDIV = ||p|| / ||[u v]||          = divergence residual
...
!</subroutine>
    ...
    ! local variables
    real(DP) :: dresF_u,dresF_v,DresTmp(4),dnormUV
    ...
    ! RESF_u := max ( ||F1||_E , ||F2||_E )
    ! RESF_v := max ( ||F3||_E , ||F4||_E )

    call lsysbl_vectorNormBlock (rrhs,Cnorms,DresTmp)
    dresF_u = max(DresTmp(1),DresTmp(2))
    dresF_v = max(DresTmp(3),DresTmp(4))
    if (dresF_u .lt. 1.0E-8_DP) dresF_u = 1.0_DP
    if (dresF_v .lt. 1.0E-8_DP) dresF_v = 1.0_DP
...
!
    ...
    Dresiduals(1) = sqrt(DresTmp(1)**2+DresTmp(2)**2)/dresF_u
    Dresiduals(2) = sqrt(DresTmp(3)**2+DresTmp(4)**2)/dresF_v

    ! DNORMU = || (U1,U2) ||_l2

    call lsysbl_vectorNormBlock (rvector,Cnorms,DresTmp)
    dnormUV = sqrt(DresTmp(1)**2+DresTmp(2)**2+DresTmp(3)**2+DresTmp(4)**2)
    if (dnormUV .lt. 1.0E-8_DP) dnormUV = 1.0_DP
    ...
    Dresiduals(3) = &
        lsyssc_vectorNorm (rdefect%RvectorBlock(5),LINALG_NORMEUCLID) / dnormUV
...
!</subroutine>
    ...
    rnlSolver%DepsRel(2) = rnlSolver%DepsRel(1) ! for x and y components

    call parlst_getvalue_double (p_rsection, 'depsVR', &
                                 rnlSolver%DepsRel(3), rnlSolver%DepsRel(3))
    rnlSolver%DepsRel(4) = rnlSolver%DepsRel(3) ! for x and y components


    call parlst_getvalue_double (p_rsection, 'depsPR', &
                                 rnlSolver%DepsRel(5), rnlSolver%DepsRel(5))

    call parlst_getvalue_double (p_rsection, 'depsD_u', &
                                 rnlSolver%DepsAbs(1), rnlSolver%DepsAbs(1))
    rnlSolver%DepsAbs(2) = rnlSolver%DepsAbs(1) ! for x and y components

    call parlst_getvalue_double (p_rsection, 'depsD_v', &
                                 rnlSolver%DepsAbs(3), rnlSolver%DepsAbs(3))
    rnlSolver%DepsAbs(4) = rnlSolver%DepsAbs(3) ! for x and y components

    call parlst_getvalue_double (p_rsection, 'depsDiv', &
                                 rnlSolver%DepsAbs(5), rnlSolver%DepsAbs(5))
