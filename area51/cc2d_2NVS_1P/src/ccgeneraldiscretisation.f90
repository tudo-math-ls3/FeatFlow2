...
!</subroutine>
  ...
  character(LEN=SYS_NAMELEN) :: sstr
    ...
    do i=rproblem%NLMIN,rproblem%NLMAX
      ...
      call spdiscr_duplicateDiscrSc (p_rdiscretisation%RspatialDiscr(5), &
...
!<input>
  ...
  ! Cubature formula for the velocity1 & velocity2 matrices
...
!</subroutine>
  ...
  ! Number of equations in our problem. 2 velocities (for nvs1) + 
!                          2 velocities (for nvs2) + pressure = 5
  integer, parameter :: nequations = 5
  ...
  logical :: btriallowed, bquadallowed
    ...
    !  Component 1 = X-velocity for nvs1
    !  Component 2 = Y-velocity for nvs1
    !  Component 3 = X-velocity for nvs2
    !  Component 4 = Y-velocity for nvs2
    !  Component 5 = Pressure
    ...
    call spdiscr_duplicateDiscrSc(rdiscretisation%RspatialDiscr(1),&
        rdiscretisation%RspatialDiscr(3)) ! added by obaid
    call spdiscr_duplicateDiscrSc(rdiscretisation%RspatialDiscr(1),&
        rdiscretisation%RspatialDiscr(4)) ! added by obaid
    ...
    call spdiscr_deriveSimpleDiscrSc (rdiscretisation%RspatialDiscr(1),  &
        ieltypeP, icubB,rdiscretisation%RspatialDiscr(5))
...
!</subroutine>
  ...
  ! Number of equations in our problem. velocity+velocity+pressure = 5
  integer, parameter :: nequations = 5
  ...
  integer(I32) :: ieltypeUV, ieltypeP
    ...
    call spdiscr_deriveSimpleDiscrSc (rsourceDiscretisation%RspatialDiscr(3),  &
        ieltypeUV, SPDISC_CUB_NOCHANGE,rdestDiscretisation%RspatialDiscr(3))
    ...
    call spdiscr_deriveSimpleDiscrSc (rsourceDiscretisation%RspatialDiscr(5),  &
        ieltypeP, SPDISC_CUB_NOCHANGE,rdestDiscretisation%RspatialDiscr(5))
...
!<inputoutput>
  ...
  ! A t_asmTemplates structure. The matrices in this structure iprojTypeVelocityare generated.
...
!</subroutine>
  ...
  ! local variablescc_allocTemplateMatrices
  integer :: cmatBuildType,istrongDerivativeBmatrix
  integer :: iprojTypeVelocity1,iprojTypeVelocity2, iprojTypePressure
    ...
    call parlst_getvalue_int (rproblem%rparamList, 'CC-PROLREST', &
        'IPROJTYPEVELOCITY1', iprojTypeVelocity1, 0)
    call parlst_getvalue_int (rproblem%rparamList, 'CC-PROLREST', &
        'IPROJTYPEVELOCITY2', iprojTypeVelocity2, 0)
    call parlst_getvalue_int (rproblem%rparamList, 'CC-PROLREST', &
    ...
    ! Create the matrices structure of the pressure using the 5th
    ! spatial discretisation structure in rdiscretisation%RspatialDiscr.
    call bilf_createMatrixStructure (&
              rdiscretisation%RspatialDiscr(5),LSYSSC_MATRIX9,&
    ...
    call lsyssc_duplicateMatrix (rasmTempl%rmatrixTemplateFEM,&
                rasmTempl%rmatrixStokes1,LSYSSC_DUP_SHARE,LSYSSC_DUP_REMOVE)

    call lsyssc_duplicateMatrix (rasmTempl%rmatrixTemplateFEM,&
                rasmTempl%rmatrixStokes2,LSYSSC_DUP_SHARE,LSYSSC_DUP_REMOVE)
    
    ! Allocate memory for the entries; do not initialise the memory.
    call lsyssc_allocEmptyMatrix (rasmTempl%rmatrixStokes1,LSYSSC_SETM_UNDEFINED)
    call lsyssc_allocEmptyMatrix (rasmTempl%rmatrixStokes2,LSYSSC_SETM_UNDEFINED)
    
    ! In the global system, there are four coupling matrices B1 ... B4.
    ! Both have the structure of the template gradient matrix.
    ! So connect the four B-matrices to the template gradient matrix
    ...
    call lsyssc_duplicateMatrix (rasmTempl%rmatrixTemplateGradient,&
        rasmTempl%rmatrixB3,LSYSSC_DUP_SHARE,LSYSSC_DUP_REMOVE)
                
    call lsyssc_duplicateMatrix (rasmTempl%rmatrixTemplateGradient,&
        rasmTempl%rmatrixB4,LSYSSC_DUP_SHARE,LSYSSC_DUP_REMOVE)
    ...
    call lsyssc_allocEmptyMatrix (rasmTempl%rmatrixB3,LSYSSC_SETM_UNDEFINED)
    call lsyssc_allocEmptyMatrix (rasmTempl%rmatrixB4,LSYSSC_SETM_UNDEFINED)
    ...
    call lsyssc_duplicateMatrix (rasmTempl%rmatrixTemplateDivergence,&
        rasmTempl%rmatrixD3,LSYSSC_DUP_SHARE,LSYSSC_DUP_REMOVE)
                
    call lsyssc_duplicateMatrix (rasmTempl%rmatrixTemplateDivergence,&
        rasmTempl%rmatrixD4,LSYSSC_DUP_SHARE,LSYSSC_DUP_REMOVE)
    ...
    call lsyssc_allocEmptyMatrix (rasmTempl%rmatrixD3,LSYSSC_SETM_UNDEFINED)
    call lsyssc_allocEmptyMatrix (rasmTempl%rmatrixD4,LSYSSC_SETM_UNDEFINED)
    
    ! The D1^T ... D4^T matrices are by default the same as B1 ... B4.
    ! but different by scaling factor \mu
    ...
    call lsyssc_duplicateMatrix (rasmTempl%rmatrixB3,&
        rasmTempl%rmatrixD3T,LSYSSC_DUP_SHARE,LSYSSC_DUP_SHARE)
                
    call lsyssc_duplicateMatrix (rasmTempl%rmatrixB4,&
        rasmTempl%rmatrixD4T,LSYSSC_DUP_SHARE,LSYSSC_DUP_SHARE)
    ...
    if(present(rdiscretisationCoarse)) then
      ...
      if(iprojTypeVelocity1 .eq. 1) then
        ...
        call mlop_create2LvlMatrixStruct (&
            ...
            rdiscretisation%RspatialDiscr(1),&
            LSYSSC_MATRIX9, rasmTempl%rmatrixProlVelocity1)
        
        ! And let the interpolation matrix share the structure
        call lsyssc_duplicateMatrix(&
            rasmTempl%rmatrixProlVelocity1, &
            rasmTempl%rmatrixInterpVelocity1, &
            LSYSSC_DUP_SHARE, LSYSSC_DUP_REMOVE)
        
      end if
      
      if(iprojTypeVelocity2 .eq. 1) then
        
        ! Assemble the matrix structure for velocity projection
        call mlop_create2LvlMatrixStruct (&
            rdiscretisationCoarse%RspatialDiscr(3),&
            rdiscretisation%RspatialDiscr(3),&
            LSYSSC_MATRIX9, rasmTempl%rmatrixProlVelocity2)
        
        ! And let the interpolation matrix share the structure
        call lsyssc_duplicateMatrix(&
            rasmTempl%rmatrixProlVelocity2, &
            rasmTempl%rmatrixInterpVelocity2, &
            LSYSSC_DUP_SHARE, LSYSSC_DUP_REMOVE)
      ...
      if(iprojTypePressure .eq. 1) then
        ...
        call mlop_create2LvlMatrixStruct (&
            rdiscretisationCoarse%RspatialDiscr(5),&
...
!    rform%Dcoefficients(1)  = rproblem%dnu1
!    rform%Dcoefficients(2)  = rproblem%dnu1
!
...
!                                 rproblem%rcollection)
    ...
    call stdop_assembleLaplaceMatrix (rasmTempl%rmatrixStokes1,.true.,rproblem%rphysics%dnu1)
    call stdop_assembleLaplaceMatrix (rasmTempl%rmatrixStokes2,.true.,rproblem%rphysics%dnu2)
    ...
    if (istrongDerivativeBmatrix .eq. 0) then
      ...
      call stdop_assembleSimpleMatrix (rasmTempl%rmatrixB3,&
          DER_FUNC,DER_DERIV_X,-1.0_DP)

      ! Build the second pressure matrix B4.
      call stdop_assembleSimpleMatrix (rasmTempl%rmatrixB4,&
          DER_FUNC,DER_DERIV_Y,-1.0_DP)
      
      ! Set up the matrices D1 and D2 by transposing B1 ... B4.
      ...
      call lsyssc_transposeMatrix (rasmTempl%rmatrixB3,&
          rasmTempl%rmatrixD3,LSYSSC_TR_CONTENT)

      call lsyssc_transposeMatrix (rasmTempl%rmatrixB4,&
          rasmTempl%rmatrixD4,LSYSSC_TR_CONTENT)
      ...
      ! Build the 3rd pressure matrix B3.
      call stdop_assembleSimpleMatrix (rasmTempl%rmatrixB3,&
          DER_DERIV_X,DER_FUNC,1.0_DP)
      ...
      ! Build the first divergence matrix D3
      call stdop_assembleSimpleMatrix (rasmTempl%rmatrixD3,&
          DER_FUNC,DER_DERIV_X,-1.0_DP)

      ! Build the second divergence matrix D4
      call stdop_assembleSimpleMatrix (rasmTempl%rmatrixD4,&
          DER_FUNC,DER_DERIV_Y,-1.0_DP)
          
      ! Set up the D1^T ... D4^T matrices as transposed D1 ... D4.
      ...
      call lsyssc_allocEmptyMatrix(rasmTempl%rmatrixD3T,LSYSSC_SETM_UNDEFINED)
      call lsyssc_allocEmptyMatrix(rasmTempl%rmatrixD4T,LSYSSC_SETM_UNDEFINED)
      
      ! Transpose only the content, the structure is the same as B1 ... B4.
      ...
      call lsyssc_transposeMatrix (rasmTempl%rmatrixD3,&
          rasmTempl%rmatrixD3T,LSYSSC_TR_CONTENT)

      call lsyssc_transposeMatrix (rasmTempl%rmatrixD4,&
          rasmTempl%rmatrixD4T,LSYSSC_TR_CONTENT)
    ...
    if (rproblem%rstabilisation%iupwind .eq. CCMASM_STAB_FASTEDGEORIENTED) then
      ...
      rjumpStabil%dnu = rproblem%rphysics%dnu1
      ...
      call lsyssc_duplicateMatrix (rasmTempl%rmatrixTemplateFEM,&
                  rasmTempl%rmatrixStabil1,LSYSSC_DUP_SHARE,LSYSSC_DUP_EMPTY)

      call lsyssc_duplicateMatrix (rasmTempl%rmatrixTemplateFEM,&
                  rasmTempl%rmatrixStabil2,LSYSSC_DUP_SHARE,LSYSSC_DUP_EMPTY)

      call lsyssc_clearMatrix (rasmTempl%rmatrixStabil1)
      call lsyssc_clearMatrix (rasmTempl%rmatrixStabil2)
      ...
      call conv_jumpStabilisation2d (&
          rjumpStabil, CONV_MODMATRIX, rasmTempl%rmatrixStabil1,&
          rdiscretisation=rasmTempl%rdiscretisationStabil)  
    
      rjumpStabil%dnu = rproblem%rphysics%dnu2

      call conv_jumpStabilisation2d (&
          rjumpStabil, CONV_MODMATRIX, rasmTempl%rmatrixStabil2,&
          rdiscretisation=rasmTempl%rdiscretisationStabil)  
    end if
    ...
    call lsyssc_releaseMatrix (rasmTempl%rmatrixMass1)
    call lsyssc_releaseMatrix (rasmTempl%rmatrixMass2)
    ...
    call lsyssc_duplicateMatrix (rasmTempl%rmatrixTemplateFEM,&
                rasmTempl%rmatrixMass1,LSYSSC_DUP_SHARE,LSYSSC_DUP_REMOVE)
    call lsyssc_duplicateMatrix (rasmTempl%rmatrixTemplateFEM,&
                rasmTempl%rmatrixMass2,LSYSSC_DUP_SHARE,LSYSSC_DUP_REMOVE)
    ...
    call lsyssc_assignDiscrDirectMat (rasmTempl%rmatrixMass1,&
        rasmTempl%rdiscretisationMass)
    call lsyssc_assignDiscrDirectMat (rasmTempl%rmatrixMass2,&
        rasmTempl%rdiscretisationMass)
    ...
    call stdop_assembleSimpleMatrix (rasmTempl%rmatrixMass1,DER_FUNC,DER_FUNC)
    call stdop_assembleSimpleMatrix (rasmTempl%rmatrixMass2,DER_FUNC,DER_FUNC)
    ...
    if (j .eq. 0) then
      ...
      ! to one of the LSYSSC_LUMP_xxxx constants for lsyssc_lumpMatrixScalar.
      call lsyssc_lumpMatrixScalar (rasmTempl%rmatrixMass1,j)
      call lsyssc_lumpMatrixScalar (rasmTempl%rmatrixMass2,j)
...
!</subroutine>
    ...
    if(lsyssc_hasMatrixStructure(rasmTemplFine%rmatrixProlVelocity1)) then
    
      ! Assemble its entries
      call mlop_build2LvlProlMatrix(&
          rdiscretisationCoarse%RspatialDiscr(1),&
          rdiscretisationFine%RspatialDiscr(1),&
          .true., rasmTemplFine%rmatrixProlVelocity1, MLOP_AVRG_MASS)

      call mlop_build2LvlInterpMatrix(&
          rdiscretisationCoarse%RspatialDiscr(1),&
          rdiscretisationFine%RspatialDiscr(1),&
          .true., rasmTemplFine%rmatrixInterpVelocity1, MLOP_AVRG_MASS)

    end if
!----------------------/////////////////////////---------------------
    if(lsyssc_hasMatrixStructure(rasmTemplFine%rmatrixProlVelocity2)) then
      call mlop_build2LvlProlMatrix(&
          rdiscretisationCoarse%RspatialDiscr(3),&
          rdiscretisationFine%RspatialDiscr(3),&
          .true., rasmTemplFine%rmatrixProlVelocity2, MLOP_AVRG_MASS)

      call mlop_build2LvlInterpMatrix(&
          rdiscretisationCoarse%RspatialDiscr(3),&
          rdiscretisationFine%RspatialDiscr(3),&
          .true., rasmTemplFine%rmatrixInterpVelocity2, MLOP_AVRG_MASS)
...
!----------------------/////////////////////////---------------------
    ...
    if(lsyssc_hasMatrixStructure(rasmTemplFine%rmatrixProlPressure)) then
      ...
      call mlop_build2LvlProlMatrix(&
          rdiscretisationCoarse%RspatialDiscr(5),&
          rdiscretisationFine%RspatialDiscr(5),&
...
!</subroutine>
    ...
    ! Release Stokes, B1 ... B4,... matrices
    call lsyssc_releaseMatrix (rasmTempl%rmatrixD4T)
    call lsyssc_releaseMatrix (rasmTempl%rmatrixD3T)
    call lsyssc_releaseMatrix (rasmTempl%rmatrixD2T)
    call lsyssc_releaseMatrix (rasmTempl%rmatrixD1T)
    call lsyssc_releaseMatrix (rasmTempl%rmatrixD4)
    call lsyssc_releaseMatrix (rasmTempl%rmatrixD3)
    call lsyssc_releaseMatrix (rasmTempl%rmatrixD2)
    call lsyssc_releaseMatrix (rasmTempl%rmatrixD1)
    call lsyssc_releaseMatrix (rasmTempl%rmatrixB4)
    call lsyssc_releaseMatrix (rasmTempl%rmatrixB3)
    call lsyssc_releaseMatrix (rasmTempl%rmatrixB2)
    call lsyssc_releaseMatrix (rasmTempl%rmatrixB1)
    call lsyssc_releaseMatrix (rasmTempl%rmatrixStokes2)
    call lsyssc_releaseMatrix (rasmTempl%rmatrixStokes1)
    if (lsyssc_hasMatrixStructure(rasmTempl%rmatrixStabil1)) then
      call lsyssc_releaseMatrix (rasmTempl%rmatrixStabil1)
    end if
    if (lsyssc_hasMatrixStructure(rasmTempl%rmatrixStabil2)) then
      call lsyssc_releaseMatrix (rasmTempl%rmatrixStabil2)
    end if    
    ...
    if(lsyssc_hasMatrixStructure(rasmTempl%rmatrixProlVelocity1)) then
      call lsyssc_releaseMatrix(rasmTempl%rmatrixInterpVelocity1)
      call lsyssc_releaseMatrix(rasmTempl%rmatrixProlVelocity1)
    end if
    if(lsyssc_hasMatrixStructure(rasmTempl%rmatrixProlVelocity2)) then
      call lsyssc_releaseMatrix(rasmTempl%rmatrixInterpVelocity2)
      call lsyssc_releaseMatrix(rasmTempl%rmatrixProlVelocity2)
    end if
...
!</subroutine>
    ...
    real(DP), dimension(NDIM2D) :: Dvelocity
    real(DP), dimension(2*NDIM2D) :: Dacceleration
    ...
    if (rrhsAssembly%ctype .ne. 0) then
      ...
      if (rrhsAssembly%ctype .eq. 3) then
        ...
        call lsyssc_scalarMatVec (rasmTemplates%rmatrixMass1, &
        ...
        call lsyssc_scalarMatVec (rasmTemplates%rmatrixMass2, &
...
!</subroutine>
    ...
    case (1,2)
      ...
      if (ielementTypeInitialSolution .eq. -1) then
        ...
        if ((rvector2%RvectorBlock(1)%NEQ .ne. rvector1%RvectorBlock(1)%NEQ) .or.&
            ...
            (rvector2%RvectorBlock(5)%NEQ .ne. rvector1%RvectorBlock(5)%NEQ)) then
          call output_line (&
        ...
        call anprj_analytL2projectionByMass (rvector1%RvectorBlock(1),&
            rproblem%RlevelInfo(ilev)%rasmTempl%rmatrixMass1,&
        ...
        call anprj_analytL2projectionByMass (rvector1%RvectorBlock(3),&
            rproblem%RlevelInfo(ilev)%rasmTempl%rmatrixMass2,&
            fcoeff_solProjection,rcollection)
    ...
    case (3)
      ...
      ! Discretise the X-velocity part of nvs1:
      call linf_buildVectorScalar (&
                p_rdiscretisation%RspatialDiscr(1),rlinform,.true.,&
                rvector1%RvectorBlock(1),coeff_AnalyticSolution_X1,&
                rproblem%rcollection)

      ! Y-velocity of nvs1:
      call linf_buildVectorScalar (&
                p_rdiscretisation%RspatialDiscr(2),rlinform,.true.,&
                rvector1%RvectorBlock(2),coeff_AnalyticSolution_Y1,&
                rproblem%rcollection)

      ! Discretise the X-velocity part of nvs2:
      call linf_buildVectorScalar (&
                p_rdiscretisation%RspatialDiscr(3),rlinform,.true.,&
                rvector1%RvectorBlock(3),coeff_AnalyticSolution_X2,&
                rproblem%rcollection)

      ! Y-velocity of nvs2:
      call linf_buildVectorScalar (&
                p_rdiscretisation%RspatialDiscr(4),rlinform,.true.,&
                rvector1%RvectorBlock(4),coeff_AnalyticSolution_Y2,&
                rproblem%rcollection)

      ! Pressure:
      call linf_buildVectorScalar (&
                p_rdiscretisation%RspatialDiscr(5),rlinform,.true.,&
                rvector1%RvectorBlock(5),coeff_AnalyticSolution_P,&
                rproblem%rcollection)
      ...
      ! We solve separately for the five components.
      ! Prepare the solver for the velocity of nvs1
      ! Prepare the solver for the X-velocity.
      call lsysbl_createMatFromScalar(&
          rproblem%RlevelInfo(rproblem%NLMAX)%rasmTempl%rmatrixMass1,Rmatrices(1))
      ...
      if (p_rsolverNode%iresult .ne. 0) then
        ...
        ! -----
        ! Solve for the Y-velocity of nvs1.
...
!----------------------/////////////////////////-----------------------------
      ...
      ! Prepare the solver for the velocity of nvs2
      ! Prepare the solver for the X-velocity.
      call lsysbl_createMatFromScalar(&
          rproblem%RlevelInfo(rproblem%NLMAX)%rasmTempl%rmatrixMass2,Rmatrices(1))
      call linsol_setMatrices (p_rsolverNode,Rmatrices)
      call linsol_initStructure (p_rsolverNode,ierror)
      call linsol_initData (p_rsolverNode,ierror)
      ...
      if (p_rsolverNode%iresult .ne. 0) then
        ...
        ! -----
        ! Solve for the Y-velocity of nvs2.
        ! Throw away the temporary references to the 1st component in rvector1/rvector
        ! and replace them by references to the 2nd component.
        call lsysbl_releaseVector (rsingleRHS)
        call lsysbl_releaseVector (rsingleSol)
