...
!<input>
  ...
  ! Type of matrix.
  ! =CCMASM_MTP_AUTOMATIC: standard matrix, A11=A22 & A33=A44
  ! =CCMASM_MTP_DECOUPLED: Decoupled velocity matrices A11, A22, A33 and A44
...
!</subroutine>
    ...
    ! (Newton1=A12/A21 , Newton2=A12/A21 present, UMFPACK 
    ! on pure Dirichlet=A33 present.)
    ...
    ! Initialise them with the same structure, i.e. A11 ... A44 share (!) their
    ...
    ! of those terms which should appear in the matrix to a value <> 0,
    ...
    rnonlinearCCMatrix%dtheta1 = 1.0_DP   ! A velocity block for nvs1
    rnonlinearCCMatrix%dtheta2 = 1.0_DP   ! A velocity block for nvs2

    rnonlinearCCMatrix%deta1 = 1.0_DP     ! A gradient block for nvs1
    rnonlinearCCMatrix%deta2 = 1.0_DP     ! A gradient block for nvs2

    rnonlinearCCMatrix%dtau1 = 1.0_DP     ! A divergence block for nvs1
    rnonlinearCCMatrix%dtau2 = 1.0_DP     ! A divergence block for nvs2
...
!</subroutine>
    ...
    if (.not. bneumann) then
      ...
      rnonlinearIteration%p_RfilterChain(3)%itoL20component = 2*NDIM2D+1
    end if
...
!</subroutine>
    ...
    case (1)
      ...
      call cgcor_init(p_rsolverNode%p_rsubnodeMultigrid2%rcoarseGridCorrection,2*NDIM2D+1)
      p_rsolverNode%p_rsubnodeMultigrid2%rcoarseGridCorrection%p_DequationWeights(5) &
          = -1.0_DP  ! Obaid, 3 must be replaced by 5
    ...
    case (CCPREC_LINEARSOLVER,CCPREC_NEWTON,CCPREC_NEWTONDYNAMIC)
      ...
      do i=NLMIN,NLMAX
        ...
        if (binit .or. bstructuralUpdate) then
          ...
          ! The routine also reserves memory for A(5,5) for the case it is needed,
        ...
        !
        ! In case we have a pure-dirichlet problem, we activate the 5,5-submatrix
          ...
          case (0)
            ...
            if (i .eq. NLMAX) then
              ...
              if (rnonlinearIteration%rprecSpecials%bneedPressureDiagonalBlock) then
                ! Activate the 5,5-block, UMFPACK needs it in case the pressure
                ! is globally indefinite.
                p_rmatrixPreconditioner%RmatrixBlock(5,5)%dscaleFactor = 1.0_DP
              else
                p_rmatrixPreconditioner%RmatrixBlock(5,5)%dscaleFactor = 0.0_DP
              end if
            end if
...
!</subroutine>
    ...
    ! Do we use matrix based projection for the velocity of nvs1?
    call parlst_getvalue_int (p_rsection,'iprojTypeVelocity1',i1,0)  ! see discetisation.dat
    if(i1 .eq. 1) then
      ...
      call mlprj_initMatrixProjection(&
          ...
          rlevelInfo%rasmTempl%rmatrixProlVelocity1, &
          rlevelInfo%rasmTempl%rmatrixInterpVelocity1)  ! see DOFmaintenace/multilevelprojection.f90
      call mlprj_initMatrixProjection(&
          rprojection%RscalarProjection(1,2), &
          rlevelInfo%rasmTempl%rmatrixProlVelocity1, &
          rlevelInfo%rasmTempl%rmatrixInterpVelocity1)
          
    end if
!*******************************---------------- obaid
    ! Do we use matrix based projection for the velocity of nvs2?
    call parlst_getvalue_int (p_rsection,'iprojTypeVelocity2',i1,0)  ! see discetisation.dat
    if(i1 .eq. 1) then
      ...
      call mlprj_initMatrixProjection(&
          ...
          rlevelInfo%rasmTempl%rmatrixProlVelocity2, &
          rlevelInfo%rasmTempl%rmatrixInterpVelocity2)  ! see DOFmaintenace/multilevelprojection.f90
      call mlprj_initMatrixProjection(&
          rprojection%RscalarProjection(1,4), &
          rlevelInfo%rasmTempl%rmatrixProlVelocity2, &
          rlevelInfo%rasmTempl%rmatrixInterpVelocity2)
...
!*********************************---------------- 0baid
    ...
    ! Prolongation/restriction order for velocity components of nvs1
    call parlst_getvalue_int (p_rsection,'iinterpolationOrderVel1',i1,-1)
...
!*********************************---------------- 0baid
    ! Prolongation/restriction order for velocity components of nvs1
    call parlst_getvalue_int (p_rsection,'iinterpolationOrderVel2',i1,-1)
...
!*********************************---------------- 0baid
    ...
    if (i1 .ne. -1) then
      ! Initialise order of prolongation/restriction for velocity components
      rprojection%RscalarProjection(:,2*NDIM2D+1)%iprolongationOrder  = i1
      rprojection%RscalarProjection(:,2*NDIM2D+1)%irestrictionOrder   = i1
      rprojection%RscalarProjection(:,2*NDIM2D+1)%iinterpolationOrder = i1
    end if
    
    ! Prolongation/restriction variant for velocity components1svn fo 
    ! in case of Q1~ discretisation
    call parlst_getvalue_int (p_rsection,'iinterpolationVariantVel1',i1,0)
 ...
 !*********************************---------------- 0baid
  ! Prolongation/restriction variant for velocity components of nvs2
    ! in case of Q1~ discretisation
    call parlst_getvalue_int (p_rsection,'iinterpolationVariantVel2',i1,0)
...
!*********************************---------------- 0baid   
    ...
    call parlst_getvalue_int (p_rsection,'iintARIndicatorEX3YVel1',i1,1)
...
!*********************************---------------- 0baid  
    ! Aspect-ratio indicator in case of Q1~ discretisation
    ! with extended prolongation/restriction
    call parlst_getvalue_int (p_rsection,'iintARIndicatorEX3YVel2',i1,1)
...
!*********************************---------------- 0baid  
    ...
    call parlst_getvalue_double (p_rsection,'dintARboundEX3YVel1',d1,20.0_DP)
...
!*********************************---------------- 0baid 
    ! Aspect-ratio bound for switching to constant prolongation/restriction
    ! in case of Q1~ discretisation with extended prolongation/restriction
    call parlst_getvalue_double (p_rsection,'dintARboundEX3YVel2',d1,20.0_DP)
