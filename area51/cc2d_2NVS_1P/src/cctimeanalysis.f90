...
!<typeblock>
  ...
  type t_timeDerivatives
    ...
    ! $||rel. change in V||_{L2}$
    real(DP) :: drelVL2

    ! $||rel. change in V||_{\max}$
    real(DP) :: drelVmax
...
!<constantblock description="Identifiers for norms of time-dependent solutions.">
  ...
  ! $ ||v||_{l2} $
  integer, parameter :: TNRM_L2V    = 3  

  ! $ ||v||_{\max} $
  integer, parameter :: TNRM_LMAXV   = 4

  ! $ ||p||_{\l2} $
  integer, parameter :: TNRM_P2U    = 5

  ! $ ||p||_{\max} $
  integer, parameter :: TNRM_PMAX   = 6

  ! $ \max ( ||u||_{l2} , ||v||_{l2} , ||p||_{l2} ) $
  integer, parameter :: TNRM_L2UP2U     = 7

  ! $ \max ( ||u||_{\max} ,||v||_{\max} , ||p||_{\max} ) $
  integer, parameter :: TNRM_L2MAXPMAX  = 8

  ! $ \max ( ||u||_{l2}, ||v||_{l2},  ||p||_{l2} , 
  !          ||u||_{\max}, ||v||_{\max}, ||p||_{\max} ) $
  integer, parameter :: TNRM_MAX        = 9

  ! $ \min ( ||u||_{l2}, ||v||_{l2},  ||p||_{l2} , 
  !          ||u||_{\max}, ||v||_{\max}, ||p||_{\max} ) $
  integer, parameter :: TNRM_MIN        = 10

  ! $ \max ( ||u||_{l2} , ||v||_{l2} ) $
  integer, parameter :: TNRM_L2UV     = 11

  ! $ \max ( ||u||_{\max} ,||v||_{\max} ) $
  integer, parameter :: TNRM_L2MAXUMAXV  = 12
...
!<input>
  ...
  ! =TNRM_L2V      : Calculate dtimeerror=drelVL2; standard
  ! =TNRM_LMAXU     : Calculate dtimeerror=drelUmax
  ! =TNRM_LMAXV     : Calculate dtimeerror=drelVmax
  ! =TNRM_P2U      : Calculate dtimeerror=drelPL2
  ! =TNRM_PMAX     : Calculate dtimeerror=drelPmax
  ! =TNRM_L2UP2U   : Calculate dtimeerror=max(drelUL2,drelVL2,drelPL2)
  ! =TNRM_L2MAXPMAX: Calculate dtimeerror=max(drelUmax,drelVmax,drelPmax)
  ! =TNRM_MAX      : Calculate dtimeerror=max(drelUL2,drelVL2,drelPL2,drelUmax,drelVmax,drelPmax)
  ! =TNRM_MIN      : Calculate dtimeerror=min(drelUL2,drelPL2,drelUmax,drelPmax)  
  ! =TNRM_L2UV      : Calculate dtimeerror=max(drelUL2,drelVL2)  
  ! =TNRM_L2MAXUMAXV : Calculate dtimeerror=max(drelUmax,drelVmax) 
...
!</function>
    ...
    ! local variables
    real(DP) :: dtmp
    real(DP), dimension(5) :: Dnorms1,Dnorms2
...
!---------------------------//////////////////////------------------------------ 1
    ...
    if (dtmp .eq. 0.0_DP) dtmp=1.0_DP
    rtimeErrorLocal%drelVL2 = sqrt(0.5_DP * (Dnorms1(3)**2+Dnorms1(4)**2) ) / dtmp
!---------------------------/////////////////////------------------------------- 1
    dtmp = Dnorms2(5)
    if (dtmp .eq. 0.0_DP) dtmp=1.0_DP
    rtimeErrorLocal%drelPL2 = Dnorms1(5) / dtmp
...
!---------------------------/////////////////////------------------------------- 2
    ...
    if (dtmp .eq. 0.0_DP) dtmp=1.0_DP
    rtimeErrorLocal%drelVmax = max(Dnorms1(3),Dnorms1(4)) / dtmp
!---------------------------/////////////////////------------------------------- 2

    dtmp = Dnorms2(5)
    if (dtmp .eq. 0.0_DP) dtmp=1.0_DP
    rtimeErrorLocal%drelPmax = Dnorms1(5) / dtmp
    ...
    case (TNRM_L2V)
      dtimeerror = rtimeErrorLocal%drelVL2
    case (TNRM_LMAXU)
      dtimeerror = rtimeErrorLocal%drelUmax
    case (TNRM_LMAXV)
      dtimeerror = rtimeErrorLocal%drelVmax
    case (TNRM_P2U)
      dtimeerror = rtimeErrorLocal%drelPL2
    case (TNRM_PMAX)
      dtimeerror = rtimeErrorLocal%drelPmax
    case (TNRM_L2UP2U)
      dtimeerror = max(rtimeErrorLocal%drelUL2,rtimeErrorLocal%drelVL2,rtimeErrorLocal%drelPL2)
    case (TNRM_L2MAXPMAX)
      dtimeerror = max(rtimeErrorLocal%drelUmax,rtimeErrorLocal%drelVmax,rtimeErrorLocal%drelPmax)
    case (TNRM_MAX)
      dtimeerror = max(rtimeErrorLocal%drelUL2,rtimeErrorLocal%drelVL2,rtimeErrorLocal%drelPL2,&
                       rtimeErrorLocal%drelUmax,rtimeErrorLocal%drelVmax,rtimeErrorLocal%drelPmax)
    case (TNRM_MIN)
      dtimeerror = min(rtimeErrorLocal%drelUL2,rtimeErrorLocal%drelVL2,rtimeErrorLocal%drelPL2,&
                       rtimeErrorLocal%drelUmax,rtimeErrorLocal%drelVmax,rtimeErrorLocal%drelPmax)
    case (TNRM_L2UV)
      dtimeerror = max(rtimeErrorLocal%drelUL2,rtimeErrorLocal%drelVL2)
    case (TNRM_L2MAXUMAXV)
      dtimeerror = max(rtimeErrorLocal%drelUmax,rtimeErrorLocal%drelVmax)
    end select
...
!<input>
  ! Type of norm to use in space/time; former IEPSAD.
  ! =TNRM_L2U      : Calculate dtimeerror=drelUL2; standard
  ! =TNRM_L2V      : Calculate dtimeerror=drelVL2;
  ! =TNRM_LMAXU     : Calculate dtimeerror=drelUmax
  ! =TNRM_LMAXV     : Calculate dtimeerror=drelVmax
  ! =TNRM_P2U      : Calculate dtimeerror=drelPL2
  ! =TNRM_PMAX     : Calculate dtimeerror=drelPmax
  ...
  ! =TNRM_L2MAXPMAX: Calculate dtimeerror=max(drelUmax,drelVmax,drelPmax)
  ! =TNRM_MAX      : Calculate dtimeerror=max(drelUL2,drelVL2,drelPL2,drelUmax,drelVmax,drelPmax)
  ! =TNRM_MIN      : Calculate dtimeerror=min(drelUL2,drelVL2,drelPL2,drelUmax,drelVmax,drelPmax)  
  integer                        :: ctimeErrorControl
...
!</function>
    ...
    neqv = rSolutionNew%RvectorBlock(3)%NEQ+rSolutionNew%RvectorBlock(4)%NEQ
    neqp = rSolutionNew%RvectorBlock(5)%NEQ
    ...
    p_rtimeNorm%drelVL2 = sqrt( 0.5_DP * (Dnorms1(3)**2+Dnorms1(4)**2) ) &
        / (sqrt(real(neqv,DP)) * dtstep)
    p_rtimeNorm%drelPL2 = Dnorms1(5) / (sqrt(real(neqp,DP)) * dtstep)
    ...
    p_rtimeNorm%drelVmax = max(Dnorms1(3),Dnorms1(4)) / dtstep
    p_rtimeNorm%drelPmax = Dnorms1(5) / dtstep
    ...
    case (TNRM_L2V)
      dtimenorm = p_rtimeNorm%drelVL2
    case (TNRM_LMAXU)
      dtimenorm = p_rtimeNorm%drelUmax
    case (TNRM_LMAXV)
      dtimenorm = p_rtimeNorm%drelVmax
    case (TNRM_P2U)
      dtimenorm = p_rtimeNorm%drelPL2
    case (TNRM_PMAX)
      dtimenorm = p_rtimeNorm%drelPmax
    case (TNRM_L2UP2U)
      dtimenorm = max(p_rtimeNorm%drelUL2,p_rtimeNorm%drelVL2,p_rtimeNorm%drelPL2)
    case (TNRM_L2MAXPMAX)
      dtimenorm = max(p_rtimeNorm%drelUmax,p_rtimeNorm%drelVmax,p_rtimeNorm%drelPmax)
    case (TNRM_MAX)
      dtimenorm = max(p_rtimeNorm%drelUL2,p_rtimeNorm%drelVL2,p_rtimeNorm%drelPL2,&
                      p_rtimeNorm%drelUmax,p_rtimeNorm%drelVmax,p_rtimeNorm%drelPmax)
    case (TNRM_MIN)
      dtimenorm = min(p_rtimeNorm%drelUL2,p_rtimeNorm%drelVL2,p_rtimeNorm%drelPL2,&
                      p_rtimeNorm%drelUmax,p_rtimeNorm%drelVmax,p_rtimeNorm%drelPmax)
!
    case (TNRM_L2UV)
      dtimenorm = max(p_rtimeNorm%drelUL2,p_rtimeNorm%drelVL2)
    case (TNRM_L2MAXUMAXV)
      dtimenorm = max(p_rtimeNorm%drelUmax,p_rtimeNorm%drelVmax)
    end select
