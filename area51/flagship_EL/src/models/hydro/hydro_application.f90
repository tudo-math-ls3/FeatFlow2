...
!</subroutine>
    ...
    ! One-way, two-way coupling or steady state gas solution
    ...
    ! Euler-Lagrange
    !- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    call hydro_initpart(rparlist,rproblem%p_rproblemLevelMax,rsolution,&
    ...
    if (rtimestep%dfinalTime .gt. rtimestep%dinitialTime) then
      ...
      if (trim(algorithm) .eq. 'transient_primal') then
        ...
        call hydro_outputSolution(rparlist, ssectionName,&
            rproblem%p_rproblemLevelMax, rParticles, rsolutionPrimal,&
      ...
      case(SYSTEM_INTERLEAVEFORMAT)
        ...
        do ivariable = 1, nvariable
          ...
          elseif (trim(cvariable) .eq. 'velo_part') then
            ...
            select case(ndim)
            case (NDIM1D)
              call hydro_getVarInterleaveFormat(rvector4%NEQ, NVAR1D,&
            ...
            case (NDIM3D)
              call hydro_getVarInterleaveFormat(rvector4%NEQ, NVAR3D,&
                  'velo_part_x', p_Dsolution, p_Ddata4)
              call hydro_getVarInterleaveFormat(rvector5%NEQ, NVAR3D,&
                  'velo_part_y', p_Dsolution, p_Ddata5)
              call hydro_getVarInterleaveFormat(rvector6%NEQ, NVAR3D,&
          ...
          elseif (trim(cvariable) .eq. 'velo_part') then
            ...
            case (NDIM3D)
              ...
              call hydro_getVarBlockFormat(rvector5%NEQ, NVAR2D,&
                  'velo_part_y', p_Dsolution, p_Ddata5)
              call hydro_getVarBlockFormat(rvector6%NEQ, NVAR2D,&
                  'velo_part_z', p_Dsolution, p_Ddata6)
              call ucd_addVarVertBasedVec(rexport, 'velo_part',&
                  p_Ddata4, p_Ddata5, p_Ddata6)
...
!</subroutine>
    ...
    p_rtimerParticlephase => collct_getvalue_timer(rcollection,&
        'rtimerParticlephase', ssectionName=ssectionName)
...
subroutine hydro_initpart(rparlist,p_rproblemLevel,rsolution,rtimestep,rcollection,rParticles)
...
!<input>
    ...
    call parlst_getvalue_int(rparlist, 'Hydro', "nPart", nPart)
    ...
    call parlst_getvalue_double(rparlist, 'Hydro', "gravityx", gravityx)
    call parlst_getvalue_double(rparlist, 'Hydro', "gravityy", gravityy)

    ! Get value of kinematic viscosity
    call parlst_getvalue_double(rparlist, 'Hydro', "gas_nu", gas_nu)

    ! Get boundarybehaviour
    call parlst_getvalue_int(rparlist, 'Hydro', "boundbehav", boundbehav)
...
!<input>
    ...
    ! One way or twoway coupling?
    call parlst_getvalue_int(rparlist, 'Hydro', "icouplingpart", icouplingpart)

    select case(icouplingpart)
    case(1)
        ! Subroutine to compute the movement of the particles
        call hydro_moveparticlesoneway(rparlist,p_rproblemLevel,rsolution,rParticles)
    
    case(2)
        ! Subroutine to compute the movement of the particles
        call hydro_moveparticlestwoway(rparlist,p_rproblemLevel,rsolution,rParticles)
