...
!</subroutine>
    ...
    rform%Dcoefficients(1)  = 2*mu + lemda
    rform%Dcoefficients(2)  = mu
   ...
   call bilf_buildMatrixScalar (rform,.true.,rmatrix%RmatrixBlock(1,1),&
    ...
    rform%ballCoeffConstant = .true.
    rform%BconstantCoeff = .true.
    rform%Dcoefficients(1)  = mu + lemda
   ...
   call bilf_buildMatrixScalar (rform,.true.,rmatrix%RmatrixBlock(2,1),&
    ...
    rform%ballCoeffConstant = .true.
    rform%BconstantCoeff = .true.
    rform%Dcoefficients(1)  = mu
    rform%Dcoefficients(2)  = 2*mu + lemda
   ...
   call bilf_buildMatrixScalar (rform,.true.,rmatrix%RmatrixBlock(2,2),&
    ...
    call linf_buildVectorScalarBdr2d(rlinform,CUB_G2X2,.false.,&
		  rrhs%RvectorBlock(1),coeff_RHS_neumBdr_X_2D,rboundaryRegion)

    call linf_buildVectorScalarBdr2d(rlinform,CUB_G2X2,.false.,&
		  rrhs%RvectorBlock(2),coeff_RHS_neumBdr_Y_2D,rboundaryRegion)
