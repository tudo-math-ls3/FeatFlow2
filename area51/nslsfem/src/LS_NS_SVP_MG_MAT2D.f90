...
!<types>
  ...
  ! Prolongation/Interpolation matrices for velocity
  type(t_matrixScalar) :: rmatProlVelocity, rmatInteVelocity

  ! Prolongation/Interpolation matrices for pressure
  type(t_matrixScalar) :: rmatProlPressure, rmatIntePressure

  ! Prolongation/Interpolation matrices for stress
  type(t_matrixScalar) :: rmatProlStress, rmatInteStress
...
!</subroutine>
  ...
  ! for coarse level
  allocate(Rlevels(NLMIN)%p_rprojection)
  call mlprj_initProjectionMat (Rlevels(NLMIN)%p_rprojection, &
  ...
  do i = NLMIN+1, NLMAX
    ...
    call mlop_create2LvlMatrixStruct(Rlevels(i-1)%rdiscretisation%RspatialDiscr(1), &
        Rlevels(i)%rdiscretisation%RspatialDiscr(1), &
        LSYSSC_MATRIX9, Rlevels(i)%rmatProlVelocity)
    call mlop_create2LvlMatrixStruct(Rlevels(i-1)%rdiscretisation%RspatialDiscr(3), &
        Rlevels(i)%rdiscretisation%RspatialDiscr(3), &
        LSYSSC_MATRIX9, Rlevels(i)%rmatProlPressure)
    call mlop_create2LvlMatrixStruct(Rlevels(i-1)%rdiscretisation%RspatialDiscr(4), &
        Rlevels(i)%rdiscretisation%RspatialDiscr(4), &
        LSYSSC_MATRIX9, Rlevels(i)%rmatProlStress)

    ! Assemble prolongation matrices
    call mlop_build2LvlProlMatrix (Rlevels(i-1)%rdiscretisation%RspatialDiscr(1),&
        Rlevels(i)%rdiscretisation%RspatialDiscr(1), &
        .true., Rlevels(i)%rmatProlVelocity, &
          rcubatureInfoCoarse=Rlevels(i-1)%rcubatureInfo,&
          rcubatureInfoFine=Rlevels(i)%rcubatureInfo)
    call mlop_build2LvlProlMatrix (Rlevels(i-1)%rdiscretisation%RspatialDiscr(3),&
        Rlevels(i)%rdiscretisation%RspatialDiscr(3), &
        .true., Rlevels(i)%rmatProlPressure, &
          rcubatureInfoCoarse=Rlevels(i-1)%rcubatureInfo,&
          rcubatureInfoFine=Rlevels(i)%rcubatureInfo)
    call mlop_build2LvlProlMatrix (Rlevels(i-1)%rdiscretisation%RspatialDiscr(4),&
        Rlevels(i)%rdiscretisation%RspatialDiscr(4), &
        .true., Rlevels(i)%rmatProlStress, &
          rcubatureInfoCoarse=Rlevels(i-1)%rcubatureInfo,&
          rcubatureInfoFine=Rlevels(i)%rcubatureInfo)
    ...
    ! -=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    ! Create interpolation matrix structures
    call mlop_create2LvlMatrixStruct(Rlevels(i-1)%rdiscretisation%RspatialDiscr(1), &
        Rlevels(i)%rdiscretisation%RspatialDiscr(1), &
        LSYSSC_MATRIX9, Rlevels(i)%rmatInteVelocity)
    call mlop_create2LvlMatrixStruct(Rlevels(i-1)%rdiscretisation%RspatialDiscr(3), &
        Rlevels(i)%rdiscretisation%RspatialDiscr(3), &
        LSYSSC_MATRIX9, Rlevels(i)%rmatIntePressure)
    call mlop_create2LvlMatrixStruct(Rlevels(i-1)%rdiscretisation%RspatialDiscr(4), &
        Rlevels(i)%rdiscretisation%RspatialDiscr(4), &
        LSYSSC_MATRIX9, Rlevels(i)%rmatInteStress)

    ! Assemble interpolation matrices
    call mlop_build2LvlInterpMatrix (Rlevels(i-1)%rdiscretisation%RspatialDiscr(1),&
        Rlevels(i)%rdiscretisation%RspatialDiscr(1), &
        .true., Rlevels(i)%rmatInteVelocity, &
          rcubatureInfoCoarse=Rlevels(i-1)%rcubatureInfo,&
          rcubatureInfoFine=Rlevels(i)%rcubatureInfo)
    call mlop_build2LvlInterpMatrix (Rlevels(i-1)%rdiscretisation%RspatialDiscr(3),&
        Rlevels(i)%rdiscretisation%RspatialDiscr(3), &
        .true., Rlevels(i)%rmatIntePressure, &
          rcubatureInfoCoarse=Rlevels(i-1)%rcubatureInfo,&
          rcubatureInfoFine=Rlevels(i)%rcubatureInfo)
    call mlop_build2LvlInterpMatrix(Rlevels(i-1)%rdiscretisation%RspatialDiscr(4),&
        Rlevels(i)%rdiscretisation%RspatialDiscr(4), &
        .true., Rlevels(i)%rmatInteStress, &
          rcubatureInfoCoarse=Rlevels(i-1)%rcubatureInfo,&
          rcubatureInfoFine=Rlevels(i)%rcubatureInfo)


    ! Initialise the projection based on the matrix structure on level 'i'
    call mlprj_initProjectionMat (Rlevels(i)%p_rprojection, &
                                              Rlevels(i)%rmatrix)

    ! And initialise the matrix-based projection
    call mlprj_initMatrixProjection(&
        Rlevels(i)%p_rprojection%RscalarProjection(1,1), &
        Rlevels(i)%rmatProlVelocity,&
        rmatrixInterp=Rlevels(i)%rmatInteVelocity)

    call mlprj_initMatrixProjection(&
        Rlevels(i)%p_rprojection%RscalarProjection(1,2), &
        Rlevels(i)%rmatProlVelocity,&
        rmatrixInterp=Rlevels(i)%rmatInteVelocity)

    call mlprj_initMatrixProjection(&
        Rlevels(i)%p_rprojection%RscalarProjection(1,3), &
        Rlevels(i)%rmatProlPressure,&
        rmatrixInterp=Rlevels(i)%rmatIntePressure)

    call mlprj_initMatrixProjection(&
        Rlevels(i)%p_rprojection%RscalarProjection(1,4), &
        Rlevels(i)%rmatProlStress,&
        rmatrixInterp=Rlevels(i)%rmatInteStress)

    call mlprj_initMatrixProjection(&
        Rlevels(i)%p_rprojection%RscalarProjection(1,5), &
        Rlevels(i)%rmatProlStress,&
        rmatrixInterp=Rlevels(i)%rmatInteStress)

    call mlprj_initMatrixProjection(&
        Rlevels(i)%p_rprojection%RscalarProjection(1,6), &
        Rlevels(i)%rmatProlStress,&
        rmatrixInterp=Rlevels(i)%rmatInteStress)
...
!</subroutine>
  ...
  do i = NLMIN+1, NLMAX
    ...
    call linsol_initProjMultigrid2Level(p_rlevelInfo,Rlevels(i)%p_rprojection)    
...
!</subroutine>
  ...
  do i = NLMAX, NLMIN+1, -1
    call mlprj_doneProjection(Rlevels(i)%p_rprojection)
    call lsyssc_releaseMatrix (Rlevels(i)%rmatProlVelocity)
    call lsyssc_releaseMatrix (Rlevels(i)%rmatProlPressure)
    call lsyssc_releaseMatrix (Rlevels(i)%rmatProlStress)
    call lsyssc_releaseMatrix (Rlevels(i)%rmatInteVelocity)
    call lsyssc_releaseMatrix (Rlevels(i)%rmatIntePressure)
    call lsyssc_releaseMatrix (Rlevels(i)%rmatInteStress)
  end do
  call mlprj_doneProjection(Rlevels(NLMIN)%p_rprojection)
