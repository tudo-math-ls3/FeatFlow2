...
!</subroutine>
    ...
    if(rproblem%iParticles .gt. 0)then        
      call cc_updateParticlePositionBench(rpostprocessing,rproblem,&
...
!</subroutine>
    ...
    real(DP) :: dminTime, dmaxTime, dtimeDifferenceUCD,dframeVel
    integer :: ioutputUCD,ieltype,ilevelUCD,ipolyHandle,ipart,ive
...
!      dframeVel = rproblem%dFrameVel
!      p_Ddata2(ive)=p_Ddata2(ive) + dframeVel
!    end do
...
!</subroutine>
  ...
  integer :: i
    ...
    i=floor(rproblem%rtimedependence%dtimeMax/rproblem%rtimedependence%dtimestep)
...
!</subroutine>
    ...
    real(DP) :: dCenterX,dCenterY,dCenterXold,dCenterYold,domega,d5,d4,d1,d2
    type(t_geometryObject), pointer :: p_rgeometryObject        
    ...
    do ipart=1,p_rparticleCollection%nparticles
      ...
      dvely   = dtimestep*(1.0_dp*dfy+ddmasssl*-4.81_dp)/dmasssl
      ...
      p_rparticleCollection%p_rParticles(ipart)%rResForceX(5) = &
      p_rparticleCollection%p_rParticles(ipart)%rResForceX(4)      

      p_rparticleCollection%p_rParticles(ipart)%rResForceY(5) = &
      p_rparticleCollection%p_rParticles(ipart)%rResForceY(4)      
      ...
      (p_rparticleCollection%p_rParticles(ipart)%rResForceY(6) + &
      0.5_dp*dvely)
      ...
      p_rparticleCollection%p_rParticles(ipart)%rResForceX(6)=&
      p_rparticleCollection%p_rParticles(ipart)%rResForceX(6) + dvelx
      p_rparticleCollection%p_rParticles(ipart)%rResForceY(6)=&
      p_rparticleCollection%p_rParticles(ipart)%rResForceY(6) + dvely

      print *,"--------------------------"
      print *,"VelocityYTotalnachher: ",p_rparticleCollection%p_rParticles(ipart)%rResForceY(6)     

      
      if(ipart .eq. 1)then
        ! update the frame velocity
        rproblem%ddFrameVel = dvely/dtimestep
        rproblem%dFrameVel  = rproblem%DVelY(1)
        !rproblem%dFrameVel  = dvely
      end if
      ...
      p_rparticleCollection%p_rParticles(ipart)%rResForceX(6)
      print *,"VelocityYTotalvorher: ",p_rparticleCollection%p_rParticles(ipart)%dtransVelY
      p_rparticleCollection%p_rParticles(ipart)%dtransVelY   = &
      p_rparticleCollection%p_rParticles(ipart)%rResForceY(6)


      print *,"--------------------------"
      print *,"VelocityYTotalnachher1: ",p_rparticleCollection%p_rParticles(ipart)%rResForceY(6)     

      print *,"--------------------------"
      print *,"VelocityYTotalnachher2: ",p_rparticleCollection%p_rParticles(ipart)%dtransVelY     
      
      
!      if (PARTICLES(IP,4,1).lt.0.04d0) then
!       ! position change
!       PARTICLES(IP,4,1) = 0.04d0
!       ! invert the velocity
!       PARTICLES(IP,5,1) = -PARTICLES(IP,5,1)
!      end if
!
!      if (PARTICLES(IP,4,1).gt.0.66d0) then
!       PARTICLES(IP,4,1) = 0.66d0
!       PARTICLES(IP,5,1) = -PARTICLES(IP,5,1)
!      end if
!
!      if (PARTICLES(IP,4,2).lt.0.04d0) then
!       PARTICLES(IP,4,2) = 0.04d0
!       PARTICLES(IP,5,2) = -PARTICLES(IP,5,2)
!      end if

      if (dCenterY .lt. 0.15_dp) then
       p_rgeometryObject%rcoord2D%Dorigin(2) = 0.15_dp
       p_rparticleCollection%p_rParticles(ipart)%dtransVelY = &
       -p_rparticleCollection%p_rParticles(ipart)%dtransVelY
...
!<subroutine>
  subroutine cc_updateParticlePositionBench(rpostprocessing,rproblem,dtimestep)
!<description>
  ! 
!</description>
