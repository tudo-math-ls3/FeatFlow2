...
! and non-constant coefficients or the defect vector as can be
! indicated via the constants CONV_MODMATRIX CONV_MODDEFECT
!</description>
...
!</subroutine>
    ...
    if ((cdef .eq. CONV_MODDEFECT) .or. (cdef .eq. CONV_MODBOTH)) then
      if ((.not. present(rsolution)) .or. (.not. present(rdefect))) then
        print *,'cc_NonlinBlinform: Solution/defect vector not present!'
        call sys_halt()
      end if
    end if
    ...
    if ((rmatrix%RmatrixBlock(1,1)%cmatrixFormat .ne. LSYSSC_MATRIX9) .and. &
      ...
      print *,'cc_NonlinBlinform: Unsupported matrix format'
      call sys_halt()
    end if
    ...
    if ((rconfig%dnewton .ne. 0.0_DP) .or. (rconfig%dnewtonTransposed .ne. 0.0_DP)) then
      if (.not. lsysbl_isSubmatrixPresent(rmatrix,1,2) .or. &
        ...
        print *,'cc_NonlinBlinform: For the Newton matrix, A12 and A21 must be defined!'
        call sys_halt()
      end if
      if (lsyssc_isMatrixContentShared ( &
        ...
        print *,'cc_NonlinBlinform: For the Newton matrix, the matrix blocks must be indepentent!'
        call sys_halt()
      end if
    end if
    ...
    if (rmatrix%RmatrixBlock(1,1)%p_rspatialDiscrTest%ccomplexity &
      ...
      print *,'cc_NonlinBlinform: Unsupported discretisation.'
      call sys_halt()
    end if

    if ((rvecPrimary%cdataType .ne. ST_DOUBLE) .or. &
        (rvecSecondary%cdataType .ne. ST_DOUBLE)) then
      print *,'cc_NonlinBlinform: Unsupported vector data type in velocity.'
      call sys_halt()
    end if
    ...
    if (present(rdefect)) then
      if ((rsolution%cdataType .ne. ST_DOUBLE) .or. &
        ...
        print *,'cc_NonlinBlinform: Unsupported vector data type in solution/defect'
        call sys_halt()
      end if
    end if
