...
!</subroutine>
    ...
    do i=rmeshHierarchy%nlevels+1,max(rmeshHierarchy%nmaxlevels,nlevels)
      ...
      call tria_refine2LevelOrdering (rmeshHierarchy%p_Rtriangulations(i-1),&
          rmeshHierarchy%p_Rtriangulations(i),p_rboundary,ctriaflags)
...
!<subroutine>
  ...
  subroutine mshh_initHierarchyTria (rmeshHierarchy,rtriangulation,nmaxLevels,&
      rboundary,cdupFlag)
...
!<output>
  ! The mesh space hierarch structure to create
