...
!    ilvmax = collct_getvalue_int (p_rcollection,'NLMAX')
  ...
  !</subroutine>
    ...
    contains
      ...
      ! local variables
      real(DP) :: dnewton
      integer :: ilev,icol,iblockA11,iblockA22
...
!     ! ----------------------------------------------------------------------- 
!     ! Output blockmatrices
!     ! ----------------------------------------------------------------------- 
!     ! If activated, output a matrix block (A11, A22, ...) on the maximum level. 
!     ! Default value is no output. Read in the corresponding parameters (iblockA11, iblockA22,...)

         call parlst_getvalue_int (rproblem%rparamList, 'CC-POSTPROCESSING', &
                                  'IBLOCKA11', iblockA11,0)
         call parlst_getvalue_int (rproblem%rparamList, 'CC-POSTPROCESSING', &
                                  'IBLOCKA22', iblockA22,0)
