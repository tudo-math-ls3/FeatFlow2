...
!~~~~~~~~~~~~~~~~CH problem~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  type(t_CHproblem), intent(IN),  target, optional :: rCHproblem
  type(t_vectorBlock), intent(IN), target, optional :: rCHvector
  type(t_vectorBlock), intent(IN), target, optional :: rCHrhs
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
!</inputoutput>
...
! Mcai, take care.
    ...
    ! gravity will depend on concentration
    rproblem%rcollection%p_rvectorQuickAccess1 => rCHvector
    ! Add gravity
...
!~~~~~~~~~CH problem needs to be used to generate RHS, time dependent case~~~~
    if(present(rCHvector)) then
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  
      ! Our surface tension is based on FengXiaobing's paper, so only one 1 itermCount
      rlinform%itermCount = 1
      rlinform%Idescriptors(1) = DER_FUNC


!~~~~~We neeed to modify this code so that it coupled with CahnHilliard eqn~~~~~~~
!~~~~~Here, We need information from CH problem, more than gravity~~~~~~~~~~~~~
...
! for (1-c^2)/4(\nabla \ha(w), {\bf v})
      call linf_buildVectorScalar (&
              ...
              rrhs%RvectorBlock(1),coeff_RHS_x1,&
              rproblem%rcollection)

      ! And the Y-velocity part:
      call linf_buildVectorScalar (&
              p_rdiscretisation%RspatialDiscr(2),rlinform,.false.,&
              rrhs%RvectorBlock(2),coeff_RHS_y1,&
              rproblem%rcollection)
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
! for ({\bf v} \cdot \nabal c, w)
      call linf_buildVectorScalar (&
              p_rdiscretisation%RspatialDiscr(1),rlinform,.false.,&
              rrhs%RvectorBlock(1),coeff_RHS_x2,&
              rproblem%rcollection)
...
!     print *, p_vectordata(344)
      ...
      call linf_buildVectorScalar (&
              p_rdiscretisation%RspatialDiscr(2),rlinform,.false.,&
              rrhs%RvectorBlock(2),coeff_RHS_y2,&
              rproblem%rcollection)
