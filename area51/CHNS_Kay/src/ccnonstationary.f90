...
module ccnonstationary
 ...
 ! use CahnHilliard module to provide all needed from CahnHilliard 
 ! equation    
...
contains
  ...
  ! *********************Main subroutine for CHNS eqn*********************
  
!<subroutine>

!~~~~It is modified from cc_solveNonstationary
  subroutine CHNS_solveNonstationary (rproblem,rvector,rrhs,rpostprocessing, &
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!!<inputoutput> for CahnHilliard problem 
!  ! A problem structure saving problem-dependent information.
...
!
!  ! Postprocessing structure. Defines what to do with solution vectors.
   type(t_c2d2postprocessing), intent(INOUT) :: rCHpostprocessing
!</inputoutput>
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
!~~~~~~~~~~~~~~local variables for CH problem~~~~~~~~~~~~~~~~~~~~~~~~~~
    !local variables for CH problem
    type(t_CHNonlinearIteration) :: rCHnonlinearIteration

    ! The nonlinear solver configuration
    type(t_nlsolNode) :: rCHnlSol  
!~~~~~~~~~~~~~~local variables for CH problem~~~~~~~~~~~~~~~~~~~~~~~~~~
...
!~~~~~~~~~~~~~~~~CH problem, postprocessing~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    call output_line ('Start CH problem')

!Mcai, here we use same nonlinear solver control as cc2d
    call parlst_readfromfile(rCHproblem%rparamList, 'data/CH_solver.dat')
    call CH_getNonlinearSolver (rCHnlSol, rCHproblem%rparamList, 'CH2D-NONLINEAR')

    ! Initialise the nonlinear loop. This is to prepare everything for
    ! our callback routines that are called from the nonlinear solver.
    ! The preconditioner in that structure is initialised later.
    call parlst_readfromfile(rCHproblem%rparamList, 'data/nonlinsol_cc2d.dat')
    call CH_initNonlinearLoop (rCHproblem,rCHproblem%NLMIN,rCHproblem%NLMAX,&
        rCHvector,rCHrhs,rCHnonlinearIteration,'CC2D-NONLINEAR')

!MCai, we first initialize linear and nonlinear solvers.
!    call CH_initSolver (rCHproblem)


    ! Get our right hand side / solution / matrix on the finest
    ! level from the problem structure.
!    p_rmatrix => rCHproblem%RlevelInfo(NLMAX)%rmatrix
!    p_rrhs    => rCHproblem%rrhs 

 
!~~~~~~~~~~~~~~~~~~~Preparing CH time loop~~~~~~~~~~~~~~~~~~~~~~~
!    ! Initialise the time stepping scheme according to the problem configuration
    call parlst_readfromfile(rCHproblem%rparamList, 'data/timediscr.dat')
	! MCai, the time stepping of CH problem is not necessary, because we already have one
    call CH_initTimeSteppingScheme (rCHproblem%rparamList, &
	                  rCHproblem%rtimedependence%rtimestepping)

  
    ! Initialise the preconditioner for the nonlinear iteration
    call parlst_readfromfile(rCHproblem%rparamList, 'data/nonlinsol_cc2d.dat')
  
    call CH_initPreconditioner (rCHproblem,&
         rCHnonlinearIteration,rCHvector,rCHrhs, &
		  rproblem, rvector)     ! We need to use rNSvector to generate convective term
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    ! maintained in the timeloop.
    ! Afterwards, we can start the timeloop.
!    call CH_implementBC (rCHproblem,rCHvector,rCHrhs,.true.,.false.)
    call CH_implementBC (rCHproblem,rCHvector,rCHrhs,1.0_DP)

    ! Postprocessing. Write out the initial solution.
    call output_line ('CH problem, Starting postprocessing of initial solution...')
    call CH_postprocessing (rCHproblem,rCHvector,0,0.0_DP)

!    print *, 'Before time loop, make sure everything is correct'

    rCHproblem%rtimedependence%dtime = rCHproblem%rtimedependence%dtimemin
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	...
	! we treat CH_postprocessing after 1 time step
	rCHproblem%rtimedependence%iiteration=1

!MCai, debug make sure that before time loop, everything is correct. 
!    stop
    !----------------------------------------------------
    ! Timeloop
    !----------------------------------------------------
   ...
   ! stop 
    ...
    do while ((rproblem%rtimedependence%itimeStep .le. &
              ...
              (irepetition .le. rCHproblem%rtimedependence%niterations) .and. &
              (rCHproblem%rtimedependence%dtime .lt. rCHproblem%rtimedependence%dtimemax))

! evaluate rCHproblem%rtimedependence%iiteration, so that it is equal to iprepetion
! it is better increase iiteration once after each time step
!      rCHproblem%rtimedependence%iiteration=iprepetion

!~~~~~~~~~ Combined it with CH problem
!     DO WHILE ((iiteration .LE. rCHproblem%rtimedependence%niterations) .AND. &
!              (rCHproblem%rtimedependence%dtime .LT. rCHproblem%rtimedependence%dtimemax))
 ...
 !~~~~We may need also time step...
!Mcai, we use the following to realize coupling 
         Print *, 'we first solve Cahn Hilliard problem, then solve NS problem'
         call output_separator(OU_SEP_MINUS)
         call output_line ('Time step '//TRIM(sys_siL(rCHproblem%rtimedependence%iiteration,6))// &
                       '     Time '//TRIM(sys_sdL(rCHproblem%rtimedependence%dtime,5)))
         call output_lbrk ()
     
      	 call CH_timestep (rCHproblem,rCHvector,rCHrhs, rtimestepping, &
            rCHnonlinearIteration,rCHnlSol, rproblem, rvector, rrhs)
         
!    	 print *, 'stop after one step of CH problem, make sure it is correct'
!         stop
         
! MCai, or replace the rCHproblem%rtimedependence(rCHproblem%NLMAX)%rtimestepping by rtimestepping
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 		          
 
 !~~~~~~~~~~~~~~~Perform one step for NS equation~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 !~~~~We may need other input parameters, like rCHproblem, rCHvector, rCHrhs
...
!~~~~~~~~~~~~~~~We add a code for one time step of Cahn-Hilliard Eqn~~~~~~~~~~~~~~
!         call CH_performTimestep (rCHproblem, rCHrhs, rCHtempBlock)
        call output_separator(OU_SEP_MINUS)
        call output_line ('Time step '//TRIM(sys_siL(rCHproblem%rtimedependence%iiteration,6))// &
                        '     Time '//TRIM(sys_sdL(rCHproblem%rtimedependence%dtime,5)))
        call output_lbrk ()
        ...
        call CH_timestep (rCHproblem,rCHvector,rCHrhs, rtimestepping,& 
                          rCHnonlinearIteration,rCHnlSol, &
			              rproblem, rvector, rrhs)

        write(*,*)'we finished one CH problem  step, then NS problem '
!        stop
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
!~~~~~~~~~~~~~~~~CH problem, postprocessing~~~~~~~~~~~~~~~~~~`~~~~
        call output_separator(OU_SEP_MINUS)
        call output_line ('CH problem, Starting postprocessing of the time step...')

        ! Postprocessing. Write out the solution if it was calculated successfully.
!        call CH_postprocessing (rCHproblem,rCHvector,rCHpostprocessing)
        call CH_postprocessing (rCHproblem,rCHvector,rCHproblem%rtimedependence%iiteration,&
                    rproblem%rtimedependence%dtime)
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      ...
      rCHproblem%rtimedependence%iiteration=rCHproblem%rtimedependence%iiteration+1
...
!~~~~~~~~~~~~~CH problem done loop~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!MCai, we do something here. release Preconditioner and NonlinearLoop
    ! Release the preconditioner
    call CH_releasePreconditioner (rCHnonlinearIteration)
    ! Release parameters of the nonlinear loop, final clean up
    call CH_doneNonlinearLoop (rCHnonlinearIteration)
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
!~~~~~~~~~~~~~~~~~~~CH problem~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  type(t_CHproblem), intent(INOUT) :: rCHproblem    !~~~Q: target or not?
  type(t_vectorBlock), intent(INOUT) :: rCHvector
  type(t_vectorBlock), intent(INOUT) :: rCHrhs
