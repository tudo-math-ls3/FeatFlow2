...
!</subroutine>
    ...
    character(LEN=PARLST_MLDATA), dimension(6) :: sbdex
    ...
    do ibdComponent = 1,boundary_igetNBoundComp(p_rboundary)
      ...
      if (i .ne. 0) then
        ...
        do isegment = 1,parlst_querysubstrings (p_rbdcond, cstr)
          ...
          read(cstr,*) dpar2,iintervalEnds,(ibctyp(n),n=1,6)
...
!             case (1)
              ...
              ! Read the line again, get the expressions for X- and Y- disp. and vel.
              read(cstr,*) dvalue,iintervalEnds,(ibctyp(n),n=1,6),(sbdex(n),n=1,6)
		...
		if (sbdex(k) .ne. '') then
                ...
                case (BDC_VALDOUBLE,BDC_VALPARPROFILE)
                  ...
                  rcoll%Dquickaccess(4) = &
                      collct_getvalue_real (rcoll, sbdex(k), 0, SEC_SBDEXPRESSIONS)
                case (BDC_EXPRESSION)
                  ! Expression. Write the identifier for the expression
                  ! as itag into the boundary condition structure.
                  rcoll%IquickAccess(4) = &
                      collct_getvalue_int (rcoll, sbdex(k), 0, SEC_SBDEXPRESSIONS)
                end select
              
                ! Assemble the BC`s.
                call bcasm_newDirichletBConRealBD (&
                    rdiscretisation,k,rboundaryRegion,rdynamicLevelInfo%rdiscreteBC,&
...
!               read(cstr,*) dvalue,iintervalEnds,ibctyp(1),sbdex(1)
!               
...
! !               end if
! 
!               if (sbdex(1) .ne. '') then ! sbdex(1) for pressure drop
!               
...
!                   
!                   ! IquickAccess(3) saves the type of the expression
!                   iexptyp = collct_getvalue_int (rcoll, sbdex(1))
!                   rcoll%IquickAccess(3) = iexptyp
! 
!                   ! The first element in the sting quick access array is
!                   ! the name of the expression to evaluate.
!                   rcoll%SquickAccess(1) = sbdex(1)
!                   
!                   ! Dquickaccess(4) / IquickAccess(4) saves information
!                   ! about the expression.
!                   select case (iexptyp)
...
!                     ! Constant or parabolic profile
!                     rcoll%Dquickaccess(4) = &
!                         collct_getvalue_real (rcoll,sbdex(1), 0, SEC_SBDEXPRESSIONS)
!                   case (BDC_EXPRESSION)
!                     ! Expression. Write the identifier for the expression
!                     ! as itag into the boundary condition structure.
!                     rcoll%IquickAccess(4) = &
!                         collct_getvalue_int (rcoll,sbdex(1), 0, SEC_SBDEXPRESSIONS)
!                   end select
!                 
!                   IvelEqns = (/5,6/) !/***/ (/1,2/) ?
...
!<input>
  ...
  ! =0: Any displacement and velocity component, 
  ! =1: X-displacement, =2: Y-displacement                     
  ! =3: X-velocity, =4: Y-velocity   for solid skeleton                   
  ! =5: X-velocity, =6: Y-velocity   for pore fluid                     
...
!</subroutine>
    ...
    integer :: i, j, n, icount, hedgeslocal
    integer :: ibdComponent,isegment,iintervalEnds
    integer, dimension(6) :: ibctyp
    real(DP) :: dvalue,dpar1,dpar2
    character(LEN=PARLST_MLDATA) :: cstr,cexpr
    character(LEN=PARLST_MLDATA), dimension(6) :: sbdex
    ! A set of variables describing the analytic boundary conditions.
      ...
      if (i .ne. 0) then
        ...
        do isegment = 1,parlst_querysubstrings (p_rbdcond, cstr)
          ...
          if (dpar2 .ge. dpar1) then
            ...
            case (1)
              ! Simple Dirichlet boundary
              ! Read the line again, get the expressions for X- and Y- disp. & vel.
              read(cstr,*) dvalue,iintervalEnds,(ibctyp(n),n=1,6),(sbdex(n),n=1,6)
...
!<input>
  ...
  !   should be calculated for (e.g. 1=1st solution component, e.g. X-displacement,
  !   2=2nd solution component, e.g. y-displacement,...,
  !   7=7th solution component, e.g. pressure)
  ! For pressure drop boundary / normal stress:
  !   Velocity components that are affected by the fluid normal stress
  !   (usually "5 6" for x- and y-velocity while returned value must specify
