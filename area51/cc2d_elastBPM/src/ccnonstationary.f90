...
!<description>
  ...
  !
  ! Note: This will not allocate memory but only initialise the parameters
...
!</subroutine>
    ...
    ! cc_initNonlinMatrix is found in ccnonlinearcore.f90
    ...
    if (ipressureFullyImplicit .ne. 1) then
      call lsyssc_scaleVector (rvector%RvectorBlock(3*NDIM2D+1),&
...
!</subroutine>
    ...
    if ((rtimestepping%ctimestepType .eq. TSCHM_FRACTIONALSTEP) .or. &
      ...
      call lsyssc_vectorLinearComb (rvectorOld%RvectorBlock(3),&
          rvectorInt%RvectorBlock(3),&
          1.0_DP-rtimestepping%dtheta,rtimestepping%dtheta)
      call lsyssc_vectorLinearComb (rvectorOld%RvectorBlock(4),&
          rvectorInt%RvectorBlock(4),&
          1.0_DP-rtimestepping%dtheta,rtimestepping%dtheta)

      call lsyssc_vectorLinearComb (rvectorOld%RvectorBlock(5),&
          rvectorInt%RvectorBlock(5),&
          1.0_DP-rtimestepping%dtheta,rtimestepping%dtheta)
      call lsyssc_vectorLinearComb (rvectorOld%RvectorBlock(6),&
          rvectorInt%RvectorBlock(6),&
          1.0_DP-rtimestepping%dtheta,rtimestepping%dtheta)    
    end if
...
!</subroutine>
    ...
    do while ((rproblem%rtimedependence%itimeStep .le. &
      ...
      if (.not. babortTimestep) then
	    ...
	    ! go to cctimeanalysis.f90 / function cc_timeErrorByPredictor
	    ! there search for (rtimeErrorLocal%)
            call output_line ('Time error: ' &
                ...
                //'VS(L2)=' &
                //trim(sys_sdEP(rtimeError%drelVSL2/real(i,DP),8,2)) &
                //' VS(MX)=' &
                //trim(sys_sdEP(rtimeError%drelVSmax/real(i,DP),8,2)) &
                //'VF(L2)=' &
                //trim(sys_sdEP(rtimeError%drelVFL2/real(i,DP),8,2)) &
                //' VF(MX)=' &
                //trim(sys_sdEP(rtimeError%drelVFmax/real(i,DP),8,2)) &
                //'  P(L2)=' &
      ...
      if (.not. babortTimestep) then
        ...
        call output_line ('Time derivative:  ' &
            ...
            //' RELVS(L2)=' &
            //trim(sys_sdEP(rtimeDerivative%drelVSL2,9,2)) &
            //' RELVF(L2)=' &
            //trim(sys_sdEP(rtimeDerivative%drelVFL2,9,2)) &
            //'  RELP(L2)=' &
