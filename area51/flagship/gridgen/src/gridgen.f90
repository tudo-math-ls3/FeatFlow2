program agridgen
  ...
  do i = 1, command_argument_count()
    ...
    case('-H','--help')
      ...
      write(*,'(A,T30,A)') '-D,  --delaunay','Delaunay triangulation o&
                                              &f the inner cycle'
  ...
  if (bexternalDelaunay) then
    ...
    call system('triangle -V -Y -D -e -a'//trim(adjustl(cbuffer1))&
  ...
  write(cbuffer1, '(I10)') nsegments*nlayers + nel
  write(cbuffer2, '(I10)') nsegments*nlayers + nvt
