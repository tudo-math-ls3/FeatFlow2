...
module zpinch_basic
  ...
  private
  public :: t_zpinch
  public :: zpinch_getNVAR
  public :: zpinch_getVariable
  public :: zpinch_getVarInterleaveFormat
  public :: zpinch_getVarBlockFormat

  interface zpinch_getNVAR
    module procedure zpinch_getNVARProblemLevel
    module procedure zpinch_getNVARDescriptor
  end interface
...
!<constantblock description="Global constants for number of variables">
  ...
  integer, parameter, public :: NVAR1D = 4

  ! number of solution components in 2D
  integer, parameter, public :: NVAR2D = 5

  ! number of solution components in 3D
  integer, parameter, public :: NVAR3D = 6
...
!<function>
  ...
  pure function zpinch_getNVARProblemLevel(rproblemLevel) result(NVAR)
...
!</function>
    ...
    NVAR = rproblemLevel%rtriangulation%ndim + 3
    
  end function zpinch_getNVARProblemLevel

  !*****************************************************************************

!<function>

  pure function zpinch_getNVARDescriptor(rappDescriptor) result(NVAR)
...
!</function>
    ...
    NVAR = rappDescriptor%ndimension + 3
    
  end function zpinch_getNVARDescriptor
...
!</subroutine>
    ...
    if (rvectorBlock%nblocks .eq. 1) then
      ...
      call zpinch_getVarInterleaveFormat(neq, nvar, cvariable, p_Ddata, p_Dvalue)
      ...
      ! Fill the scalar vector with data from the variable
      call zpinch_getVarBlockFormat(neq, nvar, cvariable, p_Ddata, p_Dvalue)
...
!<subroutine>
  ...
  pure subroutine zpinch_getVarInterleaveFormat(neq, nvar, cvariable, Ddata, Dvalue)
...
!</subroutine>
    ...
    case ('ENERGY')
      do ieq = 1, neq
        Dvalue(ieq) = Ddata(nvar-1, ieq)/Ddata(1, ieq)
      end do

    case ('EFFECTIVE_ENERGY')
      do ieq = 1, neq
        Dvalue(ieq) = Ddata(nvar-1, ieq)
      end do
      ...
      case (NVAR1D)
        do ieq = 1, neq
          Dvalue(ieq) = thdyn_pressure(GAMMA,&
              Ddata(nvar-1, ieq)/Ddata(1, ieq), Ddata(1, ieq),&
  ...
  end subroutine zpinch_getVarInterleaveFormat
  
  !*****************************************************************************

!<subroutine>
  
   pure subroutine zpinch_getVarBlockformat(neq, nvar, cvariable, Ddata, Dvalue)
...
!</subroutine>
    ...
    case ('ENERGY')
      do ieq = 1, neq
        Dvalue(ieq) = Ddata(ieq, nvar-1)/Ddata(ieq, 1)
      end do

    case ('EFFECTIVE_ENERGY')
      do ieq = 1, neq
        Dvalue(ieq) = Ddata(ieq, nvar-1)
      end do
    ...
    case ('PRESSURE')
      ...
      case (NVAR1D)
        do ieq = 1, neq
          Dvalue(ieq) = thdyn_pressure(GAMMA,&
              Ddata(ieq, nvar-1)/Ddata(ieq, 1), Ddata(ieq, 1),&
  ...
  end subroutine zpinch_getVarBlockformat
