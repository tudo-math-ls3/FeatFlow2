...
!#
!# 3.) zpinch_calcAdaptationIndicator
...
!<subroutine>
  ...
  subroutine zpinch_calcAdaptationIndicator(rsolutionEuler, rsolutionTransport, rindicator)
    ...
    elements: do iel = 1, p_rtriangulation%NEL
      ...
      vertices: do ive = 1, nve
        ...
        dgradient = max(dgradient,&
            abs(abs(p_Ddensity(i))  - abs(p_Ddensity(j)))  / max(abs(p_Ddensity(i)),  abs(p_Ddensity(j))),&
            abs(abs(p_Dpressure(i)) - abs(p_Dpressure(j))) / max(abs(p_Dpressure(i)), abs(p_Dpressure(j))),&
            abs(abs(dlambda_i)      - abs(dlambda_j))      / max(1e-2, abs(dlambda_i), abs(dlambda_j))     )
      end do vertices
