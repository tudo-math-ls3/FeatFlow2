...
module zpinch_application
  ...
  private
  public :: zpinch_app
  public :: zpinch_initApplication
  public :: zpinch_initCollection
  public :: zpinch_initSolvers
  public :: zpinch_initProblem
  public :: zpinch_initProblemLevel
  public :: zpinch_initAllProblemLevels
  public :: zpinch_initSolution
  public :: zpinch_outputSolution
  public :: zpinch_outputStatistics
  public :: zpinch_estimateRecoveryError
...
!</subroutine>
    ...
    call zpinch_initApplication(rparlist, 'zpinch', rappDescriptor)
    ...
    call zpinch_initCollection(rappDescriptor, rparlist, 'zpinch', rcollection)
    ...
    call zpinch_initAllProblemLevels(rappDescriptor, rproblem, rcollection)
    ...
    if (rtimestep%dfinalTime > 0) then
      ...
      call parlst_getvalue_string(rparlist, 'zpinch', 'indatfile', sindatfileName)
      ...
      call parlst_getvalue_string(rparlist, 'zpinch', 'sprimalbdrcondname', sbdrcondName)
      ...
      case ('transient_primal')
        ...
        !- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        call zpinch_solveTransientPrimal(rappDescriptor, rparlist, 'zpinch',&
    ...
    ! Output statistics
    call zpinch_outputStatistics(rappDescriptor, rtimerTotal)
...
!<subroutine>
  ...
  subroutine zpinch_initApplication(rparlist, ssectionName, rappDescriptor)
...
!<output>
    ...
    type(t_zpinch), intent(OUT) :: rappDescriptor
!</output>
!</subroutine>
  ...
  end subroutine zpinch_initApplication

  !*****************************************************************************

!<subroutine>

  subroutine zpinch_initCollection(rappDescriptor, rparlist, ssectionName, rcollection)
...
!<input>
    ! application descriptor
    type(t_zpinch), intent(IN) :: rappDescriptor
...
!</subroutine>
  ...
  end subroutine zpinch_initCollection
...
!<subroutine>
  ...
  subroutine zpinch_initProblem(rparlist, ssectionName, nlmin, nlmax,&
                               rproblem, rcollection)
...
!<subroutine>
  ...
  subroutine zpinch_initProblemLevel(rappDescriptor, rproblemLevel, rcollection)
...
!</subroutine>
    ...
    if (discretisation > 0) then
      ...
      if (p_rdiscretisation%ndimension .eq. 0) then
        ...
        case (SYSTEM_BLOCKFORMAT)
          call spdiscr_initBlockDiscr(p_rdiscretisation,&
                                      zpinch_getNVAR(rappDescriptor), p_rtriangulation)
        case DEFAULT
          call output_line('Unsupported system format!',&
                           OU_CLASS_ERROR,OU_MODE_STD,'zpinch_initProblemLevel')
          call sys_halt()
        end select
      end if
      ...
      if (rappDescriptor%isystemFormat .eq. SYSTEM_BLOCKFORMAT) then
        do ivar = 2, zpinch_getNVAR(rappDescriptor)
    ...
    if (systemMatrix > 0) then
      ...
      case (SYSTEM_INTERLEAVEFORMAT)
        ...
        if (lsyssc_hasMatrixStructure(rproblemLevel%Rmatrix(systemMatrix))) then
          ...
          rproblemLevel%Rmatrix(systemMatrix)%NVAR = zpinch_getNVAR(rappDescriptor)
      ...
      case (SYSTEM_BLOCKFORMAT)
        ...
        if ((rproblemLevel%RmatrixBlock(systemMatrix)%nblocksPerRow .ne. 0) .and.&
          ...
          case (SYSTEM_SEGREGATED)
            ...
            do ivar = 1, zpinch_getNVAR(rappDescriptor)
          ...
          case (SYSTEM_ALLCOUPLED)
            ...
            do ivar = 1, zpinch_getNVAR(rappDescriptor)
              do jvar = 1, zpinch_getNVAR(rappDescriptor)
                call lsyssc_resizeMatrix(&
  ...
  end subroutine zpinch_initProblemLevel

  !*****************************************************************************

!<subroutine>

  subroutine zpinch_initAllProblemLevels(rappDescriptor, rproblem, rcollection)
...
!</subroutine>
    ...
    do while(associated(p_rproblemLevel))
      ...
      call zpinch_initProblemLevel(rappDescriptor, p_rproblemLevel, rcollection)
      
      ! Switch to next coarser level
      p_rproblemLevel => p_rproblemLevel%p_rproblemLevelCoarse
    end do

  end subroutine zpinch_initAllProblemLevels

  !*****************************************************************************

!<subroutine>

  subroutine zpinch_initSolution(rparlist, ssectionName, rproblemLevel, dtime, rvector)
...
!</subroutine>
    ...
    case DEFAULT
      call output_line('Invalid type of solution profile!',&
                       OU_CLASS_ERROR, OU_MODE_STD, 'zpinch_initSolution')
      call sys_halt()
    end select

  end subroutine zpinch_initSolution
...
!</subroutine>
    ...
    isize = size(p_Dsolution)/zpinch_getNVAR(rproblemLevel)
    ...
    case(SYSTEM_INTERLEAVEFORMAT)
      ...
      select case(ndim)
      case (NDIM1D)
        call zpinch_getVarInterleaveFormat(rvector1%NEQ, NVAR1D, 'velocity_x', p_Dsolution, p_Ddata1)
        call ucd_addVarVertBasedVec(rexport, 'velocity', p_Ddata1)

        call zpinch_getVarInterleaveFormat(rvector1%NEQ, NVAR1D, 'density', p_Dsolution, p_Ddata1)
        call ucd_addVariableVertexBased (rexport, 'density', UCD_VAR_STANDARD, p_Ddata1)

        call zpinch_getVarInterleaveFormat(rvector1%NEQ, NVAR1D, 'energy', p_Dsolution, p_Ddata1)
        call ucd_addVariableVertexBased (rexport, 'energy', UCD_VAR_STANDARD, p_Ddata1)

        call zpinch_getVarInterleaveFormat(rvector1%NEQ, NVAR1D, 'tracer', p_Dsolution, p_Ddata1)
        call ucd_addVariableVertexBased (rexport, 'tracer', UCD_VAR_STANDARD, p_Ddata1)
        
        call zpinch_getVarInterleaveFormat(rvector1%NEQ, NVAR1D, 'pressure', p_Dsolution, p_Ddata1)
        call ucd_addVariableVertexBased (rexport, 'pressure', UCD_VAR_STANDARD, p_Ddata1)

        call zpinch_getVarInterleaveFormat(rvector1%NEQ, NVAR1D, 'machnumber', p_Dsolution, p_Ddata1)
        call ucd_addVariableVertexBased (rexport, 'machnumber', UCD_VAR_STANDARD, p_Ddata1)
        
        
      case (NDIM2D)
        call zpinch_getVarInterleaveFormat(rvector1%NEQ, NVAR2D, 'velocity_x', p_Dsolution, p_Ddata1)
        call zpinch_getVarInterleaveFormat(rvector2%NEQ, NVAR2D, 'velocity_y', p_Dsolution, p_Ddata2)
        call ucd_addVarVertBasedVec(rexport, 'velocity', p_Ddata1, p_Ddata2)

        call zpinch_getVarInterleaveFormat(rvector1%NEQ, NVAR2D, 'density', p_Dsolution, p_Ddata1)
        call ucd_addVariableVertexBased (rexport, 'density', UCD_VAR_STANDARD, p_Ddata1)

        call zpinch_getVarInterleaveFormat(rvector1%NEQ, NVAR2D, 'energy', p_Dsolution, p_Ddata1)
        call ucd_addVariableVertexBased (rexport, 'energy', UCD_VAR_STANDARD, p_Ddata1)
        
        call zpinch_getVarInterleaveFormat(rvector1%NEQ, NVAR2D, 'tracer', p_Dsolution, p_Ddata1)
        call ucd_addVariableVertexBased (rexport, 'tracer', UCD_VAR_STANDARD, p_Ddata1)

        call zpinch_getVarInterleaveFormat(rvector1%NEQ, NVAR2D, 'pressure', p_Dsolution, p_Ddata1)
        call ucd_addVariableVertexBased (rexport, 'pressure', UCD_VAR_STANDARD, p_Ddata1)

        call zpinch_getVarInterleaveFormat(rvector1%NEQ, NVAR2D, 'machnumber', p_Dsolution, p_Ddata1)
        call ucd_addVariableVertexBased (rexport, 'machnumber', UCD_VAR_STANDARD, p_Ddata1)

      case (NDIM3D)
        call zpinch_getVarInterleaveFormat(rvector1%NEQ, NVAR3D, 'velocity_x', p_Dsolution, p_Ddata1)
        call zpinch_getVarInterleaveFormat(rvector2%NEQ, NVAR3D, 'velocity_y', p_Dsolution, p_Ddata2)
        call zpinch_getVarInterleaveFormat(rvector3%NEQ, NVAR3D, 'velocity_z', p_Dsolution, p_Ddata3)
        call ucd_addVarVertBasedVec(rexport, 'velocity', p_Ddata1, p_Ddata2, p_Ddata3)
        
        call zpinch_getVarInterleaveFormat(rvector1%NEQ, NVAR3D, 'density', p_Dsolution, p_Ddata1)
        call ucd_addVariableVertexBased (rexport, 'density', UCD_VAR_STANDARD, p_Ddata1)

        call zpinch_getVarInterleaveFormat(rvector1%NEQ, NVAR3D, 'energy', p_Dsolution, p_Ddata1)
        call ucd_addVariableVertexBased (rexport, 'energy', UCD_VAR_STANDARD, p_Ddata1)

        call zpinch_getVarInterleaveFormat(rvector1%NEQ, NVAR3D, 'tracer', p_Dsolution, p_Ddata1)
        call ucd_addVariableVertexBased (rexport, 'tracer', UCD_VAR_STANDARD, p_Ddata1)
        
        call zpinch_getVarInterleaveFormat(rvector1%NEQ, NVAR3D, 'pressure', p_Dsolution, p_Ddata1)
        call ucd_addVariableVertexBased (rexport, 'pressure', UCD_VAR_STANDARD, p_Ddata1)

        call zpinch_getVarInterleaveFormat(rvector1%NEQ, NVAR3D, 'machnumber', p_Dsolution, p_Ddata1)
        call ucd_addVariableVertexBased (rexport, 'machnumber', UCD_VAR_STANDARD, p_Ddata1)
    ...
    case (SYSTEM_BLOCKFORMAT)
      ...
      select case(ndim)
      case (NDIM1D)
        call zpinch_getVarBlockFormat(rvector1%NEQ, NVAR1D, 'velocity_x', p_Dsolution, p_Ddata1)
        call ucd_addVarVertBasedVec(rexport, 'velocity', p_Ddata1)

        call zpinch_getVarBlockFormat(rvector1%NEQ, NVAR1D, 'density', p_Dsolution, p_Ddata1)
        call ucd_addVariableVertexBased (rexport, 'density', UCD_VAR_STANDARD, p_Ddata1)

        call zpinch_getVarBlockFormat(rvector1%NEQ, NVAR1D, 'energy', p_Dsolution, p_Ddata1)
        call ucd_addVariableVertexBased (rexport, 'energy', UCD_VAR_STANDARD, p_Ddata1)

        call zpinch_getVarBlockFormat(rvector1%NEQ, NVAR1D, 'tracer', p_Dsolution, p_Ddata1)
        call ucd_addVariableVertexBased (rexport, 'tracer', UCD_VAR_STANDARD, p_Ddata1)
        
        call zpinch_getVarBlockFormat(rvector1%NEQ, NVAR1D, 'pressure', p_Dsolution, p_Ddata1)
        call ucd_addVariableVertexBased (rexport, 'pressure', UCD_VAR_STANDARD, p_Ddata1)

        call zpinch_getVarBlockFormat(rvector1%NEQ, NVAR1D, 'machnumber', p_Dsolution, p_Ddata1)
        call ucd_addVariableVertexBased (rexport, 'machnumber', UCD_VAR_STANDARD, p_Ddata1)
        
        
      case (NDIM2D)
        call zpinch_getVarBlockFormat(rvector1%NEQ, NVAR2D, 'velocity_x', p_Dsolution, p_Ddata1)
        call zpinch_getVarBlockFormat(rvector2%NEQ, NVAR2D, 'velocity_y', p_Dsolution, p_Ddata2)
        call ucd_addVarVertBasedVec(rexport, 'velocity', p_Ddata1, p_Ddata2)

        call zpinch_getVarBlockFormat(rvector1%NEQ, NVAR2D, 'density', p_Dsolution, p_Ddata1)
        call ucd_addVariableVertexBased (rexport, 'density', UCD_VAR_STANDARD, p_Ddata1)

        call zpinch_getVarBlockFormat(rvector1%NEQ, NVAR2D, 'energy', p_Dsolution, p_Ddata1)
        call ucd_addVariableVertexBased (rexport, 'energy', UCD_VAR_STANDARD, p_Ddata1)
        
        call zpinch_getVarBlockFormat(rvector1%NEQ, NVAR2D, 'tracer', p_Dsolution, p_Ddata1)
        call ucd_addVariableVertexBased (rexport, 'tracer', UCD_VAR_STANDARD, p_Ddata1)

        call zpinch_getVarBlockFormat(rvector1%NEQ, NVAR2D, 'pressure', p_Dsolution, p_Ddata1)
        call ucd_addVariableVertexBased (rexport, 'pressure', UCD_VAR_STANDARD, p_Ddata1)

        call zpinch_getVarBlockFormat(rvector1%NEQ, NVAR2D, 'machnumber', p_Dsolution, p_Ddata1)
        call ucd_addVariableVertexBased (rexport, 'machnumber', UCD_VAR_STANDARD, p_Ddata1)

      case (NDIM3D)
        call zpinch_getVarBlockFormat(rvector1%NEQ, NVAR3D, 'velocity_x', p_Dsolution, p_Ddata1)
        call zpinch_getVarBlockFormat(rvector2%NEQ, NVAR3D, 'velocity_y', p_Dsolution, p_Ddata2)
        call zpinch_getVarBlockFormat(rvector3%NEQ, NVAR3D, 'velocity_z', p_Dsolution, p_Ddata3)
        call ucd_addVarVertBasedVec(rexport, 'velocity', p_Ddata1, p_Ddata2, p_Ddata3)
        
        call zpinch_getVarBlockFormat(rvector1%NEQ, NVAR3D, 'density', p_Dsolution, p_Ddata1)
        call ucd_addVariableVertexBased (rexport, 'density', UCD_VAR_STANDARD, p_Ddata1)

        call zpinch_getVarBlockFormat(rvector1%NEQ, NVAR3D, 'energy', p_Dsolution, p_Ddata1)
        call ucd_addVariableVertexBased (rexport, 'energy', UCD_VAR_STANDARD, p_Ddata1)

        call zpinch_getVarBlockFormat(rvector1%NEQ, NVAR3D, 'tracer', p_Dsolution, p_Ddata1)
        call ucd_addVariableVertexBased (rexport, 'tracer', UCD_VAR_STANDARD, p_Ddata1)

        call zpinch_getVarBlockFormat(rvector1%NEQ, NVAR3D, 'pressure', p_Dsolution, p_Ddata1)
        call ucd_addVariableVertexBased (rexport, 'pressure', UCD_VAR_STANDARD, p_Ddata1)

        call zpinch_getVarBlockFormat(rvector1%NEQ, NVAR3D, 'machnumber', p_Dsolution, p_Ddata1)
        call ucd_addVariableVertexBased (rexport, 'machnumber', UCD_VAR_STANDARD, p_Ddata1)
...
!<subroutine>
  ...
  subroutine zpinch_outputStatistics(rappDescriptor, rtimerTotal)
...
!<input>
    ! application descriptor
    type(t_zpinch), intent(INOUT) :: rappDescriptor
...
!<subroutine>
  ...
  subroutine zpinch_estimateRecoveryError(rparlist, ssectionName, rproblemLevel,&
...
!</subroutine>
    ...
    do ierrorVariable = 1, nerrorVariables
      ...
      call zpinch_getVariable(rsolution, serrorVariable, rvectorScalar)
      ...
      case DEFAULT
        call output_line('Invalid type of error estimator!',&
                         OU_CLASS_ERROR,OU_MODE_STD,'zpinch_estimateRecoveryError')
        call sys_halt()
      end select
  ...
  end subroutine zpinch_estimateRecoveryError
...
!</subroutine>
    ...
    case (SYSTEM_INTERLEAVEFORMAT)
      ...
      case (NDIM1D)
        call zpinch_hadaptCallbackScalar1D(rcollection, HADAPT_OPR_INITCALLBACK, Ivalue, Ivalue)
        call hadapt_performAdaptation(rhadapt, rindicator, rcollection, zpinch_hadaptCallbackScalar1D)
      ...
      case (NDIM3D)
        call zpinch_hadaptCallbackScalar3D(rcollection, HADAPT_OPR_INITCALLBACK, Ivalue, Ivalue)
        call hadapt_performAdaptation(rhadapt, rindicator, rcollection, zpinch_hadaptCallbackScalar3D)
      end select
    ...
    case (SYSTEM_BLOCKFORMAT)
      ...
      select case(rtriangulationSrc%ndim)
      case (NDIM1D)
        call zpinch_hadaptCallbackBlock1D(rcollection, HADAPT_OPR_INITCALLBACK, Ivalue, Ivalue)
        call hadapt_performAdaptation(rhadapt, rindicator, rcollection, zpinch_hadaptCallbackBlock1D)
      ...
      case (NDIM3D)
        call zpinch_hadaptCallbackBlock3D(rcollection, HADAPT_OPR_INITCALLBACK, Ivalue, Ivalue)
        call hadapt_performAdaptation(rhadapt, rindicator, rcollection, zpinch_hadaptCallbackBlock3D)
      end select
...
!<subroutine>
    ...
    subroutine zpinch_solveTransientPrimal(rappDescriptor, rparlist, ssectionName,&
...
!</subroutine>
    ...
    if (p_rdiscretisation%ncomponents .ne. zpinch_getNVAR(p_rproblemLevel)) then
      rsolution%RvectorBlock(1)%NVAR = zpinch_getNVAR(p_rproblemLevel)
      isize = rsolution%NEQ*zpinch_getNVAR(p_rproblemLevel)
    ...
    call zpinch_initSolution(rparlist, ssectionName, p_rproblemLevel,&
    ...
    case (NDIM1D)
      call bdrf_filterVectorExplicit(rbdrCond, p_rproblemLevel%rtriangulation,&
                                     ...
                                     rproblem%rboundary, zpinch_calcBoundaryvalues1d)
    case (NDIM2D)
      call bdrf_filterVectorExplicit(rbdrCond, p_rproblemLevel%rtriangulation,&
                                     rsolution, rtimestep%dinitialTime,&
                                     rproblem%rboundary, zpinch_calcBoundaryvalues2d)
    case (NDIM3D)
      call bdrf_filterVectorExplicit(rbdrCond, p_rproblemLevel%rtriangulation,&
                                     rsolution, rtimestep%dinitialTime,&
                                     rproblem%rboundary, zpinch_calcBoundaryvalues3d)
    end select
      ...
      if ((dstepAdapt > 0.0_DP) .or. (npreadapt > 0)) then
        ...
        if (npreadapt > 0) then
          ...
          do ipreadapt = 1, npreadapt
            ...
            call zpinch_estimateRecoveryError(rparlist, ssectionname, p_rproblemLevel,&
                                             rsolution, rtimestep%dinitialTime,&
    ...
    timeloop: do
      ...
      call zpinch_outputSolution(rparlist, ssectionName, p_rproblemLevel,&
      ...
      case (TSTEP_RK_SCHEME)
        ...
        call tstep_performRKStep(p_rproblemLevel, rtimestep, rsolver,&
                                 rsolution, zpinch_nlsolverCallback, rcollection)
      ...
      case DEFAULT
        call output_line('Unsupported time-stepping algorithm!',&
                         OU_CLASS_ERROR,OU_MODE_STD,'zpinch_solveTransient')
        call sys_halt()
      end select

      ! Perform characteristic FCT postprocessing
      call zpinch_calcLinearizedFCT(rbdrCond, p_rproblemLevel, rtimestep,&
                                    rsolution, rcollection)

      ! Apply the source term
      call zpinch_calcSourceTerm(p_rproblemLevel, rtimestep, rsolution, rcollection)
