...
extern "C"
{
  int hydro_calcFluxRusDiss2d_cuda(unsigned long * h_DcoeffsAtEdge,
				   ...
				   int * nedges,
				   int * iedgeset);
  int FNAME(hydro_calcfluxrusdiss2d_cuda)(unsigned long * h_DcoeffsAtEdge,
					  unsigned long * h_IedgeList,
					  unsigned long * h_Dx,
					  unsigned long * h_Dy,
					  double * dscale,
...
__global__ void hydro_calcFluxRusDiss2d_knl(double * DcoeffsAtEdge,
					    int * IedgeList,
					    double * Dx,
					    double * Dy,
					    double dscale,
  ...
  if (idx<nedges)
  {
    ...
    double d_ij = max( abs(RCONST(0.5)*(IDX3T(DcoeffsAtEdge,1,1,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge)-
		       ...
		       RCONST(0.5)*sqrt(POW(IDX3T(DcoeffsAtEdge,1,1,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge)-
					...
					POW(IDX3T(DcoeffsAtEdge,2,1,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge)-
					    IDX3T(DcoeffsAtEdge,2,2,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge),2))*cj,
		       ...
		       RCONST(0.5)*sqrt(POW(IDX3T(DcoeffsAtEdge,1,2,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge)-
					...
					POW(IDX3T(DcoeffsAtEdge,2,2,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge)-
					    IDX3T(DcoeffsAtEdge,2,1,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge),2))*ci );
#else
    ...
    double d_ij = max( abs(IDX3T(DcoeffsAtEdge,1,1,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge)*uj+
		       ...
		       sqrt(POW(IDX3T(DcoeffsAtEdge,1,1,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge),2)+
			    POW(IDX3T(DcoeffsAtEdge,2,1,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge),2))*cj,
		       ...
		       sqrt(POW(IDX3T(DcoeffsAtEdge,1,2,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge),2)+
			    POW(IDX3T(DcoeffsAtEdge,2,2,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge),2))*ci );
#endif
				 ...
				 int * nedges, int * iedgeset)
{
  ...
  if (*nblocks == 1) {
    hydro_calcFluxRusDiss2d_knl<0><<<grid, block>>>(d_DcoeffsAtEdge,
						    d_IedgeList,
						    d_Dx, d_Dy, (*dscale), 
						    (*neq), (*nvar),
						    (*nedge), (*nmatcoeff),
    ...
    hydro_calcFluxRusDiss2d_knl<1><<<grid, block>>>(d_DcoeffsAtEdge,
						    d_IedgeList,
						    d_Dx, d_Dy, (*dscale), 
						    (*neq), (*nvar),
						    (*nedge), (*nmatcoeff),
...
int FNAME(hydro_calcfluxrusdiss2d_cuda)(unsigned long * h_DcoeffsAtEdge,
					...
					int * nedges,  int * iedgeset)
{
  return hydro_calcFluxRusDiss2d_cuda(h_DcoeffsAtEdge, h_IedgeList,
