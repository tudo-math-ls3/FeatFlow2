...
extern "C"
{
  int hydro_calcFluxScDissDiSp2d_cuda(unsigned long * h_DcoeffsAtEdge,
				      ...
				      int * nedges,
				      int * iedgeset);
  int FNAME(hydro_calcfluxscdissdisp2d_cuda)(unsigned long * h_DcoeffsAtEdge,
					     unsigned long * h_IedgeList,
					     unsigned long * h_Dx,
					     unsigned long * h_Dy,
					     double * dscale,
...
__global__ void hydro_calcFluxScDissDiSp2d_knl(double * DcoeffsAtEdge,
					       int * IedgeList,
					       double * Dx,
					       double * Dy,
					       double dscale,
				    ...
				    int * nedges, int * iedgeset)
{
  ...
  if (*nblocks == 1) {
    hydro_calcFluxScDissDiSp2d_knl<0><<<grid, block>>>(d_DcoeffsAtEdge,
						       d_IedgeList,
						       d_Dx, d_Dy, (*dscale), 
						       (*neq), (*nvar),
						       (*nedge), (*nmatcoeff),
    ...
    hydro_calcFluxScDissDiSp2d_knl<1><<<grid, block>>>(d_DcoeffsAtEdge,
						       d_IedgeList,
						       d_Dx, d_Dy, (*dscale), 
						       (*neq), (*nvar),
						       (*nedge), (*nmatcoeff),
...
int FNAME(hydro_calcfluxscdissdisp2d_cuda)(unsigned long * h_DcoeffsAtEdge,
					   ...
					   int * nedges,  int * iedgeset)
{
  return hydro_calcFluxScDissDiSp2d_cuda(h_DcoeffsAtEdge, h_IedgeList,
