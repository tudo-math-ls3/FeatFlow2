/*#############################################################################
 ******************************************************************************
 * <name> hydro_calcFluxRusDissDiSp3d_cuda </name>
 ******************************************************************************
 *
 * <purpose>
...
extern "C"
{
  int hydro_calcFluxRusDissDiSp3d_cuda(unsigned long * h_DcoeffsAtEdge,
				       ...
				       int * nedges,
				       int * iedgeset);
  int FNAME(hydro_calcfluxrusdissdisp3d_cuda)(unsigned long * h_DcoeffsAtEdge,
					      unsigned long * h_IedgeList,
					      unsigned long * h_Dx,
					      unsigned long * h_Dy,
					      double * dscale,
...
__global__ void hydro_calcFluxRusDissDiSp3d_knl(double * DcoeffsAtEdge,
						int * IedgeList,
						double * Dx,
						double * Dy,
						double dscale,
	...
	IDX2(DdataAtEdge,5,2,NVAR3D,2) = IDX2_FORWARD(Dx,5,j,NVAR3D,neq);
      ...
      double d_ij = max( abs(RCONST(0.5)*(IDX3T(DcoeffsAtEdge,1,1,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge)-
			 ...
			 abs(RCONST(0.5)*(IDX3T(DcoeffsAtEdge,2,2,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge)-
	          ...
	          + max( abs(RCONST(0.5)*(IDX3T(DcoeffsAtEdge,3,1,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge)-
					  IDX3T(DcoeffsAtEdge,3,2,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge))*wj)+
			 abs(RCONST(0.5)*(IDX3T(DcoeffsAtEdge,3,1,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge)-
					  IDX3T(DcoeffsAtEdge,3,2,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge)))*cj,
			 abs(RCONST(0.5)*(IDX3T(DcoeffsAtEdge,3,2,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge)-
					  IDX3T(DcoeffsAtEdge,3,1,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge))*wi)+
			 abs(RCONST(0.5)*(IDX3T(DcoeffsAtEdge,3,2,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge)-
					  IDX3T(DcoeffsAtEdge,3,1,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge)))*ci );
#else
      ...
      double d_ij = max( abs(IDX3(DcoeffsAtEdge,1,1,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge)*uj)+
			 ...
			 abs(IDX3(DcoeffsAtEdge,2,2,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge))*ci )
	          + max( abs(IDX3(DcoeffsAtEdge,3,1,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge)*wj)+
			 abs(IDX3(DcoeffsAtEdge,3,1,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge))*cj,
			 abs(IDX3(DcoeffsAtEdge,3,2,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge)*wi)+
			 abs(IDX3(DcoeffsAtEdge,3,2,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge))*ci );
#endif
				     ...
				     int * nedges, int * iedgeset)
{
  ...
  if (*nblocks == 1) {
    hydro_calcFluxRusDissDiSp3d_knl<0><<<grid, block>>>(d_DcoeffsAtEdge,
							d_IedgeList,
							d_Dx, d_Dy, (*dscale), 
							(*neq), (*nvar),
							(*nedge), (*nmatcoeff),
    ...
    hydro_calcFluxRusDissDiSp3d_knl<1><<<grid, block>>>(d_DcoeffsAtEdge,
							d_IedgeList,
							d_Dx, d_Dy, (*dscale), 
							(*neq), (*nvar),
							(*nedge), (*nmatcoeff),
...
int FNAME(hydro_calcfluxrusdissdisp3d_cuda)(unsigned long * h_DcoeffsAtEdge,
					    ...
					    int * nedges,  int * iedgeset)
{
  return hydro_calcFluxRusDissDiSp3d_cuda(h_DcoeffsAtEdge, h_IedgeList,
