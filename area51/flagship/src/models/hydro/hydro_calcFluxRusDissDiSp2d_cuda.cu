...
extern "C"
{
  int hydro_calcFluxRusDissDiSp2d_cuda(unsigned long * h_DmatrixCoeffsAtEdge,
				       ...
				       int * nedges,
				       int * iedgeset);
  int FNAME(hydro_calcfluxrusdissdisp2d_cuda)(unsigned long * h_DmatrixCoeffsAtEdge,
					      unsigned long * h_IverticesAtEdge,
					      unsigned long * h_Dx,
					      unsigned long * h_Dy,
					      double * dscale,
...
__global__ void hydro_calcFluxRusDissDiSp2d_knl(double * DmatrixCoeffsAtEdge,
						int * IverticesAtEdge,
						double * Dx,
						double * Dy,
						double dscale,
						...
						int iedgeset)
{
  ...
  if (idx<nedges)
  {
    ...
    double d_ij = max( abs(RCONST(0.5)*(IDX3T(DmatrixCoeffsAtEdge,1,1,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge)-
					IDX3T(DmatrixCoeffsAtEdge,1,2,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge))*uj)+
		       abs(RCONST(0.5)*(IDX3T(DmatrixCoeffsAtEdge,1,1,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge)-
					IDX3T(DmatrixCoeffsAtEdge,1,2,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge)))*cj,
		       abs(RCONST(0.5)*(IDX3T(DmatrixCoeffsAtEdge,1,2,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge)-
					IDX3T(DmatrixCoeffsAtEdge,1,1,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge))*ui)+
		       abs(RCONST(0.5)*(IDX3T(DmatrixCoeffsAtEdge,1,2,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge)-
					IDX3T(DmatrixCoeffsAtEdge,1,1,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge)))*ci )
                + max( abs(RCONST(0.5)*(IDX3T(DmatrixCoeffsAtEdge,2,1,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge)-
					IDX3T(DmatrixCoeffsAtEdge,2,2,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge))*vj)+
		       abs(RCONST(0.5)*(IDX3T(DmatrixCoeffsAtEdge,2,1,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge)-
					IDX3T(DmatrixCoeffsAtEdge,2,2,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge)))*cj,
		       abs(RCONST(0.5)*(IDX3T(DmatrixCoeffsAtEdge,2,2,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge)-
					IDX3T(DmatrixCoeffsAtEdge,2,1,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge))*vi)+
		       abs(RCONST(0.5)*(IDX3T(DmatrixCoeffsAtEdge,2,2,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge)-
					IDX3T(DmatrixCoeffsAtEdge,2,1,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge)))*ci );
#else
    // Compute scalar dissipation with dimensional splitting
    double d_ij = max( abs(IDX3(DmatrixCoeffsAtEdge,1,1,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge)*uj)+
		       abs(IDX3(DmatrixCoeffsAtEdge,1,1,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge))*cj,
		       abs(IDX3(DmatrixCoeffsAtEdge,1,2,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge)*ui)+
		       abs(IDX3(DmatrixCoeffsAtEdge,1,2,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge))*ci )
                + max( abs(IDX3(DmatrixCoeffsAtEdge,2,1,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge)*vj)+
		       abs(IDX3(DmatrixCoeffsAtEdge,2,1,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge))*cj,
		       abs(IDX3(DmatrixCoeffsAtEdge,2,2,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge)*vi)+
		       abs(IDX3(DmatrixCoeffsAtEdge,2,2,iedgeset+idx,HYDRO_NDIM,nmatcoeff,nedge))*ci );
#endif
				...
				int * nedges, int * iedgeset)
{
  ...
  if (*nblocks == 1) {
    hydro_calcFluxRusDissDiSp2d_knl<0><<<grid, block>>>(d_DmatrixCoeffsAtEdge,
						    d_IverticesAtEdge,
						    d_Dx, d_Dy, (*dscale), 
						    (*neq), (*nvar),
						    (*nedge), (*nmatcoeff),
    ...
    hydro_calcFluxRusDissDiSp2d_knl<1><<<grid, block>>>(d_DmatrixCoeffsAtEdge,
						    d_IverticesAtEdge,
						    d_Dx, d_Dy, (*dscale), 
						    (*neq), (*nvar),
						    (*nedge), (*nmatcoeff),
...
int FNAME(hydro_calcfluxrusdissdisp2d_cuda)(unsigned long * h_DmatrixCoeffsAtEdge,
					    ...
					    int * nedges,  int * iedgeset)
{
  return hydro_calcFluxRusDissDiSp2d_cuda(h_DmatrixCoeffsAtEdge, h_IverticesAtEdge,
