...
!    integer, dimension(:), pointer :: p_IelementGroupOld,p_IelementCounterOld
!    integer, dimension(:), pointer :: p_IelementGroupNew,p_IelementCounterNew
...
!    
!    if (p_rdiscrSubdomain%h_IelementGroup .ne. ST_NOHANDLE) then
!      call storage_getbase_int (p_rdiscrSubdomain%h_IelementGroup,p_IelementGroupOld)
!      call storage_getbase_int (p_rdiscrSubdomain%h_IelementGroup,p_IelementGroupNew)
!    else
!      ! Uniform discretisation
!      nullify(p_IelementGroupOld)
!      nullify(p_IelementGroupNew)
!    end if
!    
...
!        ielold = Ielements(i)
!        
!        if (associated(p_IelementGroupOld)) then
!          ! Insert it to its corresponding element distribution
!          if (p_IelementGroupOld(ielold) .eq. ieldist) then
!          
!            ! Global data.
!            !
...
!      if (associated(p_IelementGroupNew)) then
!        ieldist = p_IelementGroupNew(i)
!      end if
!
...
!</subroutine>
    ...
    do i=1,spdiscr_getNelemGroups(rdiscrManifold)
    
      call spdiscr_getElemGroupInfo (rdiscrManifold,1,celement)
...
!        rdiscrManifold%RelementDistr(i)%celement = EL_DG_T2_1D
    ...
    if (rdiscrManifold%h_IelemGroupIDs .ne. ST_NOHANDLE) then
      call storage_getbase_int (rdiscrManifold%h_IelemGroupIDs,p_IelementGroupOld)
      call storage_getbase_int (rdiscrManifold%h_IelemGroupIDs,p_IelementGroupNew)
    else
      ! Uniform discretisation
      nullify(p_IelementGroupOld)
      nullify(p_IelementGroupNew)
    ...
    if (spdiscr_getNelemGroups(rdiscrManifold) .eq. 1) then
      ...
      if (rdiscrManifold%p_rtriangulation%NEL .gt. 0) then
        ...
        call spdiscr_getElemGroupInfo (rdiscrManifold,1,p_IelementList=p_IelementListNew)
...
!</subroutine>
    ...
    if (bshr) then
      ...
      if (rdestDiscr%h_IelemGroupIDs .ne. ST_NOHANDLE) then
