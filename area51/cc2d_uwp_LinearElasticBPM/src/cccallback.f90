...
! !     real(DP) :: dnF0
! 
!     real(DP), dimension(:,:,:), allocatable :: divVS
!     real(DP), dimension(:,:,:), allocatable :: vS_x
!     real(DP), dimension(:,:,:), allocatable :: vS_y
!     integer :: i, j
! 
! 
...
!     allocate (vS_x(ubound(Dcoefficients,2),ubound(Dcoefficients,3),1))
!     allocate (vS_y(ubound(Dcoefficients,2),ubound(Dcoefficients,3),1))
!     allocate (divVS(ubound(Dcoefficients,2),ubound(Dcoefficients,3),1))
! 
!     call fevl_evaluate_sim (DER_DERIV_X, VS_x(:,:,1), &
!         p_rvector%Rvectorblock(3), Dpoints, &
!         rdomainIntSubset%p_Ielements, rdomainIntSubset%p_DcubPtsRef)
! 
!     call fevl_evaluate_sim (DER_DERIV_Y, vS_y(:,:,1), &
...
!            Dcoefficients(1,j,i) = dWeight*drhoFR*divVS(j,i,1)
!          end do
!        end do
! 
! 
...
!        do i=1,ubound(Dcoefficients,3)
!          do j=1,ubound(Dcoefficients,2)
!            Dcoefficients(1,j,i) = dWeight*drhoFR*divVS(j,i,1)/dnF0
!          end do
!        end do
! 
!   end subroutine
