...
!<typeblock>
  ...
  type t_ccspatialPreconditioner
    ...
    ! Projection hierarchy for all the levels.
...
contains
  ...
  !<description>
    ...
    ! needs.
    !
    ! smva_initNonlinMatrix must have been called prior to this routine to
...
!</subroutine>
  ...
  contains
    ...
    ! Spatial preconditioner structure that defines all parameters how to perform
    ! preconditioning.
    type(t_ccspatialPreconditioner), intent(INOUT),target    :: rpreconditioner
  ...
  !    call lsysbl_getbase_double (rx,p_vec)
      ...
      do ilev=rpreconditioner%NLMAX,rpreconditioner%NLMIN,-1
        ...
        if (ilev .eq. rpreconditioner%NLMAX) then
          ...
          call mlprj_performInterpolationHier (rpreconditioner%rprjHierarchy,&
              ilev+1,p_rvectorCoarse1,p_rvectorFine1)
          call mlprj_performInterpolationHier (rpreconditioner%rprjHierarchy,&
              ilev+1,p_rvectorCoarse2,p_rvectorFine2)
          call mlprj_performInterpolationHier (rpreconditioner%rprjHierarchy,&
              ilev+1,p_rvectorCoarse3,p_rvectorFine3)
