...
!<subroutine>
  ...
  subroutine cc_solveNonstationaryDirect (rproblem,rvector,ctypeStartVector)
...
!<input>
  ...
  integer, intent(in), optional :: ctypeStartVector
!</input>
...
!</subroutine>
    ...
    if (present(rvector)) then
      ...
      if (present(ctypeStartVector)) then
        if (ctypeStartVector .gt. 0) then
    ...
    ! Prepare constaints in the problem.
    ...
    case (2)
      ...
      ! Level 1 and level nlmin..nlmax.
      call output_lbrk ()
      call output_line ('Triangulation')
      call output_line ('-------------')
      if (rproblem%NLMIN .gt. 1) them
        call tria_infoStatistics (rproblem%RlevelInfo(i)%p_rtriangulation,&
          .true.,i)
      else
    ...
    call cc_doneTargetFlow (rproblem%roptcontrol)
    
    call cc_doneConstraints (rproblem%roptcontrol)
