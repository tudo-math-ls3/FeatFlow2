...
!<subroutine>
  ...
  subroutine cc_getSegmentInfo (&
      ranalyticBC,iindex,isegment,dpar,iintervalsEnd,cbcType1,cbcType2,sparams) !/***/  cbcType1,cbcType2
...
!<output>
  ...
  integer, intent(out) :: cbcType1  !/***/
  integer, intent(out) :: cbcType2  !/***/
...
!</subroutine>
    ...
    read(sstr,*) dpar,iintervalsEnd,cbcType1,cbcType2   !/***/

    ! Get the position of the parameters
    istart = 1
    call sys_getNextToken (sstr,sstr2,istart) ! jump the reading of dpar
    call sys_getNextToken (sstr,sstr2,istart) ! jump the reading of iintervalsEnd
    call sys_getNextToken (sstr,sstr2,istart) ! jump the reading of cbcType1
    call sys_getNextToken (sstr,sstr2,istart) ! jump the reading of cbcType2  so that we finally read 'exprName1' and 'exprName2'
    ...
    do ibdComponent = 1,boundary_igetNBoundComp(rbcAssemblyData%p_rboundary)
      ...
      do isegment = 1,nsegments
        ...
        call cc_getSegmentInfo (rbcAssemblyData%ranalyticBC,&
            iindex,isegment,dpar2,iintervalEnds,cbctype1,cbctype2,sparams) !/***/ cbctype1,cbctype2
            
        rbcAssemblyData%cbctype1 = cbctype1 !/***/
        rbcAssemblyData%cbctype2 = cbctype2 !/***/
	  ...
	  isNeumann =   .false. !/***/
	  isDirichlet = .false. !/***/
          if (cbctype1.eq. 0 .or. cbctype2.eq. 0) isNeumann   = .true.  !/***/
          if (cbctype1.eq. 1 .or. cbctype2.eq. 1) isDirichlet = .true.  !/***/
...
!           case (0,4)
          ...
          if (isDirichlet) then   !/***/
            ...
            if (cbctype1.eq. 1 .and. sbdex1 .ne. "") then   !/***/
              ! X-velocity
              !
              ...
              call bcasm_newDirichletBConRealBD (&
                  ...
                  cc_fcoeff_bdConditions1,rlocalCollection,casmComplexity)
                  
            end if
            
            if (cbctype2.eq. 1 .and. sbdex2 .ne. "") then
              ...
              call bcasm_newDirichletBConRealBD (&
                  ...
                  rbcAssemblyData%rboundaryRegion,&
                  rdynamicLevelInfo%rdiscreteBC,&
                  cc_fcoeff_bdConditions2,rlocalCollection,casmComplexity)

            end if
          end if !/***/  over isDirichlet
!           ! -----------------------------------------------
!           ! Pressure drop BC
!           ! -----------------------------------------------
!           case (2)
...
!           end select
        ...
        end if ! over ( dpar2 .ge. dpar1)
                                          
      end do ! over (isegment)
      
    end do ! over (ibdComponent)
...
!</subroutine>
    ...
    integer :: ibdComponent,iintervalEnds
    integer :: cbctype1,cbctype2 !/***/
    real(DP) :: dpar1,dpar2
    character(LEN=PARLST_MLDATA) :: sparams,sbdex1,sbdex2
    ...
    do ibdComponent = 1,boundary_igetNBoundComp(rbcAssemblyData%p_rboundary)
      ...
      do isegment = 1,nsegments
        ...
        call cc_getSegmentInfo (&
            rbcAssemblyData%ranalyticBC,iindex,isegment,dpar2,iintervalEnds,cbctype1,cbctype2,sparams) !/***/ cbctype1,cbctype2

        ! Form a boundary condition segment that covers that boundary part
        if (dpar2 .ge. dpar1) then
          
          ! Now, which type of BC is to be created?
          select case (cbctype1) !/***/
...
!<subroutine>
  ...
  subroutine cc_fcoeff_bdConditions1 (Icomponents,rdiscretisation,rboundaryRegion,ielement, &
...
!</subroutine>
    ...
    case (DISCBC_NEEDFUNC,DISCBC_NEEDFUNCMID,DISCBC_NEEDDERIV, &
      ...
      select case (p_rbcAssemblyData%cbcType1) !/***/
      case (1)
...
!<subroutine>
  ...
  subroutine cc_fcoeff_bdConditions2 (Icomponents,rdiscretisation,rboundaryRegion,ielement, &
      cinfoNeeded,iwhere,dwhere, Dvalues, rcollection)
...
!</subroutine>
    ...
    case (DISCBC_NEEDFUNC,DISCBC_NEEDFUNCMID,DISCBC_NEEDDERIV, &
      ...
      ! Note: The information about the BC`s can be retrieved from the
      ! quick-access arrays in the collection as initialised above.
      select case (p_rbcAssemblyData%cbcType2) !/***/
      case (1)
        ! Simple Dirichlet BCs.
        call cc_evalBoundaryValue (p_rbcAssemblyData,dwhere,Dvalues(1))
      ...
      do isegment = 1,nsegments
        ...
        if (dpar2 .ge. dpar1) then
          ...
          ! -----------------------------------------------
            ...
            ! Assemble
            if (cbctype1.eq. 0 .and. sbdex1 .ne. "") then  !/***/
              rbcAssemblyData%icomponent = 1
              call cc_prepareExpressionEval (rbcAssemblyData,sbdex1)
            ...
            if (cbctype2.eq. 0 .and. sbdex2 .ne. "") then  !/***/
              rbcAssemblyData%icomponent = 2
              call cc_prepareExpressionEval (rbcAssemblyData,sbdex2)
