    ...
    if (ctype .eq. 5.or.ctype .eq. 6) then
      ...
      do ielement = 1,nelements
        do ipoint = 1,npointsPerElement
          ...
          Rval(1) = Dpoints(1,ipoint,ielement) ! 1: x-coord.
          Rval(2) = Dpoints(2,ipoint,ielement) ! 2: y-coord.
...
!</subroutine>
    ...
    if (rrhsAssembly%ctype .ne. 0) then
      ...
      ! *****************************************
      ! Stationary RHS_P, given in a file
      ! *****************************************
      case (6)
        
        ! Compute D1*w1 + D2*w2 = div w = RHS_P
        !
        ! Multiply with D matrices to calculate div w
        call lsyssc_matVec (rasmTemplates%rmatrixD1, &
            rrhsAssembly%rrhsVector2%RvectorBlock(1), &
            rrhs%RVectorBlock(3), 0.25_DP, 0.0_DP, .false.)

 
        call lsyssc_matVec (rasmTemplates%rmatrixD2, &
            rrhsAssembly%rrhsVector2%RvectorBlock(2), &
            rrhs%RVectorBlock(3), 0.25_DP, 1.0_DP, .false.)
