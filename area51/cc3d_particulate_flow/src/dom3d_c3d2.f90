!##############################################################################
!# ****************************************************************************
!# <name> dom3d_c3d2 </name>
!# ****************************************************************************
!#
!# <purpose>
!# This module contains some auxiliary routines for the domain [0,2.2]x[0,0.41]x[0,0.41],
...
!#
!#   A := { (x,y,z) \in R^3 | 0 <= x <= 2.2 , 0 <= y, z <= 0.41 }
!#
...
!# 3. The right face,  i.e. all cells whose X-coordinate is 2.2.
...
!#
!#  1.) dom3d_c3d2_calcMeshRegion
!#      -> Creates a mesh region describing a set of specific domain faces.
!#
!#  2.) dom3d_c3d2_HitTest
!#      -> A hit-test routine that is used by dom3d_c3d2_calcMeshRegion,
!#         to identify which cells of the triangulation belong to which
!#         domain face.
!#
!#  3.) dom3d_c3d2_calcParProfile
...
!<constantblock description="Domain parametrisation">
  ...
  ! X-Coordinate ranges
  REAL(DP), PARAMETER :: DOM3D_C3D2_X_MIN  = 0.0_DP
  REAL(DP), PARAMETER :: DOM3D_C3D2_X_MAX  = 2.2_DP
  
  ! Y-Coordinate ranges
  REAL(DP), PARAMETER :: DOM3D_C3D2_Y_MIN  = 0.0_DP
  REAL(DP), PARAMETER :: DOM3D_C3D2_Y_MAX  = 0.41_DP
  
  ! Z-Coordinate ranges
  REAL(DP), PARAMETER :: DOM3D_C3D2_Z_MIN  = 0.0_DP
  REAL(DP), PARAMETER :: DOM3D_C3D2_Z_MAX  = 0.41_DP
...
!<constantblock description="Face identifiers">
  ...
  INTEGER(I32), PARAMETER :: DOM3D_C3D2_REG_BOTTOM = 2**0

  ! Identification flag for the front face:
  INTEGER(I32), PARAMETER :: DOM3D_C3D2_REG_FRONT  = 2**1

  ! Identification flag for the right face:
  INTEGER(I32), PARAMETER :: DOM3D_C3D2_REG_RIGHT  = 2**2

  ! Identification flag for the back face:
  INTEGER(I32), PARAMETER :: DOM3D_C3D2_REG_BACK   = 2**3

  ! Identification flag for the left face:
  INTEGER(I32), PARAMETER :: DOM3D_C3D2_REG_LEFT   = 2**4

  ! Identification flag for the top face:
  INTEGER(I32), PARAMETER :: DOM3D_C3D2_REG_TOP    = 2**5
  ...
  ! Neumann.
  INTEGER(I32), PARAMETER :: DOM3D_C3D2_REG_STOKES = DOM3D_C3D2_REG_BOTTOM &
                     + DOM3D_C3D2_REG_FRONT + DOM3D_C3D2_REG_BACK&
                     + DOM3D_C3D2_REG_LEFT + DOM3D_C3D2_REG_TOP

!</constantblock>

!</constants>

INTERFACE dom3d_c3d2_calcMeshRegion
  MODULE PROCEDURE dom3d_c3d2_calcMeshRegion_C
  MODULE PROCEDURE dom3d_c3d2_calcMeshRegion_I
END INTERFACE
...
!<subroutine>
  ...
  SUBROUTINE dom3d_c3d2_calcMeshRegion_C(rmeshRegion,rtriangulation,cfaces)
...
!<input>
  ...
  ! A combination of DOM3D_C3D2_REG_XXXX contants defined above specifying
...
!<subroutine>
  ...
  INTEGER, DIMENSION(6) :: Ifaces
    ...
    CALL mshreg_createFromHitTest(rmeshRegion, rtriangulation, &
         MSHREG_IDX_FACE, .TRUE., dom3d_c3d2_HitTest, Ifaces)
...
!<subroutine>
  ...
  SUBROUTINE dom3d_c3d2_calcMeshRegion_I(rmeshRegion,rtriangulation,Ifaces)
...
!<subroutine>
  ...
  SUBROUTINE dom3d_c3d2_HitTest(inumCells,Dcoords,Ihit,rcollection)
...
!</subroutine>
  ...
  REAL(DP), PARAMETER :: tol = 0.0001_DP
    ...
    DO i = 1, inumCells
      ...
      ! Bottom face?
      IF      ((Dcoords(3,i) - DOM3D_C3D2_Z_MIN) .LT. tol) THEN
        Ihit(i) = 1
      
      ! Front face?
      ELSE IF ((Dcoords(2,i) - DOM3D_C3D2_Y_MIN) .LT. tol) THEN
        Ihit(i) = 2
      
      ! Right face?
      ELSE IF ((Dcoords(1,i) - DOM3D_C3D2_X_MAX) .GT. -tol) THEN
        Ihit(i) = 3
      
      ! Back face?
      ELSE IF ((Dcoords(2,i) - DOM3D_C3D2_Y_MAX) .GT. -tol) THEN
        Ihit(i) = 4
      
      ! Left face?
      ELSE IF ((Dcoords(1,i) - DOM3D_C3D2_X_MIN) .LT. tol) THEN
        Ihit(i) = 5
      
      ! Top face?
      ELSE IF ((Dcoords(3,i) - DOM3D_C3D2_Z_MAX) .GT. -tol) THEN
        Ihit(i) = 6
...
!<subroutine>
  ...
  SUBROUTINE dom3d_c3d2_calcParProfile (dvalue, iregion, dx, dy, dz)
...
!</subroutine>
  ...
  REAL(DP) :: x,y,z
    ...
    x = (dx - DOM3D_C3D2_X_MIN) / (DOM3D_C3D2_X_MAX - DOM3D_C3D2_X_MIN)
    y = (dy - DOM3D_C3D2_Y_MIN) / (DOM3D_C3D2_Y_MAX - DOM3D_C3D2_Y_MIN)
    z = (dz - DOM3D_C3D2_Z_MIN) / (DOM3D_C3D2_Z_MAX - DOM3D_C3D2_Z_MIN)
    ...
    CASE DEFAULT
      ! Invalid region
      PRINT *, 'ERROR: dom3d_c3d2_calcParProfile: Invalid region'
      CALL sys_halt()
    END SELECT
