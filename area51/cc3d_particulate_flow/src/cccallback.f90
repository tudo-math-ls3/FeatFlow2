...
! ***************************************************************************
  ...
  !===================================================
  ! Modified callback routine ( Rapha )
  !===================================================
  !<subroutine>
  subroutine coeff_ball (rdiscretisationTrial,rdiscretisationTest,rform, &
                  nelements,npointsPerElement,Dpoints, &
                  IdofsTrial,IdofsTest,rdomainIntSubset, &
                  Dcoefficients,rcollection)
  ...
  ! Local Variables
  real(dp) :: dtimestep, dRadius, dCenterX,dCenterY,dCenterZ,dtime
  
  integer  :: i,j,res
  
  !-----------------------------------------------------------------
  !
  ! Get the time from the collection, if available
  !
  !-----------------------------------------------------------------
    ! -----------------------------------------------------------------------
    ! Get the time parameters form the collection
    ! -----------------------------------------------------------------------
    ...
    do i=1,nelements
      do j=1,npointsPerElement
        ...
        if(dRadius < 0.05_dp)then
          Dcoefficients(:,j,i) = 1E+3_DP
        else
          Dcoefficients(:,j,i) = 0.0_DP
        end if
      end do
    end do
...
!********************************************************************************  
  ...
  !</subroutine>
    ...
    ! local variables
    real(DP) :: ddistance, dxcenter, dycenter, dzcenter, dradius, dx, dy, dz,eps1
    ...
    integer(PREC_POINTIDX), dimension(:,:), pointer :: p_IverticesAtFace        
    ...
    call storage_getbase_int2d (p_rtriangulation%h_IverticesAtFace,&
                                p_IverticesAtFace)
    ...
    do idx = 1,Revaluation(1)%nvalues
      ...
      call getXYZcoord (Revaluation(1)%cinfoNeeded,ipoint,&
                        p_DvertexCoordinates,&
                        p_IverticesAtElement,p_IverticesAtFace,&
      ...
      if (ddistance .le. dradius) then
        ...
        Revaluation(3)%p_Iinside (idx) = 1
        
        ! We prescribe 0.0 as Dirichlet value here - for x,y,z-velocity
        Revaluation(1)%p_Dvalues (idx,1) = 0.0_DP
        Revaluation(2)%p_Dvalues (idx,1) = 0.0_DP
        Revaluation(3)%p_Dvalues (idx,1) = 0.0_DP
  ...
  contains
    ...
    subroutine getXYZcoord (cinfoNeeded,iwhere,DvertexCoords,&
                           IverticesAtElement,IverticesAtFace,NVT,&
                           dx,dy,dz)
    ...
    ! Array with numbers of points adjacent to all faces
    integer(PREC_POINTIDX), dimension(:,:), intent(IN) :: IverticesAtFace
    ...
    real(DP), intent(OUT) :: dz
      ...
      case (DISCFBC_NEEDFUNC)
        ...
        dz = DvertexCoords(3,iwhere)
      
      case (DISCFBC_NEEDFUNCFACEMID,DISCFBC_NEEDFACEINTMEAN)
      
        ! Not much harder; get the two points on the edge and calculate
        ! the mean coordinates. [1..NVA,1..NAT]
        iv1 = IverticesAtFace(1,iwhere)
        iv2 = IverticesAtFace(2,iwhere)
        
        iv3 = IverticesAtFace(3,iwhere)
        iv4 = IverticesAtFace(4,iwhere)
        
        dx = 0.25_DP*(DvertexCoords(1,iv1)+DvertexCoords(1,iv2)&
                     + DvertexCoords(1,iv3)+DvertexCoords(1,iv4))
        
        dy = 0.25_DP*(DvertexCoords(2,iv1)+DvertexCoords(2,iv2)&
                     + DvertexCoords(2,iv3)+DvertexCoords(2,iv4))
        
        dz = 0.25_DP*(DvertexCoords(3,iv1)+DvertexCoords(3,iv2)&
                     + DvertexCoords(3,iv3)+DvertexCoords(3,iv4))
...
!********************************************************************************  
  ...
  subroutine getBoundaryValuesFBC3 (Icomponents,rdiscretisation,&
                                   Revaluation, rcollection)
  ...
  !</subroutine>
    ...
    ! local variables
    real(DP) :: ddistance, dxcenter, dycenter, dzcenter, dradius, dx, dy, dz
    real(DP) :: dtimestep,dtime,dvelx,dvely,dvelz
    real(DP), dimension(:,:), pointer :: p_DvertexCoordinates
    integer(PREC_POINTIDX), dimension(:,:), pointer :: p_IverticesAtElement
    integer(PREC_POINTIDX), dimension(:,:), pointer :: p_IverticesAtEdge
    integer(PREC_POINTIDX), dimension(:,:), pointer :: p_IverticesAtFace        
    ...
    dzcenter = rcollection%DQuickaccess(9)

    dvelx = rcollection%DQuickaccess(10)
    dvely = rcollection%DQuickaccess(11)
    dvelz = rcollection%DQuickaccess(12)
...
!    dzcenter = 0.2
    ...
    do idx = 1,Revaluation(1)%nvalues
      ...
      if (ddistance .le. dradius) then
        ...
        ! We prescribe 0.0 as Dirichlet value here - for x,y,z-velocity
        Revaluation(1)%p_Dvalues (idx,1) = dvelx
        Revaluation(2)%p_Dvalues (idx,1) = dvely
        Revaluation(3)%p_Dvalues (idx,1) = dvelz
