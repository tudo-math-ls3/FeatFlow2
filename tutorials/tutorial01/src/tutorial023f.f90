!##############################################################################
!# Tutorial 023f: Calculate an L2 and H1 error, block integration, subdomains
!##############################################################################
...
!<subroutine>
  ...
  subroutine fcalc_L2error_subdomain(Dintvalues,rassemblyData,rintAssembly,&
      npointsPerElement,nelements,revalVectors,rcollection)

!<description>  
    ! Calculates the L2 error of the function passed by revalVectors
    ! on the subdomains (x <= 0.25) and (x >= 0.25).
!</description>
...
!</subroutine>
    ...
    Dintvalues(1) = 0.0_DP    ! x <= 0.25
    Dintvalues(2) = 0.0_DP    ! x >= 0.25
    ...
    do iel = 1,nelements
      ...
      do icubp = 1,npointsPerElement
        ...
        if (dx .le. 0.25_DP) then
          ...
          ! 2nd subdomain
          Dintvalues(2) = Dintvalues(2) + p_DcubWeight(icubp,iel) * dval**2
        end if
...
!<subroutine>
  ...
  subroutine fcalc_H1error_subdomain(Dintvalues,rassemblyData,rintAssembly,&
      npointsPerElement,nelements,revalVectors,rcollection)

!<description>  
    ! Calculates the H1 (semi-)error of the function passed by revalVectors
    ! on the subdomain (x <= 0.25) and (x >= 0.25).
!</description>
...
!</subroutine>
    ...
    do iel = 1,nelements
      ...
      do icubp = 1,npointsPerElement
        ...
        if (dx .le. 0.25_DP) then
          ...
          ! 2nd subdomain
          Dintvalues(2) = Dintvalues(2) + p_DcubWeight(icubp,iel) * &
              ( dderivX**2 + dderivY**2 )
        end if
  ...
  subroutine start_tutorial023f
    ...
    integer :: ivt
    real(DP) :: dx
    real(DP), dimension(2) :: Dintvalues
    ...
    call output_line ("This is FEAT-2. Tutorial 023f")
    ...
    ! Calculate ||u||_L2^2 simultaneously on subdomain
    ! (x <= 0.25) and (x >= 0.25).
    call bma_buildIntegrals (Dintvalues,BMA_CALC_STANDARD,&
        fcalc_L2error_subdomain,revalVectors=rcoeffVectors,rcubatureInfo=rcubatureInfo)

    call fev2_releaseVectorList(rcoeffVectors)
        
    ! Take the square roots to get ||u||
    Dintvalues(:) = sqrt(Dintvalues(:))
        
    call output_line ("L2-error(A) = "//trim(sys_sdEL(Dintvalues(1),10)))
    call output_line ("L2-error(B) = "//trim(sys_sdEL(Dintvalues(2),10)))
    ...
    ! Calculate |u_h-u|_H1^2 = ||D(u_h-u)||_L2^2 simultaneously on subdomain
    ! (x <= 0.25) and (x >= 0.25).
    call bma_buildIntegrals (Dintvalues,BMA_CALC_STANDARD,&
        fcalc_H1error_subdomain,revalVectors=rcoeffVectors,rcubatureInfo=rcubatureInfo)

    call fev2_releaseVectorList(rcoeffVectors)
        
    ! Take the square roots to get |u_h-u|_H1
    Dintvalues(:) = sqrt(Dintvalues(:))
        
    call output_line ("H1-error(A) = "//trim(sys_sdEL(Dintvalues(1),10)))
    call output_line ("H1-error(B) = "//trim(sys_sdEL(Dintvalues(2),10)))
