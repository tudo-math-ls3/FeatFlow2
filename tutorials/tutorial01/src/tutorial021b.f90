!##############################################################################
!# Tutorial 021b: Solve a simple nonlinear system using the Newton iteration
!##############################################################################
!# Solves the system
!#
!#    -Laplace(u) - u + u^3 = f
...
!#
!# Applies the block assembly for the defect and the Newton matrix.
...
!<description>  
    ! Calculates the Newton matrix (Frechet derivative) 
    ! of the operator "F(u) = -Laplace(u) - u + u^3 - f".
!</description>
...
!</subroutine>
    ...
    !
    ! Its Frechet derivative reads
    !
    !    F'(u)v = -Laplace(v) - v + 3 u^2 v
    !
    ! which induces the Newton matrix
    !
    !    A(u) = -Laplace - M + " 3*u^2*M "
    !
    ! with the last term understood as the matrix from the bilinear
    ! form "(3 u^2 phi_j, phi_i)".
    ! =====================================================
        ...
        do idofe=1,RmatrixData(1,1)%ndofTest
          ...
          do jdofe=1,RmatrixData(1,1)%ndofTrial
            ...
            p_DlocalMatrix11(jdofe,idofe,iel) = p_DlocalMatrix11(jdofe,idofe,iel) + &
                p_DcubWeight(icubp,iel) * 3.0_DP * du**2 * dbasJ*dbasI
  ...
  subroutine start_tutorial021b
    ...
    call output_line ("This is FEAT-2. Tutorial 021b")
    ...
    call ucd_startVTK (rexport,UCD_FLAG_STANDARD,rtriangulation,"post/tutorial021b.vtk")
