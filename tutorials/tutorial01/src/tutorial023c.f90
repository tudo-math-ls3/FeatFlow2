!##############################################################################
!# Tutorial 023c: Calculate an L2 and H1 norm, block integration, subdomains
!##############################################################################
...
!</subroutine>
  ...
  subroutine start_tutorial023c
    ...
    call output_line ("This is FEAT-2. Tutorial 023c")
    ...
    call output_line ("L2-norm(A) = "//trim(sys_sdL(Dintvalues(1),10)))
    call output_line ("L2-norm(B) = "//trim(sys_sdL(Dintvalues(2),10)))
    ...
    ! Calculate |u|_H1^2 = ||Du||_L2^2 simultaneously on subdomain
    ! (x <= 0.25) and (x >= 0.25).
    call bma_buildIntegrals (Dintvalues,BMA_CALC_STANDARD,&
    ...
    ! Take the square roots to get |u|_H1
    Dintvalues(:) = sqrt(Dintvalues(:))
        
    call output_line ("H1-norm(A) = "//trim(sys_sdL(Dintvalues(1),10)))
    call output_line ("H1-norm(B) = "//trim(sys_sdL(Dintvalues(2),10)))
