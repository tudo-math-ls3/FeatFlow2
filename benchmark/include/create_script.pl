...
foreach my $testfile (@defaultSettingsFiles) {
    ...
    my $fullfilepath = $testsdir . "/" . $testfile;

    if ( -e $fullfilepath) {
        print STDERR "Parsing test coding file <$fullfilepath>...\n" if ($debugScript);
        ...
        # Read the file to the default values. Do not initialise by
        # default values as there are none. 
        &read_def_file($testsdir,$testfile,\%test,\%default,\%default,0);
    
        print STDERR "Finished parsing test coding file <$testfile>.\n" if ($debugScript);
    } else {
        warn "$progname: WARNING: Cannot open file <$fullfilepath>: $!\n";
    }
...
foreach my $testfile (@testfiles) {
    ...
    # Read the file based on default values.
    &read_def_file($testsdir,$testfile,\%test,\%inherited,\%default,1);
...
sub show_version {
    ...
    print "Modifications by M.Koester.\n";
...
sub read_def_file {
    ...
    # Paraeters: Directory, Filename, Test-list, Reference to hashes, 
    # Reference to defaults, reset to derfaults,
    # Optional: Current test id.
    my ($testsdir, $testfile, $test, $inherited, $default, $resetdef, $currenttestid) = @_;
    ...
    if ( $resetdef == 1 ) {
        # Defaults are specified, take them.
    ...
    if ( defined ($currenttestid)) {
        # Continue parsing of testid $currenttestid.
        print STDERR "Continue parsing of testid $currenttestid\n" if ($debugScript);
    ...
    LINE: while (<TESTFILE>) {
        ...
        if ($_ =~ m%^\s*include\s+(.+)$%i) {
            ...
            my $fullfilepath = $testsdir . "/" . $includefilename;
            print STDERR "Including subfile: <$fullfilepath> \n" if ($debugScript);
            
            # Include the subfile. Do not reset the defaults, the file has
            # just to be parsed as it is.
            &read_def_file($testsdir,$includefilename,$test,
                $inherited,$default,0,$testid);
            
            print STDERR "Finished including subfile <$fullfilepath> \n" if ($debugScript);
