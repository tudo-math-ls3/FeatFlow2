...
# all or for different reasons one wishes to do so), add LAM/MPI-related settings
# manually here.
ifeq ($(strip $(MPIWRAPPERS)), NO)

# Set LAMHOME if unset
ifeq ($(strip $(LAMHOME)),)
LAMHOME  = /usr/local/lam
MESSAGE := $(MESSAGE) \
	    echo '*** Warning: LAMHOME unset. Has been set to $(LAMHOME)'; \
	    echo '*** Warning: This has implications on include and library paths!';
endif

# Set include and library directory
MPIINC    = -I$(LAMHOME)/include
MPILIBDIR = -L$(LAMHOME)/lib
MPILIBS  := $(MPILIBS) -llamf77mpi -lmpi -llam -lpthread -lutil

# LAM/MPI needs the dynamic linker library on Linux systems
ifeq ($(firstword $(subst -, ,$(ID))), pc)
MPILIBS  := $(MPILIBS) -ldl
endif

# LAM/MPI needs on Sun Ultrasparc systems the dynamic linker library as
# well as some additional system libraries
ifeq ($(firstword $(subst -, ,$(ID))), sun4u)
MPILIBS  := $(MPILIBS) -ldl -lsocket -lnsl
endif
