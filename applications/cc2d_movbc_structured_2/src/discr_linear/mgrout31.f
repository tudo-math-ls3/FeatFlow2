...
* the E031/EM31 element. They support adaptive switching to constant 
...
************************************************************************
* MP031  - Standard prolongation with averaging of the values of a node
...
************************************************************************
* MP031X - Extended prolongartion. Takes care of the aspect ratio
...
************************************************************************
      ...
      SUBROUTINE MP031X(DU1,DU2,KVERT1,KVERT2,KMID1,KMID2,KADJ1,KADJ2,
...
C PRWEIG (.,2) die Konstanten fuer die konstante Prolongation.
      ...
      DATA PRWEIG    /0.9375D0,-0.1875D0,-0.0625D0,0.3125D0,
     *                0.5625D0,0.1875D0,0.0625D0,0.1875D0,
     *                1D0,0D0,0D0,0D0,
     *                1D0,0D0,0D0,0D0/ 
      SAVE PRWEIG
...
C      IF (PARP.NE.-1D0) THEN
C        IF ((ARIEL.GT.PARP).OR.(ARIEL1.GT.PARP)) IGRID1=2
C        IF ((ARIEL.GT.PARP).OR.(ARIEL2.GT.PARP)) IGRID2=2
C        IF ((ARIEL.GT.PARP).OR.(ARIEL3.GT.PARP)) IGRID3=2
C        IF ((ARIEL.GT.PARP).OR.(ARIEL4.GT.PARP)) IGRID4=2
C        IF ((IGRID1.EQ.2).OR.(IGRID2.EQ.2).OR.(IGRID3.EQ.2).OR.
C     *      (IGRID4.EQ.2)) NCEL = NCEL+1
C      END IF  
...
************************************************************************
* MR031  - Standard restriction with averaging of the values of a node
*          on the edge of two elements  
*
* Parameters:
* DU2    - fine grid vector
...
************************************************************************
* MR031X - Extended restriction. Takes care of the aspect ratio
*          and allows different weights for the two contributions 
*          of the elements adjacent to an edge for the value of a node.
*
* Parameters:
...
************************************************************************
      ...
      SUBROUTINE MR031X(DU2,DU1,KVERT2,KVERT1,KMID2,KMID1,KADJ2,KADJ1,
...
C      IF (PARR.NE.-1D0) THEN
C        IF ((ARIEL.GT.PARR).OR.(ARIEL1.GT.PARR)) IGRID1=2
C        IF ((ARIEL.GT.PARR).OR.(ARIEL2.GT.PARR)) IGRID2=2
C        IF ((ARIEL.GT.PARR).OR.(ARIEL3.GT.PARR)) IGRID3=2
C        IF ((ARIEL.GT.PARR).OR.(ARIEL4.GT.PARR)) IGRID4=2
C      END IF  
...
!        CALL MP031X(DUC,DUF,KWORK(KVERTC),KWORK(KVERTF),
!     *              KWORK(KMIDC),KWORK(KMIDF),KWORK(KADJC),KWORK(KADJF),
...
!      KAREA =L(KLAREA (ILEV))
!  
!C evtl. ist IPR < 0, falls diese Rest. fuer EM30 verwendet wird.  
!  
!      IF (ABS(IPR).EQ.1) THEN    
!        CALL MR031(DDF,DDC,KWORK(KVERTF),KWORK(KVERTC),
!     *             KWORK(KMIDF),KWORK(KMIDC),KWORK(KADJF),KWORK(KADJC),
!     *             NVTF,NVT,NELF,NEL)
!      ELSE IF (ABS(IPR).EQ.2) THEN
!        CALL MR031C(DDF,DDC,KWORK(KVERTF),KWORK(KVERTC),
!     *             KWORK(KMIDF),KWORK(KMIDC),KWORK(KADJF),KWORK(KADJC),
...
!        CALL MR031X(DDF,DDC,KWORK(KVERTF),KWORK(KVERTC),
!     *             KWORK(KMIDF),KWORK(KMIDC),KWORK(KADJF),KWORK(KADJC),
...
C
************************************************************************
!      SUBROUTINE YSTP31(DX,DD,DB,NEQ,ALPHA)  
************************************************************************
*
