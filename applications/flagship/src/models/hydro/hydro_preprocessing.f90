!##############################################################################
!# ****************************************************************************
!# <Name> hydro_preprocessing </name>
!# ****************************************************************************
!#
!# <purpose>
!# This module contains all preprocessing routines which are required to
!# solve the compressible Euler/Navier-Stokes equations
!#
      ...
      if (systemMatrix > 0) then
        ...
        case (SYSTEM_INTERLEAVEFORMAT)
          ...
          if (lsyssc_hasMatrixStructure(rproblemLevel%RmatrixScalar(systemMatrix))) then
            ...
            rproblemLevel%RmatrixScalar(systemMatrix)%NVAR = hydro_getNVAR(rproblemLevel)
      ...
      do iblock = 1, rvector%nblocks
        ...
        if (rvector%RvectorBlock(iblock)%NVAR .eq. 1) then
          ...
          call linf_buildVectorScalar(rform, .true.,&
              ...
              fcoeff_buildVectorSc_sim=hydro_coeffVectorAnalytic,&
...
end module hydro_preprocessing
