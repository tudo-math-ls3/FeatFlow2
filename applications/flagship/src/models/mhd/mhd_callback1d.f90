...
!# solve the compressible ideal MHD equations in 1D.
!#
...
!</subroutine>
    ...
    do idx = 1, nedges
      ...
      vi = YVELOCITY3_1D(DdataAtEdge,IDX3,1,idx,_,_,_)
      wi = ZVELOCITY3_1D(DdataAtEdge,IDX3,1,idx,_,_,_)
      uj = XVELOCITY3_1D(DdataAtEdge,IDX3,2,idx,_,_,_)
      vj = YVELOCITY3_1D(DdataAtEdge,IDX3,2,idx,_,_,_)
      wj = ZVELOCITY3_1D(DdataAtEdge,IDX3,2,idx,_,_,_)
      
      ! Compute total pressures
      pi = TOTALPRESSURE3_1D(DdataAtEdge,IDX3,1,idx,_,_,_)
      pj = TOTALPRESSURE3_1D(DdataAtEdge,IDX3,2,idx,_,_,_)

      ! Compute auxiliary quantities
      qi = MAG_DOT_VEL3_1D(DdataAtEdge,IDX3,1,idx,_,_,_,ui,vi,wi)
      qj = MAG_DOT_VEL3_1D(DdataAtEdge,IDX3,2,idx,_,_,_,uj,vj,wj)
      
#ifdef MHD_USE_IBP
      ! Compute fluxes for x-direction
      IDX1(Fxi,1) = INVISCIDFLUX1_XDIR3_1D(DdataAtEdge,IDX3,1,idx,_,_,_,ui,vi,wi,pi,qi)
      IDX1(Fxi,2) = INVISCIDFLUX2_XDIR3_1D(DdataAtEdge,IDX3,1,idx,_,_,_,ui,vi,wi,pi,qi)
      IDX1(Fxi,3) = INVISCIDFLUX3_XDIR3_1D(DdataAtEdge,IDX3,1,idx,_,_,_,ui,vi,wi,pi,qi)
      IDX1(Fxi,4) = INVISCIDFLUX4_XDIR3_1D(DdataAtEdge,IDX3,1,idx,_,_,_,ui,vi,wi,pi,qi)
      IDX1(Fxi,5) = INVISCIDFLUX5_XDIR3_1D(DdataAtEdge,IDX3,1,idx,_,_,_,ui,vi,wi,pi,qi)
      IDX1(Fxi,6) = INVISCIDFLUX6_XDIR3_1D(DdataAtEdge,IDX3,1,idx,_,_,_,ui,vi,wi,pi,qi)
      IDX1(Fxi,7) = INVISCIDFLUX7_XDIR3_1D(DdataAtEdge,IDX3,1,idx,_,_,_,ui,vi,wi,pi,qi)

      IDX1(Fxj,1) = INVISCIDFLUX1_XDIR3_1D(DdataAtEdge,IDX3,2,idx,_,_,_,uj,vj,wj,pj,qj)
      IDX1(Fxj,2) = INVISCIDFLUX2_XDIR3_1D(DdataAtEdge,IDX3,2,idx,_,_,_,uj,vj,wj,pj,qj)
      IDX1(Fxj,3) = INVISCIDFLUX3_XDIR3_1D(DdataAtEdge,IDX3,2,idx,_,_,_,uj,vj,wj,pj,qj)
      IDX1(Fxj,4) = INVISCIDFLUX4_XDIR3_1D(DdataAtEdge,IDX3,2,idx,_,_,_,uj,vj,wj,pj,qj)
      IDX1(Fxj,5) = INVISCIDFLUX5_XDIR3_1D(DdataAtEdge,IDX3,2,idx,_,_,_,uj,vj,wj,pj,qj)
      IDX1(Fxj,6) = INVISCIDFLUX6_XDIR3_1D(DdataAtEdge,IDX3,2,idx,_,_,_,uj,vj,wj,pj,qj)
      IDX1(Fxj,7) = INVISCIDFLUX7_XDIR3_1D(DdataAtEdge,IDX3,2,idx,_,_,_,uj,vj,wj,pj,qj)
      ...
      IDX1(Fx_ij,1) = INVISCIDFLUX1_XDIR3_1D(DdataAtEdge,IDX3,1,idx,_,_,_,ui,vi,wi,pi,qi)-&
                      INVISCIDFLUX1_XDIR3_1D(DdataAtEdge,IDX3,2,idx,_,_,_,uj,vj,wj,pj,qj)
      IDX1(Fx_ij,2) = INVISCIDFLUX2_XDIR3_1D(DdataAtEdge,IDX3,1,idx,_,_,_,ui,vi,wi,pi,qi)-&
                      INVISCIDFLUX2_XDIR3_1D(DdataAtEdge,IDX3,2,idx,_,_,_,uj,vj,wj,pj,qj)
      IDX1(Fx_ij,3) = INVISCIDFLUX3_XDIR3_1D(DdataAtEdge,IDX3,1,idx,_,_,_,ui,vi,wi,pi,qi)-&
                      INVISCIDFLUX3_XDIR3_1D(DdataAtEdge,IDX3,2,idx,_,_,_,uj,vj,wj,pj,qj)
      IDX1(Fx_ij,4) = INVISCIDFLUX4_XDIR3_1D(DdataAtEdge,IDX3,1,idx,_,_,_,ui,vi,wi,pi,qi)-&
                      INVISCIDFLUX4_XDIR3_1D(DdataAtEdge,IDX3,2,idx,_,_,_,uj,vj,wj,pj,qj)
      IDX1(Fx_ij,5) = INVISCIDFLUX5_XDIR3_1D(DdataAtEdge,IDX3,1,idx,_,_,_,ui,vi,wi,pi,qi)-&
                      INVISCIDFLUX5_XDIR3_1D(DdataAtEdge,IDX3,2,idx,_,_,_,uj,vj,wj,pj,qj)
      IDX1(Fx_ij,6) = INVISCIDFLUX6_XDIR3_1D(DdataAtEdge,IDX3,1,idx,_,_,_,ui,vi,wi,pi,qi)-&
                      INVISCIDFLUX6_XDIR3_1D(DdataAtEdge,IDX3,2,idx,_,_,_,uj,vj,wj,pj,qj)
      IDX1(Fx_ij,7) = INVISCIDFLUX7_XDIR3_1D(DdataAtEdge,IDX3,1,idx,_,_,_,ui,vi,wi,pi,qi)-&
                      INVISCIDFLUX7_XDIR3_1D(DdataAtEdge,IDX3,2,idx,_,_,_,uj,vj,wj,pj,qj)
...
!</subroutine>
    ...
    do idx = 1, nedges
      ...
      bxPow2_ij = (POW(ROE_MEAN_VALUE(\
                       XMAGFIELD3_1D(DdataAtEdge,IDX3,2,idx,_,_,_),\
                       XMAGFIELD3_1D(DdataAtEdge,IDX3,1,idx,_,_,_),aux),2))/rho_ij
      ...
      bPow2_ij = (POW(ROE_MEAN_VALUE(\
                      XMAGFIELD3_1D(DdataAtEdge,IDX3,2,idx,_,_,_),\
                      XMAGFIELD3_1D(DdataAtEdge,IDX3,1,idx,_,_,_),aux),2)+\
                  POW(ROE_MEAN_VALUE(\
                      YMAGFIELD3_1D(DdataAtEdge,IDX3,2,idx,_,_,_),\
                      YMAGFIELD3_1D(DdataAtEdge,IDX3,1,idx,_,_,_),aux),2)+\
                  POW(ROE_MEAN_VALUE(\
                      ZMAGFIELD3_1D(DdataAtEdge,IDX3,2,idx,_,_,_),\
                      ZMAGFIELD3_1D(DdataAtEdge,IDX3,1,idx,_,_,_),aux),2))/rho_ij
...
!</subroutine>
    ...
    do idx = 1, nedges
      ...
      if (anorm .gt. SYS_EPSREAL_DP) then
        ...
        auxy = ROE_MEAN_VALUE(\
                YMAGFIELD3_1D(DdataAtEdge,IDX3,2,idx,_,_,_),\
                YMAGFIELD3_1D(DdataAtEdge,IDX3,1,idx,_,_,_),aux)
        auxz = ROE_MEAN_VALUE(\
                ZMAGFIELD3_1D(DdataAtEdge,IDX3,2,idx,_,_,_),\
                ZMAGFIELD3_1D(DdataAtEdge,IDX3,1,idx,_,_,_),aux)
...
!</subroutine>
    ...
    do idx = 1, nedges
      ...
      cai = abs(XMAGFIELD3_1D(DdataAtEdge,IDX3,1,idx,_,_,_))
      ...
      ! Compute auxiliary quantities
      astPow2i = MAGFIELDMAGNITUDE3_1D(DdataAtEdge,IDX3,1,idx,_,_,_)/&
                 DENSITY3_1D(DdataAtEdge,IDX3,1,idx,_,_,_) + aPow2i
      astPow2j = MAGFIELDMAGNITUDE3_1D(DdataAtEdge,IDX3,2,idx,_,_,_)/&
    ...
    do idx = 1, nedges
      ...
      IDX3(DtransformedFluxesAtEdge,1,1,idx,_,_,_) =&
          ...
          (ui*ui+vi*vi+wi*wi)*DENSITY2_1D(DfluxesAtEdge,IDX2,idx,_,_)-&
                         ...
                         wi*ZMOMENTUM2_1D(DfluxesAtEdge,IDX2,idx,_,_)-&
                            XMAGFIELD3_1D(DdataAtEdge,IDX3,1,idx,_,_,_)*&
                            XMAGFIELD2_1D(DfluxesAtEdge,IDX2,idx,_,_)-&
                            YMAGFIELD3_1D(DdataAtEdge,IDX3,1,idx,_,_,_)*&
                            YMAGFIELD2_1D(DfluxesAtEdge,IDX2,idx,_,_)-&
                            ZMAGFIELD3_1D(DdataAtEdge,IDX3,1,idx,_,_,_)*&
      ...
      IDX3(DtransformedFluxesAtEdge,1,2,idx,_,_,_) =&
          ...
          (uj*uj+vj*vj+wj*wj)*DENSITY2_1D(DfluxesAtEdge,IDX2,idx,_,_)-&
                         ...
                         wj*ZMOMENTUM2_1D(DfluxesAtEdge,IDX2,idx,_,_)-&
                            XMAGFIELD3_1D(DdataAtEdge,IDX3,2,idx,_,_,_)*&
                            XMAGFIELD2_1D(DfluxesAtEdge,IDX2,idx,_,_)-&
                            YMAGFIELD3_1D(DdataAtEdge,IDX3,2,idx,_,_,_)*&
                            YMAGFIELD2_1D(DfluxesAtEdge,IDX2,idx,_,_)-&
...
!</subroutine>
    ...
    do idx = 1, nedges
      ...
      IDX3(DtransformedFluxesAtEdge,2,1,idx,_,_,_) =&
          (YMOMENTUM2_1D(DfluxesAtEdge,IDX2,idx,_,_)-&
          vi*DENSITY2_1D(DfluxesAtEdge,IDX2,idx,_,_))/&
             DENSITY3_1D(DdataAtEdge,IDX3,1,idx,_,_,_)
      IDX3(DtransformedFluxesAtEdge,2,2,idx,_,_,_) =&
         -(YMOMENTUM2_1D(DfluxesAtEdge,IDX2,idx,_,_)-&
          vj*DENSITY2_1D(DfluxesAtEdge,IDX2,idx,_,_))/&
             DENSITY3_1D(DdataAtEdge,IDX3,2,idx,_,_,_)

      ! Transformed velocity fluxes in z-direction
      IDX3(DtransformedFluxesAtEdge,3,1,idx,_,_,_) =&
          (ZMOMENTUM2_1D(DfluxesAtEdge,IDX2,idx,_,_)-&
          wi*DENSITY2_1D(DfluxesAtEdge,IDX2,idx,_,_))/&
             DENSITY3_1D(DdataAtEdge,IDX3,1,idx,_,_,_)
      IDX3(DtransformedFluxesAtEdge,3,2,idx,_,_,_) =&
         -(ZMOMENTUM2_1D(DfluxesAtEdge,IDX2,idx,_,_)-&
          wj*DENSITY2_1D(DfluxesAtEdge,IDX2,idx,_,_))/&
             DENSITY3_1D(DdataAtEdge,IDX3,2,idx,_,_,_)
    end do
...
!</subroutine>
    ...
    do idx = 1, nedges
      ...
      IDX2(DtransformedDataAtEdge,2,idx,_,_) =&
          YVELOCITY3_1D(DdataAtEdge,IDX3,2,idx,_,_,_)-&
          YVELOCITY3_1D(DdataAtEdge,IDX3,1,idx,_,_,_)

      ! Transformed velocity difference in z-direction
      IDX2(DtransformedDataAtEdge,3,idx,_,_) =&
          ZVELOCITY3_1D(DdataAtEdge,IDX3,2,idx,_,_,_)-&
          ZVELOCITY3_1D(DdataAtEdge,IDX3,1,idx,_,_,_)
    end do
...
!</subroutine>
    ...
    do idx = 1, nnodes
      ...
      IDX2(DtransformedDataAtNode,2,idx,_,_) =&
          YVELOCITY2_1D(DdataAtNode,IDX2,idx,_,_)

      ! Transformed z-velocity values
      IDX2(DtransformedDataAtNode,3,idx,_,_) =&
          ZVELOCITY2_1D(DdataAtNode,IDX2,idx,_,_)
    end do
...
!</subroutine>
    ...
    do idx = 1, nedges
      ...
      IDX2(DtransformedDataAtEdge,2,idx,_,_) =&
          YMOMENTUM3_1D(DdataAtEdge,IDX3,2,idx,_,_,_)-&
          YMOMENTUM3_1D(DdataAtEdge,IDX3,1,idx,_,_,_)

      ! Transformed momentum difference in z-direction
      IDX2(DtransformedDataAtEdge,3,idx,_,_) =&
          ZMOMENTUM3_1D(DdataAtEdge,IDX3,2,idx,_,_,_)-&
          ZMOMENTUM3_1D(DdataAtEdge,IDX3,1,idx,_,_,_)
    end do
...
!</subroutine>
    ...
    do idx = 1, nnodes
      ...
      IDX2(DtransformedDataAtNode,2,idx,_,_) =&
          YMOMENTUM2_1D(DdataAtNode,IDX2,idx,_,_)

      ! Transformed z-momentum values
      IDX2(DtransformedDataAtNode,3,idx,_,_) =&
          ZMOMENTUM2_1D(DdataAtNode,IDX2,idx,_,_)
    end do
...
!</subroutine>
    ...
    do idx = 1, nnodes
      ...
      IDX2(DtransformedDataAtNode,1,idx,_,_) =&
          YMAGFIELD2_1D(DdataAtNode,IDX2,idx,_,_)

      ! Transformed z-component of the magnetic field
      IDX2(DtransformedDataAtNode,2,idx,_,_) =&
          ZMAGFIELD2_1D(DdataAtNode,IDX2,idx,_,_)
    end do
...
!</subroutine>
  ...
  contains
    ...
    subroutine doGalerkinFlux(Dstate, dnx, Dflux)
      ...
      v = YVELOCITY1_1D(Dstate,IDX1)
      w = ZVELOCITY1_1D(Dstate,IDX1)
      p = TOTALPRESSURE1_1D(Dstate,IDX1)
      q = MAG_DOT_VEL1_1D(Dstate,IDX1,u,v,w)
      
      ! Calculate ${\bf n}\cdot{\bf F}(U)$
      Dflux(1) = dnx*(XMOMENTUM1_1D(Dstate,IDX1))
      Dflux(2) = dnx*(XMOMENTUM1_1D(Dstate,IDX1)*u+p)
      Dflux(3) = dnx*(YMOMENTUM1_1D(Dstate,IDX1)*u-&
                      XMAGFIELD1_1D(Dstate,IDX1)*&
                      YMAGFIELD1_1D(Dstate,IDX1))
      Dflux(4) = dnx*(ZMOMENTUM1_1D(Dstate,IDX1)*u-&
                      XMAGFIELD1_1D(Dstate,IDX1)*&
                      ZMAGFIELD1_1D(Dstate,IDX1))
      Dflux(5) = dnx*(YMAGFIELD1_1D(Dstate,IDX1)*u-&
                      XMAGFIELD1_1D(Dstate,IDX1)*v)
      Dflux(6) = dnx*(ZMAGFIELD1_1D(Dstate,IDX1)*u-&
                      XMAGFIELD1_1D(Dstate,IDX1)*w)
      Dflux(7) = dnx*((TOTALENERGY1_1D(Dstate,IDX1)+p)*u-&
                      XMAGFIELD1_1D(Dstate,IDX1)*q)
