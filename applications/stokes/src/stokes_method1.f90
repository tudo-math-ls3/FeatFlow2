  ...
  INTEGER :: i
    ...
    DO i=rproblem%ilvmin,rproblem%ilvmax
      ...
      CALL lsyssc_duplicateMatrix (p_rmatrix%RmatrixBlock(1,3),&
                  p_rmatrix%RmatrixBlock(2,3),LSYSSC_DUP_STRNOCONT)
      ...
      CALL lsyssc_duplicateMatrix (p_rmatrix%RmatrixBlock(1,1),&
                  p_rmatrix%RmatrixBlock(2,2),LSYSSC_DUP_NONE)
      ...
      CALL bilf_buildMatrixScalar (rform,.TRUE.,&
                                  p_rmatrix%RmatrixBlock(1,3),coeff_Pressure,&
                                  rproblem%rcollection)
      ...
      CALL bilf_buildMatrixScalar (rform,.TRUE.,&
                                  p_rmatrix%RmatrixBlock(2,3),coeff_Pressure,&
                                  rproblem%rcollection)
                                   
      ! Put the transposed 1st pressure matrix to position (3,1) of the block matrix:
      CALL lsyssc_transposeMatrix (p_rmatrix%RmatrixBlock(1,3), &
                                   p_rmatrix%RmatrixBlock(3,1))

      ! Put the transposed 2nd pressure matrix to position (3,1) of the block matrix:
      CALL lsyssc_transposeMatrix (p_rmatrix%RmatrixBlock(2,3), &
