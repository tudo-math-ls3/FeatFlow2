!##############################################################################
!# ****************************************************************************
!# <name> stokes2d_method0_resort </name>
!# ****************************************************************************
!#
!# <purpose>
...
!#
!# The routine uses the BiCGStab solver.
!#
!# The linear system of the 1st and 2nd block is resorted using Cuthill McKee.
!# </purpose>
!##############################################################################

module stokes2d_method0_resort
...
!</subroutine>
    ...
    ! Duplicate the matrix to the Y-velocity matrix.
    !
    ! NOTE: We copy everything, structure and content, since the matrices
    ! are resorted later! Shared structures would not be sorted!
    call lsyssc_duplicateMatrix (rmatrix%RmatrixBlock(1,1),&
        rmatrix%RmatrixBlock(2,2),LSYSSC_DUP_COPY,LSYSSC_DUP_COPY)
    ...
    !
    ! WARNING: There is no preconditioner attached. No VANKA variant
    ! currently supports sorted matrices!!!
    ...
    ! Residual output every 10 iterations
    p_rsolverNode%niteResOutput = 10
    ...
    ! -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    ! Sorting back the solution vector
    ! -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-

    ! Sorting back the solution is necessary for postprocessing.
    call lsysbl_sortVector(rvector,.false.)
