...
!# The code uses resorting of the matrix/vectors with the Cuthill-McKee
!# algorithm
!# </purpose>
!##############################################################################

module poisson2d_method0_cmsort
...
!</subroutine>
    ...
    ! -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    ! Impose a resorting into matrix and vectors
    ! -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    ...
    call sstrat_initCuthillMcKee (rsortStrategy%p_Rstrategies(1),rmatSystem%RmatrixBlock(1,1))
    
    ! Impose the sorting strategy to the matrix and the vectors.
    call lsysbl_setSortStrategy (rvecRhs,rsortStrategy)
    call lsysbl_setSortStrategy (rvecSol,rsortStrategy)
    call lsysbl_setSortStrategy (rmatSystem,rsortStrategy,rsortStrategy)
    
    ! Resort matrix and vectors.
    call lsysbl_sortVector (rvecRhs,.true.)
    call lsysbl_sortVector (rvecSol,.true.)
    call lsysbl_sortMatrix (rmatSystem,.true.)
    ...
    ! Unsort the solution
    call lsysbl_sortVector (rvecSol,.false.)
