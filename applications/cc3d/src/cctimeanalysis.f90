...
!</function>
    ...
    ! local variables
    REAL(DP) :: dtmp
    REAL(DP), DIMENSION(4) :: Dnorms1,Dnorms2
    INTEGER(PREC_VECIDX), DIMENSION(4) :: Cnorms
    ...
    dtmp = SQRT( 0.5_DP * (Dnorms2(1)**2+Dnorms2(2)**2+Dnorms2(3)**2) )
    IF (dtmp .EQ. 0.0_DP) dtmp=1.0_DP
    rtimeErrorLocal%drelUL2 = SQRT(0.5_DP * (Dnorms1(1)**2+Dnorms1(2)**2+&
                                             Dnorms1(3)**2) ) / dtmp

    dtmp = Dnorms2(4)
    IF (dtmp .EQ. 0.0_DP) dtmp=1.0_DP
    rtimeErrorLocal%drelPL2 = Dnorms1(4) / dtmp
    ...
    dtmp = MAX(Dnorms2(1),Dnorms2(2),Dnorms2(3))
    IF (dtmp .EQ. 0.0_DP) dtmp=1.0_DP
    rtimeErrorLocal%drelUmax = MAX(Dnorms1(1),Dnorms1(2),Dnorms1(3)) / dtmp

    dtmp = Dnorms2(4)
    IF (dtmp .EQ. 0.0_DP) dtmp=1.0_DP
    rtimeErrorLocal%drelPmax = Dnorms1(4) / dtmp
...
!</function>
    ...
    nequ = rSolutionNew%RvectorBlock(1)%NEQ+rSolutionNew%RvectorBlock(2)%NEQ &
         + rSolutionNew%RvectorBlock(3)%NEQ
    neqp = rSolutionNew%RvectorBlock(4)%NEQ
    ...
    p_rtimeNorm%drelUL2 = SQRT( 0.5_DP * (Dnorms1(1)**2+Dnorms1(2)**2+Dnorms1(3)**2) ) &
