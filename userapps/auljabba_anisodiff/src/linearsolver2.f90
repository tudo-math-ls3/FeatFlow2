...
!#                  <tex>$$ Ax=b $$</tex>
!#
!# For this we have a linear solver <tex>$P^{-1} \approx A^{-1}$</tex> (Gauss el1imination
...
!<typeblock>
  ...
  type t_linsolNode
    ...
    ! By default, every solver in a solver tree belongs to solver
    ! subgroup 0. This means, the solver is initialised by the default
...
!<typeblock>
  ...
  type t_linsolSubnodeDeflGMRES
    ...
    ! preconditioner.
    ! =false: The preconditoner is used as left preconditioner. This is
    !    the default setting.
...
!<description>
  ...
  ! can be get by calling linsol_getMultigrid2Level).
  ! The matrices <tex>$ A $</tex> on all levels must be attached to the solver previously
  ! by linsol_setMatrices.
...
!<inputoutput>
  ...
  ! Solution vector on level ilevel.
  ! Is replaced by a new approximation to the solution
  type(t_vectorBlock), intent(inout)        :: rx
...
!!!!Check test: Apply filtr to the preconditioned RHS

    if (associated(p_RfilterChain)) then
      call filter_applyFilterChainVec (p_ractualRHS, p_RfilterChain)
    end if
...
!!!!Now there is no need to apply this filter again
...
!     end if
    ...
    ! initialise the queue of the last residuals with RES
    Dresqueue = dfr
!!!! The following If statement will be executed only when the current level is finest level(NLMAX), 
!!!! where we want to solve our problem
    if (ilevel .eq. nlmax) then
      ! Initialise starting residuum
      rsolverNode%dinitialDefect = dfr
      rsolverNode%dlastDefect = 0.0_DP

      ! Check if our initial defect is zero. This may happen if the filtering
      ! routine filters "everything out"!
      ! In that case we can directly stop our computation.
    ...
    do while (ite < nmaxIterations)
      ...
      ! Step O.2:
      ! Now scale the defect by the inverse of its norm. v(1) = v(1) / ||v(1)||
      call lsysbl_scaleVector (p_Rv(1), 1.0_DP / dres)
      ...
      do i = 1, idim
        ...
        ! Step I.1:Solve P * z(i) = v(i), where P is the preconditioner matrix
        call lsysbl_copyVector (p_Rv(i), p_Rz(i))
    !!!! The following If statement will be executed for all the levels, except for the COARSEST(NLMIN) level     
        ! Apply deflation preconditioning to z(i)
        if (associated(p_rlevelInfoCoarse)) then
          ...
          ! p_rdefTempVector = A * z
          call lsysbl_matVec (p_rlevelInfo%rsystemMatrix, p_Rz(i),p_rdefTempVector, &
              1.0_DP, 0.0_DP)
          
          ! Preconditioning: p_rdefTempVector = P * p_rdefTempVector
          if (bprec) then
            call linsol_precondDefect (&
                p_rlevelInfo%p_rpreconditioner,p_rdefTempVector)
          end if
          ! p_rtempVector = (P A z  -  drelax lambda_max v(i))
