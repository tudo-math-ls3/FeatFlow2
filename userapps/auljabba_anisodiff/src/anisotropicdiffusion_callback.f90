...
!<subroutine>
  ...
  !</subroutine>
    ...
    case (0,2)
      ...
      ! => f(x,y) = - A(1,1)*2*(y^2-1) - A(1,2)*4*x*y - A(2,1)*4*x*y &
      !  		- A(2,2)*2*(x^2-1)   
      
      Dcoefficients (1,:,:) = &
        -A(1,1)*2.0_DP*((DPoints(2,:,:)*DPoints(2,:,:))-1.0_DP) &
	-A(1,2)*4.0_DP*DPoints(1,:,:)*DPoints(2,:,:) &
	-A(2,1)*4.0_DP*DPoints(1,:,:)*DPoints(2,:,:) &
	-A(2,2)*2.0_DP*((DPoints(1,:,:)*DPoints(1,:,:))-1.0_DP)
...
!</subroutine>
  ...
  case (DER_FUNC)
    ! u(x,y) = (x-1)(x+1)(y-1)(y+1)
    Dvalues (:,:) = ((Dpoints(1,:,:)*Dpoints(1,:,:))-1.0_DP)* &
...
!     
!       Dvalues(1) = - 0.5_DP*dx*dx + 0.5_DP
!   else if ((dwhere .GE. 2.0_DP) .AND. (dwhere .LE. 3.0_DP)) then
!       Dvalues(1) = - 0.5_DP*dx*dx + 0.5_DP
!   else 
!       Dvalues(1) = 0.0_DP
!   end if
