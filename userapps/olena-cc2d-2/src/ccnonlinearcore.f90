...
!#   $\mu$    = 0/1     - Switches the 'P'-term on/off,
!#   $\kappa$ = 0/1     - Switches the 'K'-term on/off,
!#   $\jota$  = 0/1     - Switches the 'C'-term on/off,
!#
...
!<typeblock>
  ...
  type t_ccNonlinearIteration
    ...
    ! MU-parameter that switch the P-term on/off
    real(DP) :: dmu = 0.0_DP

    ! KAPPA-parameter that switch the K-term on/off
    real(DP) :: dkappa = 0.0_DP

    ! JOTA-parameter that switch the P-term on/off
    ...
    real(DP), dimension(5) :: DresidualInit = 0.0_DP

    ! Auxiliary variable: Saves the last defect in the nonlinear iteration
    real(DP), dimension(5) :: DresidualOld = 0.0_DP
...
contains
  ...
  !</subroutine>
      ...
      rnonlinearCCMatrix%dmu = rnonlinearIteration%dmu
      rnonlinearCCMatrix%dkappa = rnonlinearIteration%dkappa
      rnonlinearCCMatrix%djota = rnonlinearIteration%djota
...
!    ilvmax = collct_getvalue_int (p_rcollection,'NLMAX')
  ...
  !</output>
      ...
      Dresiduals(5) = sqrt(Dresiduals(1)**2 + Dresiduals(2)**2 + Dresiduals(3)**2)
      ...
      if (ite .eq. 0) then
        ...
        call output_line (' IT  RELU     RELP     DEF-U    DEF-DIV   DEF-C '// &
        ...
        call output_line ('  0                   '// &
            ...
            trim(sys_sdEP(Dresiduals(5),9,2)),coutputMode=rsolverNode%coutputMode)
        call output_separator (OU_SEP_MINUS,coutputMode=rsolverNode%coutputMode)
        ...
        drhoNL = (Dresiduals(5)/dresINIT) ** (1.0_DP/real(ite,DP))

        ! Calculate norms of the solution/defect vector, calculated above
        dresU   = Dresiduals(1)
        dresDIV = Dresiduals(2)
        dresC   = Dresiduals(3)
        dres    = sqrt(dresU**2 + dresDIV**2 + dresC**2)
...
!<output>
  ...
  ! Dresiduals(1) = RESU   = ||defect_u|| / ||rhs_u|| = velocity residual
  ! Dresiduals(2) = RESDIV = ||p|| / ||u||            = divergence residual
  ! Dresiduals(3) = RESC   = ||defect_c|| / ||rhs_c|| = concentration residual
...
!</subroutine>
    ...
    ! local variables
    real(DP) :: dresFU,dresFC,DresTmp(4),dnormU
    integer, dimension(4) :: Cnorms
    ...
    dresFU = max(DresTmp(1),DresTmp(2))
    if (dresFU .lt. 1.0E-8_DP) dresFU = 1.0_DP

    dresFC = DresTmp(4)
    if (dresFC .lt. 1.0E-8_DP) dresFC = 1.0_DP
    ...
    Dresiduals(1) = sqrt(DresTmp(1)**2+DresTmp(2)**2)/dresFU

    Dresiduals(3) = DresTmp(4)/dresFC
