...
contains
 ...
 subroutine calcPressureIntMean(rvector)

type(t_vectorBlock), intent(in) :: rvector

real(DP), dimension(:), pointer :: p_Dp, p_Dvol
...
! get triangulation
p_rtria => rvector%p_rblockDiscr%p_rtriangulation

! get pressure array
call lsyssc_getbase_double(rvector%RvectorBlock(3), p_Dp)

! get element volumes
call storage_getbase_double(p_rtria%h_DelementVolume, p_Dvol)

! compute integral mean
intmean = 0.0_DP

do i = 1, p_rtria%NEL
  intmean = intmean + p_Dp(i)*p_Dvol(i)
end do

intmean = intmean / p_Dvol(p_rtria%NEL+1)

! print to screen and log file
call output_line('Pressure Integral Mean: ' // trim(sys_sde(intmean, 10)))
  ...
  subroutine test1234
    ...
    ! call output_lbrk()
     call output_line ("Featflow rocks!",&
...
!</subroutine>
    ...
    if ((rsolution%p_rblockDiscr%RspatialDiscr(1)% &
      ...
      case (2)
        ...
        ! >>>>> NEW ASSEMBLY >>>>>          
        if ((rproblem%rphysics%cviscoModel .eq. 0) .or. (rproblem%rphysics%cviscoModel .eq. 100)) then
