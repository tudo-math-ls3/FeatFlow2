...
!</subroutine>
    ...
    case (1,2,3,4)
      ...
      !                          rdomainIntSubset, DER_FUNC, p_Ddata(:,:,6))
      ...
      case (4)
        ...
        !   z = ||D(u)||^2 + dviscoEps^2
        Dcoefficients(:,:) = dnu * (p_Ddata(:,:,1)+dviscoEps)**(0.5_DP*dviscoexponent-1.0_DP)
        !Dcoefficients(:,:) = dnu * p_Ddata(:,:,6) * (p_Ddata(:,:,1)+dviscoEps)**(0.5_DP*dviscoexponent-1.0_DP)
      !Dcoefficients(:,:) = (p_Ddata(:,:,6) / sqrt(p_Ddata(:,:,1))) + &
             !(p_Ddata(:,:,6) / (sqrt(p_Ddata(:,:,1)) + sqrt(p_Ddata(:,:,1))))
