  ...
  do ilev=NLMAX,NLMIN,-1
    ...
    case (1)
      ...
      if (rcollection%DquickAccess(4) .eq. 1.0_DP) then
        ...
        call bma_buildMatrix (p_rmatrix,BMA_CALC_STANDARD,ls_svp2D_NewtonMatrix,&
           rcubatureInfo=Rlevels(ilev)%rcubatureInfo,rcollection=rcollection, &
           revalVectors=revalVectors)      
      end if
    end select
...
!</subroutine>
  ...
  do iel = 1,nelements
    ...
    do idofe=1,p_rmatrixDataA11%ndofTest
      ...
      dval = p_DcubWeight(icubp,iel) * (   beta*( dUy*dU*dbasJ*dbasIx + & 
                dUy*dV*dbasJ*dbasIy ) + gama*dnu*dnu*2.0_DP*dbasJx*dbasIy + &
                alpha*(dbasJy*dbasIx)   ) 
      ! A21
  ...
  do iel = 1,nelements
    ...
    do idofe=1,p_rmatrixDataA14%ndofTest
      ...
      ! A41
      dval = p_DcubWeight(icubp,iel) * ( -2.0_DP*gama*dnu*dbasJ*dbasIx - &
             ( dbasJx*dbasIx*dU+dbasJx*dbasIy*dV )   )
      ...
      ! A42
      p_DlocalMatrixA42(idofe,jdofe,iel) = p_DlocalMatrixA42(idofe,jdofe,iel) + 0.0_DP
      ...
      ! A51
      dval = p_DcubWeight(icubp,iel)*(  -2.0_DP*gama*dnu*dbasJ*dbasIy -&
            ( dbasJy*dbasIx*dU+dbasJy*dbasIy*dV )   )
      ...
      ! A61
      p_DlocalMatrixA61(idofe,jdofe,iel) = p_DlocalMatrixA61(idofe,jdofe,iel) + 0.0_DP      
...
!</subroutine>
  ...
  real(DP), dimension(:,:), pointer :: p_DcubWeight
  type(t_bmaMatrixData), pointer :: p_rmatrixDataA11,p_rmatrixDataA14
  ...
  do iel = 1,nelements
    ...
    do idofe=1,p_rmatrixDataA11%ndofTest
      ...
      ! the cubature weight and sum up into the local matrices.
      ! A11
      dval = p_DcubWeight(icubp,iel) * (   dbasJ*dbasI*(dUx*dUx+dVx*dVx) + & 
                dUx*dU*dbasJ*dbasIx + dUx*dV*dbasJ*dbasIy   )
      p_DlocalMatrixA11(jdofe,idofe,iel) = p_DlocalMatrixA11(jdofe,idofe,iel) + dval
      
      ! A22
      dval = p_DcubWeight(icubp,iel) * (   dbasJ*dbasI*(dUy*dUy+dVy*dVy) + & 
                dVy*dU*dbasJ*dbasIx + dVy*dV*dbasJ*dbasIy   )      
      p_DlocalMatrixA22(jdofe,idofe,iel) = p_DlocalMatrixA22(jdofe,idofe,iel) + dval
      
      ! A12
      dval = p_DcubWeight(icubp,iel) * (   dbasJ*dbasI*(dUy*dUx+dVy*dVx) + & 
                dUy*dU*dbasJ*dbasIx + dUy*dV*dbasJ*dbasIy   )      
      p_DlocalMatrixA12(jdofe,idofe,iel) = p_DlocalMatrixA12(jdofe,idofe,iel) + dval
      
      ! A21
      dval = p_DcubWeight(icubp,iel) * (   dbasJ*dbasI*(dUy*dUx+dVy*dVx) + &
                dU*dVx*dbasJx*dbasI + dV*dVx*dbasJy*dbasI    ) 
      p_DlocalMatrixA21(idofe,jdofe,iel) = p_DlocalMatrixA21(idofe,jdofe,iel) + dval      
  ...
  do iel = 1,nelements
    ...
    do idofe=1,p_rmatrixDataA14%ndofTest
      ...
      ! A41
      dval = p_DcubWeight(icubp,iel) * (   -dUx*dbasJx*dbasI   )      
      p_DlocalMatrixA41(idofe,jdofe,iel) = p_DlocalMatrixA41(idofe,jdofe,iel) + dval

      ! A24
      
      ! A42
      dval = p_DcubWeight(icubp,iel) * (   -dUy*dbasJx*dbasI   )      
      ...
      ! A61
      dval = p_DcubWeight(icubp,iel) * (   -dVx*dbasJy*dbasI   )
      p_DlocalMatrixA61(idofe,jdofe,iel) = p_DlocalMatrixA61(idofe,jdofe,iel) + dval      
      
      ! A26
      
      ! A62
      dval = p_DcubWeight(icubp,iel)*(  -dVy*dbasJy*dbasI   )
      p_DlocalMatrixA62(idofe,jdofe,iel) = p_DlocalMatrixA62(idofe,jdofe,iel) + dval      
      end do ! idofe
...
!<subroutine>
  ...
  do iel = 1,nelements
    ...
    do idofe=1,p_rvectorData1%ndofTest
      ...
      dval1 = dfx*(dU*dbasIx+dV*dbasIy) + &
            beta*(  dfx*dUx*dbasI + dfy*dVx*dbasI  )
          
      dval2 = dfy*(dU*dbasIx+dV*dbasIy) + &
            beta*(  dfx*dUy*dbasI + dfy*dVy*dbasI  )
