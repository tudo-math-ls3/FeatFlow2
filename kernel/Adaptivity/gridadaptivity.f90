...
MODULE gridadaptivity
  ...
  PRIVATE
  PUBLIC :: gridadapt_initFromParameterlist
  PUBLIC :: gridadapt_initFromTriangulation
  PUBLIC :: gridadapt_generateRawMesh
  PUBLIC :: gridadapt_releaseAdaptation
  PUBLIC :: gridadapt_setVertexCoordinates2D
  PUBLIC :: gridadapt_getVertexCoordinates2D
  PUBLIC :: gridadapt_setVerticesAtElement
  PUBLIC :: gridadapt_getVerticesAtElement
  PUBLIC :: gridadapt_setNeighboursAtElement
  PUBLIC :: gridadapt_getNeighboursAtElement
  PUBLIC :: gridadapt_setNelOfType
  PUBLIC :: gridadapt_getNelOfType
  PUBLIC :: gridadapt_setBoundary
  PUBLIC :: gridadapt_getBoundary
  PUBLIC :: gridadapt_setNodalProperty
  PUBLIC :: gridadapt_getNodalProperty
  PUBLIC :: gridadapt_performAdaptation
  PUBLIC :: gridadapt_info
  PUBLIC :: gridadapt_writeGridSVG
...
!<constantblock description="Global flags for grid refinement/coarsening">
  ...
  ! No refinement
  INTEGER, PARAMETER, PUBLIC :: ADAPT_NOREFINEMENT  = 0

  ! No coarsening
  INTEGER, PARAMETER, PUBLIC :: ADAPT_NOCOARSENING  = 0

  ! Red-Green refinement strategy (R. Banks)
  INTEGER, PARAMETER, PUBLIC :: ADAPT_REDGREEN      = 1

  ! Longest edge bisection strategy (M. Rivara)
  INTEGER, PARAMETER, PUBLIC :: ADAPT_LONGESTEDGE   = 2
...
!<constantblock description="Bitfield identifiers for state of adaptation">
  ...
  INTEGER, PARAMETER, PUBLIC :: ADAPT_UNDEFINED     = 2**0

  ! Parameters of adaptivity structure are initialized
  INTEGER, PARAMETER :: ADAPT_HAS_PARAMETERS        = 2**1

  ! Quadtree for vertex coordinates is generated
  INTEGER, PARAMETER :: ADAPT_HAS_QUADTREE          = 2**2

  ! Array for IverticesAtElement is generated
  INTEGER, PARAMETER :: ADAPT_HAS_VERTATELEM        = 2**3

  ! Array for IneighboursAtElement is generated
  INTEGER, PARAMETER :: ADAPT_HAS_NEIGHATELEM       = 2**4

  ! Boundary data is generated
  INTEGER, PARAMETER :: ADAPT_HAS_BOUNDARY          = 2**5

  ! Nodal property is generated
  INTEGER, PARAMETER :: ADAPT_HAS_NODALPROP         = 2**6

  ! Number of elements for predefined type
  INTEGER, PARAMETER :: ADAPT_HAS_NELOFTYPE         = 2**7

  ! Dynamic data structures are all generated
  INTEGER, PARAMETER :: ADAPT_HAS_DYNAMICDATA       = ADAPT_HAS_PARAMETERS+&
                                                      ADAPT_HAS_QUADTREE+&
                                                      ADAPT_HAS_VERTATELEM+&
                                                      ADAPT_HAS_NEIGHATELEM+&
                                                      ADAPT_HAS_BOUNDARY+&
                                                      ADAPT_HAS_NODALPROP+&
                                                      ADAPT_HAS_NELOFTYPE

  ! Cells are marked for refinement
  INTEGER, PARAMETER, PUBLIC :: ADAPT_MARKEDREFINE  = 2**8

  ! Cells are marked for coarsening
  INTEGER, PARAMETER, PUBLIC :: ADAPT_MARKEDCOARSEN = 2**9

  ! Grid has been refined
  INTEGER, PARAMETER, PUBLIC :: ADAPT_REFINED       = 2**10
  
  ! Grid has been coarsened
  INTEGER, PARAMETER, PUBLIC :: ADAPT_COARSENED     = 2**11
...
!<types>
  ...
  TYPE, PUBLIC :: t_gridadapt
    ! Format Tag: Specifies the state of adaptation
    INTEGER :: iSpec                                 = ADAPT_UNDEFINED

    ! Tag: Specified the strategy for grid refinement
    INTEGER :: irefinementStrategy                   = ADAPT_NOREFINEMENT

    ! Tag: Specifies the coarsening strategy
    INTEGER :: icoarseningStrategy                   = ADAPT_NOCOARSENING
    ...
    TYPE(t_tree), DIMENSION(:), POINTER :: rBoundary => NULL()

    ! Sparsity graph
    TYPE(t_graph) :: rsparsityGraph
  END TYPE t_gridadapt
...
!<subroutine>
  ...
  SUBROUTINE gridadapt_initFromParameterlist(rgridadapt,rparlist,ssection)
...
!<output>
    ...
    TYPE(t_gridadapt), INTENT(OUT) :: rgridadapt
!</output>
!</subroutine>

    ! Get mandatory parameters from list
    CALL parlst_getvalue_int   (rparlist,ssection,"nsubdividemax",rgridadapt%nsubdividemax)
    CALL parlst_getvalue_int   (rparlist,ssection,"irefinementStrategy",rgridadapt%irefinementStrategy)
    CALL parlst_getvalue_int   (rparlist,ssection,"icoarseningStrategy",rgridadapt%icoarseningStrategy)
    CALL parlst_getvalue_double(rparlist,ssection,"drefinementTolerance",rgridadapt%drefinementTolerance)
    CALL parlst_getvalue_double(rparlist,ssection,"dcoarseningTolerance",rgridadapt%dcoarseningTolerance)

    ! Initialize data
    rgridadapt%iSpec = ADAPT_HAS_PARAMETERS
  END SUBROUTINE gridadapt_initFromParameterlist

  ! ***************************************************************************

!<subroutine>

  SUBROUTINE gridadapt_initFromTriangulation(rgridadapt,rtriangulation)
...
!<inputoutput>
    ! Adaptivity structure
    TYPE(t_gridadapt), INTENT(INOUT) :: rgridadapt
!</inputoutput>
!</subroutine>

    ! Set dimension
    rgridadapt%ndim = rtriangulation%ndim

    ! Set coordinates
    CALL gridadapt_setVertexCoordinates2D(rgridadapt,&
        rtriangulation%h_DvertexCoords,rtriangulation%NVT)

    ! Set nodal properties
    CALL gridadapt_setNodalProperty(rgridadapt,&
        rtriangulation%h_InodalProperty)

    ! Set element numbers
    CALL gridadapt_setNelOfType(rgridadapt,&
        rtriangulation%InelOfType)
    
    ! Set vertices at element
    CALL gridadapt_setVerticesAtElement(rgridadapt,&
        rtriangulation%h_IverticesAtElement,&
        rtriangulation%NEL)

    ! Set elements adjacent to element
    CALL gridadapt_setNeighboursAtElement(rgridadapt,&
        rtriangulation%h_IneighboursAtElement)
    
    ! Set boundary
    CALL gridadapt_setBoundary(rgridadapt,&
  ...
  END SUBROUTINE gridadapt_initFromTriangulation

  ! ***************************************************************************

!<subroutine>

  SUBROUTINE gridadapt_generateRawMesh(rgridadapt,rtriangulation)
...
!</subroutine>
    ...
    ! Get dimensions
    rtriangulation%ndim = rgridadapt%ndim

    ! Get coordinates
    CALL gridadapt_getVertexCoordinates2D(rgridadapt,&
        rtriangulation%h_DvertexCoords,&
        rtriangulation%NVT)

    ! Get number of elements
    CALL gridadapt_getNelOfType(rgridadapt,rtriangulation%InelOfType)

    ! Get vertices at element list
    CALL gridadapt_getVerticesAtElement(rgridadapt,&
        rtriangulation%h_IverticesAtElement,&
        rtriangulation%NEL)

    ! Get element neighbours
    CALL gridadapt_getNeighboursAtElement(rgridadapt,&
        rtriangulation%h_IneighboursAtElement)

    ! Get nodal property list
    CALL gridadapt_getNodalProperty(rgridadapt,&
        rtriangulation%h_InodalProperty)

    ! Get boundary
    CALL gridadapt_getBoundary(rgridadapt,&
        rtriangulation%h_IboundaryCpIdx,&
        rtriangulation%h_IverticesAtBoundary,&
        rtriangulation%h_DvertexParameterValue,&
        rtriangulation%NVBD,rtriangulation%NBCT)
  END SUBROUTINE gridadapt_generateRawMesh
  
  ! ***************************************************************************

!<subroutine>

  SUBROUTINE gridadapt_releaseAdaptation(rgridadapt)

!<description>
    ! This subroutine releases all internal structures of the
    ! adaptivity data structure rgridadapt.
!</description>
...
!</subroutine>
    ...
    IF (IAND(rgridadapt%iSpec,ADAPT_HAS_QUADTREE) .NE. 0) THEN
      CALL qtree_releaseQuadtree(rgridadapt%rVertexCoordinates2D)
    END IF

    ! Check if boundary structure exists
    IF (ASSOCIATED(rgridadapt%rBoundary)) THEN
      DO ibct=1,SIZE(rgridadapt%rBoundary)
        CALL tree_releaseTree(rgridadapt%rBoundary(ibct))
      END DO
      DEALLOCATE(rgridadapt%rBoundary)
      NULLIFY(rgridadapt%rBoundary)
    END IF

    ! Release storage which is no longer in use
    IF (rgridadapt%h_Imarker /= ST_NOHANDLE)&
        CALL storage_free(rgridadapt%h_Imarker)
    IF (rgridadapt%h_IvertexAge /= ST_NOHANDLE)&
        CALL storage_free(rgridadapt%h_IvertexAge)
    IF (rgridadapt%h_ImidneighboursAtElement /= ST_NOHANDLE)&
        CALL storage_free(rgridadapt%h_ImidneighboursAtElement)
    
    ! Nullify "performance-pointers"
    NULLIFY(rgridadapt%p_IvertexAge)
    NULLIFY(rgridadapt%p_InodalProperty)
    NULLIFY(rgridadapt%p_IverticesAtElement)
    NULLIFY(rgridadapt%p_IneighboursAtElement)
    NULLIFY(rgridadapt%p_ImidneighboursAtElement)
    
    ! Clear parameters
    rgridadapt%irefinementStrategy  = ADAPT_NOREFINEMENT
    rgridadapt%icoarseningStrategy  = ADAPT_NOCOARSENING
    rgridadapt%drefinementTolerance = 0._DP
    rgridadapt%dcoarseningTolerance = 0._DP

    ! Clear data
    rgridadapt%iSpec            = ADAPT_UNDEFINED
    rgridadapt%nRefinementSteps = 0
    rgridadapt%nCoarseningSteps = 0
    rgridadapt%nSmoothingSteps  = 0
    rgridadapt%nGreenElements   = 0
    rgridadapt%ndim             = 0
    rgridadapt%NVT              = 0
    rgridadapt%NVT0             = 0
    ...
    rgridadapt%NEL0             = 0
    rgridadapt%NELMAX           = 0
    rgridadapt%InelOfType       = 0
    rgridadapt%InelOfType0      = 0
  END SUBROUTINE gridadapt_releaseAdaptation

  ! ***************************************************************************

!<subroutine>

  SUBROUTINE gridadapt_setVertexCoordinates2D(rgridadapt,h_DvertexCoords,nvt)
...
!</subroutine>
    ...
    IF (h_DvertexCoords == ST_NOHANDLE) THEN
      PRINT *, "gridadapt_setVertexCoordinates2D: Invalid handle!"
      STOP
    END IF
    ...
    CALL qtree_createQuadtree(rgridadapt%rVertexCoordinates2D,nvt,&
        nquad,xmin,ymin,xmax,ymax)
    
    ! Copy vertex coordinates to quadtree
    CALL qtree_copyToQuadtree(p_DvertexCoords,rgridadapt%rVertexCoordinates2D)

    ! Set specifier for quadtree
    rgridadapt%iSpec=IOR(rgridadapt%iSpec,ADAPT_HAS_QUADTREE)

    ! Set dimensions
    rgridadapt%ndim = NDIM2D
    rgridadapt%NVT  = nvt
  END SUBROUTINE gridadapt_setVertexCoordinates2D

  ! ***************************************************************************

!<subroutine>

  SUBROUTINE gridadapt_getVertexCoordinates2D(rgridadapt,h_DvertexCoords,nvt,ndim)
...
!<input>
    ! Adaptivity structure
    TYPE(t_gridadapt), INTENT(IN) :: rgridadapt
!</input>
...
!</subroutine>
    ...
    ! Check if quadtree exists
    IF (IAND(rgridadapt%iSpec,ADAPT_HAS_QUADTREE) .EQ. 0) THEN
      PRINT *, "gridadapt_getVertexCoordinates2D: quadtree does not exist!"
      STOP
    END IF

    ! Copy quadtree to handle h_DvertexCoords
    CALL qtree_copyFromQuadtree(rgridadapt%rVertexCoordinates2D,h_DvertexCoords)

    ! Set dimension
    IF (PRESENT(ndim)) ndim=rgridadapt%ndim
    IF (PRESENT(nvt)) nvt=rgridadapt%NVT
  END SUBROUTINE gridadapt_getVertexCoordinates2D
  
  ! ***************************************************************************

!<subroutine>
  
  SUBROUTINE gridadapt_setVerticesAtElement(rgridadapt,h_IverticesAtElement,nel)
...
!</subroutine>
    ...
    IF (h_IverticesAtElement == ST_NOHANDLE) THEN
      PRINT *, "gridadapt_setVerticesAtElement: Invalid handle!"
      STOP
    END IF

    rgridadapt%h_IverticesAtElement = h_IverticesAtElement
    ...
    rgridadapt%iSpec=IOR(rgridadapt%iSpec,ADAPT_HAS_VERTATELEM)

    ! Set dimensions
    rgridadapt%NEL  = nel     
  END SUBROUTINE gridadapt_setVerticesAtElement

  ! ***************************************************************************

!<subroutine>

  SUBROUTINE gridadapt_getVerticesAtElement(rgridadapt,h_IverticesAtElement,nel)
...
!</subroutine>
    ...
    IF (IAND(rgridadapt%iSpec,ADAPT_HAS_VERTATELEM) .EQ. 0) THEN
      PRINT *, "gridadapt_getVerticesAtElement: structure does not exist!"
      STOP
    END IF

    ! Check if handle needs to be freed first
    IF (h_IverticesAtElement /= ST_NOHANDLE .AND.&
        h_IverticesAtElement /= rgridadapt%h_IverticesAtElement)&
    ...
    ! Set dimensions
    IF(PRESENT(nel))   nel=rgridadapt%NEL
  END SUBROUTINE gridadapt_getVerticesAtElement
  
  ! ***************************************************************************

!<subroutine>
  
  SUBROUTINE gridadapt_setNeighboursAtElement(rgridadapt,h_IneighboursAtElement)
...
!</subroutine>
    ...
    IF (h_IneighboursAtElement == ST_NOHANDLE) THEN
      PRINT *, "gridadapt_setNeighboursAtElement: Invalid handle!"
      STOP
    END IF

    rgridadapt%h_IneighboursAtElement = h_IneighboursAtElement
    CALL storage_getbase_int2D(rgridadapt%h_IneighboursAtElement,&
        rgridadapt%p_IneighboursAtElement)
    
    ! Create structure of "mid-adjacent" neighbours
    CALL storage_copy(rgridadapt%h_IneighboursAtElement,&
        rgridadapt%h_ImidneighboursAtElement)
    CALL storage_getbase_int2D(rgridadapt%h_ImidneighboursAtElement,&
        rgridadapt%p_ImidneighboursAtElement)
    
    ! Set specifier for IverticesAtElement
    rgridadapt%iSpec=IOR(rgridadapt%iSpec,ADAPT_HAS_NEIGHATELEM)
  END SUBROUTINE gridadapt_setNeighboursAtElement

  ! ***************************************************************************

!<subroutine>

  SUBROUTINE gridadapt_getNeighboursAtElement(rgridadapt,&
...
!</subroutine>
    ...
    IF (IAND(rgridadapt%iSpec,ADAPT_HAS_NEIGHATELEM) .EQ. 0) THEN
      PRINT *, "gridadapt_getNeighboursAtElement: structure does not exist!"
      STOP
    END IF

    ! Check if handle needs to be freed first
    IF (h_IneighboursAtElement /= ST_NOHANDLE .AND.&
        h_IneighboursAtElement /= rgridadapt%h_IneighboursAtElement)&
    ...
    ! Set dimensions
    IF(PRESENT(nel))   nel=rgridadapt%NEL
  END SUBROUTINE gridadapt_getNeighboursAtElement

  ! ***************************************************************************

!<subroutine>

  SUBROUTINE gridadapt_setNelOfType(rgridadapt,InelOfType)
...
!</subroutine>
    ...
    rgridadapt%InelOfType = InelOfType

    ! Set specifier for InelOfType
    rgridadapt%iSpec=IOR(rgridadapt%iSpec,ADAPT_HAS_NELOFTYPE)
  END SUBROUTINE gridadapt_setNelOfType

  ! ***************************************************************************

!<subroutine>

  SUBROUTINE gridadapt_getNelOfType(rgridadapt,InelOfType)
...
!</subroutine>
    ...
    IF (IAND(rgridadapt%iSpec,ADAPT_HAS_NELOFTYPE) .EQ. 0) THEN
      PRINT *, "gridadapt_getNelOfType: structure does not exist!"
      STOP
    END IF
    
    InelOfType=rgridadapt%InelOfType
  END SUBROUTINE gridadapt_getNelOfType

  ! ***************************************************************************

!<subroutine>

  SUBROUTINE gridadapt_setBoundary(rgridadapt,h_IboundaryCpIdx,&
...
!</subroutine>
    ...
    IF ((h_IboundaryCpIdx == ST_NOHANDLE) .OR.&
      ...
      PRINT *, "gridadapt_setNodalProperty: Invalid handle!"
      STOP
    END IF
    ...
    ALLOCATE(rgridadapt%rBoundary(nbct))
    
    ! Create auxiliary array
    Isize=(/2,nvbd/)
    CALL storage_new('gridadapt_setBoundary','IData',Isize,ST_INT,&
    ...
    DO ibct=1,nbct
      ...
      CALL tree_createTree(rgridadapt%rBoundary(ibct),lvbd,ST_INT,1,0,2)
      ...
      ! Fill search tree
      CALL tree_copyToTree(p_IverticesAtBoundary(ivbdStart:ivbdEnd),rgridadapt%rBoundary(ibct),&
          p_DData = p_DvertexParameterValue2D(:,ivbdStart:ivbdEnd),&
          p_IData = p_IneighboursAtBoundary(:,ivbdStart:ivbdEnd))
    END DO
    
    ! Set dimensions
    rgridadapt%NBCT  = nbct
    rgridadapt%NVBD  = nvbd
    rgridadapt%NVBD0 = nvbd

    ! Free auxiliary storage
    CALL storage_free(h_IneighboursAtBoundary)

    ! Set specifier for boundary
    rgridadapt%iSpec=IOR(rgridadapt%iSpec,ADAPT_HAS_BOUNDARY)
  ...
  END SUBROUTINE gridadapt_setBoundary
  
  ! ***************************************************************************

!<subroutine>

  SUBROUTINE gridadapt_getBoundary(rgridadapt,h_IboundaryCpIdx,&
...
!</subroutine>
    ...
    IF (IAND(rgridadapt%iSpec,ADAPT_HAS_BOUNDARY) .EQ. 0) THEN
      PRINT *, "gridadapt_getBoundary: structure does not exist!"
      STOP
    END IF
    ...
    IF (h_IboundaryCpIdx == ST_NOHANDLE) THEN
      CALL storage_new('gridadapt_getBoundary','p_IboundaryCpIdx',&
          rgridadapt%NBCT+1,ST_INT,h_IboundaryCpIdx,ST_NEWBLOCK_NOINIT)
    ELSE
      CALL storage_getsize(h_IboundaryCpIdx,isize)
      IF (isize < rgridadapt%NBCT+1) THEN
        CALL storage_realloc('gridadapt_getBoundary',rgridadapt%NBCT+1,&
            h_IboundaryCpIdx,ST_NEWBLOCK_NOINIT,.FALSE.)
      END IF
    END IF

    IF (h_IverticesAtBoundary == ST_NOHANDLE) THEN
      CALL storage_new('gridadapt_getBoundary','p_IverticesAtBoundary',&
          rgridadapt%NVBD,ST_INT,h_IverticesAtBoundary,ST_NEWBLOCK_NOINIT)
    ELSE
      CALL storage_getsize(h_IverticesAtBoundary,isize)
      IF (isize < rgridadapt%NVBD) THEN
        CALL storage_realloc('gridadapt_getBoundary',rgridadapt%NVBD,&
            h_IverticesAtBoundary,ST_NEWBLOCK_NOINIT,.FALSE.)
      END IF
    END IF

    IF (h_DvertexParameterValue == ST_NOHANDLE) THEN
      CALL storage_new('gridadapt_getBoundary','p_DvertexParameterValue',&
          rgridadapt%NVBD,ST_DOUBLE,h_DvertexParameterValue,ST_NEWBLOCK_NOINIT)
    ELSE
    ...
    ! Set pointers
    CALL storage_getbase_int(h_IboundaryCpIdx,p_IboundaryCpIdx,rgridadapt%NBCT+1)
    CALL storage_getbase_int(h_IverticesAtBoundary,p_IverticesAtBoundary,rgridadapt%NVBD)
    CALL storage_getbase_double(h_DvertexParameterValue,p_DvertexParameterValue,rgridadapt%NVBD)

    ! Initialization
    p_IboundaryCpIdx(1) = 1
    ioff = 0

    DO ibct=1,rgridadapt%NBCT
      
      ! Set subdimensions
      lvbd      = rgridadapt%rBoundary(ibct)%NA
      ivbdStart = ioff+1
      ivbdEnd   = ioff+lvbd
      ioff      = ioff+lvbd
      ...
      CALL tree_copyFromTreeKey(rgridadapt%rBoundary(ibct),p_IverticesAtBoundary(ivbdStart:ivbdEnd))
      CALL tree_copyFromTreeDble(rgridadapt%rBoundary(ibct),p_DvertexParameterValue(ivbdStart:ivbdEnd),1)
    ...
    ! Set dimension
    IF(PRESENT(nvbd)) nvbd=rgridadapt%NVBD
    IF(PRESENT(nbct)) nbct=rgridadapt%NBCT
  END SUBROUTINE gridadapt_getBoundary

  ! ***************************************************************************

!<subroutine>

  SUBROUTINE gridadapt_setNodalProperty(rgridadapt,h_InodalProperty)
...
!</subroutine>
    ...
    rgridadapt%h_InodalProperty = h_InodalProperty
    CALL storage_getbase_int(rgridadapt%h_InodalProperty,rgridadapt%p_InodalProperty)
    
    ! Set specifier for InodalProperty
    rgridadapt%iSpec=IOR(rgridadapt%iSpec,ADAPT_HAS_NODALPROP)

  END SUBROUTINE gridadapt_setNodalProperty

  ! ***************************************************************************

!<subroutine>

  SUBROUTINE gridadapt_getNodalProperty(rgridadapt,h_InodalProperty)
...
!</subroutine>
    ...
    IF (IAND(rgridadapt%iSpec,ADAPT_HAS_NODALPROP) .EQ. 0) THEN
      PRINT *, "gridadapt_getNodalProperty: structure does not exist!"
      STOP
    END IF

    ! Check if handle needs to be freed first
    IF (h_InodalProperty /= ST_NOHANDLE .AND.&
        h_InodalProperty /= rgridadapt%h_InodalProperty)&
        CALL storage_free(h_InodalProperty)

    ! Assign handle
    h_InodalProperty = rgridadapt%h_InodalProperty
  END SUBROUTINE gridadapt_getNodalProperty

  ! ***************************************************************************

!<subroutine>

  SUBROUTINE gridadapt_performAdaptation(rgridadapt,rindicator,&
...
!</subroutine>
    ...
    IF (IAND(rgridadapt%iSpec,ADAPT_HAS_DYNAMICDATA).EQ.0) THEN
      PRINT *, "gridadapt_performAdaptation: dynamic data structures are not generated!"
      STOP
    END IF

    ! Initialize initial dimensions
    CALL storage_getsize(rgridadapt%h_IverticesAtElement,Isize)
    rgridadapt%NELMAX      = Isize(2)
    CALL storage_getsize(rgridadapt%h_IneighboursAtElement,Isize)
    rgridadapt%NELMAX      = MIN(rgridadapt%NELMAX,Isize(2))

    rgridadapt%InelOfType0 = rgridadapt%InelOfType
    ...
    CASE (ADAPT_NOREFINEMENT)   ! No grid refinement
      
    CASE (ADAPT_REDGREEN)   ! Red-green grid refinement

      ! Mark elements for refinement based on indicator function
      CALL mark_refinement2D(rgridadapt,rindicator)

      ! Mark additional elements to restore conformity
      CALL redgreen_mark_refinement2D(rgridadapt,fcb_adjustDimension,fcb_insertVertex)
      ...
      CALL storage_realloc('gridadapt_performAdaptation',nvt,&
          rgridadapt%h_IvertexAge,ST_NEWBLOCK_NOINIT,.TRUE.)
      CALL storage_realloc('gridadapt_performAdaptation',nvt,&
          rgridadapt%h_InodalProperty,ST_NEWBLOCK_NOINIT,.TRUE.)
     
      ! Adjust elemental arrays
      CALL storage_realloc('gridadapt_performAdaptation',nel,&
          rgridadapt%h_IverticesAtElement,ST_NEWBLOCK_NOINIT,.TRUE.)
      CALL storage_realloc('gridadapt_performAdaptation',nel,&
          rgridadapt%h_IneighboursAtElement,ST_NEWBLOCK_NOINIT,.TRUE.)
      CALL storage_realloc('gridadapt_performAdaptation',nel,&
          rgridadapt%h_ImidneighboursAtElement,ST_NEWBLOCK_NOINIT,.TRUE.)
    ...
    CASE (ADAPT_LONGESTEDGE)   ! Bisection of Longest edge
      ...
      ! Perform refinement
      CALL bisection_refine(rgridadapt)

      
    CASE DEFAULT
      PRINT *, "gridadapt_performAdaptation: Unsupported refinement strategy!"
      STOP
    END SELECT
  ...
  CONTAINS
    ...
    FUNCTION NumberOfElements(rgridadapt) RESULT(nel)
      ...
      CALL storage_getbase_int(rgridadapt%h_Imarker,p_Imarker)
  ...
  END SUBROUTINE gridadapt_performAdaptation

  ! ***************************************************************************

!<subroutine>
  
  SUBROUTINE gridadapt_info(rgridadapt)
...
!</subroutine>
    ...
    WRITE(*,FMT='(2X,72("-"))')
    WRITE(*,FMT='(2X,A,T52,I15)') 'Total number of grid refinement steps:', rgridadapt%nRefinementSteps
    WRITE(*,FMT='(2X,A,T52,I15)') 'Total number of grid coarsening steps:', rgridadapt%nCoarseningSteps
    WRITE(*,FMT='(2X,A,T52,I15)') 'Total number of grid smoothing steps:',  rgridadapt%nSmoothingSteps
    WRITE(*,*)
    WRITE(*,FMT='(2X,A,T52,I3)')  'Strategy for grid refinement:', rgridadapt%irefinementStrategy
    WRITE(*,FMT='(2X,A,T52,I3)')  'Strategy for grid coarsening:', rgridadapt%icoarseningStrategy
    WRITE(*,*)
    WRITE(*,FMT='(2X,A,T40,I8,1X,A,1X,I8)') 'Number of elements:',               rgridadapt%NEL,'initially',rgridadapt%NEL0
    WRITE(*,FMT='(2X,A,T40,I8,1X,A,1X,I8)') 'Number of vertices:',               rgridadapt%NVT,'initially',rgridadapt%NVT0
    WRITE(*,FMT='(2X,A,T40,I8,1X,A,1X,I8)') 'Number of boundary vertices:',      rgridadapt%NVBD,'initially',rgridadapt%NVBD0
    WRITE(*,*)
    WRITE(*,FMT='(2X,A)') 'Coordinates:'
    WRITE(*,FMT='(2X,72("-"))')
    CALL qtree_infoQuadtree(rgridadapt%rVertexCoordinates2D)
    DO ibct=1,rgridadapt%NBCT
      WRITE(*,FMT='(2X,A,I2,A)') 'Boundary component',ibct,':'
      WRITE(*,FMT='(2X,72("-"))')
      CALL tree_infoTree(rgridadapt%rBoundary(ibct))
    END DO
    WRITE(*,FMT='(2X,72("+"))')
    WRITE(*,*)
  END SUBROUTINE gridadapt_info

  ! ***************************************************************************

!<subroutine>

  SUBROUTINE gridadapt_writeGridSVG(rgridadapt,coutputFile,cinfo)
...
!</subroutine>
    ...
    bbox = qtree_getBoundingBox(rgridadapt%rVertexCoordinates2D)
    ...
    IF (IAND(rgridadapt%iSpec,ADAPT_MARKEDREFINE).NE.0) THEN
      ...
      DO iel=1,rgridadapt%NEL
        ...
        DO ive=1,nve
          xdim = qtree_getX(rgridadapt%rVertexCoordinates2D,Kvert(ive))-x0
          ydim = qtree_getY(rgridadapt%rVertexCoordinates2D,Kvert(ive))-y0
        ...
        ! ... end element.
        xdim = qtree_getX(rgridadapt%rVertexCoordinates2D,Kvert(1))-x0
        ydim = qtree_getY(rgridadapt%rVertexCoordinates2D,Kvert(1))-y0
        WRITE(UNIT=cx,FMT='(I9)') xoff+INT(xscale*xdim)
        WRITE(UNIT=cy,FMT='(I9)') ysize+yoff-INT(yscale*ydim)
        ...
        IF (btest(cinfo,3) .OR. btest(cinfo,4) .OR. btest(cinfo,5)) THEN
          IF (nve == 3) THEN
            xdim   = SUM(qtree_getX(rgridadapt%rVertexCoordinates2D,Kvert(1:3)))/3._DP-x0
            ydim   = SUM(qtree_getY(rgridadapt%rVertexCoordinates2D,Kvert(1:3)))/3._DP-y0
            istate = redgreen_getStateTria(rgridadapt%p_IvertexAge(Kvert(1:3)))
          ELSE
            xdim   = SUM(qtree_getX(rgridadapt%rVertexCoordinates2D,Kvert(1:4)))/4._DP-x0
            ydim   = SUM(qtree_getY(rgridadapt%rVertexCoordinates2D,Kvert(1:4)))/4._DP-y0
            istate = redgreen_getStateQuad(rgridadapt%p_IvertexAge(Kvert(1:4)))
          END IF
          WRITE(UNIT=cx,FMT='(I9)') xoff+INT(xscale*xdim)
          WRITE(UNIT=cy,FMT='(I9)') ysize+yoff-INT(yscale*ydim)
        ...
        IF (btest(cinfo,6)) THEN
          xmid = SUM(qtree_getX(rgridadapt%rVertexCoordinates2D,Kvert(1:nve)))/REAL(nve,DP)-x0
          ymid = SUM(qtree_getY(rgridadapt%rVertexCoordinates2D,Kvert(1:nve)))/REAL(nve,DP)-y0
          
          DO ive=1,nve
            xdim = (2*qtree_getX(rgridadapt%rVertexCoordinates2D,Kvert(ive))+xmid)/3._DP
            ydim = (2*qtree_getY(rgridadapt%rVertexCoordinates2D,Kvert(ive))+ymid)/3._DP
    ...
    IF (btest(cinfo,0) .OR. btest(cinfo,1) .OR. btest(cinfo,2)) THEN
      ...
      DO ivt=1,qtree_getsize(rgridadapt%rVertexCoordinates2D)
        xdim = qtree_getX(rgridadapt%rVertexCoordinates2D,ivt)-x0
        ydim = qtree_getY(rgridadapt%rVertexCoordinates2D,ivt)-y0
        WRITE(UNIT=cx,FMT='(I9)') xoff+INT(xscale*xdim)
        WRITE(UNIT=cy,FMT='(I9)') ysize+yoff-INT(yscale*ydim)
        ...
        ELSEIF (btest(cinfo,2)) THEN
          WRITE(UNIT=cnvt,FMT='(I9)') rgridadapt%p_IvertexAge(ivt)
          IF (rgridadapt%p_IvertexAge(ivt) > 0) THEN
            WRITE(UNIT=iunit,FMT='(A)') '<text x="'//TRIM(ADJUSTL(cx))//'" y="'//TRIM(ADJUSTL(cy))//&
                '" font-family="Verdana" font-size="48" fill="black">'//TRIM(ADJUSTL(cnvt))//'</text>'
          ELSE
            WRITE(UNIT=iunit,FMT='(A)') '<text x="'//TRIM(ADJUSTL(cx))//'" y="'//TRIM(ADJUSTL(cy))//&
    ...
    WRITE(UNIT=iunit,FMT='(A)') '</svg>'
    CLOSE (UNIT=iunit)
  END SUBROUTINE gridadapt_writeGridSVG
...
!<subroutine>
  ...
  SUBROUTINE add_vertex_atEdgeMidpoint2D(rgridadapt,i1,i2,e1,i12,fcb_insertVertex)
...
!</subroutine>
    ...
    x1 = qtree_getX(rgridadapt%rVertexCoordinates2D,i1)
    y1 = qtree_getY(rgridadapt%rVertexCoordinates2D,i1)
    x2 = qtree_getX(rgridadapt%rVertexCoordinates2D,i2)
    y2 = qtree_getY(rgridadapt%rVertexCoordinates2D,i2)
    ...
    IF (qtree_searchInQuadtree(rgridadapt%rVertexCoordinates2D,&
      ...
      ! Set nodal property
      IF (e1 == 0) THEN
        rgridadapt%p_InodalProperty(i12) = rgridadapt%p_InodalProperty(i1)
      ELSE
        rgridadapt%p_InodalProperty(i12) = 0
      END IF
      
      ! Add new entry to vertex coordinates
      CALL qtree_insertIntoQuadtree(rgridadapt%rVertexCoordinates2D,i12,Dcoord,iquad)
      
      ! Are we at the boundary?
      IF (e1 == 0) THEN
        ! Increment number of boundary nodes
        rgridadapt%NVBD = rgridadapt%NVBD+1
        
        ! Get number of boundary component
        ibct = rgridadapt%p_InodalProperty(i1)
        
        ! Get parameter values of the boundary nodes
        IF (tree_searchInTree(rgridadapt%rBoundary(ibct),i1,ipred) == TREE_NOT_FOUND) THEN
          PRINT *, "add_vertex: Unable to find vertex in boudary data structure!"
          STOP
        END IF
        ipos   = rgridadapt%rBoundary(ibct)%Kchild(MERGE(TLEFT,TRIGHT,ipred < 0),ABS(ipred))
        dvbdp1 = rgridadapt%rBoundary(ibct)%DData(BdrValue,ipos)
        
        IF (tree_searchInTree(rgridadapt%rBoundary(ibct),i2,ipred) == TREE_NOT_FOUND) THEN
          PRINT *, "add_vertex: Unable to find vertex in boudary data structure!"
          STOP
        END IF
        ipos   = rgridadapt%rBoundary(ibct)%Kchild(MERGE(TLEFT,TRIGHT,ipred < 0),ABS(ipred))
        dvbdp2 = rgridadapt%rBoundary(ibct)%DData(BdrValue,ipos)
        ...
        CALL tree_insertIntoTree(rgridadapt%rBoundary(ibct),i12,&
    ...
    CALL grph_insertVertex(rgridadapt%rsparsitygraph,i12)
...
!<subroutine>
  ...
  SUBROUTINE add_vertex_atElementCenter2D(rgridadapt,i1,i2,i3,i4,i5,fcb_insertVertex)
...
!</subroutine>
    ...
    x2 = qtree_getX(rgridadapt%rVertexCoordinates2D,i2)
    y2 = qtree_getY(rgridadapt%rVertexCoordinates2D,i2)
    x3 = qtree_getX(rgridadapt%rVertexCoordinates2D,i3)
    y3 = qtree_getY(rgridadapt%rVertexCoordinates2D,i3)
    x4 = qtree_getX(rgridadapt%rVertexCoordinates2D,i4)
    y4 = qtree_getY(rgridadapt%rVertexCoordinates2D,i4)
    ...
    IF (qtree_searchInQuadtree(rgridadapt%rVertexCoordinates2D,&
      ...
      rgridadapt%p_IvertexAge(I5) = 1+MAX(ABS(rgridadapt%p_IvertexAge(i1)),&
          ABS(rgridadapt%p_IvertexAge(i2)),ABS(rgridadapt%p_IvertexAge(i3)),&
          ABS(rgridadapt%p_IvertexAge(i4)))

      ! Set nodal property
      rgridadapt%p_InodalProperty(i5) = 0
      
      ! Add new entry to DCORVG
      CALL qtree_insertIntoQuadtree(rgridadapt%rVertexCoordinates2D,i5,Dcoord,iquad)
    END IF
    

    ! Insert vertex into graph
    CALL grph_insertVertex(rgridadapt%rsparsitygraph,i5)
...
!<subroutine>
  ...
  SUBROUTINE remove_vertex2D(rgridadapt,ivt,jvt,fcb_removeVertex)
...
!</subroutine>
    ...
    ibct = rgridadapt%p_InodalProperty(ivt)

    ! Are we at the boundary?
    IF (ibct /= 0) THEN

      ! Find position of vertex IVT in boundary array
      IF (tree_searchInTree(rgridadapt%rBoundary(ibct),ivt,ipred) == TREE_NOT_FOUND) THEN
      ...
      i1 = rgridadapt%rBoundary(ibct)%IData(BdrPrev,ipos)
      i2 = rgridadapt%rBoundary(ibct)%IData(BdrNext,ipos)
      ...
      END IF
      ipos = rgridadapt%rBoundary(ibct)%Kchild(MERGE(TLEFT,TRIGHT,ipred < 0),ABS(ipred))
      rgridadapt%rBoundary(ibct)%IData(BdrNext,ipos) = i2
      ...
      END IF
      ipos = rgridadapt%rBoundary(ibct)%Kchild(MERGE(TLEFT,TRIGHT,ipred < 0),ABS(ipred))
      rgridadapt%rBoundary(ibct)%IData(BdrPrev,ipos) = i1
      
      IF (tree_deleteFromTree(rgridadapt%rBoundary(ibct),ivt) == TREE_NOT_FOUND) THEN
    ...
    IF (ivt < jvt) THEN
      ...
      ibct = rgridadapt%p_InodalProperty(jvt)

      ! Are we at the boundary?
      IF (ibct /= 0) THEN

        IF (tree_searchInTree(rgridadapt%rBoundary(ibct),jvt,ipred) == TREE_NOT_FOUND) THEN
        ...
        CALL tree_insertIntoTree(rgridadapt%rBoundary(ibct),ivt,&
            Idata=rgridadapt%rBoundary(ibct)%IData(:,ipos),&
            Ddata=rgridadapt%rBoundary(ibct)%DData(:,ipos))
        ...
        END IF
        ipos = rgridadapt%rBoundary(ibct)%Kchild(MERGE(TLEFT,TRIGHT,ipred < 0),ABS(ipred))
        rgridadapt%rBoundary(ibct)%IData(BdrNext,ipos) = ivt
        ...
        END IF
        ipos = rgridadapt%rBoundary(ibct)%Kchild(MERGE(TLEFT,TRIGHT,ipred < 0),ABS(ipred))
        rgridadapt%rBoundary(ibct)%IData(BdrPrev,ipos) = ivt
        
        ! Delete JVT from the boundary
        IF (tree_deleteFromTree(rgridadapt%rBoundary(ibct),jvt) == TREE_NOT_FOUND) THEN
      ...
      rgridadapt%p_InodalProperty(ivt) = rgridadapt%p_InodalProperty(jvt)
      rgridadapt%p_IvertexAge(ivt)     = rgridadapt%p_IvertexAge(jvt)
      

!!$      ! Replace column IVT by column JVT in KCOL
!!$      CALL arrlst_swapArraylist(rgridadapt%rKcol,ivt,jvt)
!!$      
!!$      ! Modify diagonal entry in KCOL
!!$      ipos = arrlst_getNextInArraylist(rgridadapt%rKcol,ivt,.TRUE.)
!!$      rgridadapt%rKcol%IData(ipos) = ivt
!!$      
!!$      ! Eliminate JVT from the KCOL and insert the replacement IVT
!!$      DO
!!$        ipos = arrlst_getNextInArraylist(rgridadapt%rKcol,ivt,.FALSE.)
!!$        IF (ipos == ANULL) EXIT
!!$        kvt = rgridadapt%rKcol%IData(ipos)
!!$        
!!$        IF (arrlst_deleteFromArraylist(rgridadapt%rKcol,kvt,jvt) == ARRAYLIST_NOT_FOUND) THEN
!!$          PRINT *, "remove_vertex: Unable to delete vertex JVT=",jvt," from list KCOL"
!!$          STOP
!!$        END IF
!!$
!!$        IF (arrlst_searchInArraylist(rgridadapt%rKcol,kvt,ivt,ipred) == ARRAYLIST_NOT_FOUND) THEN
!!$          PRINT *, "remove_vertex: Unable to find vertex IVT=",ivt," in list KCOL"
!!$          STOP
!!$        END IF
!!$        CALL arrlst_insertIntoArraylist(rgridadapt%rKcol,kvt,ivt,ipred,ipos)
!!$      END DO
    END IF
 
    ! Eliminate column JVT in KCOL
!    CALL arrlst_releaseArraylist(rgridadapt%rKcol,jvt)
    
    ! Decrease number of entries in KCOL by one
 !   rgridadapt%NA = rgridadapt%NA-1

!!$    IF (tree_deleteFromTree(rgridadapt%rKld,jvt+1) == TREE_NOT_FOUND) THEN
...
!<subroutine>
  ...
  SUBROUTINE add_edge(rgridadapt,i1,i2)
...
!</subroutine>
    ...
    CALL grph_insertEdge(rgridadapt%rsparsitygraph,i1,i2)
...
!<subroutine>
  ...
  SUBROUTINE remove_edge(rgridadapt,i1,i2)
...
!</subroutine>
    ...
    CALL grph_removeEdge(rgridadapt%rsparsitygraph,i1,i2)   
...
!<subroutine>
  ...
  SUBROUTINE replace_elementTria(rgridadapt,ipos,i1,i2,i3,e1,e2,e3,e4,e5,e6)
...
!</subroutine
    ...
    rgridadapt%p_IverticesAtElement(:,ipos)      = (/i1,i2,i3,0/)
    rgridadapt%p_IneighboursAtElement(:,ipos)    = (/e1,e2,e3,0/)
    rgridadapt%p_ImidneighboursAtElement(:,ipos) = (/e4,e5,e6,0/)    
...
!<subroutine>
  ...
  SUBROUTINE replace_elementQuad(rgridadapt,ipos,i1,i2,i3,i4,e1,e2,e3,e4,e5,e6,e7,e8)
...
!</subroutine
    ...
    rgridadapt%p_IverticesAtElement(:,ipos)      = (/i1,i2,i3,i4/)
    rgridadapt%p_IneighboursAtElement(:,ipos)    = (/e1,e2,e3,e4/)
    rgridadapt%p_ImidneighboursAtElement(:,ipos) = (/e5,e6,e7,e8/)
...
!<subroutine>
  ...
  SUBROUTINE add_elementTria(rgridadapt,i1,i2,i3,e1,e2,e3,e4,e5,e6)
...
!</subroutine
    ...
    rgridadapt%p_ImidneighboursAtElement(:,rgridadapt%NEL) = (/e4,e5,e6,0/)
...
!<subroutine>
  ...
  SUBROUTINE add_elementQuad(rgridadapt,i1,i2,i3,i4,e1,e2,e3,e4,e5,e6,e7,e8)
...
!</subroutine
    ...
    rgridadapt%p_ImidneighboursAtElement(:,rgridadapt%NEL) = (/e5,e6,e7,e8/)
...
!<subroutine>
  ...
  SUBROUTINE remove_element2D(rgridadapt,iel,ielReplace)
...
!</subroutine>
    ...
    IF (iel /= ielReplace) THEN
      ...
      rgridadapt%p_IverticesAtElement(:,iel)      = rgridadapt%p_IverticesAtElement(:,ielReplace)
      rgridadapt%p_IneighboursAtElement(:,iel)    = rgridadapt%p_IneighboursAtElement(:,ielReplace)
      rgridadapt%p_ImidneighboursAtElement(:,iel) = rgridadapt%p_ImidneighboursAtElement(:,ielReplace)

      ! Update the adjacency list of surrounding elements
      nve = MERGE(3,4,rgridadapt%p_IverticesAtElement(4,iel).EQ.0)
      DO ive=1,nve
        
        ! Get element number of adjacent element jel
        jel = rgridadapt%p_IneighboursAtElement(ive,iel)
        ...
        mve = MERGE(3,4,rgridadapt%p_IverticesAtElement(4,jel).EQ.0)
        DO jve=1,mve
          IF (rgridadapt%p_IneighboursAtElement(jve,jel).EQ.ielReplace) THEN
            rgridadapt%p_IneighboursAtElement(jve,jel)    = iel
            rgridadapt%p_ImidneighboursAtElement(jve,jel) = iel
            EXIT
          END IF
        END DO
      END DO
      ...
      rgridadapt%p_IverticesAtElement(:,ielReplace)      = 0
      rgridadapt%p_IneighboursAtElement(:,ielReplace)    = 0
      rgridadapt%p_ImidneighboursAtElement(:,ielReplace) = 0
      
    ELSE

      ! Delete element
      rgridadapt%p_IverticesAtElement(:,iel)      = 0
      rgridadapt%p_IneighboursAtElement(:,iel)    = 0
      rgridadapt%p_ImidneighboursAtElement(:,iel) = 0
...
!<subroutine>
  ...
  SUBROUTINE update_ElementNeighbors2D(rgridadapt,e0adj,e0midadj,e0,eadj,emidadj)
...
!</subroutine>
    ...
    IF (e0adj == e0midadj) THEN
      ...
      nve = MERGE(3,4,rgridadapt%p_IverticesAtElement(4,e0adj)==0)
      ...
      IF (e0 < 0) THEN
        ...
        DO ive=1,nve
          IF (rgridadapt%p_IverticesAtElement(ive,e0adj)   == e0) THEN
            rgridadapt%p_IneighboursAtElement(ive,e0adj)    = eadj
            rgridadapt%p_ImidneighboursAtElement(ive,e0adj) = emidadj
            EXIT
          END IF
        END DO
      ...
      ELSE
        ...
        ! entries in the lists of (mid-)adjacent element neighbors.
        DO ive=1,nve
          IF (rgridadapt%p_IneighboursAtElement(ive,e0adj) == e0) THEN
            rgridadapt%p_IneighboursAtElement(ive,e0adj)    = eadj
            rgridadapt%p_ImidneighboursAtElement(ive,e0adj) = emidadj
            EXIT
          END IF
    ...
    ELSE
      ...
      IF (e0 < 0) THEN
        ...
        DO ive=1,nve
          IF (rgridadapt%p_IverticesAtElement(ive,e0adj)   == e0) THEN
            rgridadapt%p_IneighboursAtElement(ive,e0adj)    = emidadj
            rgridadapt%p_ImidneighboursAtElement(ive,e0adj) = emidadj
            EXIT
          END IF
        END DO
      ...
      ! What kind of element is neighboring element 
      nve = MERGE(3,4,rgridadapt%p_IverticesAtElement(4,e0midadj)==0)
      ...
      IF (e0 < 0) THEN
        ...
        ! entries in the lists of (mid-)adjacent element neighbors.
        DO ive=1,nve
          IF (rgridadapt%p_IverticesAtElement(ive,e0midadj) == e0) THEN
            rgridadapt%p_IneighboursAtElement(ive,e0midadj)    = eadj
            rgridadapt%p_ImidneighboursAtElement(ive,e0midadj) = eadj
            EXIT
          END IF
        END DO
      ...
      ELSE
        ...
        ! entries in the lists of (mid-)adjacent element neighbors.
        DO ive=1,nve
          IF (rgridadapt%p_IneighboursAtElement(ive,e0midadj) == e0) THEN
            rgridadapt%p_IneighboursAtElement(ive,e0midadj)    = eadj
            rgridadapt%p_ImidneighboursAtElement(ive,e0midadj) = eadj
            EXIT
          END IF
  ...
  SUBROUTINE refine_Tria2Tria(rgridadapt,iel,imarker,fcb_insertVertex)
...
!</subroutine>
    ...
    i1 = rgridadapt%p_IverticesAtElement(loc1,iel)
    i2 = rgridadapt%p_IverticesAtElement(loc2,iel)
    i3 = rgridadapt%p_IverticesAtElement(loc3,iel)

    e1 = rgridadapt%p_IneighboursAtElement(loc1,iel)
    e2 = rgridadapt%p_IneighboursAtElement(loc2,iel)
    e3 = rgridadapt%p_IneighboursAtElement(loc3,iel)
    e4 = rgridadapt%p_ImidneighboursAtElement(loc1,iel)
    e5 = rgridadapt%p_ImidneighboursAtElement(loc2,iel)
    e6 = rgridadapt%p_ImidneighboursAtElement(loc3,iel)
    
    ! Store values before refinement
    nel0 = rgridadapt%NEL
    
    ! Add one new vertex I4 
    CALL add_vertex2D(rgridadapt,i1,i2,e1,i4,fcb_insertVertex)
    
    ! Replace element IEL and add one new element E4
    CALL replace_element2D(rgridadapt,iel,i1,i4,i3,e1,nel0+1,e3,e1,nel0+1,e6)
    CALL add_element2D(rgridadapt,i2,i3,i4,e2,iel,e4,e5,iel,e4)
    
    ! Update list of neighboring elements
    CALL update_ElementNeighbors2D(rgridadapt,e1,e4,iel,nel0+1,iel)
    CALL update_ElementNeighbors2D(rgridadapt,e2,e5,iel,nel0+1,nel0+1)
    ...
    IF (e1 == e4) THEN
      ...
      CALL add_edge(rgridadapt,i1,i4)
      CALL add_edge(rgridadapt,i2,i4)
    END IF
    CALL add_edge(rgridadapt,i3,i4)
...
!<subroutine>
  ...
  SUBROUTINE refine_Tria3Tria(rgridadapt,iel,imarker,fcb_insertVertex)
...
!</subroutine>
    ...
    CALL add_vertex2D(rgridadapt,i1,i2,e1,i4,fcb_insertVertex)
    CALL add_vertex2D(rgridadapt,i2,i3,e2,i5,fcb_insertVertex)
    
    ! Compute the length of edges (I1,I2) and (I2,I3)
    x = qtree_getX(rgridadapt%rVertexCoordinates2D,i1)-&
        qtree_getX(rgridadapt%rVertexCoordinates2D,i2)
    y = qtree_getY(rgridadapt%rVertexCoordinates2D,i1)-&
        qtree_getY(rgridadapt%rVertexCoordinates2D,i2)
    dlen12=SQRT(x*x+y*y)

    x = qtree_getX(rgridadapt%rVertexCoordinates2D,i2)-&
        qtree_getX(rgridadapt%rVertexCoordinates2D,i3)
    y = qtree_getY(rgridadapt%rVertexCoordinates2D,i2)-&
        qtree_getY(rgridadapt%rVertexCoordinates2D,i3)
    dlen23=SQRT(x*x+y*y)
    ...
    IF (dlen12 > dlen23) THEN
      ...
      CALL replace_element2D(rgridadapt,iel,i1,i4,i3,e1,nel0+2,e3,e1,nel0+2,e6)
      CALL add_element2D(rgridadapt,i2,i5,i4,e2,nel0+2,e4,e2,nel0+2,e4)
      CALL add_element2D(rgridadapt,i3,i4,i5,iel,nel0+1,e5,iel,nel0+1,e5)
      
      ! Update list of nieghboring elements
      CALL update_ElementNeighbors2D(rgridadapt,e1,e4,iel,nel0+1,iel)
      CALL update_ElementNeighbors2D(rgridadapt,e2,e5,iel,nel0+2,nel0+1)
            
      ! Delete broken edges (I1,I2), (I2,I3)
      IF (e1 == e4) CALL remove_edge(rgridadapt,i1,i2)
      IF (e2 == e5) CALL remove_edge(rgridadapt,i2,i3)
      ...
      END IF
      IF (e2 == e5) THEN
        CALL add_edge(rgridadapt,i2,i5)
        CALL add_edge(rgridadapt,i3,i5)
      END IF
      CALL add_edge(rgridadapt,i3,i4)
      CALL add_edge(rgridadapt,i4,i5)
    ...
    ELSE
      ...
      ! Replace element IEL and add two new elements E4 and E5
      CALL replace_element2D(rgridadapt,iel,i1,i5,i3,nel0+2,e5,e3,nel0+2,e5,e6)
      CALL add_element2D(rgridadapt,i2,i5,i4,e2,nel0+2,e4,e2,nel0+2,e4)
      CALL add_element2D(rgridadapt,i1,i4,i5,e1,nel0+1,iel,e1,nel0+1,iel)
      
      ! Update list of neighboring elements
      CALL update_ElementNeighbors2D(rgridadapt,e1,e4,iel,nel0+1,nel0+2)
      ...
      END IF
      CALL add_edge(rgridadapt,i4,i5)
      CALL add_edge(rgridadapt,i1,i5)
    END IF
...
!<subroutine>
  ...
  SUBROUTINE refine_Tria4Tria(rgridadapt,iel,fcb_insertVertex)
...
!</subroutine>
    ...
    i2 = rgridadapt%p_IverticesAtElement(2,iel)
    i3 = rgridadapt%p_IverticesAtElement(3,iel)

    e1 = rgridadapt%p_IneighboursAtElement(1,iel)
    e2 = rgridadapt%p_IneighboursAtElement(2,iel)
    e3 = rgridadapt%p_IneighboursAtElement(3,iel)
    e4 = rgridadapt%p_ImidneighboursAtElement(1,iel)
    e5 = rgridadapt%p_ImidneighboursAtElement(2,iel)
    e6 = rgridadapt%p_ImidneighboursAtElement(3,iel)
    ...
    CALL add_vertex2D(rgridadapt,i1,i2,e1,i4,fcb_insertVertex)
    CALL add_vertex2D(rgridadapt,i2,i3,e2,i5,fcb_insertVertex)
    CALL add_vertex2D(rgridadapt,i3,i1,e3,i6,fcb_insertVertex)
    
    ! Replace element IEL and add three new elements E4, E5, and E6
    CALL replace_element2D(rgridadapt,iel,i1,i4,i6,e1,nel0+3,e6,e1,nel0+3,e6)
    CALL add_element2D(rgridadapt,i2,i5,i4,e2,nel0+3,e4,e2,nel0+3,e4)
    CALL add_element2D(rgridadapt,i3,i6,i5,e3,nel0+3,e5,e3,nel0+3,e5)
    CALL add_element2D(rgridadapt,i4,i5,i6,nel0+1,nel0+2,iel,nel0+1,nel0+2,iel)
    
    ! Update list of neighboring elements
    CALL update_ElementNeighbors2D(rgridadapt,e1,e4,iel,nel0+1,iel)
    CALL update_ElementNeighbors2D(rgridadapt,e2,e5,iel,nel0+2,nel0+1)
    CALL update_ElementNeighbors2D(rgridadapt,e3,e6,iel,iel,nel0+2)
        
    ! Delete broken edges (I1,I2),(I1,I3),(I2,I3) (if required)
    IF (e1 == e4) CALL remove_edge(rgridadapt,i1,i2)
    IF (e2 == e5) CALL remove_edge(rgridadapt,i2,i3)
    IF (e3 == e6) CALL remove_edge(rgridadapt,i1,i3)
    ...
    IF (e3 == e6) THEN
      ...
      CALL add_edge(rgridadapt,i1,i6)
    END IF
    CALL add_edge(rgridadapt,i4,i5)
    CALL add_edge(rgridadapt,i4,i6)
...
!<subroutine>
  ...
  SUBROUTINE refine_Quad2Quad(rgridadapt,iel,imarker,fcb_insertVertex)
...
!</subroutine>    
    ...
    i2 = rgridadapt%p_IverticesAtElement(loc2,iel)
    i3 = rgridadapt%p_IverticesAtElement(loc3,iel)
    i4 = rgridadapt%p_IverticesAtElement(loc4,iel)

    e1 = rgridadapt%p_IneighboursAtElement(loc1,iel)
    e2 = rgridadapt%p_IneighboursAtElement(loc2,iel)
    e3 = rgridadapt%p_IneighboursAtElement(loc3,iel)
    e4 = rgridadapt%p_IneighboursAtElement(loc4,iel)
    e5 = rgridadapt%p_ImidneighboursAtElement(loc1,iel)
    e6 = rgridadapt%p_ImidneighboursAtElement(loc2,iel)
    e7 = rgridadapt%p_ImidneighboursAtElement(loc3,iel)
    e8 = rgridadapt%p_ImidneighboursAtElement(loc4,iel)
    
    ! Store values before refinement
    nel0 = rgridadapt%NEL
    
    ! Add two new vertices I5 and I6
    CALL add_vertex2D(rgridadapt,i1,i2,e1,i5,fcb_insertVertex)
    CALL add_vertex2D(rgridadapt,i3,i4,e3,i6,fcb_insertVertex)
    
    ! Replace element IEL and add one new element E5
    CALL replace_element2D(rgridadapt,iel,i1,i5,i6,i4,e1,nel0+1,e7,e4,e1,nel0+1,e7,e8)
    CALL add_element2D(rgridadapt,i3,i6,i5,i2,e3,iel,e5,e2,e3,iel,e5,e6)
    
    ! Update list of neighboring elements
    CALL update_ElementNeighbors2D(rgridadapt,e1,e5,iel,nel0+1,iel)
    CALL update_ElementNeighbors2D(rgridadapt,e2,e6,iel,nel0+1,nel0+1)
    CALL update_ElementNeighbors2D(rgridadapt,e3,e7,iel,iel,nel0+1)
    
    ! Delete broken edges (I1,I3) and (I2,I4)
    CALL remove_edge(rgridadapt,i1,i3)
    CALL remove_edge(rgridadapt,i2,i4)

    ! Delete broken edges (I1,I2) and (I3,I4)
    IF (e1 == e5) CALL remove_edge(rgridadapt,i1,i2)
    IF (e3 == e7) CALL remove_edge(rgridadapt,i3,i4)
...
!<subroutine>
  ...
  SUBROUTINE refine_Quad3Tria(rgridadapt,iel,imarker,fcb_insertVertex)
...
!</subroutine>
    ...
    CALL replace_element2D(rgridadapt,iel,i1,i5,i4,e1,nel0+2,e4,e1,nel0+2,e8)
    CALL add_element2D(rgridadapt,i2,i3,i5,e2,nel0+2,e5,e6,nel0+2,e5)
    CALL add_element2D(rgridadapt,i5,i3,i4,nel0+1,e3,iel,nel0+1,e7,iel)    
...
!<subroutine>
  ...
  SUBROUTINE refine_Quad4Tria(rgridadapt,iel,imarker,fcb_insertVertex)
...
!</subroutine>
    ...
    CALL replace_element2D(rgridadapt,iel,i1,i5,i4,e1,nel0+3,e4,e1,nel0+3,e8)
    CALL add_element2D(rgridadapt,i2,i6,i5,e2,nel0+3,e5,e2,nel0+3,e5)
    CALL add_element2D(rgridadapt,i3,i4,i6,e3,nel0+3,e6,e7,nel0+3,e6)
    CALL add_element2D(rgridadapt,i4,i5,i6,iel,nel0+1,nel0+2,iel,nel0+1,nel0+2)
    
    ! Update list of neighboring elements
    CALL update_ElementNeighbors2D(rgridadapt,e1,e5,iel,nel0+1,iel)
    CALL update_ElementNeighbors2D(rgridadapt,e2,e6,iel,nel0+2,nel0+1)
    CALL update_ElementNeighbors2D(rgridadapt,e3,e7,iel,nel0+2,nel0+2)
    ...
    IF (e1 == e5) CALL remove_edge(rgridadapt,i1,i2)
    IF (e2 == e6) CALL remove_edge(rgridadapt,i2,i3)
...
!<subroutine>
  ...
  SUBROUTINE refine_Quad4Quad(rgridadapt,iel,fcb_insertVertex)
...
!</subroutine>
    ...
    e6 = rgridadapt%p_ImidneighboursAtElement(2,iel)
    ...
    CALL add_vertex2D(rgridadapt,i1,i2,e1,i5,fcb_insertVertex)
    CALL add_vertex2D(rgridadapt,i2,i3,e2,i6,fcb_insertVertex)
    CALL add_vertex2D(rgridadapt,i3,i4,e3,i7,fcb_insertVertex)
    CALL add_vertex2D(rgridadapt,i4,i1,e4,i8,fcb_insertVertex)
    CALL add_vertex2D(rgridadapt,i1,i2,i3,i4,i9,fcb_insertVertex)
    
    ! Replace element IEL and add three new elements E5, E6, and E7
    CALL replace_element2D(rgridadapt,iel,i1,i5,i9,i8,e1,nel0+1,nel0+3,e8,e1,nel0+1,nel0+3,e8)
    CALL add_element2D(rgridadapt,i2,i6,i9,i5,e2,nel0+2,iel,e5,e2,nel0+2,iel,e5)
    CALL add_element2D(rgridadapt,i3,i7,i9,i6,e3,nel0+3,nel0+1,e6,e3,nel0+3,nel0+1,e6)
    CALL add_element2D(rgridadapt,i4,i8,i9,i7,e4,iel,nel0+2,e7,e4,iel,nel0+2,e7)
    
    ! Update list of neighboring elements
    CALL update_ElementNeighbors2D(rgridadapt,e1,e5,iel,nel0+1,iel)
    CALL update_ElementNeighbors2D(rgridadapt,e2,e6,iel,nel0+2,nel0+1)
    CALL update_ElementNeighbors2D(rgridadapt,e3,e7,iel,nel0+3,nel0+2)
    CALL update_ElementNeighbors2D(rgridadapt,e4,e8,iel,iel,nel0+3)
    ...
    IF (e2 == e6) CALL remove_edge(rgridadapt,i2,i3)
    IF (e3 == e7) CALL remove_edge(rgridadapt,i3,i4)
    IF (e4 == e8) CALL remove_edge(rgridadapt,i4,i1)
    ...
    END IF
    IF (e3 == e7) THEN
      CALL add_edge(rgridadapt,i3,i7)
      CALL add_edge(rgridadapt,i4,i7)
    END IF
    IF (e4 == e8) THEN
...
!<subroutine>
  ...
  SUBROUTINE convert_Tria2Tria(rgridadapt,iel1,istate1,iel2,istate2,fcb_insertVertex)
...
!</subroutine>
    ...
    CASE(STATE_TRIA_GREENOUTER_LEFT)
      i1 = rgridadapt%p_IverticesAtElement(2,iel)
      i2 = rgridadapt%p_IverticesAtElement(3,iel)
      i3 = rgridadapt%p_IverticesAtElement(1,iel)

      e2 = rgridadapt%p_IneighboursAtElement(3,iel)
      e3 = rgridadapt%p_IneighboursAtElement(1,iel)

      e5 = rgridadapt%p_ImidneighboursAtElement(3,iel)
      e6 = rgridadapt%p_ImidneighboursAtElement(1,iel)

    CASE(STATE_TRIA_GREENOUTER_RIGHT)
      i1 = rgridadapt%p_IverticesAtElement(3,iel)
      i2 = rgridadapt%p_IverticesAtElement(1,iel)
      i3 = rgridadapt%p_IverticesAtElement(2,iel)

      e2 = rgridadapt%p_IneighboursAtElement(1,iel)
      e3 = rgridadapt%p_IneighboursAtElement(2,iel)
    ...
    SELECT CASE(jstate)
    CASE(STATE_TRIA_GREENINNER)
      j2 = rgridadapt%p_IverticesAtElement(2,jel)

      f1 = rgridadapt%p_IneighboursAtElement(1,jel)
      f2 = rgridadapt%p_IneighboursAtElement(2,jel)

      f4 = rgridadapt%p_ImidneighboursAtElement(1,jel)
      f5 = rgridadapt%p_ImidneighboursAtElement(2,jel)
      
    CASE(STATE_TRIA_GREENOUTER_LEFT)
      j2 = rgridadapt%p_IverticesAtElement(3,jel)

      f1 = rgridadapt%p_IneighboursAtElement(2,jel)
      f2 = rgridadapt%p_IneighboursAtElement(3,jel)

      f4 = rgridadapt%p_ImidneighboursAtElement(2,jel)
      f5 = rgridadapt%p_ImidneighboursAtElement(3,jel)
      
    CASE(STATE_TRIA_GREENOUTER_RIGHT)
      j2 = rgridadapt%p_IverticesAtElement(1,jel)

      f1 = rgridadapt%p_IneighboursAtElement(3,jel)
      f2 = rgridadapt%p_IneighboursAtElement(1,jel)

      f4 = rgridadapt%p_ImidneighboursAtElement(3,jel)
    ...
    CALL add_vertex2D(rgridadapt,j2,i2,f2,k1,fcb_insertVertex)
    CALL add_vertex2D(rgridadapt,i2,i3,e2,k2,fcb_insertVertex)

    ! Replace elements IEL and JEL and add two new elements G1 and G2
    CALL replace_element2D(rgridadapt,iel,i3,i1,k2,e3,nel0+2,e5,e6,nel0+2,e5)
    CALL replace_element2D(rgridadapt,jel,j2,k1,i1,f2,nel0+2,f1,f2,nel0+2,f4)
    CALL add_element2D(rgridadapt,i2,k2,k1,e2,nel0+2,f5,e2,nel0+2,f5)
    CALL add_element2D(rgridadapt,i1,k1,k2,jel,nel0+1,iel,jel,nel0+1,iel)
    ...
    CALL remove_edge(rgridadapt,i1,i2)
    IF (f2 == f5) CALL remove_edge(rgridadapt,j2,i2)
    IF (e2 == e5) CALL remove_edge(rgridadapt,i2,i3)

    ! Add new edges (J2,K1),(I2,K1),(I2,K2),(I3,K2),
    ! (K1,K2),(K1,I1), and (K2,I1)
    IF(f2 == f5) THEN
      CALL add_edge(rgridadapt,j2,k1)
      CALL add_edge(rgridadapt,i2,k1)
    END IF
    IF (e2 == e5) THEN
      CALL add_edge(rgridadapt,i2,k2)
      CALL add_edge(rgridadapt,i3,k2)
    END IF
...
!</subroutine>
    ...
    i1 = rgridadapt%p_IverticesAtElement(1,iel)
    i5 = rgridadapt%p_IverticesAtElement(2,iel)
...
!<subroutine>
  ...
  SUBROUTINE convert_Quad3Tria(rgridadapt,iel1,iel2,iel3,fcb_insertVertex)
...
!</subroutine>
    ...
    i1 = rgridadapt%p_IverticesAtElement(1,iel1)
    i5 = rgridadapt%p_IverticesAtElement(2,iel1)
    i4 = rgridadapt%p_IverticesAtElement(3,iel1)

    e1 = rgridadapt%p_IneighboursAtElement(1,iel1)
    e4 = rgridadapt%p_IneighboursAtElement(3,iel1)

    e9 = rgridadapt%p_ImidneighboursAtElement(1,iel1)
    e8 = rgridadapt%p_ImidneighboursAtElement(3,iel1)
    
    ! Get local data from element iel2
    i2 = rgridadapt%p_IverticesAtElement(1,iel2)
    i3 = rgridadapt%p_IverticesAtElement(2,iel2)
    
    e2 = rgridadapt%p_IneighboursAtElement(1,iel2)
    e5 = rgridadapt%p_IneighboursAtElement(3,iel2)

    e6 = rgridadapt%p_ImidneighboursAtElement(1,iel2)
    e10= rgridadapt%p_ImidneighboursAtElement(3,iel2)

    ! Get local data from element iel3
    e3 = rgridadapt%p_IneighboursAtElement(2,iel3)
    e7 = rgridadapt%p_ImidneighboursAtElement(2,iel3)
    ...
    CALL replace_element2D(rgridadapt,iel1,i1,i5,i9,i8,e1,iel2,nel0+1,e8,e9,iel2,nel0+1,e8)
    CALL replace_element2D(rgridadapt,iel2,i2,i6,i9,i5,e2,iel3,iel1,e5,e2,iel3,iel1,e10)
    CALL replace_element2D(rgridadapt,iel3,i3,i7,i9,i6,e3,nel0+1,iel2,e6,e3,nel0+1,iel2,e6)
    CALL add_element2D(rgridadapt,i4,i8,i9,i7,e4,iel1,iel3,e7,e4,iel1,iel3,e7)

    ! Update element neighbors
    CALL update_ElementNeighbors2D(rgridadapt,e2,e6,iel2,iel3,iel2)
    CALL update_ElementNeighbors2D(rgridadapt,e3,e7,iel3,nel0+1,iel3)
    CALL update_ElementNeighbors2D(rgridadapt,e4,e8,iel1,iel1,nel0+1)
    
    ! Delete broken edges (I2,I3),(I3,I4),(I4,I1),(I5,I3) and (I5,I4)
    IF (e2 == e6) CALL remove_edge(rgridadapt,i2,i3)
    IF (e3 == e7) CALL remove_edge(rgridadapt,i3,i4)
    IF (e4 == e8) CALL remove_edge(rgridadapt,i4,i1)
    CALL remove_edge(rgridadapt,i5,i3)
    CALL remove_edge(rgridadapt,i5,i4)
    ...
    rgridadapt%InelOfType(TRIA_NVETRI2D)  = rgridadapt%InelOfType(TRIA_NVETRI2D)-3
    rgridadapt%InelOfType(TRIA_NVEQUAD2D) = rgridadapt%InelOfType(TRIA_NVEQUAD2D)+3
...
!<subroutine>
  ...
  SUBROUTINE convert_Quad4Tria(rgridadapt,iel1,iel2,iel3,iel4,fcb_insertVertex)
...
!</subroutine>
    ...
    ! Get local data from element iel2
    i2 = rgridadapt%p_IverticesAtElement(1,iel2)
    i6 = rgridadapt%p_IverticesAtElement(2,iel2)

    e2 = rgridadapt%p_IneighboursAtElement(1,iel2)
    e5 = rgridadapt%p_IneighboursAtElement(3,iel2)

    e11= rgridadapt%p_ImidneighboursAtElement(1,iel2)
    e10= rgridadapt%p_ImidneighboursAtElement(3,iel2)

    ! Get local data from element iel3
    i3 = rgridadapt%p_IverticesAtElement(1,iel3)

    e3 = rgridadapt%p_IneighboursAtElement(1,iel3)
    e6 = rgridadapt%p_IneighboursAtElement(3,iel3)

    e7 = rgridadapt%p_ImidneighboursAtElement(1,iel3)
    e12= rgridadapt%p_ImidneighboursAtElement(3,iel3)
    ...
    CALL replace_element2D(rgridadapt,iel1,i1,i5,i9,i8,e1,iel2,iel4,e8,e9,iel2,iel4,e8)
    CALL replace_element2D(rgridadapt,iel2,i2,i6,i9,i5,e2,iel3,iel1,e5,e11,iel3,iel1,e10)
    CALL replace_element2D(rgridadapt,iel3,i3,i7,i9,i6,e3,iel4,iel2,e6,e3,iel4,iel2,e12)
    CALL replace_element2D(rgridadapt,iel4,i4,i8,i9,i7,e4,iel1,iel3,e7,e4,iel1,iel3,e7)

    ! Update element neighbors
    CALL update_ElementNeighbors2D(rgridadapt,e3,e7,iel3,iel4,iel3)
    CALL update_ElementNeighbors2D(rgridadapt,e4,e8,iel1,iel1,iel4)
    ...
    IF (e4 == e8) CALL remove_edge(rgridadapt,i4,i1)
    CALL remove_edge(rgridadapt,i4,i5)
    CALL remove_edge(rgridadapt,i5,i6)
    CALL remove_edge(rgridadapt,i4,i6)
    ...
    ! Finally, adjust numbers of triangles/quadrilaterals
    rgridadapt%InelOfType(TRIA_NVETRI2D)  = rgridadapt%InelOfType(TRIA_NVETRI2D)-4
    rgridadapt%InelOfType(TRIA_NVEQUAD2D) = rgridadapt%InelOfType(TRIA_NVEQUAD2D)+4
...
!<subroutine>
  ...
  SUBROUTINE mark_refinement2D(rgridadapt,rindicator)
...
!</subroutine>
    ...
    IF (rgridadapt%h_Imarker /= ST_NOHANDLE) &
        CALL storage_free(rgridadapt%h_Imarker)
    CALL storage_new('mark_refinement2D','Imarker',rgridadapt%NEL0,&
        ST_INT,rgridadapt%h_Imarker,ST_NEWBLOCK_ZERO)
    ...
    DO ivt=1,SIZE(rgridadapt%p_IvertexAge)
      rgridadapt%p_IvertexAge(ivt) = ABS(rgridadapt%p_IvertexAge(ivt))
    END DO
    ...
    mark: DO iel=1,SIZE(p_Dindicator)
      ...
      IF (p_Dindicator(iel) > rgridadapt%drefinementTolerance) THEN
        ...
        IF (Kvert(4) == 0) THEN
          ...
          DO ive=1,3
            IF (ABS(rgridadapt%p_IvertexAge(Kvert(ive))).EQ.&
                rgridadapt%NSUBDIVIDEMAX) THEN
          ...
          DO ive=1,3
            rgridadapt%p_IvertexAge(Kvert(ive)) = -ABS(rgridadapt%p_IvertexAge(Kvert(ive)))
          END DO
    ...
    rgridadapt%iSpec=IOR(rgridadapt%iSpec,ADAPT_MARKEDREFINE)
...
!<subroutine>
  ...
  SUBROUTINE mark_coarsening2D(rgridadapt,rindicator)
...
!</subroutine>
    ...
    ! Check if dynamic data structures are generated and contain data
    IF (IAND(rgridadapt%iSpec,ADAPT_HAS_DYNAMICDATA).EQ.0 .OR.&
        IAND(rgridadapt%iSpec,ADAPT_MARKEDREFINE).EQ.0) THEN
    ...
    ! Set pointers
    IF (rgridadapt%h_Imarker == ST_NOHANDLE) THEN
    ...
    mark: DO iel=1,SIZE(p_Dindicator)
      ...
      CASE(STATE_TRIA_GREENOUTER_RIGHT)
        ...
        i = Kvert(1)
        rgridadapt%p_IvertexAge(i) = -ABS(rgridadapt%p_IvertexAge(i))
        
        ! In addition, check if the second vertex is older and "lock" it if required
        j = Kvert(2)
        IF (ABS(rgridadapt%p_IvertexAge(i)) > ABS(rgridadapt%p_IvertexAge(j))) &
            rgridadapt%p_IvertexAge(j) = -ABS(rgridadapt%p_IvertexAge(j))
      ...
      CASE(STATE_QUAD_RED4)
        ...
        IF (p_Dindicator(iel) >= rgridadapt%dcoarseningTolerance) THEN
          ! "Lock" all four nodes
          DO ive=1,4
            i = Kvert(ive)
            rgridadapt%p_IvertexAge(i) = -ABS(rgridadapt%p_IvertexAge(i))
      ...
      CASE(STATE_TRIA_REDINNER)
        ...
        DO ive=1,3
          ...
          IF (jel > SIZE(p_Dindicator)) THEN
            ...
            DO jve=1,4
              i = rgridadapt%p_IverticesAtElement(ive,jel)
              rgridadapt%p_IvertexAge(i) = -ABS(rgridadapt%p_IvertexAge(i))
            END DO

          ELSEIF(p_Dindicator(jel) >= rgridadapt%dcoarseningTolerance) THEN
          ...
          ELSE
            ...
            i = rgridadapt%p_IverticesAtElement(1,jel)
            rgridadapt%p_IvertexAge(i) = -ABS(rgridadapt%p_IvertexAge(i))
    ...
    rgridadapt%iSpec=IOR(rgridadapt%iSpec,ADAPT_MARKEDCOARSEN)
...
!<subroutine>
  ...
  SUBROUTINE redgreen_mark_refinement2D(rgridadapt,fcb_adjustDimension,fcb_insertVertex)
...
!</subroutine>
    ...
    IF (rgridadapt%nGreenElements > 0) THEN

      ! Predict new dimensions for the worst case
      nvt = CEILING(rgridadapt%NVT0+1.5*rgridadapt%nGreenElements)
      nel = rgridadapt%NEL0+2*rgridadapt%nGreenElements

      ! Adjust nodal/elemental arrays
      CALL storage_realloc('redgreen_mark_refinement2D',nvt,&
          rgridadapt%h_IvertexAge,ST_NEWBLOCK_ZERO,.TRUE.)
      CALL storage_realloc('redgreen_mark_refinement2D',nvt,&
          rgridadapt%h_InodalProperty,ST_NEWBLOCK_ZERO,.TRUE.)
      CALL storage_realloc('redgreen_mark_refinement2D',nel,&
          rgridadapt%h_Imarker,ST_NEWBLOCK_ZERO,.TRUE.)
      CALL storage_realloc('redgreen_mark_refinement2D',nel,&
          rgridadapt%h_IverticesAtElement,ST_NEWBLOCK_NOINIT,.TRUE.)
      CALL storage_realloc('redgreen_mark_refinement2D',nel,&
          rgridadapt%h_IneighboursAtElement,ST_NEWBLOCK_NOINIT,.TRUE.)
    ...
    ELSE
      ...
      CALL storage_new('redgreen_mark_refinement2D','p_Imodified',rgridadapt%NEL0,&
          ST_INT,h_Imodified,ST_NEWBLOCK_ZERO)
      CALL storage_getbase_int(h_Imodified,p_Imodified)
    ...
    ! refinement can be uniquely determined (see above; data type t_gridadapt).
    ...
    conformity: DO
      ...
      DO iel=1,rgridadapt%NEL0
        ...
        CASE(STATE_TRIA_OUTERINNER)
          ...
          jstate = redgreen_getState(rgridadapt,jel)
          ...
          IF (jstate .EQ. STATE_TRIA_OUTERINNER) THEN
            ...
            IF (iel > jel) THEN
              ...
              iel1 = rgridadapt%p_IneighboursAtElement(3,kel)
              iel2 = rgridadapt%p_ImidneighboursAtElement(3,kel)
              IF (iel1 .NE. 0 .AND. iel1 .EQ. iel2) CALL mark_edge(kel,iel1)
              
              ! Mark the edge of the element adjacent to LEL for subdivision
              iel1 = rgridadapt%p_IneighboursAtElement(1,lel)
              iel2 = rgridadapt%p_ImidneighboursAtElement(1,lel)
              ...
              CALL convert_Quad4Tria(rgridadapt,kel,jel,lel,iel,fcb_insertVertex)
              isConform=.FALSE.
            ...
            ELSE
              ! To begin with, we need to find the two missing triangles KEL and LEL
              ! which make up the original quadrilateral
              kel  = rgridadapt%p_IneighboursAtElement(1,jel)
              ...
              ! Now, we can physically convert the four triangles into four quadrilaterals
              CALL convert_Quad4Tria(rgridadapt,kel,iel,lel,jel,fcb_insertVertex)
              isConform=.FALSE.
        ...
        CASE(STATE_QUAD_HALF1)
          ...
          iel1 = rgridadapt%p_IneighboursAtElement(4,iel)
          iel2 = rgridadapt%p_ImidneighboursAtElement(4,iel)
          IF (iel1 .NE. 0 .AND. iel1 .EQ. iel2) CALL mark_edge(iel,iel1)

          ! Mark the fourth edge of elements JEL for subdivision
          iel1 = rgridadapt%p_IneighboursAtElement(4,jel)
          iel2 = rgridadapt%p_ImidneighboursAtElement(4,jel)
          ...
          CALL convert_Quad2Quad(rgridadapt,iel,jel,fcb_insertVertex)
          isConform=.FALSE.
...
!          Kmidadj = rgridadapt%p_ImidneighboursAtElement(:,iel)
        ...
        CASE(STATE_TRIA_GREENINNER)
          ...
          iel1 = rgridadapt%p_IneighboursAtElement(3,jel)
          iel2 = rgridadapt%p_ImidneighboursAtElement(3,jel)
          IF (iel1 .NE. 0 .AND. iel1 .EQ. iel2) CALL mark_edge(jel,iel1)

          ! Mark the edge of the element adjacent to KEL for subdivision
          iel1 = rgridadapt%p_IneighboursAtElement(1,kel)
          iel2 = rgridadapt%p_ImidneighboursAtElement(1,kel)
          IF (iel1 .NE. 0 .AND. iel1 .EQ. iel2) CALL mark_edge(kel,iel1)

          ! Now, we can physically convert the three elements IEL,JEL and KEL
          ! into four similar quadrilaterals
          CALL convert_Quad3Tria(rgridadapt,jel,kel,iel,fcb_insertVertex)
          isConform=.FALSE.
        ...
        CASE(STATE_TRIA_GREENOUTER_LEFT)
          ...
          CASE(STATE_TRIA_GREENOUTER_RIGHT)
            ...
            iel1 = rgridadapt%p_IneighboursAtElement(1,jel)
            iel2 = rgridadapt%p_ImidneighboursAtElement(1,jel)
            IF (iel1 .NE. 0 .AND. iel1 .EQ. iel2) CALL mark_edge(jel,iel1)
            
            ! Now, we can physically convert the two elements IEL and JEL into four similar triangles
            CALL convert_Tria2Tria(rgridadapt,iel,istate,jel,jstate,fcb_insertVertex)
            isConform=.FALSE.
          ...
          CASE(STATE_TRIA_GREENINNER)
            ...
            iel1 = rgridadapt%p_IneighboursAtElement(2,jel)
            iel2 = rgridadapt%p_ImidneighboursAtElement(2,jel)
            IF (iel1 .NE. 0 .AND. iel1 .EQ. iel2) CALL mark_edge(jel,iel1)
            ...
            ! Now, we can physically convert the three elements IEL,JEL and KEL
            ! into four similar quadrilaterals
            CALL convert_Quad3Tria(rgridadapt,iel,kel,jel,fcb_insertVertex)
            isConform=.FALSE.
          ...
          CASE(STATE_TRIA_OUTERINNER)
            ...
            iel1 = rgridadapt%p_IneighboursAtElement(3,iel)
            iel2 = rgridadapt%p_ImidneighboursAtElement(3,iel)
            IF (iel1 .NE. 0 .AND. iel1 .EQ. iel2) CALL mark_edge(iel,iel1)
            ...
            ! Now, we can physically convert the four triangles into four quadrilaterals
            CALL convert_Quad4Tria(rgridadapt,iel,kel,lel,jel,fcb_insertVertex)
            isConform=.FALSE.
        ...
        CASE(STATE_TRIA_GREENOUTER_RIGHT)
          ...
          CASE(STATE_TRIA_GREENOUTER_LEFT)
            ...
            ! Now, we can physically convert the two elements IEL and JEL into four similar triangles
            CALL convert_Tria2Tria(rgridadapt,jel,jstate,iel,istate,fcb_insertVertex)
            isConform=.FALSE.
          ...
          CASE(STATE_TRIA_GREENINNER)
            ...
            ! be found in the dynamic data structure first
            kel = rgridadapt%p_IneighboursAtElement(3,jel)
            ...
            ! Now, we can physically convert the three elements IEL,JEL and KEL
            ! into four similar quadrilaterals
            CALL convert_Quad3Tria(rgridadapt,kel,iel,jel,fcb_insertVertex)
            isConform=.FALSE.
          ...
          CASE(STATE_TRIA_OUTERINNER)
            ...
            kel = rgridadapt%p_IneighboursAtElement(2,jel)
            lel = rgridadapt%p_IneighboursAtElement(1,jel)

            ! Mark the edge of the element adjacent to IEL for subdivision
            iel1 = rgridadapt%p_IneighboursAtElement(1,iel)
            iel2 = rgridadapt%p_ImidneighboursAtElement(1,iel)
            IF (iel1 .NE. 0 .AND. iel1 .EQ. iel2) CALL mark_edge(iel,iel1)

            ! Mark the edge of the element adjacent to LEL for subdivision
            iel1 = rgridadapt%p_IneighboursAtElement(3,lel)
            iel2 = rgridadapt%p_ImidneighboursAtElement(3,lel)
            IF (iel1 .NE. 0 .AND. iel1 .EQ. iel2) CALL mark_edge(lel,iel1)

            ! Now, we can physically convert the four triangles into four quadrilaterals
            CALL convert_Quad4Tria(rgridadapt,lel,kel,iel,jel,fcb_insertVertex)
            isConform=.FALSE.
...
100     CONTINUE
        ...
        DO ive=1,nve
          ...
          IF (btest(p_Imarker(iel),ive)) THEN
            ...
            Kvert = rgridadapt%p_IverticesAtElement(:,jel)
            Kjadj = rgridadapt%p_IneighboursAtElement(:,jel)
            ...
            rgridadapt%p_IvertexAge(Kvert(1:mve)) = -ABS(rgridadapt%p_IvertexAge(Kvert(1:mve)))
          END IF
        END DO
      END DO
  ...
  CONTAINS
    ...
    SUBROUTINE mark_edge(iel,jel)
      ...
      Kvert = rgridadapt%p_IverticesAtElement(:,jel)
      Kadj  = rgridadapt%p_IneighboursAtElement(:,jel)
...
!<subroutine>
  ...
  SUBROUTINE redgreen_refine(rgridadapt,fcb_insertVertex)
...
!</subroutine>
    ...
    DO iel=1,SIZE(p_Imarker)
      ...
      CASE(MARK_TRIA4TRIA)
        ...
        CALL refine_Tria4Tria(rgridadapt,iel,fcb_insertVertex)
      ...
      CASE(MARK_TRIA2TRIA_1,MARK_TRIA2TRIA_2,MARK_TRIA2TRIA_3)   
        ...
        rgridadapt%nGreenElements = rgridadapt%nGreenElements+2
        
      CASE(MARK_QUAD3TRIA_1,MARK_QUAD3TRIA_2,MARK_QUAD3TRIA_3,MARK_QUAD3TRIA_4)
        ! Green refinement quadrilateral
        CALL refine_Quad3Tria(rgridadapt,iel,p_Imarker(iel),fcb_insertVertex)
        rgridadapt%nGreenElements = rgridadapt%nGreenElements+3
        
      CASE(MARK_QUAD2QUAD_13,MARK_QUAD2QUAD_24)
        ! Green refinement quadrilateral
        CALL refine_Quad2Quad(rgridadapt,iel,p_Imarker(iel),fcb_insertVertex)
        rgridadapt%nGreenElements = rgridadapt%nGreenElements+2

      CASE(MARK_QUAD4TRIA_12,MARK_QUAD4TRIA_23,MARK_QUAD4TRIA_34,MARK_QUAD4TRIA_14)
        ! Green refinement quadrilateral
        CALL refine_Quad4Tria(rgridadapt,iel,p_Imarker(iel),fcb_insertVertex)
        rgridadapt%nGreenElements = rgridadapt%nGreenElements+4
    ...
    rgridadapt%nRefinementSteps = rgridadapt%nRefinementSteps+1
    
    ! Refinement has been performed.
    rgridadapt%iSpec = IOR(rgridadapt%ispec,ADAPT_REFINED)
    
    ! Hence, the markers are no longer valid
    rgridadapt%iSpec = IAND(rgridadapt%iSpec,NOT(ADAPT_MARKEDREFINE))
...
!<function>
  ...
  PURE FUNCTION redgreen_getState(rgridadapt,iel) RESULT(istate)
...
!</function>
    ...
    ! Get local data
    Kvert = rgridadapt%p_IverticesAtElement(1:4,iel)
...
!<subroutine>
  ...
  SUBROUTINE bisection_refine(rgridadapt)
