...
!<constantblock description="Identifiers for the different problem classes that can be handled by VANKA">
  ...
  INTEGER, PARAMETER :: VANKAPC_2DBOUSSINESQ = 4
...
!<typeblock>
  ...
  ! A structure that saves matrix pointers for the 2D-Boussinesq
  ! VANKA method for Boussinesq systems.
  ...
  !                     \ 0    0    0  N  /
  !
  ! The following matrices are optional: A12, A21, C, M3
  ! The following matrices are silently assumed to have the same matrix structure:
  ! 1.) A11, A12, A21, A22
  ! 2.) B1, B2, B1^T, B2^T
  ! 3.) M1, M2
  
  TYPE t_vankaPointer2DBouss
    
    ! Pointer to the column structure of the velocity matrix A
    INTEGER, DIMENSION(:), POINTER              :: p_KcolA => NULL()
    
    ! Pointer to the row structure of the velocity matrix A
    INTEGER, DIMENSION(:), POINTER              :: p_KldA => NULL()
    
    ! Pointer to diagonal entries in the velocity matrix A
...
!<typeblock>
  ...
  TYPE t_vanka
    ...
    ! only vaid if if cproblemClassVanka==VANKAPC_2DBOUSSINESQ.
    TYPE(t_vankaPointer2DBouss) :: rvanka2DBouss
...
!</subroutine>
    ...
    CASE (VANKAPC_2DBOUSSINESQ)
      ! Vanka for 2D Boussinesq problems
      CALL vanka_init2DBoussinesq (rmatrix,rvanka,csubtype)
...
!</subroutine>
    ...
    CASE (VANKAPC_2DBOUSSINESQ)
      ! 2D Boussinesq problem.
      CALL vanka_2DBoussinesq (rvanka%rvanka2DBouss, rvector, rrhs, domega,&
          rvanka%csubtype,rvanka%csubsubtype)
      ...
      ELSE IF &
        ...
        CASE (VANKATP_DIAGONAL)
          ...
          ELSE
            ...
            ! in A11, A12, A21 and A22!
            CALL output_line (&
                'VANKA has no support for A12 and A21!',&
                OU_CLASS_ERROR,OU_MODE_STD,'vanka_2DNavierStokes')
            CALL sys_halt()
          END IF
...
! *****************************************************************************
! Problem class: VANKA variants for 2D Boussinesq problems
! *****************************************************************************

!<subroutine>
  
  SUBROUTINE vanka_init2DBoussinesq (rmatrix,rvanka,csubtype)
...
!</subroutine>
    ...
    IF (rmatrix%ndiagBlocks .NE. 4) THEN
      CALL output_line ('System matrix is not 4x4.',&
          OU_CLASS_ERROR,OU_MODE_STD,'vanka_init2DBoussinesq')
      CALL sys_halt()
    END IF
...
!    END DO
    ...
    ! Fill the output structure with data of the matrices.
    CALL lsyssc_getbase_double(rmatrix%RmatrixBlock(1,1),&
        rvanka%rvanka2DBouss%p_DA11)
    CALL lsyssc_getbase_double(rmatrix%RmatrixBlock(2,2),&
        rvanka%rvanka2DBouss%p_DA22)
    CALL lsyssc_getbase_double(rmatrix%RmatrixBlock(1,3),&
        rvanka%rvanka2DBouss%p_DB1)
    CALL lsyssc_getbase_double(rmatrix%RmatrixBlock(2,3),&
        rvanka%rvanka2DBouss%p_DB2)
    CALL lsyssc_getbase_double(rmatrix%RmatrixBlock(3,1),&
        rvanka%rvanka2DBouss%p_DD1)
    CALL lsyssc_getbase_double(rmatrix%RmatrixBlock(3,2),&
        rvanka%rvanka2DBouss%p_DD2)
    CALL lsyssc_getbase_double(rmatrix%RmatrixBlock(1,4),&
        rvanka%rvanka2DBouss%p_DM1)
    CALL lsyssc_getbase_double(rmatrix%RmatrixBlock(2,4),&
        rvanka%rvanka2DBouss%p_DM2)
    CALL lsyssc_getbase_double(rmatrix%RmatrixBlock(4,4),&
        rvanka%rvanka2DBouss%p_DN)
    CALL lsyssc_getbase_Kcol(rmatrix%RmatrixBlock(1,1),&
        rvanka%rvanka2DBouss%p_KcolA)
    CALL lsyssc_getbase_Kld(rmatrix%RmatrixBlock(1,1), &
        rvanka%rvanka2DBouss%p_KldA )
    CALL lsyssc_getbase_Kcol(rmatrix%RmatrixBlock(1,3),&
        rvanka%rvanka2DBouss%p_KcolB)
    CALL lsyssc_getbase_Kld(rmatrix%RmatrixBlock(1,3), &
        rvanka%rvanka2DBouss%p_KldB)
    CALL lsyssc_getbase_Kcol(rmatrix%RmatrixBlock(1,4),&
        rvanka%rvanka2DBouss%p_KcolM)
    CALL lsyssc_getbase_Kld(rmatrix%RmatrixBlock(1,4), &
        rvanka%rvanka2DBouss%p_KldM)
    CALL lsyssc_getbase_Kcol(rmatrix%RmatrixBlock(4,4),&
        rvanka%rvanka2DBouss%p_KcolN)
    CALL lsyssc_getbase_Kld(rmatrix%RmatrixBlock(4,4), &
        rvanka%rvanka2DBouss%p_KldN)
    IF (rmatrix%RmatrixBlock(1,1)%cmatrixFormat .EQ. LSYSSC_MATRIX9) THEN
      CALL lsyssc_getbase_Kdiagonal(rmatrix%RmatrixBlock(1,1), &
                              rvanka%rvanka2DBouss%p_KdiagonalA)
    ELSE
      rvanka%rvanka2DBouss%p_KdiagonalA => rvanka%rvanka2DBouss%p_KldA
    END IF
    IF (rmatrix%RmatrixBlock(4,4)%cmatrixFormat .EQ. LSYSSC_MATRIX9) THEN
      CALL lsyssc_getbase_Kdiagonal(rmatrix%RmatrixBlock(4,4), &
                              rvanka%rvanka2DBouss%p_KdiagonalN)
    ELSE
      rvanka%rvanka2DBouss%p_KdiagonalN => rvanka%rvanka2DBouss%p_KldN
    END IF
    ...
    IF (lsysbl_isSubmatrixPresent(rmatrix,1,2)) THEN
      ...
      CALL lsyssc_getbase_double(rmatrix%RmatrixBlock(1,2),&
          rvanka%rvanka2DBouss%p_DA12 )
      
      CALL lsyssc_getbase_double(rmatrix%RmatrixBlock(2,1),&
          rvanka%rvanka2DBouss%p_DA21 )
    END IF
    ...
    IF (lsysbl_isSubmatrixPresent(rmatrix,3,3)) THEN
      ...
      CALL lsyssc_getbase_double(rmatrix%RmatrixBlock(3,3),&
          rvanka%rvanka2DBouss%p_DC)

      CALL lsyssc_getbase_Kcol(rmatrix%RmatrixBlock(3,3),&
          rvanka%rvanka2DBouss%p_KcolC)
      CALL lsyssc_getbase_Kld(rmatrix%RmatrixBlock(3,3), &
          rvanka%rvanka2DBouss%p_KldC)

      IF (rmatrix%RmatrixBlock(3,3)%cmatrixFormat .EQ. LSYSSC_MATRIX9) THEN
        CALL lsyssc_getbase_Kdiagonal(rmatrix%RmatrixBlock(3,3), &
                                rvanka%rvanka2DBouss%p_KdiagonalC)
      ELSE
        rvanka%rvanka2DBouss%p_KdiagonalC => rvanka%rvanka2DBouss%p_KldC
      END IF
    ...
    IF(lsysbl_isSubmatrixPresent(rmatrix,3,4)) THEN
      ...
      CALL lsyssc_getbase_double(rmatrix%RmatrixBlock(3,4),&
          rvanka%rvanka2DBouss%p_DM3)
          
      CALL lsyssc_getbase_Kcol(rmatrix%RmatrixBlock(3,4),&
          rvanka%rvanka2DBouss%p_KcolM3)
      CALL lsyssc_getbase_Kld(rmatrix%RmatrixBlock(3,4), &
          rvanka%rvanka2DBouss%p_KldM3)
    
    END IF

    ! Get the multiplication factors of the submatrices.
    rvanka%rvanka2DBouss%Dmultipliers(1:4,1:4) = &
        rmatrix%RmatrixBlock(1:4,1:4)%dscaleFactor
    ...
    ! discretisation structure.
    rvanka%rvanka2DBouss%p_rspatialDiscrU => p_rblockDiscr%RspatialDiscr(1)
    rvanka%rvanka2DBouss%p_rspatialDiscrV => p_rblockDiscr%RspatialDiscr(2)
    rvanka%rvanka2DBouss%p_rspatialDiscrP => p_rblockDiscr%RspatialDiscr(3)
    rvanka%rvanka2DBouss%p_rspatialDiscrT => p_rblockDiscr%RspatialDiscr(4)
    
    IF (rvanka%rvanka2DBouss%p_rspatialDiscrU%inumFESpaces .NE. &
        rvanka%rvanka2DBouss%p_rspatialDiscrV%inumFESpaces) THEN
      CALL output_line (&
          'Discretisation structures of X- and Y-velocity incompatible!',&
          OU_CLASS_ERROR,OU_MODE_STD,'vanka_init2DBoussinesq')
      CALL sys_halt()
    ...
    IF ((rvanka%rvanka2DBouss%p_rspatialDiscrP%inumFESpaces .NE. 1) .AND. &
        (rvanka%rvanka2DBouss%p_rspatialDiscrP%inumFESpaces .NE. &
          rvanka%rvanka2DBouss%p_rspatialDiscrU%inumFESpaces)) THEN
      ! Either there must be only one element type for the pressure, or there one
      ! pressure element distribution for every velocity element distribution!
      ! If this is not the case, we cannot determine (at least not in reasonable time)
      ! which element type the pressure represents on a cell!
...
!<subroutine>
  ...
  SUBROUTINE vanka_2DBoussinesq (rvanka2DBouss, rvector, rrhs, domega, &
      csubtype, csubsubtype)
...
!<inputoutput>
  ...
  ! t_vanka structure that saves algorithm-specific parameters.
  TYPE(t_vankaPointer2DBouss), INTENT(INOUT) :: rvanka2DBouss
!</inputoutput>
...
!</subroutine>
    ...
    DO ielementdist = 1,rvanka2DBouss%p_rspatialDiscrU%inumFESpaces
      ...
      IF ((elem_getPrimaryElement(p_relementDistrU%celement) .EQ. EL_Q1T) .AND. &
        ...
        CASE DEFAULT
          CALL output_line (&
              'Unknown VANKA subtype!',&
              OU_CLASS_ERROR,OU_MODE_STD,'vanka_2DBoussinesq')
          CALL sys_halt()  
  ...
  ! ***************************************************************************
  ! 2D Boussinesq VANKA, jacobi-style version
  ! Supports Q1~/Q1~/Q0/Q1 discretisation only.
  ! Matrix must be of the form
  !
  !    / A11  A12  B1 M1 \
  !    | A21  A22  B2 M2 |
  ...
  !    \ 0    0    0  N  /
  !
  ! where A12,A21,C and M3 are optional.
  !
  ! D1/D2 must have the same structure as B1/B2 and the 'transposed'
  ! flag set (LSYSSC_MSPEC_TRANSPOSED).
  ! ***************************************************************************
...
!<input>
  ! t_vankaPointer2DBouss structure that saves algorithm-specific parameters.
  TYPE(t_vankaPointer2DBouss), INTENT(IN) :: rvanka
...
!</subroutine>
  ...
  real(DP), dimension(:), pointer :: p_DA11,p_DA12,p_DA21,p_DA22,p_DB1,p_DB2,&
    ...
    real(DP) :: daux
    logical :: bHaveA12, bHaveC, bHaveM3
    ...
    DO ielidx=1, SIZE(IelementList)
      ...
      ! Get all 13 DOFs for this element
        ...
        do k = 1, ndofV
          ...
          do i = i1, i2
            ...
            Df(k)       = Df(k)       - Dmult(1,1)*p_DA11(i)*p_DvecU(j) &
                                      - Dmult(1,2)*p_DA12(i)*p_DvecV(j)
            Df(ndofV+k) = Df(ndofV+k) - Dmult(2,1)*p_DA21(i)*p_DvecU(j) &
                                      - Dmult(2,2)*p_DA22(i)*p_DvecV(j)
          end do
        end do
      else
        ! f_u := f_u - A11*u
      ...
      call vanka_aux_solve_BS2D_js_414(Du,Df,Da1,Da2,Db1,Db2,&
...
!<description>
  ...
  ! this routine, as this holds a reference to the system matrix.
  !
  ! This is the block-diagonal version:
  ! 
  !           / A1 0  B1 M1 \
  !           | 0  A2 B2 M2 |
  !           | D1 D2 C  M3 |
...
!</subroutine>
  ...
  logical :: bHaveC, bHaveM3
    ...
    DO ielidx=1, SIZE(IelementList)
      ...
      ! Solve the local system
      call vanka_aux_solve_BS2D_bd_414(Du,Df,Da1,Da2,Db1,Db2,&
                                       Dd1,Dd2,Dc,Dm1,Dm2,Dm3,Dn)
...
!<description>
  ...
  ! this routine, as this holds a reference to the system matrix.
  !
  ! This is the fully coupled version:
  ! 
  !           / A11 A12 B1 M1 \
  !           | A21 A22 B2 M2 |
  !           | D1  D2  C  M3 |
