...
!<typeblock>
  ...
  type t_spsor
    ...
    ! Matrix pointers to diagonal A sub-matrices
    type(t_matrixScalar), pointer :: p_rA11 => null()
    type(t_matrixScalar), pointer :: p_rA22 => null()
    type(t_matrixScalar), pointer :: p_rA33 => null()

    ! Matrix pointers to off-diagonal A sub-matrices
    type(t_matrixScalar), pointer :: p_rA12 => null()
    type(t_matrixScalar), pointer :: p_rA13 => null()
    type(t_matrixScalar), pointer :: p_rA21 => null()
    type(t_matrixScalar), pointer :: p_rA23 => null()
    type(t_matrixScalar), pointer :: p_rA31 => null()
    type(t_matrixScalar), pointer :: p_rA32 => null()

    ! Matrix pointers to B sub-matrices
    type(t_matrixScalar), pointer :: p_rB1 => null()
    type(t_matrixScalar), pointer :: p_rB2 => null()
    type(t_matrixScalar), pointer :: p_rB3 => null()

    ! Matrix pointers to D sub-matrices
    type(t_matrixScalar), pointer :: p_rD1 => null()
    type(t_matrixScalar), pointer :: p_rD2 => null()
    type(t_matrixScalar), pointer :: p_rD3 => null()
...
!</subroutine>
  ...
  integer(I32) :: cflag
    ...
    case(SPSOR_SYSTEM_NAVST3D)
      ! 3D Navier-Stokes system
      call spsor_initNavSt3D(rdata, rmatrix, cflag)
...
!</subroutine>
    ...
    case (SPSOR_SYSTEM_NAVST3D)
      ...
      call spsor_initData_NavSt3D(rdata)
...
!</subroutine>
  ...
  real(DP) :: domA, domS
    ...
    case (SPSOR_SYSTEM_NAVST3D)
      ...
      ! Call the routine for 3D Navier-Stokes systems
      call spsor_solve_NavSt3D(rdata, rsol, rrhs, niterations, domA, domS)
...
!</subroutine>
  ...
  real(DP) :: domA, domS
    ...
    case (SPSOR_SYSTEM_NAVST3D)
      ...
      ! Do we apply SP-SOR or SP-SSOR?
      if(iand(rdata%cflags, SPSOR_FLAG_SYM) .ne. 0_I32) then
        ! Apply SP-SSOR for 3D Navier-Stokes systems
        call spsor_prec_sym_NavSt3D(rdata, rdef, domA, domS)
      else
        ! Apply SP-SOR for 3D Navier-Stokes systems
        call spsor_prec_NavSt3D(rdata, rdef, domA, domS)
      end if
...
!</subroutine>
  ...
  integer :: ndofPres, ndofVelo, ieldist
    ...
    rdata%bdiagS = (rdata%cpressure .eq. SPSOR_PRES_P0)

    if(rdata%bdiagS) then

      ! Create a diagonal matrix
      call lsyssc_createDiagMatrixStruc(rdata%rS, ndofPres, LSYSSC_MATRIX9)
    ...
    do i = 1, 2
      do j = 1, 2
        if(i .eq. j) then
          ...
          if(i .gt. 1) then
            ! Make sure all main diagonal blocks have the same structure
...
!      if(i .gt. 1) then
!        ! Make sure all B matrices have the same structure
!        if(.not. lsyssc_isMatrixStructureShared(&
...
!      if(j .gt. 1) then
!        ! Make sure all D matrices have the same structure
!        if(.not. lsyssc_isMatrixStructureShared(&
...
!</subroutine>
  ...
  logical :: bHaveA12
    ...
    if(bHaveA12) then
      ...
      ! Solve for A11 * u1 = u1 - A12 * u2 - B1*p
  ...
  ! -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  !
  !  R O U T I N E S   F O R   3 D   N A V I E R - S T O K E S   S Y S T E M S
  !
  ! -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
...
!<subroutine>
  ...
  subroutine spsor_initNavSt3D(rdata, rmatrix, cflags)

!<description>
  ! Initialises a SP-SOR data structure for 3D Navier-Stokes systems.
!</description>
...
!</subroutine>
  ...
  integer :: ndofPres, ndofVelo, ieldist
    ...
    ! Check if the matrix is valid
    call spsor_checkMatrixNavSt3D(rmatrix)

    ! Hang in the matrix pointers
    rdata%p_rA11 => rmatrix%RmatrixBlock(1,1)
    rdata%p_rA22 => rmatrix%RmatrixBlock(2,2)
    rdata%p_rA33 => rmatrix%RmatrixBlock(3,3)
    rdata%p_rB1 => rmatrix%RmatrixBlock(1,4)
    rdata%p_rB2 => rmatrix%RmatrixBlock(2,4)
    rdata%p_rB3 => rmatrix%RmatrixBlock(3,4)
    rdata%p_rD1 => rmatrix%RmatrixBlock(4,1)
    rdata%p_rD2 => rmatrix%RmatrixBlock(4,2)
    rdata%p_rD3 => rmatrix%RmatrixBlock(4,3)

    ! Are the off-diagonal A sub-matrices present?
    if(rmatrix%RmatrixBlock(1,2)%NEQ .gt. 0) then
      rdata%p_rA12 => rmatrix%RmatrixBlock(1,2)
      rdata%p_rA13 => rmatrix%RmatrixBlock(1,3)
      rdata%p_rA21 => rmatrix%RmatrixBlock(2,1)
      rdata%p_rA23 => rmatrix%RmatrixBlock(2,3)
      rdata%p_rA31 => rmatrix%RmatrixBlock(3,1)
      rdata%p_rA32 => rmatrix%RmatrixBlock(3,2)
    end if

    ! Is the C sub-matrix present?
    if(rmatrix%RmatrixBlock(4,4)%NEQ .gt. 0) &
      rdata%p_rC => rmatrix%RmatrixBlock(4,4)
...
!<subroutine>
  ...
  subroutine spsor_checkMatrixNavSt3D(rmatrix, bvalid)

!<description>
  ! Checks whether a given block matrix is valid for the 3D Navier-Stokes
  ! SP-SOR solver.
!</description>
...
!</subroutine>
  ...
  logical :: bHaveA12, bHaveAij
    ...
    ! First of all, the matrix must be 4x4
    if((rmatrix%nblocksPerRow .ne. 4) .or. (rmatrix%nblocksPerCol .ne. 4)) then
      if(present(bvalid)) then
        return
      else
        call output_line ('Matrix must be 4x4',&
            OU_CLASS_ERROR,OU_MODE_STD,'spsor_checkMatrixNavSt3D')
        call sys_halt()
      end if
    end if
    ...
    do i = 1, 3
      ...
      ! Make sure the matrix exists
      if((.not. lsyssc_hasMatrixStructure(rmatrix%RmatrixBlock(i,4))) .or. &
         (abs(rmatrix%RmatrixBlock(i,4)%dscaleFactor) .le. SYS_EPSREAL_DP)) then
        if(present(bvalid)) then
          return
        else
          call output_line ('Sub-Matrix B(' // trim(sys_sil(i,2)) // ') does not exist',&
...
!        if(.not. lsyssc_isMatrixStructureShared(&
!            rmatrix%RmatrixBlock(1,4), rmatrix%RmatrixBlock(i,4))) then
!          if(present(bvalid)) then
!            return
!          else
!            call output_line ('All B(i) sub-matrices must share the same structure',&
...
!      end if
    ...
    do j = 1, 3
      ...
      ! Make sure the matrix exists
      if((.not. lsyssc_hasMatrixStructure(rmatrix%RmatrixBlock(4,j))) .or. &
         (abs(rmatrix%RmatrixBlock(4,j)%dscaleFactor) .le. SYS_EPSREAL_DP)) then
        if(present(bvalid)) then
          return
        else
          call output_line ('Sub-Matrix D(' // trim(sys_sil(j,2)) // ') does not exist',&
...
!        if(.not. lsyssc_isMatrixStructureShared(&
!            rmatrix%RmatrixBlock(4,1), rmatrix%RmatrixBlock(4,j))) then
!          if(present(bvalid)) then
!            return
!          else
!            call output_line ('All D(j) sub-matrices must share the same structure',&
...
!<subroutine>
  ...
  subroutine spsor_initData_NavSt3D(rdata)

!<description>
  ! This subroutine performs data-dependent initialisation of the SP-SOR
  ! solver for 3D Navier-Stokes systems.
!</description>
...
!</subroutine>
  ...
  !logical :: bdone, bHaveA12
    ...
    !
    !      ! Okay, we have a special implementation for this case...
    !      call spsor_initData_NavSt3D_Q1T_Q0(rdata)
    !      bdone = .true.
    !
    !    end if
    !
    ...
    !
    !    ! P0/Q0 pressure, arbitrary velocity
    !    call spsor_initData_NavSt3D_P0(rdata)
    !
    !  end if
    !
    !case default
...
!<subroutine>
  ...
  subroutine spsor_initData_NavSt3D_full(rdata)

!<description>
  ! Internal subroutine:
  ! Peforms data-dependent initialisation of the 'full' SP-SOR algorithm
  ! for 3D Navier-Stokes systems: arbitrary pressure, arbitrary velocity
!</description>
...
!</subroutine>
  ...
  ! Multiplication factors
  real(DP) :: dsfA11, dsfA12, dsfA13, dsfA21, dsfA22, dsfA23, &
      dsfA31, dsfA32, dsfA33, dsfB1, dsfB2, dsfB3, dsfD1, dsfD2, dsfD3, dsfC

  ! Quick access for the matrix arrays
  integer, dimension(:), pointer :: p_KldA,p_KldA12,p_KldB,p_KldC,p_KldD,p_KldS,&
      p_KcolA,p_KcolA12,p_KcolB,p_KcolC,p_KcolD,p_KcolS,p_KdiagC
  real(DP), dimension(:), pointer :: p_DA11,p_DA12,p_DA13,p_DA21,p_DA22,p_DA23, &
      p_DA31,p_DA32,p_DA33,p_DB1,p_DB2,p_DB3,p_DD1,p_DD2,p_DD3,p_DC,p_DS
  ...
  integer :: id1,id2,is1
    ...
    call lsyssc_getbase_double(rdata%p_rA11, p_DA11)
    call lsyssc_getbase_double(rdata%p_rA22, p_DA22)
    call lsyssc_getbase_double(rdata%p_rA33, p_DA33)

    call lsyssc_getbase_Kld(rdata%p_rB1, p_KldB)
    call lsyssc_getbase_Kcol(rdata%p_rB1, p_KcolB)
    call lsyssc_getbase_double(rdata%p_rB1, p_DB1)
    call lsyssc_getbase_double(rdata%p_rB2, p_DB2)
    call lsyssc_getbase_double(rdata%p_rB3, p_DB3)

    call lsyssc_getbase_Kld(rdata%p_rD1, p_KldD)
    call lsyssc_getbase_Kcol(rdata%p_rD1, p_KcolD)
    call lsyssc_getbase_double(rdata%p_rD1, p_DD1)
    call lsyssc_getbase_double(rdata%p_rD2, p_DD2)
    call lsyssc_getbase_double(rdata%p_rD3, p_DD3)
    ...
    if(associated(rdata%p_rA12)) then
      ...
      call lsyssc_getbase_Kcol(rdata%p_rA12, p_KcolA12)
      call lsyssc_getbase_double(rdata%p_rA12, p_DA12)
      call lsyssc_getbase_double(rdata%p_rA13, p_DA13)
      call lsyssc_getbase_double(rdata%p_rA21, p_DA21)
      call lsyssc_getbase_double(rdata%p_rA23, p_DA23)
      call lsyssc_getbase_double(rdata%p_rA31, p_DA31)
      call lsyssc_getbase_double(rdata%p_rA32, p_DA32)
    end if
    ...
    dsfA11 = rdata%p_rA11%dscaleFactor
    dsfA22 = rdata%p_rA22%dscaleFactor
    dsfA33 = rdata%p_rA33%dscaleFactor
    dsfB1 = rdata%p_rB1%dscaleFactor
    dsfB2 = rdata%p_rB2%dscaleFactor
    dsfB3 = rdata%p_rB3%dscaleFactor
    dsfD1 = rdata%p_rD1%dscaleFactor
    dsfD2 = rdata%p_rD2%dscaleFactor
    dsfD3 = rdata%p_rD3%dscaleFactor
    ...
    if(bHaveA12) then
      dsfA12 = rdata%p_rA12%dscaleFactor
      dsfA13 = rdata%p_rA13%dscaleFactor
      dsfA21 = rdata%p_rA21%dscaleFactor
      dsfA23 = rdata%p_rA23%dscaleFactor
      dsfA31 = rdata%p_rA31%dscaleFactor
      dsfA32 = rdata%p_rA32%dscaleFactor
    end if
        ...
        do k = p_KldA(idofu), p_KldA(idofu+1)-1
          ...
          do j = 1, ndofV
            if(idx .eq. IdofsV(j)) then
              DA(        i,        j) = dsfA11*p_DA11(k)
              DA(  ndofV+i,  ndofV+j) = dsfA22*p_DA22(k)
              DA(2*ndofV+i,2*ndofV+j) = dsfA33*p_DA33(k)
              exit
            end if
          end do ! j
        end do ! k
        ...
        if(bHaveA12) then
          ...
          do k = p_KldA12(idofu), p_KldA12(idofu+1)-1
            ...
            do j = 1, ndofV
              if(idx .eq. IdofsV(j)) then
                DA(        i,  ndofV+j) = dsfA12*p_DA12(k)
                DA(        i,2*ndofV+j) = dsfA13*p_DA13(k)
                DA(  ndofV+i,        j) = dsfA21*p_DA21(k)
                DA(  ndofV+i,2*ndofV+j) = dsfA23*p_DA23(k)
                DA(2*ndofV+i,        j) = dsfA31*p_DA31(k)
                DA(2*ndofV+i,  ndofV+j) = dsfA32*p_DA32(k)
                exit
              end if
            end do ! j
          end do ! k
      ...
      do i = 1, ndofP
        ...
        do j = 1, ndofP
          ...
          do k = 1, ndofV
            daux1 = daux1 + p_DD1(id1+k-1)*DB(        k,j)
            daux2 = daux2 + p_DD2(id1+k-1)*DB(  ndofV+k,j)
            daux3 = daux3 + p_DD3(id1+k-1)*DB(2*ndofV+k,j)
          end do ! k
          DC(i,j) = DC(i,j) - dsfD1*daux1 - dsfD2*daux2 - dsfD3*daux3
...
!<subroutine>
  ...
  subroutine spsor_solve_NavSt3D(rdata, rsol, rrhs, niterations, domegaA, domegaS)

!<description>
  ! INTERNAL ROUTINE:
  ! Performs a specifies number of SP-SOR iterations for 3D Navier-Stokes systems.
!</description>
...
!</subroutine>
  ...
  ! Multiplication factors
  real(DP) :: dsfA11, dsfA12, dsfA13, dsfA21, dsfA22, dsfA23, dsfA31, dsfA32, dsfA33, &
      dsfB1, dsfB2, dsfB3, dsfD1, dsfD2, dsfD3

  ! access for the matrix arrays
  integer, dimension(:), pointer :: p_KldA,p_KldA12,p_KldB,p_KldD,p_KldS,&
      p_KcolA,p_KcolA12,p_KcolB,p_KcolD,p_KcolS,p_KdiagA,p_KdiagS
  real(DP), dimension(:), pointer :: p_DA11,p_DA12,p_DA13,p_DA21,p_DA22,p_DA23,&
      p_DA31,p_DA32,p_DA33,p_DB1,p_DB2,p_DB3,p_DD1,p_DD2,p_DD3,p_DS

  ! vector arrays
  real(DP), dimension(:), pointer :: p_Du1, p_Du2, p_Du3, p_Dup, p_Df1, p_Df2, p_Df3, &
      p_Dfp,p_Dw
  ...
  logical :: bHaveA12
    ...
    call lsyssc_getbase_double(rsol%RvectorBlock(1), p_Du1)
    call lsyssc_getbase_double(rsol%RvectorBlock(2), p_Du2)
    call lsyssc_getbase_double(rsol%RvectorBlock(3), p_Du3)
    call lsyssc_getbase_double(rsol%RvectorBlock(4), p_Dup)
    call lsyssc_getbase_double(rrhs%RvectorBlock(1), p_Df1)
    call lsyssc_getbase_double(rrhs%RvectorBlock(2), p_Df2)
    call lsyssc_getbase_double(rrhs%RvectorBlock(3), p_Df3)
    call lsyssc_getbase_double(rrhs%RvectorBlock(4), p_Dfp)
    if(.not. rdata%bdiagS) call lsyssc_getbase_double(rdata%rw, p_Dw)
    ...
    do iter = 1, niterations
      ...
      if(bHaveA12) then
        ...
        ! Solve A11 * u1 = f1 - A12*u2 - A13*u3 - B1*p
        call spsor_aux_solveAAAB(m,p_Du1,p_Du2,p_Du3,p_Dup,p_Df1,&
            p_KldA,p_KcolA,p_KdiagA,p_DA11,dsfA11,&
            p_KldA12,p_KcolA12,p_DA12,p_DA13,dsfA12,dsfA13,&
            p_KldB,p_KcolB,p_DB1,dsfB1,domegaA)

        ! Solve A22 * u2 = f2 - A21*u1 - A23*u3 - B2*p
        call spsor_aux_solveAAAB(m,p_Du2,p_Du1,p_Du3,p_Dup,p_Df2,&
            p_KldA,p_KcolA,p_KdiagA,p_DA22,dsfA22,&
            p_KldA12,p_KcolA12,p_DA21,p_DA23,dsfA21,dsfA23,&
            p_KldB,p_KcolB,p_DB2,dsfB2,domegaA)

        ! Solve A33 * u3 = f3 - A31*u1 - A32*u2 - B3*p
        call spsor_aux_solveAAAB(m,p_Du3,p_Du1,p_Du2,p_Dup,p_Df3,&
            p_KldA,p_KcolA,p_KdiagA,p_DA33,dsfA33,&
            p_KldA12,p_KcolA12,p_DA31,p_DA32,dsfA31,dsfA32,&
            p_KldB,p_KcolB,p_DB3,dsfB3,domegaA)

      else

        ! A12/A21 are not present
        call spsor_aux_solveAB_3D(m,p_Du1,p_Du2,p_Du3,p_Dup,p_Df1,p_Df2,p_Df3,&
            p_KldA,p_KcolA,p_KdiagA,p_DA11,p_DA22,p_DA33,dsfA11,dsfA22,dsfA33,&
            p_KldB,p_KcolB,p_DB1,p_DB2,p_DB3,dsfB1,dsfB2,dsfB3,domegaA)
      ...
      if(rdata%bdiagS) then
        ...
        ! Calculate p := p + omega * S * (g - D*u)
        call spsor_aux_solveDDDS(n,p_Du1,p_Du2,p_Du3,p_Dup,p_Dfp,&
            p_KldD,p_KcolD,p_DD1,p_DD2,p_DD3,dsfD1,dsfD2,dsfD3,p_DS,domegaS)

      else

        ! Calculate w := g - D*u
        call spsor_aux_multDDD(n,p_Du1,p_Du2,p_Du3,p_Dw,p_Dfp,&
...
!<subroutine>
  ...
  subroutine spsor_prec_NavSt3D(rdata, rdef, domegaA, domegaS)

!<description>
  ! INTERNAL ROUTINE:
  ! Applies the SP-SOR preconditioner for 3D Navier-Stokes systems.
!</description>
...
!</subroutine>
  ...
  ! Multiplication factors
  real(DP) :: dsfA11, dsfA21, dsfA22, dsfA31, dsfA32, dsfA33, dsfD1, dsfD2, dsfD3

  ! access for the matrix arrays
  integer, dimension(:), pointer :: p_KldA,p_KldA12,p_KldD,p_KldS,&
      p_KcolA,p_KcolA12,p_KcolD,p_KcolS,p_KdiagA,p_KdiagS
  real(DP), dimension(:), pointer :: p_DA11,p_DA21,p_DA22,p_DA31,p_DA32,p_DA33,&
      p_DD1,p_DD2,p_DD3,p_DS

  ! vector arrays
  real(DP), dimension(:), pointer :: p_Du1, p_Du2, p_Du3, p_Dup, p_Dw
  ...
  logical :: bHaveA12
    ...
    call lsyssc_getbase_double(rdef%RvectorBlock(1), p_Du1)
    call lsyssc_getbase_double(rdef%RvectorBlock(2), p_Du2)
    call lsyssc_getbase_double(rdef%RvectorBlock(3), p_Du3)
    call lsyssc_getbase_double(rdef%RvectorBlock(4), p_Dup)
    if(.not. rdata%bdiagS) call lsyssc_getbase_double(rdata%rw, p_Dw)
    ...
    if(bHaveA12) then
      ...
      ! Solve for A33 * u3 = u3 - A31 * u1 - A32 * u2
      call spsor_aux_precLAAA(m,p_Du1,p_Du2,p_Du3,p_KldA,p_KcolA,p_KdiagA,&
          p_DA33,dsfA33,p_KldA12,p_KcolA12,p_DA31,p_DA32,dsfA31,dsfA32,domegaA)

    else

      ! A12/A21 are not present
      call spsor_aux_precLA_3D(m,p_Du1,p_Du2,p_Du3,p_KldA,p_KcolA,p_KdiagA,&
          p_DA11,p_DA22,p_DA33,dsfA11,dsfA22,dsfA33,domegaA)
    ...
    if(rdata%bdiagS) then
      ...
      ! Calculate p := omega * S * (p - D*u)
      call spsor_aux_precDDDS(n,p_Du1,p_Du2,p_Du3,p_Dup,p_KldD,p_KcolD,&
          p_DD1,p_DD2,p_DD3,dsfD1,dsfD2,dsfD3,p_DS,domegaS)

    else

      ! Calculate w := p - D*u
      call spsor_aux_multDDD(n,p_Du1,p_Du2,p_Du3,p_Dw,p_Dup,&
          p_KldD,p_KcolD,p_DD1,p_DD2,p_DD3,dsfD1,dsfD2,dsfD3)
...
!<subroutine>
  ...
  subroutine spsor_prec_sym_NavSt3D(rdata, rdef, domegaA, domegaS)

!<description>
  ! INTERNAL ROUTINE:
  ! Applies the SP-SSOR preconditioner for 3D Navier-Stokes systems.
!</description>
...
!</subroutine>
  ...
  logical :: bHaveA12
    ...
    if(bHaveA12) then
      ...
      ! Solve for A33 * u3 = u3 - B3*p
      call spsor_aux_precUAB(m,p_Du3,p_Dup,p_KldA,p_KcolA,p_KdiagA,&
          p_DA33,dsfA33,p_KldB,p_KcolB,p_DB3,dsfB3,domegaA)

      ! Solve for A22 * u2 = u2 - A23 * u3 - B2*p
      call spsor_aux_precUAAB(m,p_Du2,p_Du3,p_Dup,p_KldA,p_KcolA,p_KdiagA,&
          p_DA22,dsfA22,p_KldA12,p_KcolA12,p_DA23,dsfA23,p_KldB,p_KcolB,&
          p_DB2,dsfB2,domegaA)

      ! Solve for A11 * u1 = u1 - A12 * u2 - A13 * u3 - B1*p
      call spsor_aux_precUAAAB(m,p_Du1,p_Du2,p_Du3,p_Dup,p_KldA,p_KcolA,p_KdiagA,&
          p_DA11,dsfA11,p_KldA12,p_KcolA12,p_DA12,p_DA13,dsfA12,dsfA13,p_KldB,p_KcolB,&
          p_DB1,dsfB1,domegaA)

    else

      ! A12/A21 are not present
      call spsor_aux_precUAB_3D(m,p_Du1,p_Du2,p_Du3,p_Dup,p_KldA,p_KcolA,p_KdiagA,&
          p_DA11,p_DA22,p_DA33,dsfA11,dsfA22,dsfA33,p_KldB,p_KcolB,p_DB1,p_DB2,p_DB3,&
          dsfB1,dsfB2,dsfB3,domegaA)
