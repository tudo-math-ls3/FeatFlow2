...
!#
!#  <code>
!#    call linf_buildVectorScalar (rform,.true.,rvector,rcubatureInfo,coeff_RHS)
!#  </code>
!#
...
!</subroutine>
    ...
    call output_line ("Using deprecated feature. Please update your code.", &
        OU_CLASS_WARNING,OU_MODE_STD,"linf_buildVectorScalar1")
#endif
    ...
    if (rvector%h_Ddata .eq. ST_NOHANDLE) then
      ...
      ! Create the vector.
      ! Only clear it if it is not cleared below.
      call lsyssc_createVector (rdiscretisation,rvector,.not. bclear)
...
!<description>
  ! This routine assembles the entries of a vector according to a linear form
  ! (typically used for assembling RHS vectors). All subvectors in rvectorBlock
  ! must be associated to the same discretisation (i.e. discretised on the same
  ! mesh with the same element(s)).
  !
  ! If bclear=TRUE, the vector is cleared before the assembly and any
  ! sorting of the entries is switched off - the vector is set up unsorted.
  !
...
!</subroutine>
    ...
    ! defines how to do the assembly.
    if (.not. present(rcubatureInfo)) then
      call spdiscr_createDefCubStructure(rvectorBlock%p_rblockDiscr%RspatialDiscr(1),&
          rtempCubatureInfo,CUB_GEN_DEPR_LINFORM)
      p_rcubatureInfo => rtempCubatureInfo
    else
      p_rcubatureInfo => rcubatureInfo
