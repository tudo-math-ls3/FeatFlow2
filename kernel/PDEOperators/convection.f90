...
!<description>
  ! Standard streamline diffusion method to set up the convection operator
  !  
  !            $$ u_1 * grad(u_2) $$
  !
  ! in a matrix or to build a defect vector.
      ...
      IF (ddelta .NE. 0.0_DP) THEN
        DO IEL=1,IELmax-IELset+1
          CALL getLocalDelta (u1Xvel,u1Yvel,u2Xvel,u2Yvel,dweight1,dweight2, &
                      ...
                      p_IverticesAtElement,p_IedgesAtElement,p_DcornerCoordinates,Idofs(:,IEL),indof, &
                      dupsam,dre)
...
!      END DO
      ...
      DO IEL=1,IELmax-IELset+1
        ...
        DO ICUBP = 1, ncubp
          ...
          !
          ! Outer loop over the DOF's j=1..indof on our current element, 
          ! which corresponds to the basis functions Phi_j:
          ...
          DO IDOFE=1,indof
            ...
            ! Inner loop over the DOF's i=1..indof, which corresponds to
            ! the basis function Phi_i:
