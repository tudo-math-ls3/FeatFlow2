...
!$ use omp_lib
  ...
  public :: elem_eval_QP1NP_2D
  public :: elem_eval_QP1NPD_2D
  ...
  public :: elem_eval_DG_T0_2D
  public :: elem_eval_DG_T1_2D
  public :: elem_eval_DG_T2_2D
  public :: elem_eval_DG_T3_2D
...
!<subroutine>
  ...
  subroutine elem_eval_Q0_2D (celement, reval, Bder, Dbas)
...
! </subroutine>
  ...
  ! Element Description
  ! -------------------
  ! The Q0_2D element is specified by a single basis function, constant in the element.
  !
...
!<subroutine>
  ...
  subroutine elem_eval_QP1NP_2D (celement, reval, Bder, Dbas)
...
! </subroutine>
  ...
  ! Element Description
  ! -------------------
  ! The QP1NP_2D element is specified by three polynomials per element.
  !
  ! The basis polynomials are constructed from the following set of monomials:
  ! { 1, x, y }
  !
  ...
  real(dp) :: deta1,dxi1,deta2,dxi2
  real(DP) :: ddet
  integer :: i,j,IVE

    ! Calculate function values or derivatives?
    if(Bder(DER_FUNC2D) .or. Bder(DER_DERIV2D_X) .or. Bder(DER_DERIV2D_Y)) then

      ! Loop through all elements
      !$omp parallel do default(shared) private(DXM,DYM,IVE,ddet,deta1,deta2,&
      !$omp          dx,dxi1,dxi2,dy)&
      !$omp if(reval%nelements > reval%p_rperfconfig%NELEMMIN_OMP)
      do j = 1, reval%nelements
        ...
        do IVE=1,NVE
          DXM(IVE)=0.5_DP*(reval%p_Dcoords(1,IVE,j)+reval%p_Dcoords(1,mod(IVE,4)+1,j))
          DYM(IVE)=0.5_DP*(reval%p_Dcoords(2,IVE,j)+reval%p_Dcoords(2,mod(IVE,4)+1,j))
        end do
        ...
        do i = 1, reval%npointsPerElement
          ...
          dx = (reval%p_DpointsRef(1,i,j)-0.5_DP*(DXM(1)+DXM(3)))
          dy = (reval%p_DpointsRef(2,i,j)-0.5_DP*(DYM(1)+DYM(3)))

          ! Now calculate the (inverse of the) Jacobian determinant of the linear mapping.
          ddet = 2.0_DP/(deta1*dxi2 - deta2*dxi1)
          ...
          Dbas(1,DER_FUNC2D,i,j) = 1.0_DP
          Dbas(2,DER_FUNC2D,i,j) = ddet * (dxi2*dx - dxi1*dy)
          Dbas(3,DER_FUNC2D,i,j) = ddet * (deta1*dy - deta2*dx)
          
          ! X-derivatives on real element (at no additional costs)
          Dbas(1,DER_DERIV2D_X,i,j) = 0.0_DP
          Dbas(2,DER_DERIV2D_X,i,j) = ddet * dxi2
          Dbas(3,DER_DERIV2D_X,i,j) = -ddet * dxi1

          ! Y-derivatives on real element (at no additional costs)
          Dbas(1,DER_DERIV2D_Y,i,j) = 0.0_DP
          Dbas(2,DER_DERIV2D_Y,i,j) = -ddet * deta2
          Dbas(3,DER_DERIV2D_Y,i,j) = ddet * deta1
...
!<subroutine>
  ...
  subroutine elem_eval_QP1NPD_2D (celement, reval, Bder, Dbas)
...
! </subroutine>
  ...
  ! of the edges.
  !
  ! QP1-element defining the basis functions directly on the real element.
  ...
  real(DP),dimension(4) :: DXM,DYM
  real(dp) :: deta1,dxi1,deta2,dxi2,dnormeta,dnormxi,dmx,dmy
  integer :: i,j,IVE
  logical :: bsuccess
    ...
    if(Bder(DER_FUNC2D) .or. Bder(DER_DERIV2D_X) .or. Bder(DER_DERIV2D_Y)) then
      ...
      ! Loop through all elements
      !$omp parallel do default(shared) private(Da,Db,DXM,DYM,IVE,deta1,deta2,&
      !$omp          dnormeta,dnormxi,dxi1,dxi2,dmx,dmy,bsuccess)&
      !$omp if(reval%nelements > reval%p_rperfconfig%NELEMMIN_OMP)
      do j = 1, reval%nelements
        ...
        do i = 1, reval%npointsPerElement
          ...
          dx = deta1*reval%p_DpointsReal(1,i,j) + deta2*reval%p_DpointsReal(2,i,j)
          dy = dxi1*reval%p_DpointsReal(1,i,j) + dxi2*reval%p_DpointsReal(2,i,j)
          ...
          ! in Db.
          Dbas(1,DER_FUNC2D,i,j) = 1.0_DP
          Dbas(2,DER_FUNC2D,i,j) = Db(2,2)*dx + Db(3,2)*dy ! + Db(1,2), but this is zero
          Dbas(3,DER_FUNC2D,i,j) = Db(2,3)*dx + Db(3,3)*dy ! + Db(1,3), but this is zero
                    
          ! X-derivatives on real element (at no additional costs)
          Dbas(1,DER_DERIV2D_X,i,j) = 0.0_DP
          Dbas(2,DER_DERIV2D_X,i,j) = Db(2,2)*deta1 + Db(3,2)*dxi1
          Dbas(3,DER_DERIV2D_X,i,j) = Db(2,3)*deta1 + Db(3,3)*dxi1
          
          ! Y-derivatives on real element (at no additional costs)
          Dbas(1,DER_DERIV2D_Y,i,j) = 0.0_DP
          Dbas(2,DER_DERIV2D_Y,i,j) = Db(2,2)*deta2 + Db(3,2)*dxi2
          Dbas(3,DER_DERIV2D_Y,i,j) = Db(2,3)*deta2 + Db(3,3)*dxi2
...
!<subroutine>
  ...
  subroutine elem_eval_EM30_2D (celement, reval, Bder, Dbas)
...
! </subroutine>
  ...
  npointsderx = 0
  npointsdery = 0
  if (Bder(DER_FUNC)) npointsfunc = reval%npointsPerElement
  if (Bder(DER_DERIV_X)) npointsderx = reval%npointsPerElement
  if (Bder(DER_DERIV_Y)) npointsdery = reval%npointsPerElement
    ...
    if (iand(celement,int(2**18,I32)) .eq. 0) then
      ...
      do j=1,reval%nelements
        ...
        do IVE=1,NVE
          DXM(IVE)=0.5_DP*(reval%p_Dcoords(1,IVE,j)+reval%p_Dcoords(1,mod(IVE,4)+1,j))
          DYM(IVE)=0.5_DP*(reval%p_Dcoords(2,IVE,j)+reval%p_Dcoords(2,mod(IVE,4)+1,j))
          DLX(IVE)=0.5_DP*(reval%p_Dcoords(1,mod(IVE,4)+1,j)-reval%p_Dcoords(1,IVE,j))
          DLY(IVE)=0.5_DP*(reval%p_Dcoords(2,mod(IVE,4)+1,j)-reval%p_Dcoords(2,IVE,j))
        end do
        ...
        ! Function values
        do i=1,npointsfunc   ! either 0 or reval%npointsPerElement

          Dbas(1,DER_FUNC,i,j)=  COB(1,1)*reval%p_DpointsReal(1,i,j)**2 &
                                +COB(2,1)*reval%p_DpointsReal(2,i,j)**2 &
                                +COB(3,1)*reval%p_DpointsReal(1,i,j)*reval%p_DpointsReal(2,i,j) &
                                +COB(4,1)*reval%p_DpointsReal(1,i,j)   &
                                +COB(5,1)*reval%p_DpointsReal(2,i,j)   &
                                +COB(6,1)
          Dbas(2,DER_FUNC,i,j)=  COB(1,2)*reval%p_DpointsReal(1,i,j)**2 &
                                +COB(2,2)*reval%p_DpointsReal(2,i,j)**2 &
                                +COB(3,2)*reval%p_DpointsReal(1,i,j)*reval%p_DpointsReal(2,i,j) &
                                +COB(4,2)*reval%p_DpointsReal(1,i,j)   &
                                +COB(5,2)*reval%p_DpointsReal(2,i,j)   &
                                +COB(6,2)
          Dbas(3,DER_FUNC,i,j)=  COB(1,3)*reval%p_DpointsReal(1,i,j)**2 &
                                +COB(2,3)*reval%p_DpointsReal(2,i,j)**2 &
                                +COB(3,3)*reval%p_DpointsReal(1,i,j)*reval%p_DpointsReal(2,i,j) &
                                +COB(4,3)*reval%p_DpointsReal(1,i,j)   &
                                +COB(5,3)*reval%p_DpointsReal(2,i,j)   &
                                +COB(6,3)
          Dbas(4,DER_FUNC,i,j)=  COB(1,4)*reval%p_DpointsReal(1,i,j)**2 &
                                +COB(2,4)*reval%p_DpointsReal(2,i,j)**2 &
                                +COB(3,4)*reval%p_DpointsReal(1,i,j)*reval%p_DpointsReal(2,i,j) &
                                +COB(4,4)*reval%p_DpointsReal(1,i,j)    &
                                +COB(5,4)*reval%p_DpointsReal(2,i,j)    &
                                +COB(6,4)
        end do

        ! x-derivatives

        do i=1,npointsderx   ! either 0 or reval%npointsPerElement
          Dbas(1,DER_DERIV_X,i,j) = 2.0_DP*COB(1,1)*reval%p_DpointsReal(1,i,j) &
                                          +COB(3,1)*reval%p_DpointsReal(2,i,j) &
                                          +COB(4,1)
          Dbas(2,DER_DERIV_X,i,j) = 2.0_DP*COB(1,2)*reval%p_DpointsReal(1,i,j) &
                                          +COB(3,2)*reval%p_DpointsReal(2,i,j) &
                                          +COB(4,2)
          Dbas(3,DER_DERIV_X,i,j) = 2.0_DP*COB(1,3)*reval%p_DpointsReal(1,i,j) &
                                          +COB(3,3)*reval%p_DpointsReal(2,i,j) &
                                          +COB(4,3)
          Dbas(4,DER_DERIV_X,i,j) = 2.0_DP*COB(1,4)*reval%p_DpointsReal(1,i,j) &
                                          +COB(3,4)*reval%p_DpointsReal(2,i,j) &
                                          +COB(4,4)
        end do

        ! y-derivatives

        do i=1,npointsdery   ! either 0 or reval%npointsPerElement
          Dbas(1,DER_DERIV_Y,i,j) = 2.0_DP*COB(2,1)*reval%p_DpointsReal(2,i,j) &
                                          +COB(3,1)*reval%p_DpointsReal(1,i,j) &
                                          +COB(5,1)
          Dbas(2,DER_DERIV_Y,i,j) = 2.0_DP*COB(2,2)*reval%p_DpointsReal(2,i,j) &
                                          +COB(3,2)*reval%p_DpointsReal(1,i,j) &
                                          +COB(5,2)
          Dbas(3,DER_DERIV_Y,i,j) = 2.0_DP*COB(2,3)*reval%p_DpointsReal(2,i,j) &
                                          +COB(3,3)*reval%p_DpointsReal(1,i,j) &
                                          +COB(5,3)
          Dbas(4,DER_DERIV_Y,i,j) = 2.0_DP*COB(2,4)*reval%p_DpointsReal(2,i,j) &
                                          +COB(3,4)*reval%p_DpointsReal(1,i,j) &
                                          +COB(5,4)
...
!<subroutine>
  ...
  subroutine elem_eval_E031_2D (celement, reval, Bder, Dbas)
...
! </subroutine>
  ...
  ! Element Description
  ! -------------------
  ! The E031_2D element is specified by four polynomials per element.
  !
  ! The basis polynomials are constructed from the following set of monomials:
  !
  ! { 1, x, y, x^2 - y^2 }
  ...
  ! basis polynomial conditions leads to the following basis polynomials:
  !
  !  P1(x,y) = -1/4 * (x^2 - y^2)  -  1/2 * y  +  1/4
  !  P2(x,y) =  1/4 * (x^2 - y^2)  +  1/2 * x  +  1/4
  !  P3(x,y) = -1/4 * (x^2 - y^2)  +  1/2 * y  +  1/4
  !  P4(x,y) =  1/4 * (x^2 - y^2)  -  1/2 * x  +  1/4
    ...
    if(Bder(DER_DERIV2D_X) .or. Bder(DER_DERIV2D_Y)) then
      ...
      do j = 1, reval%nelements
        ...
        do i = 1, reval%npointsPerElement
          ...
          ! X-derivatives
          DrefDer(1,1) = -dx
          DrefDer(2,1) =  dx + 1.0_DP
          DrefDer(3,1) = -dx
          DrefDer(4,1) =  dx - 1.0_DP
          ! Y-derivatives
          DrefDer(1,2) = -dy - 1.0_DP
          DrefDer(2,2) =  dy
          DrefDer(3,2) = -dy + 1.0_DP
          DrefDer(4,2) =  dy
          ...
          ! Get Jacobian determinant
          ddet = 0.5_DP / reval%p_Ddetj(i,j)

          ! X-derivatives on real element
          Dbas(1,DER_DERIV2D_X,i,j) = ddet * (Reval%p_Djac(4,i,j) * DrefDer(1,1) -&
                                              Reval%p_Djac(2,i,j) * DrefDer(1,2))
          Dbas(2,DER_DERIV2D_X,i,j) = ddet * (Reval%p_Djac(4,i,j) * DrefDer(2,1) -&
                                              Reval%p_Djac(2,i,j) * DrefDer(2,2))
          Dbas(3,DER_DERIV2D_X,i,j) = ddet * (Reval%p_Djac(4,i,j) * DrefDer(3,1) -&
                                              Reval%p_Djac(2,i,j) * DrefDer(3,2))
          Dbas(4,DER_DERIV2D_X,i,j) = ddet * (Reval%p_Djac(4,i,j) * DrefDer(4,1) -&
                                              Reval%p_Djac(2,i,j) * DrefDer(4,2))

          ! Y-derivatives on real element
          Dbas(1,DER_DERIV2D_Y,i,j) = -ddet * (Reval%p_Djac(3,i,j) * DrefDer(1,1) -&
                                               Reval%p_Djac(1,i,j) * DrefDer(1,2))
          Dbas(2,DER_DERIV2D_Y,i,j) = -ddet * (Reval%p_Djac(3,i,j) * DrefDer(2,1) -&
                                               Reval%p_Djac(1,i,j) * DrefDer(2,2))
          Dbas(3,DER_DERIV2D_Y,i,j) = -ddet * (Reval%p_Djac(3,i,j) * DrefDer(3,1) -&
                                               Reval%p_Djac(1,i,j) * DrefDer(3,2))
          Dbas(4,DER_DERIV2D_Y,i,j) = -ddet * (Reval%p_Djac(3,i,j) * DrefDer(4,1) -&
                                               Reval%p_Djac(1,i,j) * DrefDer(4,2))
...
!<subroutine>
  ...
  subroutine elem_eval_EM31_2D (celement, reval, Bder, Dbas)
...
! </subroutine>
  ...
  ! Element Description
  ! -------------------
  ! The EM31_2D element is specified by five polynomials per element.
  !
  ! The standard integral-based Q1~-element looks locally as follows:
  !
  !                 phi_3
...
!<subroutine>
  ...
  subroutine elem_eval_DG_T0_2D (celement, reval, Bder, Dbas)
...
! </subroutine>
  ...
  ! Element Description
  ! -------------------
  ! The DG_T0_2D element is specified by a single basis function, constant in the element.
  !
  ! The function value of the basis function is =1, the derivatives are all 0!
...
!<subroutine>
  ...
  subroutine elem_eval_DG_T1_2D (celement, reval, Bder, Dbas)
    ...
    if(Bder(DER_DERIV2D_X) .or. Bder(DER_DERIV2D_Y)) then
      ...
      do j = 1, reval%nelements
        ...
        do i = 1, reval%npointsPerElement
          ...
          ! X-derivatives on real element
          Dbas(1,DER_DERIV2D_X,i,j) = ddet * (reval%p_Djac(4,i,j) * 0.0_DP &
                                            - reval%p_Djac(2,i,j) * 0.0_DP)
          Dbas(2,DER_DERIV2D_X,i,j) = ddet * (reval%p_Djac(4,i,j) * 1.0_DP &
                                            - reval%p_Djac(2,i,j) * 0.0_DP)
          Dbas(3,DER_DERIV2D_X,i,j) = ddet * (reval%p_Djac(4,i,j) * 0.0_DP &
                                            - reval%p_Djac(2,i,j) * 1.0_DP)

          ! Y-derivatives on real element
          Dbas(1,DER_DERIV2D_Y,i,j) = -ddet * (reval%p_Djac(3,i,j) * 0.0_DP &
                                             - reval%p_Djac(1,i,j) * 0.0_DP)
          Dbas(2,DER_DERIV2D_Y,i,j) = -ddet * (reval%p_Djac(3,i,j) * 1.0_DP &
                                             - reval%p_Djac(1,i,j) * 0.0_DP)
          Dbas(3,DER_DERIV2D_Y,i,j) = -ddet * (reval%p_Djac(3,i,j) * 0.0_DP &
                                             - reval%p_Djac(1,i,j) * 1.0_DP)
...
!<subroutine>
  ...
  subroutine elem_eval_DG_T2_2D (celement, reval, Bder, Dbas)
...
! </subroutine>
  ...
  real(DP) :: ddet,dx,dy,dc1,dc2,dc3
  integer :: i,j,k
  ...
  real(DP), parameter :: Q6 = 1.0_DP/6.0_DP
    ...
    if(Bder(DER_FUNC2D)) then
      ...
      do j = 1, reval%nelements
        ...
        do i = 1, reval%npointsPerElement
          ...
          ! Evaluate basis functions
          Dbas(1,DER_FUNC2D,i,j)= 1.0_DP
          Dbas(2,DER_FUNC2D,i,j)= dx
          Dbas(3,DER_FUNC2D,i,j)= dy
          Dbas(4,DER_FUNC2D,i,j)= Q2*dx*dx-Q6
          Dbas(5,DER_FUNC2D,i,j)= Q2*dy*dy-Q6
          Dbas(6,DER_FUNC2D,i,j)= dx*dy
    ...
    if(Bder(DER_DERIV2D_X) .or. Bder(DER_DERIV2D_Y)) then
      ...
      ! Loop through all elements
      !$omp parallel do default(shared) private(Dhelp,i,k,ddet,dx,dy)&
      !$omp if(reval%nelements > reval%p_rperfconfig%NELEMMIN_OMP)
      do j = 1, reval%nelements
        ...
        do i = 1, reval%npointsPerElement
          ...
          ! X-derivatives on real element
          do k = 1,6
            Dbas(k,DER_DERIV2D_X,i,j) = ddet &
                * (reval%p_Djac(4,i,j) * Dhelp(k,1) &
                -  reval%p_Djac(2,i,j) * Dhelp(k,2))
          end do

          ! Y-derivatives on real element
          do k = 1,6
            Dbas(k,DER_DERIV2D_Y,i,j) = -ddet &
                * (reval%p_Djac(3,i,j) * Dhelp(k,1) &
                -  reval%p_Djac(1,i,j) * Dhelp(k,2))
          end do
    ...
    ! Calculate derivatives?
    if(Bder(DER_DERIV2D_XX) .or. Bder(DER_DERIV2D_XY)&
                            .or. Bder(DER_DERIV2D_YY)) then
      ...
      ! XX-, XY- and YY-derivatives on reference element are constant
      ! so we can compute them in advance
      Dhelp(1,3) = 0.0_DP
      Dhelp(2,3) = 0.0_DP
      Dhelp(3,3) = 0.0_DP
      Dhelp(4,3) = 1.0_DP
      ...
      ! Loop through all elements
      !$omp parallel do default(shared) private(Dhelp,i,k,ddet,dx,dy,dc1,dc2,dc3)&
      !$omp if(reval%nelements > reval%p_rperfconfig%NELEMMIN_OMP)
      do j = 1, reval%nelements
        ...
        do i = 1, reval%npointsPerElement
          ...
          ! Get square of Jacobian determinant
          ddet = 1.0_DP / reval%p_Ddetj(i,j)**2

          ! Coefficients for XX-derivatices
          dc1 = ddet * reval%p_Djac(4,i,j)**2
          dc2 = ddet * ( -2.0_DP * reval%p_Djac(4,i,j) * reval%p_Djac(2,i,j) )
          dc3 = ddet * reval%p_Djac(2,i,j)**2

          ! XX-derivatives on real element
          do k = 1,6
            Dbas(k,DER_DERIV2D_XX,i,j) = &
                dc1 * Dhelp(k,3) + dc2 * Dhelp(k,4) + dc3 * Dhelp(k,5)
          end do

          ! Coefficients for XY-derivatives
          dc1 = - ddet * reval%p_Djac(4,i,j) * reval%p_Djac(3,i,j)
          dc2 =   ddet * ( reval%p_Djac(4,i,j) * reval%p_Djac(1,i,j)&
                         + reval%p_Djac(3,i,j) * reval%p_Djac(2,i,j) )
          dc3 = - ddet * reval%p_Djac(1,i,j) * reval%p_Djac(4,i,j)

          ! XY-derivatives on real element
          do k = 1,6
            Dbas(k,DER_DERIV2D_XY,i,j) = &
                dc1 * Dhelp(k,3) + dc2 * Dhelp(k,4) + dc3 * Dhelp(k,5)
          end do

          ! Coefficients for YY-derivatives
          dc1 = ddet * reval%p_Djac(3,i,j)**2
          dc2 = ddet * ( -2.0_DP * reval%p_Djac(3,i,j) * reval%p_Djac(1,i,j) )
          dc3 = ddet * reval%p_Djac(1,i,j)**2

          ! YY-derivatives on real element
          do k = 1,6
            Dbas(k,DER_DERIV2D_YY,i,j) = &
                dc1 * Dhelp(k,3) + dc2 * Dhelp(k,4) + dc3 * Dhelp(k,5)
          end do
...
!<subroutine>
  ...
  subroutine elem_eval_DG_T3_2D (celement, reval, Bder, Dbas)
...
! </subroutine>
  ...
  ! Element Description
  ! -------------------
  ! The DG_T3_2D element is specified by ten polynomials on the reference element.
    ...
    if(Bder(DER_FUNC2D)) then
      ...
      do j = 1, reval%nelements
        ...
        do i = 1, reval%npointsPerElement
          ...
          Dbas(5,DER_FUNC2D,i,j)= Q2*dy*dy-Q6
          Dbas(6,DER_FUNC2D,i,j)= dx*dy
          Dbas(7,DER_FUNC2D,i,j)= Q48*dx*dx*dx
          Dbas(8,DER_FUNC2D,i,j)= Q48*dx*dx*dy
          Dbas(9,DER_FUNC2D,i,j)= Q48*dx*dy*dy
          Dbas(10,DER_FUNC2D,i,j)= Q48*dy*dy*dy
