...
!</subroutine>
    ...
    select case(rgeomObject%ctype)
    case (GEOM_SPHERE)
      call geom_triangulateSphere(rgeomObject,hpolyHandle,30,30)
    case default
...
!<subroutine>
  ...
  subroutine geom_triangulateSphere(rgeomObject, hpolyHandle, &
                                    nSlices,nStacks)
...
!<input>
  ! The geometry object, that will be triangulated
  type(t_geometryObject), intent(in)  :: rgeomObject
  
  !  
  ! To triangulate the 3d object, it will be subdivided into triangles, that form the
  ! surface of the 3d object. The subdivision is generated by parameterizing the object
  ! and subdividing the 2D parameter domain. We choose nSlices subdivisions of the x-parameter
  ! domain and nStacks subdivision of the y-parameter domain. The intersection of slices and stacks
  ! are the parameter coordinates of the vertices of the triangulation
  !
  !Parameter domain:    Sl1
  !                   ----------------
  !                  |   |            |
  !                  |   |            |
  !                  |   |Vertex1     |
...
!</subroutine>
  ...
  real(DP), dimension(:,:), pointer :: p_Dvertices
  real(DP) :: dangle, dradius,phi,theta
  ...
  integer, dimension(2) :: Isize
    ...
    ! Allocate desired number of vertices
    Isize = (/ 3, nSlices*nStacks/)
    call storage_new('geom_triangulateSphere', 'hpolyHandle', Isize, &
                       ST_DOUBLE, hpolyHandle, ST_NEWBLOCK_NOINIT)
    ...
    do i=0, nSlices-1
      do j=1,nStacks    
        p_DVertices(1,i*nStacks+j) = dradius * cos(theta) * cos(phi)
        p_DVertices(2,i*nStacks+j) = dradius * cos(theta) * sin(phi)
        p_DVertices(3,i*nStacks+j) = dradius * cos(theta)      
      end do
    end do
