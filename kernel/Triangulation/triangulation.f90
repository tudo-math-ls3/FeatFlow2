...
!#
!# 32.) tria_createRawManifTria1D
!#      -> Creates a 1D manifold raw triangulation in a 2D or 3D domain from a set
!#         of points forming a polygon. Used to express, e.g., a boundary triangulation.
!#
!# 33.) tria_createRawBdryTria2D
!#      -> Create a 1D manifold mesh from the boundary of a 2D mesh.
!#
!# Auxiliary routines:
!#
...
!#
!#   The first NBCT boundary components refer to the original BC`s in the
...
!</typeblock>
  ...
  type t_triangulation
    ...
    !
    ! Note that the triangulation supports 1D meshes in 2D and 3D, 
    ! so ubound(p_DvertexCoords,1) may be =2 or =3 although ndim=1.
    !
...
!</types>
  ...
  public :: tria_createRawManifTria1D
  public :: tria_createRawBdryTria2D
...
!</subroutine>
  ...
  contains
    ...
    type(t_triangulation), intent(out) :: rdestTriangulation
      ...
      integer :: iel,iel2
      integer :: ivt1,ivt2, ivtoffset, ivt, idim
      ...
      Isize = (/ubound(p_DcoordSource,1),rdestTriangulation%NVT/)
      ...
      do ivt=1,rsourceTriangulation%NVT
        do idim = 1,ubound(p_DcoordSource,1)
          p_DcoordDest(idim,ivt) = p_DcoordSource(idim,ivt)
        end do
      end do
      ...
      do iel=1, rsourceTriangulation%NEL
        ...
        do idim = 1,ubound(p_DcoordSource,1)
          p_DcoordDest(idim,ivtoffset+iel) = &
              0.5_DP * ( p_DcoordSource (idim,ivt1) + p_DcoordSource (idim,ivt2) )
        end do
      end do
      ...
      if (rsourceTriangulation%h_IverticesAtBoundary .ne. ST_NOHANDLE) then
        call storage_copy (rsourceTriangulation%h_IverticesAtBoundary,&
            rdestTriangulation%h_IverticesAtBoundary)
      end if
      
      if (rsourceTriangulation%h_IboundaryCpIdx .ne. ST_NOHANDLE) then
    ...
    type(t_boundary), intent(in), optional :: rboundary
      ...
      if (rsourceTriangulation%NVBD .gt. 0) then
      ...
      if ((rsourceTriangulation%h_DvertexParameterValue .ne. ST_NOHANDLE) .and. &
        ...
        if (present(rboundary) .and. brecalcBoundaryCoords) then
          ...
          ! coordinates. Points that are not on the physical boundary but
...
!</subroutine>
    ...
    case (NDIM2D)
      ...
      if (present(rboundary) .and. (rtriaDest%NVBD .gt. 0)) then
  ...
  contains
    ...
    type(t_triangulation), intent(inout) :: rtriangulation
      ...
      if (rtriangulation%NVBD .gt. 0) then
...
!<subroutine>
  ...
  subroutine tria_createRawManifTria1D(rtriangulation, Dpoints, IpointsIdx)

!<description>
  ! This routine creates a "raw" 1D triangulation consisting of
  ! a couple of 2d line segments. Dpoints specifies a couple of polygons.
  ! IpointsIdx defines the start indices of the polygons in Dpoints.
!</description>

!<input>
  ! List of points defining the polygons.
  ! If the first and last point of the polygon matches, the polygon
  ! is assumed to be closed.
  real(DP), dimension(:,:), intent(in) :: Dpoints

  ! Index array of size(Dpoints)+1 indices defining the start positions
  ! of the polygons in Dpoints. Each polygon can be seen as a boundary
  ! component in 2D. There must be IpointsIdx(size+1)=size+1 pointing
  ! behind the array.
  integer, dimension(:), intent(in) :: IpointsIdx
!</input>

!<output>
  ! The triangulation to create.
  type(t_triangulation), intent(out) :: rtriangulation
!</output>
...
!</subroutine>
  ...
  ! Some local variables
  integer :: i,j,k,nel,nct,iv1,iv2,ivt,ivtstart,iel,nbct,ibcidx,nvt
  real(DP), dimension(:,:), pointer :: p_Dcoords
  integer, dimension(:,:), pointer :: p_Iverts
  integer, dimension(:), pointer :: p_IbcVertsIdx,p_IbcVerts,p_Inpr
  integer, dimension(2) :: Isize
  logical :: bclosed
    ...
    ! Calculate the number of components and vertices.
    ! If the first and last vertex on every component matches,
    ! the polygon is assumed to be closed, so one vertex must be removed.
    nct = size(IpointsIdx)-1
    nvt = 0
    do i=1,nct
      iv1 = IpointsIdx(i)
      iv2 = IpointsIdx(i+1)-1
      if (sqrt((Dpoints(1,iv2)-Dpoints(1,iv1))**2 + &
               (Dpoints(2,iv2)-Dpoints(2,iv1))**2) .lt. 100.0_DP*SYS_EPSREAL_DP ) then
        ! Closed polygon
        nvt = nvt + iv2-iv1
      else
        ! Open polygon
    ...
    ! Allocate vertices
    Isize = (/2, nvt/)
    call storage_new("tria_createRawManifTria1D", "DCORVG",&
        Isize, ST_DOUBLE,&
        rtriangulation%h_DvertexCoords, ST_NEWBLOCK_NOINIT)
    call storage_getbase_double2D(rtriangulation%h_DvertexCoords,&
        p_Dcoords)
    ...
    ! Allocate elements.
    ! We have as many "elements" as points minus the number of components
    ! (since the components are not connected)
    nel = ubound(Dpoints,2)-nct
    Isize = (/2, nel/)
    call storage_new("tria_createRawManifTria1D", "KVERT", Isize, &
        ST_INT, rtriangulation%h_IverticesAtElement, ST_NEWBLOCK_NOINIT)
    call storage_getbase_int2d(rtriangulation%h_IverticesAtElement, p_Iverts)

    ! Calculate the number of boundary components.
    nbct = 0
    do i=1,nct
      iv1 = IpointsIdx(i)
      iv2 = IpointsIdx(i+1)-1
      bclosed = sqrt((Dpoints(1,iv2)-Dpoints(1,iv1))**2 + &
                     (Dpoints(2,iv2)-Dpoints(2,iv1))**2) .lt. 100.0_DP*SYS_EPSREAL_DP 
      if (.not. bclosed) then
        ! An open component has two boundary components = vertices.
        nbct = nbct + 2
      end if
    end do

    ! Every open component has a boundary component of two
    ! vertices at the ends. Closed components have no boundary.
    rtriangulation%NBCT = nbct
    
    if (nbct .gt. 0) then
      ! Allocate memory for boundary components
      call storage_new ("tria_createRawManifTria1D", "KBCT", nbct+1, ST_INT, &
          rtriangulation%h_IboundaryCpIdx, ST_NEWBLOCK_NOINIT)
      call storage_getbase_int(rtriangulation%h_IboundaryCpIdx, p_IbcVertsIdx)
      
      ! Allocate memory for boundary components
      call storage_new ("tria_createRawManifTria1D", "KVBD", nbct, ST_INT, &
          rtriangulation%h_IverticesAtBoundary, ST_NEWBLOCK_NOINIT)
      call storage_getbase_int(rtriangulation%h_IverticesAtBoundary, p_IbcVerts)
    end if
    
    ! Allocate memory for nodal property
    call storage_new ("tria_createRawManifTria1D", "KNPR", nvt, ST_INT, &
        rtriangulation%h_InodalProperty, ST_NEWBLOCK_ZERO)
    call storage_getbase_int(rtriangulation%h_InodalProperty,p_Inpr)

    ! Each boundary vertex is a boundary component.
    rtriangulation%NVBD = nbct
    
    ! Initialise the index array for the boundary vertices
    if (nbct .gt. 0) then
      do i=1,nbct+1
        p_IbcVertsIdx(i) = i
      end do
    ...
    do i=1,nct
      ...
      if (bclosed) then
        ! Closed polygon, one vertex less
        iv2 = iv2 - 1
      end if
      
      ! Real start index in the current component
      ivtstart = ivt
      
      do j = iv1,iv2
  
        ! Transfer the vertices
        do k=1,ubound(Dpoints,1)
          p_Dcoords(k,ivt) = Dpoints(k,j)
        end do
        
        ! Initialise elements
        if (j .lt. iv2) then
        
          ! Two consecutive vertices form an element
          p_Iverts(1,iel) = ivt
          p_Iverts(2,iel) = ivt+1
          iel = iel + 1
          ...
          ! Closed component
          p_Iverts(1,iel) = ivt
          p_Iverts(2,iel) = ivtstart
          iel = iel + 1
          
        else
        
          ! Open component. The start and end vertex is each a
          ! boundary vertex and component.
          p_IbcVerts(ibcidx) = ivtstart
          p_IbcVerts(ibcidx+1) = ivt
          
          ! Nodal property is the boundary component number = index.
          p_Inpr(ivtstart) = ibcidx
          p_Inpr(ivt) = ibcidx+1
    ...
    ! Initialise some statistical data.
    rtriangulation%NVT = nvt
    rtriangulation%NEL = nel
    rtriangulation%InelOfType(TRIA_NVELINE1D) = nel
    rtriangulation%NNVE = 2
...
!<subroutine>
  ...
  subroutine tria_createRawBdryTria2D(rtriangulation, rsourceTria, ibc)

!<description>
  ! This routine crteates a 1D boundary triangulation from the vertices on
  ! the boundary of a 2D mesh. It may consist of multiple
  ! components, each component closed.
!</description>

!<input>
  ! Source triangulation, 2D
  type(t_triangulation), intent(in) :: rsourceTria

  ! OPTIONAL: Number of the boundary component to extract. If not
  ! specified, a boundary mesh for all components is created.
  integer, intent(in), optional :: ibc
!</input>

!<output>
  ! target triangulation. A RAW mesh is created here.
  type(t_triangulation), intent(out) :: rtriangulation
!</output>

!</subroutine>

    integer :: ibc1,ibc2,ibct,nvt,ivt,ivtout
    integer, dimension(:), pointer :: p_IverticesAtBoundary,p_IboundaryCpIdx
    real(DP), dimension(:,:), pointer :: p_DvertexCoords
    real(DP), dimension(:,:), allocatable :: p_Dpoints
    integer, dimension(:), allocatable :: p_Iidx

    ! Cancel if nothing to do.
    if (rsourceTria%nvbd .eq. 0) return

    if (rsourceTria%ndim .ne. NDIM2D) then
      call output_line ("Only 2D mesh supported", &
                        OU_CLASS_ERROR,OU_MODE_STD,"tria_createRawBdryTria2D")
      call sys_halt()
    end if
    
    ! Get some pointers
    call storage_getbase_int (rsourceTria%h_IboundaryCpIdx,p_IboundaryCpIdx)
    call storage_getbase_int (rsourceTria%h_IverticesAtBoundary,p_IverticesAtBoundary)
    call storage_getbase_double2d (rsourceTria%h_DvertexCoords,p_DvertexCoords)

    ! By default, process all boundary components.
    ibc1 = 1
    ibc2 = rsourceTria%NBCT
    ...
    ! Count the number of vertices in the output array.
    ! Add one for closing the polygon.
    nvt = 0
    do ibct = ibc1,ibc2
      nvt = nvt + p_IboundaryCpIdx(ibct+1)-p_IboundaryCpIdx(ibct)+1
    end do
    
    ! Allocate temp memory.
    allocate(p_Dpoints(2,nvt))
    allocate(p_Iidx(ibc2-ibc1+2))
    
    ! Get the point coordinates
    ivtout = 1
    do ibct = ibc1,ibc2
      ! Start index
      p_Iidx (ibct-ibc1+1) = ivtout
    
      ! Transfer the points
      do ivt = p_IboundaryCpIdx(ibct),p_IboundaryCpIdx(ibct+1)-1
        ! Coordinates of the next point
        p_Dpoints(1:2,ivtout) = p_DvertexCoords(1:2,p_IverticesAtBoundary(ivt))
        ivtout = ivtout + 1
      end do
      
      ! Last point is the first one.
      p_Dpoints(1:2,ivtout) = p_DvertexCoords(1:2,p_IverticesAtBoundary(p_IboundaryCpIdx(ibct)))
      ivtout = ivtout + 1
    end do
    
    ! Final index
    p_Iidx (ibc2-ibc1+2) = ivtout
    
    ! Create the mesh
    call tria_createRawManifTria1D(rtriangulation, p_Dpoints,p_Iidx)
    
    ! Release memory
    deallocate(p_Iidx)
    deallocate(p_Dpoints)
...
!</subroutine>
    ...
    case (NDIM1D)
      do iel=1,rtriangulation%NEL
        ...
        ! line element
        iv1 = p_IverticesAtElement(1,iel)
        iv2 = p_IverticesAtElement(2,iel)
        
        p_DelementVolume(iel) = abs(p_DvertexCoords(1,iv2)-p_DvertexCoords(1,iv1))
    ...
    case (NDIM2D)
      do iel=1,rtriangulation%NEL
        ...
        p_DelementVolume(iel) = &
            sqrt((p_DvertexCoords(1,iv2)-p_DvertexCoords(1,iv1))**2 + &
                 (p_DvertexCoords(2,iv2)-p_DvertexCoords(2,iv1))**2 )
    ...
    case (NDIM3D)
      do iel=1,rtriangulation%NEL
        ...
        p_DelementVolume(iel) = &
            sqrt((p_DvertexCoords(1,iv2)-p_DvertexCoords(1,iv1))**2 + &
                 (p_DvertexCoords(2,iv2)-p_DvertexCoords(2,iv1))**2 + &
                 (p_DvertexCoords(3,iv2)-p_DvertexCoords(3,iv1))**2 )
...
!</subroutine>
    ...
    if (present(rboundary) .and. (rtriangulation%NVBD .gt. 0)) then
...
!</subroutine>
    ...
    ! Cancel if there is nothing on the boundary
    if (rtriangulation%NVBD .eq. 0) return
