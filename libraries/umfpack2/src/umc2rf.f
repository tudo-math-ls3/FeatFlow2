        SUBROUTINE UMC2RF (N, NE, JOB, TRANSA, LVALUE, LINDEX, VALUE,
...
C  a prior factorization by UMC2FA or UMC2RF.  The matrix A should have
C  the same nonzero pattern as the matrix factorized by UMC2FA or
C  UMC2RF.  The matrix can have different numerical values.  No
C  variations are made in the pivot order computed by UMC2FA.  If a
...
C  factors of the matrix factorized by UMC2FA.  If BTF (block triangular
...
C
C  This routine must be preceded by a call to UMC2FA or UMC2RF.
C  A call to UMC2RF can be followed by any number of calls to UMC2SO,
C  which solves a linear system using the LU factors computed by this
C  routine or by UMC2FA.  A call to UMC2RF can also be followed by any
C  number of calls to UMC2RF.
...
C           the same, or different, as the last call to UMC2FA.
...
C           by the last call to UMC2FA or UMC2RF (Info (23)).
...
C               from the last call to UMC2FA or UMC2RF). Modified on
C               output.
...
C               used in the last call to UMC2FA).  On output, the new
C               LU factors needed for UMC2RF start here in Index.
...
C
C           Keep (6 ... 8):  Unused.  These are used by UMC2FA only.
...
C           Must be set by caller on input (not modified).
C           Control arguments, see UMC21I for a
C           description, which sets the default values.  The current
C           version of UMC2RF does not actually use Cntl.  It is
C           included to make the argument list of UMC2RF the same as
C           UMC2FA.  UMC2RF may use Cntl in future releases.

C           ------------------------------------------------------------
C  Icntl:   An integer array of size 20.
C           Must be set by caller on input (not modified).
C           Integer control arguments, see UMC21I for a description,
C           which sets the default values.  UMC2RF uses Icntl (1),
C           Icntl (2), Icntl (3), and Icntl (7).

C           ------------------------------------------------------------
C  Info:    An integer array of size 40.
C           Need not be set by caller on input.  Modified on output.
C           It contains information about the execution of UMC2RF.
C
...
C
C               Subsequent calls to UMC2RF and UMC2SO can only be made
...
C
C           Info (14): zero.  Used by UMC2FA only.
C
...
C               The factorization still proceeds, and UMC2SO can still
...
C               UMC2RF can be called if the value of Info (17) 
C               returned by UMC2FA was less than n, but the order
...
C               UMC2RF.
C
C           Info (24): zero.  Used by UMC2SO only.
C
...
C
C           Rinfo (3): zero.  Used by UMC2FA only.
C
...
C
C           Rinfo (7): zero.  Used by UMC2SO only.
C
C           Rinfo (8): zero.  Used by UMC2SO only.
C
C           Rinfo (9 ... 20): reserved for future releases

C=======================================================================
C  TO BE PRESERVED BETWEEN CALLS TO UMC2FA, UMC2RF, UMC2SO:
C=======================================================================
C
C  When calling UMC2SO to solve a linear system using the factors
C  computed by UMC2FA or UMC2RF, the following must be preserved:
C
C       n
C       Value (Keep (1)...Keep (2))
C       Index (Keep (3)...Keep (5))
C       Keep (1 ... 20)
C
C  When calling UMC2RF to factorize a subsequent matrix with a pattern
C  similar to that factorized by UMC2FA, the following must be
C  preserved:
C
C       n
C       Index (Keep (4)...Keep (5))
...
C## End of user documentation for UMC2RF ###############################
...
C       subroutines called:     UMC2ER, UMC2P1, UMC2CO, UMC2R0
...
C  luir1:   Index (luir1 ... lui2) is needed for this call to UMC2RF
...
C  n1:      N argument in UMC2FA or UMC2RF when prior matrix factorized 
C
C  Other:
C  ------
C  i:       loop index
...
C-----------------------------------------------------------------------
        ...
        IF (N .LT. 1) THEN 
C          n is too small
           CALL UMC2ER (2, ICNTL, INFO, -1, -1)
           GO TO 9000
        ENDIF 
        IF (NE .LT. 1) THEN 
C          ne is too small
           CALL UMC2ER (2, ICNTL, INFO, -2, -1)
           GO TO 9000
        ENDIF 

        IF (TRANSA) THEN 
C          transa must be false
           CALL UMC2ER (2, ICNTL, INFO, -8, 0) 
           GO TO 9000
        ENDIF 
...
C-----------------------------------------------------------------------
        ...
        IF (BADLU) THEN 
           CALL UMC2ER (2, ICNTL, INFO, -7, 0)
C          error return, LU factors are corrupted:
           GO TO 9000
        ENDIF 
        IF (2*NE .GT. LUIR1) THEN 
           CALL UMC2ER (2, ICNTL, INFO, -2, LUIR1/2)
...
C  lindex are unmodified from the prior call to UMC2FA, then
...
C       ne1 = Index (lindex), not required for UMC2RF
        N1 = INDEX (LINDEX-1)
        NZ1 = INDEX (LINDEX-2)
C       presr1 = Index (lindex-3) .ne. 0, not required for UMC2RF
        NBLKS = INDEX (LINDEX-4)
C       nzoff1 = Index (lindex-5), not required for UMC2RF
C       trans1 = Index (lindex-6) .ne. 0, not required for UMC2RF
...
C          set error flag if out of integer memory
           CALL UMC2ER (2, ICNTL, INFO, -3, IUSE)
        ENDIF 
        IF (LVALUE .LT. XUSE) THEN 
C          set error flag if out of real memory
           CALL UMC2ER (2, ICNTL, INFO, -4, XUSE)
        ENDIF 
        IF (INFO (1) .LT. 0) THEN 
...
C-----------------------------------------------------------------------
        ...
        CALL UMC2CO (N, NZ, TRANSA, VALUE, LVALUE, INFO, ICNTL,
...
40         CONTINUE 
     ...
     $          INDEX (CPERMP), INDEX (RPERMP), NE)
           IF (INFO (1) .LT. 0) THEN 
C             error return, if UMC2R0 fails
              GO TO 9000
           ENDIF 
