        SUBROUTINE UMC2S2 (N, JOB, TRANSC, LUXSIZ, LUX,
...
C       Cntl:           control parameters, see UMC21I
...
C
C       X (1..n):       solution
C       Info:           integer informational output, see UMC21I
C       Rinfo:          real informational output, see UMC21I
C
...
C=======================================================================
C
C       called by subroutine:   UMC2SO
C       subroutines called:     UMC2ER, UMC2SL, UMC2SU
C       functions called:       ICAMAX, ABS, MAX
        INTRINSIC ABS, MAX
        INTEGER ICAMAX
...
10            CONTINUE 
              IF (NBLKS .EQ. 1) THEN 
                 CALL UMC2SL (NLU, NPIV, N, LUI(6), LUI(6+NLU), LUX,X,Z)
              ELSE 
                 DO 20 BLK = 1, NBLKS 
                    K1 = BLKP (BLK)
                    K2 = BLKP (BLK+1) - 1
                    ...
                    IF (KN .GT. 1) THEN 
                       ...
                       NLU = LUI (P+1)
                       NPIV = LUI (P+2)
                       CALL UMC2SL (NLU, NPIV, KN, LUI (P+5),
...
C             ----------------------------------------------------------
              ...
              IF (NBLKS .EQ. 1) THEN 
                 DO 110 I = 1, N 
                    R (I) = B (I)
110              CONTINUE 
                 CALL UMC2SU (NLU, NPIV, N, LUI(6), LUI(6+NLU), LUX,R,Z)
              ELSE 
...
130                 CONTINUE 
     ...
     $                    LUI (P+5+NLU), LUX (LUI (P)), X (K1), Z)
                       DO 140 I = K1, K2 
                          R (I) = B (I) - X (I)
140                    CONTINUE 
                       CALL UMC2SU (NLU, NPIV, KN, LUI (P+5),
...
210                 CONTINUE 
                 ENDIF 
                 IF (NBLKS .EQ. 1) THEN 
                    CALL UMC2SL (NLU, NPIV, N,LUI(6),LUI(6+NLU),LUX,R,Z)
                    CALL UMC2SU (NLU, NPIV, N,LUI(6),LUI(6+NLU),LUX,R,Z)
                 ELSE 
