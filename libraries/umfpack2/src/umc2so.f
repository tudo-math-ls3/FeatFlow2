        SUBROUTINE UMC2SO (N, JOB, TRANSC, LVALUE, LINDEX, VALUE,
...
C=======================================================================
C
C  Given LU factors computed by UMC2FA or UMC2RF, and the
...
C           Must be the same as passed to UMC2FA or UMC2RF.
...
C           preserved (job = 1 in UMC2FA or UMC2RF).

C           ------------------------------------------------------------
C  transc:  A logical variable.
C           Must be set by caller on input (not modified) to .false.
C           The current version of this routine does not solve with the
C           transposed or complex-conjugate-transposed systems.
C
C           PAQ = LU was performed by the factorization routine.
C           The following systems are solved:
C
C                  job = 0      solve Ax = b    
C                  job = 1      solve P'Lx = b  
C                  job = 2      solve UQ'x = b  
C
C           Other values of job are treated as zero.  Iterative
C           refinement can be done only when solving Ax=b.
C
...
C           UMC2FA or UMC2RF) (not modified).
...
C               last call to UMC2FA or UMC2RF) (not modified).
C               Layout of the LU factors in Value and Index

C           ------------------------------------------------------------
C  B:       A complex array of size n.
C           Must be set by caller on input (not modified).
C           The right hand side, b, of the system to solve.

C           ------------------------------------------------------------
C  W:       A complex array of size 2*n or 4*n.
...
C           Must be set by caller on input (not modified).
C           real control parameters, see UMC21I for a description,
C           which sets the defaults.  

C           ------------------------------------------------------------
C  Icntl:   An integer array of size 20.
C           Must be set by caller on input (not modified).
C           Integer control parameters, see UMC21I for a description,
...
C           Need not be set by caller on input.  Modified on output.
C           It contains information about the execution of UMC2SO.
C
C           Info (1) is the error flag.  If Info (1) is -7, then
C           the LU factors are uncomputed, or have been corrupted since
C           the last call to UMC2FA or UMC2RF.  No system is solved,
...
C           preserved (job = 1 in UMC2FA or UMC2RF) and Ax=b or A'x=b
C           must be solved (job = 0 in UMC2SO).  Info (24) is the
...
C=======================================================================
C
C  The following must be unchanged since the call to UMC2FA or UMC2RF
...
C## End of user documentation for UMC2SO ###############################
...
C       subroutines called:     UMC2ER, UMC2P1, UMC2S2
...
C  nz:      entries in A
C  n1:      N argument in UMC2FA or UMC2RF when matrix factorized
C  ne:      NE argument in UMC2FA or UMC2RF when matrix factorized
C
...
C-----------------------------------------------------------------------
     ...
     $     .OR. LUI1 .LE. 0 .OR. LUIR1 .LT. LUI1 .OR. LUIR1 .GT. LUI2
        IF (BADLU) THEN 
           CALL UMC2ER (3, ICNTL, INFO, -7, 0)
C          error return, LU factors are corrupted:
           GO TO 9000
        ENDIF 

        IF (TRANSC) THEN 
C          transc must be false
           CALL UMC2ER (3, ICNTL, INFO, -8, 0) 
           GO TO 9000
        ENDIF 
...
C          -------------------------------------------------------------
C          original matrix not preserved, pass dummy argument to UMC2S2
C          -------------------------------------------------------------
...
C          original matrix not preserved (UMC2FA/UMC2RF job .ne. 1)
           CALL UMC2ER (3, ICNTL, INFO, 8, 0)
           IRSTEP = 0
        ENDIF 
        IF (IRSTEP .GT. 0 .AND. (JOB .EQ. 1 .OR. JOB .EQ. 2)) THEN 
C          iterative refinement for Ax=b and A'x=b only (job = 0)
           CALL UMC2ER (3, ICNTL, INFO, 8, 1)
           IRSTEP = 0
        ENDIF 
        IF (IRSTEP .EQ. 0) THEN 
C          pass a dummy argument as Y, which is not accessed in UMC2S2
           YP = 1
           LY = 1
           SP = 1
           LW = 2*N
        ELSE 
C          pass W (yp ... yp+n-1) as Y (1..n) to UMC2S2
           YP = 2*N+1
           LY = N
           SP = 3*N+1
           LW = 4*N
...
C-----------------------------------------------------------------------
        ...
        CALL UMC2S2 (N, JOB, TRANSC, LUX2-XP1+1, VALUE (XP1),
