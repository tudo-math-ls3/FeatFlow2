        SUBROUTINE UMC2R0 (N, NZ, CP, XX, XSIZE, II, ISIZE, XTAIL,
...
C=======================================================================
C
C       called by subroutine:   UMC2RF
C       subroutines called:     UMC2ER, UMC2R2, UMC2RA, UMC2OF
...
C  wrksiz:  total size of work arrays need in II for call to UMC2R2
C  xrmax:   memory needed in Value for next call to UMC2RF
C
...
C  dummy1:  argument returned by UMC2RA, but not needed
C  dummy2:  argument returned by UMC2RA, but not needed
...
C-----------------------------------------------------------------------
C  get memory usage estimate for next call to UMC2RF
C-----------------------------------------------------------------------
...
C          -------------------------------------------------------------
           ...
           IF (NBLKS .EQ. 1) THEN 
              IF (PRESRV) THEN 
                 CALL UMC2RA (PRESRV, N, NZ, CPERM, RPERM, II (PRP),
...
C                   ----------------------------------------------------
                    ...
                    CALL UMC2RA (PRESRV, N, NZ, CPERM, RPERM, OFFP,
...
C                         printed) in the call to UMC2OF below.
                       ENDIF 
60                  CONTINUE 
...
C                   error return, if not enough integer memory:
                 ...
                 CALL UMC2R2 (CP (K1), ARNZ, KN, XTAIL,
...
C                already deallocated in UMC2R2)
C                -------------------------------------------------------
...
C          set error flag if not enough integer memory
           CALL UMC2ER (2, ICNTL, INFO, -3, INFO (18))
        ENDIF 
        IF (XHEAD .GT. XTAIL) THEN 
C          set error flag if not enough real memory
           CALL UMC2ER (2, ICNTL, INFO, -4, INFO (21))
        ENDIF 

C       error return label, for error return from UMC2R2:
9010    CONTINUE

C       Cp can now be deallocated in UMC2RF:
        IUSE = IUSE - (N+1)
...
C       set warning flag if entries outside prior pattern are present
        IF (INFO (4) .GT. 0) THEN 
           CALL UMC2ER (2, ICNTL, INFO, 1, -INFO (4))
        ENDIF 

C       set warning flag if matrix is singular
        IF (INFO (1) .GE. 0 .AND. INFO (17) .LT. N) THEN 
           CALL UMC2ER (2, ICNTL, INFO, 4, INFO (17))
        ENDIF 

C       ----------------------------------------------------------------
C       return memory usage estimate for next call to UMC2RF
C       ----------------------------------------------------------------
