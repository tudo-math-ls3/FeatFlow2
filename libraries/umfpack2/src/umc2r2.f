        SUBROUTINE UMC2R2 (CP, NZ, N, XTAIL, XX, XSIZE, XUSE, ARI,
...
C=======================================================================
C
C  UMC2R2 refactorizes the n-by-n input matrix at the head of XX
...
C       subroutines called:     UMC2ER, UMC2P2, UMC2RG, CGEMV,
C                               CGEMM, CTRSV, CTRSM
...
C  overwritten, since UMC2R2 does not require dynamic allocation of
C  integer memory.
...
C  xruse:   estimated memory needed in XX for next call to UMC2RF,
...
C             ----------------------------------------------------------
              ...
              IF (FFSIZE .GT. XTAIL-XHEAD) THEN 
                 INFO (15) = INFO (15) + 1
                 CALL UMC2RG (XX, XSIZE, XHEAD, XTAIL, XUSE,
...
C                         get original row and column index and print it
                          ROW = RPERM (J)
                          COL = CPERM (I)
                          CALL UMC2P2 (2, 97, ROW, COL, XX (P), IO)
                       ENDIF 
                    ELSE 
                       XX (XP) = XX (XP) + XX (P)
                    ENDIF 
...
C                error return, if pivot order from UMC2FA not acceptable
                 GO TO 9010
              ENDIF 
              PIV = 1 / PIV
...
C             ----------------------------------------------------------
              ...
              IF (K-K0 .GE. NB .OR. K .EQ. LUK) THEN 
                 CALL CTRSM ('L', 'U', 'N', 'U', K-K0, FFLEFR, ONE,
     $                      XX (FFPP), FFDIMC,
     $                      XX (FFRP), FFDIMC)
                 TMP = FFLEFR
                 TMP = TMP * (K-K0-1)
...
C                      get original row and column index
                       ...
                       COL = CPERM (-J)
                    ENDIF 
                    CALL UMC2P2 (2, 95, ROW, COL, XX(P), IO)
610              CONTINUE 
620           CONTINUE 
           ENDIF 
...
C       error return label:
9010    CONTINUE
C       original pivot order computed by UMC2FA is no longer acceptable
        CALL UMC2ER (2, ICNTL, INFO, -6, 0)
        RETURN
        END 
