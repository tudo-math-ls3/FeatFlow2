        SUBROUTINE UMC2FA (N, NE, JOB, TRANSA, LVALUE, LINDEX, VALUE,
...
C
C  This routine is normally preceded by a call to UMC21I to
C  initialize the default control parameters.  UMC21I need only be
C  called once.  A call to UMC2FA can be followed by any number of
C  calls to UMC2SO, which solves a linear system using the LU factors
C  computed by this routine.  A call to UMC2FA can also be followed by
C  any number of calls to UMC2RF, which factorizes another matrix with
C  the same nonzero pattern as the matrix factorized by UMC2FA (but with
...
C  NOTE:" comment in UMC2FB.
C
C  To permamently disable any diagnostic and/or error printing, see
C  the "INSTALLATION NOTE:" comments in UMC2P1 and UMC2P2.
C
C  To change the default control parameters, see the
C  "INSTALLATION NOTE:" comments in UMC21I
...
C           in UMC2SO, (Icntl (8) > 0), then job must be set to 1.
...
C               Keep (4): LU factors needed for UMC2RF start here
C                             in Index
...
C               user.  See UMC21I for details, which sets the defaults.
...
C           Must be set by caller on input (not modified).
C           real control arguments, see UMC21I for a description,
C           which sets the defaults. UMC2FA uses Cntl (1) and Cntl (2).

C           ------------------------------------------------------------
C  Icntl:   An integer array of size 20.
C           Must be set by caller on input (not modified).
C           Integer control arguments, see UMC21I for a description,
C           which sets the defaults.  UMC2FA uses Icntl (1..7).

C           ------------------------------------------------------------
C  Info:    An integer array of size 40.
C           Need not be set by caller on input.  Modified on output.
C           It contains information about the execution of UMC2FA.
C
...
C               in Info (1).
C
C           Info (4): zero.  Used by UMC2RF only.
C
...
C## End of user documentation for UMC2FA ###############################
...
C       subroutines called:     UMC2ER, UMC2P1, UMC2CO, UMC2F0
...
C  luir1:   Index (luir1 ... lindex) must be preserved for UMC2RF
C
C  Memory usage:
C  -------------
...
C-----------------------------------------------------------------------
        ...
        IF (N .LT. 1) THEN 
C          n is too small
           CALL UMC2ER (1, ICNTL, INFO, -1, -1)
           GO TO 9000
        ENDIF 
        IF (NE .LT. 1) THEN 
C          ne is too small
           CALL UMC2ER (1, ICNTL, INFO, -2, -1)
           GO TO 9000
        ENDIF 

        IF (TRANSA) THEN 
C          transa must be false
           CALL UMC2ER (1, ICNTL, INFO, -8, 0) 
           GO TO 9000
        ENDIF 
...
C          set error flag if out of integer memory:
           CALL UMC2ER (1, ICNTL, INFO, -3, IUSE)
        ENDIF 
        IF (LVALUE .LT. XUSE) THEN 
C          set error flag if out of real memory:
           CALL UMC2ER (1, ICNTL, INFO, -4, XUSE)
        ENDIF 
        IF (INFO (1) .LT. 0) THEN 
...
30         CONTINUE 
     ...
     $          PRESRV, INDEX, INDEX (N+2), VALUE, N, NZ, KEEP, NE)
           IF (INFO (1) .LT. 0) THEN 
C             error return, if UMC2F0 fails
              GO TO 9000
           ENDIF 
...
C       (nzoff) for UMC2RF:
        LUIR1 = LUI1
        IF (PRESRV) THEN 
C          do not need preserved matrix for UMC2RF
           LUIR1 = LUIR1 + N+1 + NZ
        ENDIF 
        IF (NBLKS .GT. 1) THEN 
C          do not need off-diagonal part for UMC2RF
           LUIR1 = LUIR1 + NZOFF
        ENDIF 
