        SUBROUTINE UMC2CO (N, NZ, TRANSA, XX, XSIZE, INFO, ICNTL,
     $          II, ISIZE, W, WP, WHO)
...
C       who:            who called UMC2CO, 1: UMC2FA, 2: UMC2RF
C
...
C=======================================================================
C
C       called by subroutines:  UMC2FA, UMC2RF
C       subroutines called:     UMC2ER, UMC2P2
...
C                print the offending entry on the diagnostic I/O unit
                 CALL UMC2P2 (WHO, 99, ROW, COL, XX(P), IO)
              ENDIF 
              II (P)    = II (NZ1)
              II (NZ+P) = II (NZ+NZ1)
              XX (P)    = XX (NZ1)
...
C          invalid entries found - set warning flag and continue
           CALL UMC2ER (WHO, ICNTL, INFO, 1, NINVLD)
        ENDIF 
...
C                   the input matrix.
                    ...
                    CALL UMC2P2 (WHO, 98, ROW, COL, XX (P), IO)
...
C          duplicate entries found - set warning flag and continue
           CALL UMC2ER (WHO, ICNTL, INFO, 2, NDUPL)
        ENDIF 
        NZ = NZ1
...
C          set error flag if all entries are invalid
           CALL UMC2ER (WHO, ICNTL, INFO, -2, -1)
        ENDIF 
        RETURN
        END 
