        SUBROUTINE UMZ2R2 (CP, NZ, N, XTAIL, XX, XSIZE, XUSE, ARI,
...
C=======================================================================
C
C  UMZ2R2 refactorizes the n-by-n input matrix at the head of XX
...
C       subroutines called:     UMZ2ER, UMZ2P2, UMZ2RG, ZGEMV,
C                               ZGEMM, ZTRSV, ZTRSM
...
C  overwritten, since UMZ2R2 does not require dynamic allocation of
C  integer memory.
...
C  xruse:   estimated memory needed in XX for next call to UMZ2RF,
...
C             ----------------------------------------------------------
              ...
              IF (FFSIZE .GT. XTAIL-XHEAD) THEN 
                 INFO (15) = INFO (15) + 1
                 CALL UMZ2RG (XX, XSIZE, XHEAD, XTAIL, XUSE,
...
C                         get original row and column index and print it
                          ROW = RPERM (J)
                          COL = CPERM (I)
                          CALL UMZ2P2 (2, 97, ROW, COL, XX (P), IO)
                       ENDIF 
                    ELSE 
                       XX (XP) = XX (XP) + XX (P)
                    ENDIF 
...
C                that u1 is stored with rows in reverse order.
                 CALL ZTRSV ('U', 'N', 'U', K-K0-1,
     $                         XX (FFPP         ), FFDIMC,
     $                         XX (FFPP - FFDIMC), 1)
                 TMP = K-K0-2
                 TMP = TMP * (K-K0-1)
                 RINFO (5) = RINFO (5) + 8.0*(TMP) / 2.0
              ENDIF 
              IF (K-K0-1 .GT. 0) THEN 
C                l1 = l1 - L2*u1
                 CALL ZGEMV ('N', FFLEFC, K-K0-1,
     $                   -ONE, XX (FFCP         ), FFDIMC,
     $                         XX (FFPP - FFDIMC), 1,
...
C                error return, if pivot order from UMZ2FA not acceptable
                 GO TO 9010
              ENDIF 
              PIV = 1 / PIV
...
C             ----------------------------------------------------------
              ...
              IF (K-K0 .GE. NB .OR. K .EQ. LUK) THEN 
                 CALL ZTRSM ('L', 'U', 'N', 'U', K-K0, FFLEFR, ONE,
     $                      XX (FFPP), FFDIMC,
     $                      XX (FFRP), FFDIMC)
                 TMP = FFLEFR
                 TMP = TMP * (K-K0-1)
                 TMP = TMP * (K-K0)
                 RINFO (6) = RINFO (6) + 8.0*(TMP) / 2.0
...
C                      get original row and column index
                       ...
                       COL = CPERM (-J)
                    ENDIF 
                    CALL UMZ2P2 (2, 95, ROW, COL, XX(P), IO)
610              CONTINUE 
620           CONTINUE 
           ENDIF 
...
C       error return label:
9010    CONTINUE
C       original pivot order computed by UMZ2FA is no longer acceptable
        CALL UMZ2ER (2, ICNTL, INFO, -6, 0)
        RETURN
        END 
