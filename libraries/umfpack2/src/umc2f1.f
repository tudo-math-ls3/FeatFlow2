        SUBROUTINE UMC2F1 (CP, N, CPERM, RPERM, NZOFF,
...
C=======================================================================
C
C  UMC2F1 factorizes the n-by-n column-form matrix at the head of II/XX
...
C       rmax:           maximum ludegr seen so far (see UMC2F2 for info)
C       cmax:           maximum ludegc seen so far (see UMC2F2 for info)
...
C                       See UMC2F2 for a description.
C
...
C
C       iout:           true if ran out of integer memory in UMC2F1
C       xout:           true if ran out of real memory in UMC2F1
...
C  Memory usage:
C  -------------
C  wrksiz:  size of integer workspace needed by UMC2F2
C
...
C          Cperm (1..n) is used as WiC (1..n) in UMC2F2
           WRKSIZ = 8*N + 3*DN
        ELSE 
           WRKSIZ = 10*N + 3*DN
        ENDIF 
...
C          pass Rperm and Cperm as the WiR and WiC arrays in UMC2F2:
           CALL UMC2F2 (CP, NZ, N, 1, 1, 1, ITAIL, XTAIL,
...
C          final permutations (including BTF and numerical pivoting).
           CALL UMC2F2 (CP, NZ, N, N, CPERM, RPERM, ITAIL, XTAIL,
     $          XX (XP), XSIZE-XP+1, II (IP), ISIZE-IP+1, ICNTL, CNTL,
     ...
     $          DN, DSIZ, KEEP, RMAX, CMAX, TOTNLU, XRMAX, XRUSE)
        ...
        IF (INFO (1) .LT. 0) THEN 
C          error return, if error occured in UMC2F2:
           RETURN
        ENDIF 
