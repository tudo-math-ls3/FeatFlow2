...
* BSNGL    LOG    no meaning here                                      *
* ARR      C*6    Names of blocks (for error messages only)            *
* For the description of the remaining parameters see AB17             *
*                                                                      *
* OUTPUT   TYPE                                                        *
* ------   ----                                                        *
* LA       I*4    Vector of numbers of arrays on VA                    *
...
* INPUT    TYPE                                                        *
* -----    ----                                                        *
* KCOL     I*4    Pointer vectors for each matrix on VA corresponding  *
...
* OUTPUT   TYPE                                                        *
* ------   ----                                                        *
* VA       R*8    Calculated matrices                                  *
* IER      I*4    Error indicator                                      *
*                 -116 Wrong value in array KAB                        *
*                                                                      *
************************************************************************
C
      SUBROUTINE XAB17(LA,LCOL,LLD,NA,NEQ,NBLOC,ICLEAR,ELE,
...
C
      SUB='XAB17'
      IF (ICHECK.GE.997) CALL OTRC('XAB17 ','06/18/93')
      IER=0
C
      DO 1 IBLOC=1,NBLOC
      IF (LA(IBLOC).EQ.0) THEN
       CALL ZNEW(NA,2,LA(IBLOC),ARR(IBLOC))
...
3     CONTINUE
C
      CALL AB17(VWORK(1),KWORK(L(LCOL)),KWORK(L(LLD)),NA,NEQ,
...
C
C
      SUBROUTINE AB17(VA,KCOL,KLD,NA,NEQ,NBLOC,KOFF,KVERT,KEDGE,KAREA,
...
C
      SUB='AB17'
      IF (ICHECK.GE.997) CALL OTRC('AB17  ','06/18/93')
C
...
303   DB(IDIM)=DBAS(IDIM,KDFL(JDOFE),IA)
      ...
      DO 304 IDIM=1,NDIM
      DAHH=DB(IDIM)*DBAS(IDIM,KDFL(IDOFE),IB)
304   DAH(JDOFE,IDOFE)=DAH(JDOFE,IDOFE)+DAHH*AUX
302   CONTINUE
301   CONTINUE
C
      DO 310 JDOFE=1,IDFL
...
c      IF (ABS(DAH(JDOFE,IDOFE)).LT.1D-15) DAH(JDOFE,IDOFE)=0D0
      VA(JCOLB)=VA(JCOLB)+REAL(DAH(JDOFE,IDOFE))
310   CONTINUE
C
      BFIRST=.FALSE.
300   CONTINUE
