...
C
      ICOUNT=KLEN(LNR)
      ILNR= L(LNR)
      ILNR4V=(L(LNR)-NREMV)*NV2D+NREMV
      ILNR4I=(L(LNR)-NREMI)*NI2D+NREMI
C
C
...
121   VWORK(ILNR4V+I)=REAL(DWORK(ILNR+I))
      L(LNR)=ILNR4V
      KLEN(LNR)=NV2D*KLEN(LNR)
      GOTO 140
130   DO 131 I=0,ICOUNT-1
131   KWORK(ILNR4I+I)=INT(DWORK(ILNR+I))
      L(LNR)=ILNR4I
      KLEN(LNR)=NI2D*KLEN(LNR)
140   CONTINUE      
      KTYPE(LNR)=ITYPE
