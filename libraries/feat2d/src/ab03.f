...
C
      SUB='XAB03'
      IF (ICHECK.GE.997) CALL OTRC('XAB03 ','12/04/90')
      IER=0
C
C
      DO 1 IBLOC=1,NBLOC
...
C
C
      SUBROUTINE AB03(DA,KDIA,KDIAS,NDIA,NA,NEQ,NBLOC,KOFF,KVERT,
     *                KMID,DCORVG,ELE,COEFF,BCON,
     *                COECON,KAB,KABN,ICUB,ISYMM)
C
      IMPLICIT DOUBLE PRECISION (A,C-H,O-U,W-Z),LOGICAL(B)
      CHARACTER SUB*6,FMT*15,CPARAM*120
C
...
C
      SUB='AB03'
      IF (ICHECK.GE.997) CALL OTRC('AB03  ','12/04/90')
C
...
4     CONTINUE
************************************************************************
C *** Calculation of the matrix - storage technique 3
************************************************************************
