...
* E061                                                                 *
*                                                                      *
* Purpose  Calculation of values and derivatives of element  61        *
*          Linear Hermite type element with 3 dof (quadrilat. element) *
*                                                                      *
...
* IPAR     I*4    Set to  61  if called with IPAR = -1                 *
...
C
      SUB='E061'
      IF (ICHECK.GE.998) CALL OTRC('E061  ','03/21/89')
C
      IER=0
...
C
3     CONTINUE
10    IF (ICHECK.EQ.0) GOTO 6100
      IF (DETJ.LT.0D0) CALL WERR(-130,'E061  ')
      IF (DETJ.GT.0D0.AND.DETJ.LT.1D-70) CALL WERR(-132,'E061  ')
C *** No second order derivatives available
C *** Used for second order problems only
      IF (BDER(4).OR.BDER(5).OR.BDER(6)) CALL WERR(-131,'E061  ')
      IF (IER.NE.0) GOTO 99999
C
6100  IF (.NOT.BDER(1)) GOTO 6101
C *** Function values
      DBAS(1,1)= 1D0
      DBAS(2,1)= XI1
      DBAS(3,1)= XI2
6101  IF (.NOT.(BDER(2).OR.BDER(3))) GOTO 99999
C
C *** First order derivatives
      XJ=1D0/DETJ
      IF (.NOT.BDER(2)) GOTO 6102
      DBAS(1,2)= 0D0
      DBAS(2,2)= XJ*DJAC(2,2)
      DBAS(3,2)=-XJ*DJAC(2,1)
6102  IF (.NOT.BDER(3)) GOTO 99999
      DBAS(1,3)= 0D0
      DBAS(2,3)=-XJ*DJAC(1,2)
      DBAS(3,3)= XJ*DJAC(1,1)
C
99999 END
