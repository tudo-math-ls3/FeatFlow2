...
C
      SUB='E031'
      IF (ICHECK.GE.998) CALL OTRC('E031  ','01/02/89')
C
      IER=0
...
C
3     CONTINUE
10    IF (ICHECK.EQ.0) GOTO 3100
      IF (DETJ.LT.0D0) CALL WERR(-130,'E031  ')
      IF (DETJ.GT.0D0.AND.DETJ.LT.1D-70) CALL WERR(-132,'E031  ')
C *** No second order derivatives available
C *** Used for second order problems only
      IF (BDER(4).OR.BDER(5).OR.BDER(6)) CALL WERR(-131,'E031  ')
      IF (IER.NE.0) GOTO 99999
C
3100  IF (.NOT.BDER(1)) GOTO 101
C
C *** Function values
      DBAS(1,1)=Q4*(-XI1**2+XI2**2-2D0*XI2+1D0)
      DBAS(2,1)=Q4*( XI1**2-XI2**2+2D0*XI1+1D0)
      DBAS(3,1)=Q4*(-XI1**2+XI2**2+2D0*XI2+1D0)
      DBAS(4,1)=Q4*( XI1**2-XI2**2-2D0*XI1+1D0)
