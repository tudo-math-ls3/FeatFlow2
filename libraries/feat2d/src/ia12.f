...
C
      ...
      SAVE /ERRCTL/
C
      IF (ICHECK.GE.998) CALL OTRC('IA123 ','11/19/90')
C
      DO 1 IEQ=1,NEQ
1     VX(IEQ)=VX(IEQ)/VA(IEQ)
C
      END
C
C
...
C
      ...
      SAVE /ERRCTL/
C
      IF (ICHECK.GE.998) CALL OTRC('IA127 ','01/02/89')
C
      DO 1 IEQ=1,NEQ
1     VX(IEQ)=VX(IEQ)/VA(KLD(IEQ))
C
      END
C
C
...
C
      IMPLICIT DOUBLE PRECISION (A,C-H,O-U,W-Z),LOGICAL(B)
      DIMENSION VA(*),KLD(*),VX(*),KOP(*)
      COMMON /ERRCTL/ IER,ICHECK
      SAVE /ERRCTL/
C
      IF (ICHECK.GE.998) CALL OTRC('IA12A ','01/02/89')
C
      DO 1 IEQ=1,NEQ
1     VX(IEQ)=VX(IEQ)/VA(KLD(KOP(IEQ)))
C
      END
