...
* E021                                                                 *
*                                                                      *
* Purpose  Calculation of values and derivatives of element  21        *
*          Mini element                                                *
*                                                                      *
...
* IPAR     I*4    Set to  21  if called with IPAR = -1                 *
...
************************************************************************
C
      SUBROUTINE E021 (XI1,XI2,XI3,IPAR)
C
...
C
      SUB='E021'
      IF (ICHECK.GE.998) CALL OTRC('E021  ','01/02/89')
C
      IER=0
...
C
3     CONTINUE
10    IF (ICHECK.EQ.0) GOTO 2100
      IF (DETJ.LT.0D0) CALL WERR(-130,'E021  ')
      IF (DETJ.GT.0D0.AND.DETJ.LT.1D-70) CALL WERR(-132,'E021  ')
C *** No second order derivatives available
C *** Used for second order problems only
      IF (BDER(4).OR.BDER(5).OR.BDER(6)) CALL WERR(-131,'E021  ')
      IF (IER.NE.0) GOTO 99999
C
C *** Function values
2100  IF (.NOT.BDER(1)) GOTO 2102
      DBAS(1,1)=XI1
      DBAS(2,1)=XI2
      DBAS(3,1)=XI3
      DBAS(4,1)=27D0*XI1*XI2*XI3
C
2102  XJ1= 1.0D0/DETJ
