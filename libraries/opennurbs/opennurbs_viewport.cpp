/* $Header: /src4/opennurbs/opennurbs_viewport.cpp 24    9/06/06 5:10p Dalelear $ */
/* $NoKeywords: $ */
/*
//
    ...
    while ( near_dist < 1.0001*min_near_over_far*far_dist )
    {
      ...
      if ( near_dist < target_dist && target_dist < far_dist )
      {
        ...
        if ( b > 0.0)
        {
          ...
          if ( s > 1.0 || s <= ON_ZERO_TOLERANCE )
          {
            ON_ERROR("ON_Viewport::SetFrustumNearFar aritimetic problem 1.");
            s = 1.0; // should never happen
          }
          ...
          if ( m < 0.95 || m > 1.05 )
          {
            ON_ERROR("ON_Viewport::SetFrustumNearFar aritimetic problem 2.");
          }

          if ( n < near_dist || n >= target_dist)
          {
            ON_ERROR("ON_Viewport::SetFrustumNearFar aritimetic problem 3.");
          ...
          {
            ON_ERROR("ON_Viewport::SetFrustumNearFar aritimetic problem 4.");
