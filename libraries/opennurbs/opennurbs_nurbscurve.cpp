/* $Header: /src4/opennurbs/opennurbs_nurbscurve.cpp 18    7/24/06 2:57p Dalelear $ */
/* $NoKeywords: $ */
/*
//
        ...
        ) const
{
  bool rc = (this == &other);
  if (    !rc 
       && m_V.Count() == other.m_V.Count()
       && m_E.Count() == other.m_E.Count()
       && m_L.Count() == other.m_L.Count()
       && m_F.Count() == other.m_F.Count()
       && (m_F.Count() > 0 || m_E.Count() > 0 || m_V.Count() > 0)
       )
  {
    // TODO
    /*
    ...
    for ( i = 0; i < count && rc; i++ )
    {
      rc = ON_IsDuplicateBrepVertex()
    }

    count = m_E.Count();
    for ( i = 0; i < count && rc; i++ )
    {
      rc = ON_IsDuplicateBrepEdge()
    }

    count = m_F.Count();
    for ( i = 0; i < count && rc; i++ )
    {
      rc = ON_IsDuplicateBrepFace()
    }
    */
  }
  return rc;
