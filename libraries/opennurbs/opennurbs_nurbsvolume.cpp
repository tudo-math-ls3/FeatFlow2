/* $Header: /src4/opennurbs/opennurbs_nurbsvolume.cpp 35    9/06/06 4:21p Dalelear $ */
/* $NoKeywords: $ */
/*
//
...
ON_3dPoint ON_MorphControl::MorphPoint( ON_3dPoint point ) const
{
  ...
  for ( i = m_localizers.Count()-1; i >= 0 && w > 0.0; i-- )
  {
  ...
  if ( w > 0.0 )
  {
    ...
    {
    case 1:
      if ( m_nurbs_curve0.GetClosestPoint(Q,&t) )
      {
        ON_3dVector D;
        ON_3dPoint B0, B;
        ON_Interval d = m_nurbs_curve0.Domain();
        if ( (t <= d[0] || t >= d[1]) && !m_nurbs_curve0.IsClosed() )
        {
          // use C1 extension
          m_nurbs_curve0.Ev1Der(t,B0,D);
          if ( distloc )
            w *= distloc->Value(B0.DistanceTo(point));
          s = D*D;
          if ( s > 0.0 )
          {
            s = ((Q-B0)*D)/s;
          ...
          m_nurbs_curve.Ev1Der(t,B,D);
          B = B + s*D;
        }
        else
        {
          B0 = m_nurbs_curve0.PointAt(t);
          B  = m_nurbs_curve.PointAt(t);
          if ( distloc )
            w *= distloc->Value(B0.DistanceTo(point));
        }
        D = Q - B0;
    ...
    case 2:
      if ( m_nurbs_surface0.GetClosestPoint(Q,&s,&t) )
      {
        ON_3dVector Du, Dv, D, N;
        ON_3dPoint B0, B;
        ON_Interval d0 = m_nurbs_surface0.Domain(0);
        ON_Interval d1 = m_nurbs_surface0.Domain(1);
        bool bDuTweak = ((s <= d0[0] || s >= d0[1]) && !m_nurbs_surface0.IsClosed(0));
        bool bDvTweak = ((t <= d1[0] || t >= d1[1]) && !m_nurbs_surface0.IsClosed(1));
        if ( bDuTweak || bDvTweak )
        {
          // use C1 extension
          double e, pr, du=0.0, dv=0.0;
          m_nurbs_surface0.EvNormal(s,t,B0,Du,Dv,N);
          if ( distloc )
            w *= distloc->Value(B0.DistanceTo(point));
          D = (Q - B0);
          e = D*N;
          ...
          if ( pr != 0.0 )
          {
            if ( bDuTweak )
            {
              if ( bDvTweak )
              {
                if ( 2 != ON_Solve3x2(Du,Dv,D[0],D[1],D[2],&du,&dv,&e,&pr) )
                {
                  du = dv = 0.0;
                }
              }
          ...
          B0 = B0 + du*Du + dv*Dv;
          m_nurbs_surface.Ev1Der(s,t,B,Du,Dv);
          B = B + du*Du + dv*Dv;
        }
        else
        {
          B0 = m_nurbs_surface0.PointAt(s,t);
          B  = m_nurbs_surface.PointAt(s,t); 
          if ( distloc )
            w *= distloc->Value(B0.DistanceTo(point));
        }
        D = Q - B0;
