/* $Header: /src4/opennurbs/opennurbs_archive.cpp 40    9/11/06 9:09a Dalelear $ */
/* $NoKeywords: $ */
/*
//
...
ON_BinaryArchive::~ON_BinaryArchive()
{
  if ( ON::read3dm == m_mode
       ...
       && 1 == m_3dm_version
       && 0 == m_zlib.mode 
       && !memcmp("BD7CDDCB-D3DB-45d0-832D-A59082C1BE4D",&m_zlib.buffer[0], 36)
       && !memcmp("BD7CDDCB-D3DB-45d0-832D-A59082C1BE4D",&m_zlib.buffer[44],36) )
  {
    m_zlib.buffer[0] = 0;
    m_zlib.buffer[44] = 0;
    struct ON__3dmV1LayerIndex* p;
    struct ON__3dmV1LayerIndex* next;
    memcpy(&next,&m_zlib.buffer[36],sizeof(next));
    int i;
    for ( i = 0; 0 != next && i < 1000; i++ )
    {
      p = next;
...
int ON_BinaryArchive::Read3dmV1LayerIndex(const char* sV1LayerName) const
{
  ...
  // "BD7CDDCB-D3DB-45d0-832D-A59082C1BE4D" // 36
  // guid used to insure that value in m_zlib.buffer[] is a pointer
  // V4 SDK - add a proper private member to ON_BinaryArchive to save
  // the V1 layer list.
  ...
  {
    struct ON__3dmV1LayerIndex* p;
    memcpy(&p,&m_zlib.buffer[36],sizeof(p));
    int i;
...
bool ON_BinaryArchive::Read3dmV1Layer( ON_Layer*& layer )
{
  ...
  else if (rc && layer)
  {
         ...
         )
    {
      ...
      // To avoid breaking the V3 SDK, the pointer to the head of the list
      // is saved in unused m_zlib.buffer[] memory. (V1 files have no compression.)
      // V4 SDK TODO - add a member to ON_BinaryArchive and do this in a more
      // transparent manner.
      int s_length = s.Length();
      const char* s_name = s.Array();
      if ( layer->LayerIndex() >= 0 && s_length > 0 && s_length < 256 && 0 != s_name && 0 != s_name[0] )
      {
        if ( 1 == m_3dm_v1_layer_index )
        {
          if (   !memcmp("BD7CDDCB-D3DB-45d0-832D-A59082C1BE4D",&m_zlib.buffer[0], 36)
               || !memcmp("BD7CDDCB-D3DB-45d0-832D-A59082C1BE4D",&m_zlib.buffer[44],36))
          {
          ...
          {
            // first layer - initialize the m_zlib.buffer[] space we are using
            // until the V4 SDK 
            memcpy(&m_zlib.buffer[0], "BD7CDDCB-D3DB-45d0-832D-A59082C1BE4D",36);
            memset(&m_zlib.buffer[36],0,8);
            memcpy(&m_zlib.buffer[44],"BD7CDDCB-D3DB-45d0-832D-A59082C1BE4D",36);
          }
        }
        if (   !memcmp("BD7CDDCB-D3DB-45d0-832D-A59082C1BE4D",&m_zlib.buffer[0], 36)
             && !memcmp("BD7CDDCB-D3DB-45d0-832D-A59082C1BE4D",&m_zlib.buffer[44],36))
        {
          ...
          memcpy(&p->m_next,&m_zlib.buffer[36],sizeof(p));
          p->m_layer_name = (char*)(p+1);
          p->m_layer_index = layer->LayerIndex();
          p->m_layer_name_length = s_length;
          memcpy(p->m_layer_name,s_name,s_length);
          p->m_layer_name[s_length] = 0;
          memcpy(&m_zlib.buffer[36],&p,sizeof(p));
        }
      }
    }
  }
  ...
  )
{
  ...
  if ( tcode == TCODE_RHINOIO_OBJECT_DATA ) for (;;) {
    ...
    for ( i = 0; i < sz; i++ ) {
      ...
      if ( !ReadInt( &trim.m_bRev3d ) ) break;
      if ( !ReadInt( &j ) ) break;
      switch(j) {
