/* $Header: /src4/opennurbs/opennurbs_brep_io.cpp 4     5/20/05 8:36a Dalelear $ */
/* $NoKeywords: $ */
/*
//
...
BOOL ON_BrepFace::Write( ON_BinaryArchive& file ) const
{
  ...
  if ( rc )
    rc = file.WriteInt( -1 ); // OBSOLETE m_material_index
  return rc;
}
...
BOOL ON_BrepFace::Read( ON_BinaryArchive& file )
{
  ...
  if ( rc )
    rc = file.ReadInt( &m_bRev );
  if ( rc )
    rc = file.ReadInt( &i ); // OBSOLETE m_material_index
  return rc;
}
...
bool ON_Brep::ReadOld200( ON_BinaryArchive& file, int minor_version )
{
  ...
  for ( fi = 0; rc && fi < face_count; fi++ ) 
  {
    ...
    if (rc) rc = file.ReadInt( &f.m_bRev );
    ...
    for ( fbi = 0; rc && fbi < fbcnt; fbi++ ) {
      ...
      for ( bti = 0; rc && bti < btcnt; bti++ ) {
        ...
        if (rc) rc = file.ReadInt(&trim.m_bRev3d);
