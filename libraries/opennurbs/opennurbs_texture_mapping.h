/* $Header: /src4/opennurbs/opennurbs_texture_mapping.h 17    9/05/06 11:32a Dalelear $ */
/* $NoKeywords: $ */
/*
//
...
public:
  ...
  /*
  Description:
    Create a mapping that will convert surface paramters into texture
    coordinates.
  Parameters:
    surface_u_domain - [in] Increasing surface "u" domain.
    surface_v_domain - [in] Increasing surface "v" domain.
    texture_u_domain - [in] Texture domain (can be decreasing)
    texture_v_domain - [in] Texture domain (can be decreasing)
    bRotateTexture - [in] 
  Example:
    Get the "canonical" texture coordinates for an ON_Mesh

          ON_Mapping srfuv_mapping;
          ON_Mesh* mesh = ...;
          const int vcount = mesh->VertexCount()
          ON_2dPointArray T(vcount);
          if ( srfuv_mapping.CreateSurfaceParameterMapping(
                    mesh->m_srf_domain[0],
                    mesh->m_srf_domain[1],
                    mesh->m_tex_domain[0],
                    mesh->m_tex_domain[1],
                    mesh->m_srf_tex_rotate) )
          {
            T.Reserve(vcount);
            T.SetCount(vcount);
            if (!srfuv_mapping.GetTextureCoordinates(mesh,2,2,&T[0].x))
              T.SetCount(0);
          }

  Remarks:
    Tiling and offsets ares easily accomplished using the 
    texture_u_domain and texture_u_domain paramters.  For example
    if you want to have 13x17 tiles and offset the u tiles by 1/4,
    then you would use

        texture_u_domain = (0.25, 13.25)
        texture_v_domain = (0.0, 17.0)

    You can reverse texture orientations by using decreasing intervals
    for the texture_domains.  For example, if you want the u direction
    reversed you would use

        texture_u_domain = (1.0, 0.0)

  Returns:
    True if input is valid.
  See Also:
    ON_Mesh::GetSurfaceParameterTextureMapping
  */
  bool CreateSurfaceParameterMapping( 
  ...
  Description:
    ...
    texture coordinates.
  Remarks:
    This does not change the If you are 
	*/
...
public:
  ...
  // ON_Mesh::m_mapping_id = ON_TextureMapping::m_default_srfp_mapping_id
  // when the m_T[] values are the default texture coordinates
  // produced by the OpenNURBS parameteric surface meshers like
  // ON_Surface::CreateMesh() and ON_Brep::CreateMesh(). 
  // {B988A6C2-61A6-45a7-AAEE-9AED7EF4E316}
  static const ON_UUID m_default_srfp_mapping_id;
};
