/* $Header: /src4/opennurbs/opennurbs_3dm_attributes.cpp 21    9/06/06 11:56a Dalelear $ */
/* $NoKeywords: $ */
/*
//
...
bool ON_3dmObjectAttributes::operator==(const ON_3dmObjectAttributes& other) const
{
  ...
  if ( m_bSpecialViewport != other.m_bSpecialViewport)
    return false;
        ...
        if ( rc && minor_version >= 3 )
        {
          ...
          if (rc && minor_version >= 4 )
          {
            ...
            if (rc && minor_version >= 5 )
            {
              ...
              {
                rc = file.ReadBool(&m_bSpecialViewport);
                if (rc)
                {
                  m_dmref.Empty();
                  int i, count=0;
...
BOOL ON_3dmObjectAttributes::Write( ON_BinaryArchive& file ) const
{
  ...
  // version 1.6 fields 2 September 2005
  if (rc) file.WriteBool(m_bSpecialViewport);
  if (rc)
  {
...
int  ON_3dmObjectAttributes::GetDisplayMaterialRefList(
  ON_SimpleArray<ON_DisplayMaterialRef>& list
  ) const
{
  list.Append(m_dmref.Count(),m_dmref.Array());
  return m_dmref.Count();
}
