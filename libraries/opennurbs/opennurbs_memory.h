/* $Header: /src4/opennurbs/opennurbs_memory.h 4     9/22/05 2:52p Dalelear $ */
/* $NoKeywords: $ */
/*
//
...
ON_DECL
void ON_OpenNURBS_Begin(void); // call to begin using OpenNURBS

ON_DECL
void ON_OpenNURBS_End(void);   // call to finish using OpenNURBS
...
struct tagON_MEMORY_POOL
{
  ...
  */
  ...
  // worker thead and m_h is the thread's handle.
  */
  unsigned long m_h;
  ...
  //  >  1: pool is or was a worker thread pool.  If m_h is nonzero,
  //        the worker thread is still active.  If m_h is zero, the
  //        worker thread successfully completed its task and created
  //        objects that use memory from this pool.
  //  < -1: pool was a worker thread pool and the pool is no longer
  //        in use.  This can happen because the worker thread was
...
ON_DECL
ON_MEMORY_POOL* ON_SetCurrentMemoryPool( ON_MEMORY_POOL* ); // returns pointer to pool you are replacing

ON_DECL
ON_MEMORY_POOL* ON_GetCurrentMemoryPool(void);

ON_DECL
ON_MEMORY_POOL* ON_SetMainMemoryPool( ON_MEMORY_POOL* ); // returns pointer to pool you are replacing
