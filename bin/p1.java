...
class p1
{
  ...
  public static void main(String args[])
  {
    ...
    switch (args.length)
    {
	  ...
	  break;
      // three given: parsing mode given and file name and directory prefix to store output files
      default:
	  if (args[0].equals("html"))
	  {
	    System.out.println(progname + ": HTML output selected.");
	  ...
	  dirprefix = args[2];
	  System.out.println(progname + ": Using prefix <" + dirprefix + "> for output directory.");
	  break;
    }
	...
	basename = dirprefix + "/" + filename.substring(0, filename.lastIndexOf("."));
      }
...
//           out.writeBytes("<H1>FEAT2 Dokumentation</H1>\n<br><br>");
        ...
        if (type.equals("name"))
        {
          ...
          switch(modus)
          {
            case FORMAT_HTML:
              ...
              break;
            case FORMAT_TEXMOD:
	      // escape any underscore for LaTeX
	      value1 = value1.replaceAll("_", "\\\\_");
              out.writeBytes("\\subsubsection{Module " + value1 + "}\n");
              break;
            case FORMAT_TEX:
	      // escape any underscore for LaTeX
	      value1 = value1.replaceAll("_", "\\\\_");
	  ...
	  case FORMAT_HTML:
	      ...
	      break;
	  case FORMAT_TEX:
	      out.writeBytes("{\\bf Purpose:}\n");
	      break;
	  }

	  for (int i11 = 0; i11 < lists1.getLength(); i11++)
	  {
            String type1 = lists1.item(i11).getNodeName();

	    if (type1.equals("#text"))
	    {
	      String value1 = lists1.item(i11).getNodeValue();
		  ...
		  break;
            ...
            else if (type1.equals("tex")) {
	      String value1 = lists1.item(i11).getChildNodes().item(0).getNodeValue();
	      //	      value1 = value1.replaceAll("\n","");
	      value1 = value1.replaceAll("!#","");
	      value1 = value1.replaceAll("#","");
	      value1 = value1.trim();
	  ...
	  switch(modus)
	  {
        ...
        if (type.equals("subroutine"))
        {
	      ...
	      if (patternSubroutine.matcher(value1).matches())
              {
	        // determine name of subroutine:
		// everything after "subroutine" and before any opening parenthesis
		String mame = value1.substring(value1.toLowerCase().indexOf("subroutine") + 10).trim();

		// There is no guarantee we have a "()" after a subroutine definition.
		int pos = mame.indexOf("(");
		if (pos > 0) {
		    mame = mame.substring(0, pos).trim();
		}
		...
		// everything after "function" and before any opening parenthesis
                String mame = value1.substring(value1.toLowerCase().indexOf("function") + 9).trim();

		// There is no guarantee we have a "()" after a function definition.
		int pos = mame.indexOf("(");
		if (pos > 0) {
		    mame = mame.substring(0, pos).trim();
		}
            ...
            if (type1.equals("constantblock"))
            {
              ...
              if (varset==0)
              {
                switch(modus)
                {
                  ...
                  case FORMAT_TEX:
                    out.writeBytes("\\\\\n");
                    if (! constdescr.equals("")) out.writeBytes("\\multicolumn{4}{l}{Purpose: " + constdescr + "}\\\\\n");
     ...
     //     break;
      ...
      for (int i = 0; i < lists.getLength(); i++)
      {
		...
		// There is no guarantee we have a "()" after a subroutine definition.
		int pos = mame1.indexOf("(");
		if (pos > 0) {
		    mame1 = mame1.substring(0, pos).trim();
		}

                switch(modus)
                {
                  case FORMAT_HTML:
                    out.writeBytes("<h3><a name=" + mame1 + ">Subroutine " + mame.replaceAll("!","").trim() + "</a></h3>\n");
                    break;
                  case FORMAT_TEX:
		...
		if (pos > 0) {
		    ...
		    pos = mame2.indexOf("(");
		    if (pos > 0) {
			mame1 = mame2.substring(0, pos).trim();
		    }
